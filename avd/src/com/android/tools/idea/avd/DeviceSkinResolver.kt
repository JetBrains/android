...
internal object DeviceSkinResolver {
  /**
   * If a system image skin has the same suffix as deviceSkin (including separators), returns the system image skin. Otherwise, resolves
   * deviceSkin against sdk or deviceArtResources (depending on which one is null). If resolving against deviceArtResources, also renames a
   * handful of Wear OS skins.
   *
   * @param deviceSkin the device definition skin
   * @param imageSkins the system image skins
   * @param sdk the path to the Android SDK
   * @param deviceArtResources the device-art-resources subdirectory, under the Android Studio application directory, containing the skins
   *   shipped with Studio
   * @return an absolute path if imageSkins, sdk, and deviceArtResources are absolute. Returns deviceSkin as is if it's already absolute,
   *   the empty path, _no_skin, or if both sdk and deviceArtResources are null.
   */
  fun resolve(deviceSkin: Path, imageSkins: Iterable<Path>, sdk: Path?, deviceArtResources: Path?): Path {
    when {
