...
import com.android.tools.idea.insights.ui.actions.JListSimpleColoredComponent
import com.android.tools.idea.insights.ui.actions.ResizedSimpleColoredComponent
  ...
  fun `gutter popup shows correct information`() =
    runBlocking(AndroidDispatchers.uiThread) {
      ...
      with(list.model) {
        ...
        insights.forEachIndexed { index, AppInsight ->
          assertThat(getElementAt(index)).isEqualTo(AppInsight)
        }
      }
