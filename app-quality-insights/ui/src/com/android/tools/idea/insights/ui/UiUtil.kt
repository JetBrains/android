...
/** Fixes spacing around headers in insights */
fun correctInsightHtmlText(htmlText: String): String {
  val lines = htmlText.split("\n")
  return lines
    .mapIndexed { idx, s ->
      var text = s
      when (idx) {
        0 -> {
          if (s.containsStrong() && lines.size >= 2 && !lines[1].containsHtmlListStart()) {
            text = "$text<br>"
          }
        }
        lines.size - 1 -> {
          if (s.containsStrong() && !lines[idx - 1].containsHtmlListEnd()) {
            text = "<br>$text"
          }
        }
        else -> {
          // List items can have <strong> but don't require a <br>.
          if (s.containsStrong() && !s.isListItem(lines[idx - 1], lines[idx + 1])) {
            // Lists in HTML get space above and below in the HtmlEditorKit
            // Add a space before header if previous element is not a list end
            // Add a space after header if next element is not a list start
            if (!lines[idx - 1].containsHtmlListEnd()) {
              text = "<br>$text"
            }
            if (!lines[idx + 1].containsHtmlListStart()) {
              text = "$text<br>"
            }
          }
        }
...
private fun String.containsStrong() = contains("<strong>")

private fun String.containsHtmlListStart() = contains("<ol>") || contains("<ul>")

private fun String.containsHtmlListEnd() = contains("</ol>") || contains("</ul>")

private fun String.containsHtmlListItemStart() = contains("<li>")

private fun String.containsHtmlListItemEnd() = contains("</li>")

private fun String.isListItem(prev: String, next: String) =
  containsHtmlListItemStart() ||
    containsHtmlListItemEnd() ||
    prev.containsHtmlListItemStart() ||
    next.containsHtmlListItemEnd()
