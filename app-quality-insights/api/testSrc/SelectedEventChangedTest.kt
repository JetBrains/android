...
class SelectedEventChangedTest {
  ...
  fun `get next event`() {
    ...
    val transition =
      SelectedEventChanged(EventMovement.NEXT).transition(state, TestAppInsightsTracker)
  ...
  fun `get previous event`() {
    ...
    val transition =
      SelectedEventChanged(EventMovement.PREVIOUS).transition(state, TestAppInsightsTracker)
    assertThat(transition.newState.currentEvents)
  ...
  fun `get next or previous event when none are available results in noop`() {
    ...
    transition = SelectedEventChanged(EventMovement.NEXT).transition(state, TestAppInsightsTracker)
    assertThat(transition.newState.currentEvents)
      .isEqualTo(LoadingState.Ready(DynamicEventGallery(listOf(Event("1")), 0, "")))
    assertThat(transition.action).isEqualTo(Action.NONE)
  }
