...
class AppInsightsProjectLevelControllerTest {
  ...
  fun `when model is not available, fetch insight emits correct error state when flag is enabled`() = runBlocking {
    ...
    controllerRule.fakeAiInsightContributor.modelAvailable = false
    controllerRule.controller.refreshInsight(false)
    assertThat(controllerRule.consumeNext().currentInsight).isEqualTo(LoadingState.NoModelAvailable)
  }
