...
class StreamQueryUtilsTest {
  ...
  @get:Rule val grpcServerRule = FakeGrpcServer.createFakeGrpcServer("StreamQuerySupportTest", transportService)
  ...
  fun queryForDevices() {
    ...
    assertThat(devices.map { it.device })
      .containsExactly(
  ...
  fun queryForProcesses() {
    ...
    // Verify with filter
    assertThat(
        StreamQueryUtils.queryForProcesses(transportClient.transportStub, device.deviceId) { _, lastAliveEvent -> lastAliveEvent.pid == 20 }
      )
  ...
  fun processesQueryReturnsHighestExposureLevel() {
    ...
    transportService.addProcess(
      device,
      createProcess(device.deviceId, 20, "FakeProcess1", Common.Process.State.ALIVE, Common.Process.ExposureLevel.PROFILEABLE),
    )
  ...
  private fun createDevice(featureLevel: Int, serial: String, state: Common.Device.State): Common.Device {
