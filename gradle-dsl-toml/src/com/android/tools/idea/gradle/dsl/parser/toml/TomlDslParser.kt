...
class TomlDslParser(private val psiFile: TomlFile, private val dslFile: GradleDslFile) : GradleDslParser, TomlDslNameConverter {
  ...
  override fun parse() {
    fun getVisitor(context: GradlePropertiesDslElement, name: GradleNameElement): TomlRecursiveVisitor =
  ...
  private fun GradlePropertiesDslElement.getOrCreateMap(segment: TomlKeySegment, psiElement: PsiElement): GradleDslExpressionMap {
    ...
    return getPropertyElement(description)
      ?: GradleDslExpressionMap(this, psiElement, GradleNameElement.from(segment, this@TomlDslParser), true).also { addParsedElement(it) }
  }
  ...
  override fun convertToPsiElement(context: GradleDslSimpleExpression, literal: Any): PsiElement? {
    return when (literal) {
      ...
      is Int,
      is Boolean,
      is BigDecimal -> TomlPsiFactory(context.dslFile.project, true).createLiteral(literal.toString())
