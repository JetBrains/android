  ...
  init {
    ApplicationManager.getApplication().invokeAndWait {
      ...
      ToolWindowManager.getInstance(project)
        .registerToolWindow(RegisterToolWindowTask.closable(TOOL_WINDOW_ID, icons.StudioIcons.Shell.ToolWindows.AGP_UPGRADE_ASSISTANT))
    }
  }
...
fun AgpUpgradeComponentNecessity.treeText() =
  when (this) {
...
fun AgpUpgradeComponentNecessity.checkboxToolTipText(enabled: Boolean, selected: Boolean) =
  if (enabled) null
    ...
    when (this to selected) {
      AgpUpgradeComponentNecessity.MANDATORY_INDEPENDENT to true ->
        "Cannot be deselected while ${AgpUpgradeComponentNecessity.MANDATORY_CODEPENDENT.treeText()} is selected"
      AgpUpgradeComponentNecessity.MANDATORY_CODEPENDENT to false ->
        "Cannot be selected while ${AgpUpgradeComponentNecessity.MANDATORY_INDEPENDENT.treeText()} is unselected"
      AgpUpgradeComponentNecessity.MANDATORY_CODEPENDENT to true ->
        "Cannot be deselected while ${AgpUpgradeComponentNecessity.OPTIONAL_CODEPENDENT.treeText()} is selected"
      AgpUpgradeComponentNecessity.OPTIONAL_CODEPENDENT to false ->
        "Cannot be selected while ${AgpUpgradeComponentNecessity.MANDATORY_CODEPENDENT.treeText()} is unselected"
      else -> {
...
fun AgpUpgradeComponentNecessity.description() =
  when (this) {
