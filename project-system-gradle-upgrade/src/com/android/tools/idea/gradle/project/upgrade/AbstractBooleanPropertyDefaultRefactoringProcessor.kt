...
class GradlePropertyUsageInfo(
  private val wrappedElement: WrappedPsiElement,
  val key: String,
  val oldDefault: Boolean,
  val tooltip: String,
  ...
  override fun performBuildModelRefactoring(processor: GradleBuildModelRefactoringProcessor) {
    val propertiesFile =
      when (val realElement = wrappedElement.realElement) {
        ...
        is PsiDirectory ->
          (realElement.findFile(FN_GRADLE_PROPERTIES) ?: realElement.createFile(FN_GRADLE_PROPERTIES)) as? PropertiesFile ?: return
        else -> return
      }
