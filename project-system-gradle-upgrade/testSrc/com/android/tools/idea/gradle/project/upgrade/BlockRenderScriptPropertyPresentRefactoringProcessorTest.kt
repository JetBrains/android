...
@RunsInEdt
class BlockRenderScriptPropertyPresentRefactoringProcessorTest: UpgradeGradleFileModelTestCase() {
  override val projectRule = AndroidProjectRule.onDisk()
  private val propertyKey = "android.defaults.buildfeatures.renderscript"

  @Test
  fun `Non blocked if property is not present`() {
    val processor = BlockRenderScriptPropertyPresentRefactoringProcessor(project, AgpVersion.parse("8.0.0"), AgpVersion.parse("9.0.0"))
  ...
  fun `Blocked if property present and is true`() {
    ...
    verifyBlockedReasons("8.0.0", listOf("Property android.defaults.buildfeatures.renderscript has been removed in 9.0.0-alpha01."))
  }

  @Test
  fun `Blocked if property is not present but upgrading from pre 8_0`() {
    verifyBlockedReasons("7.4.2", listOf("There have been changes in how Render Script is configured."))
  }

  @Test
  fun `Refactoring enabled for 9_0_0-alpha01`() {
    val project = projectRule.project
    val processor = BlockRenderScriptPropertyPresentRefactoringProcessor(project, AgpVersion.parse("8.0.0"), AgpVersion.parse("9.0.0-alpha01"))
  ...
  fun `Refactoring not enabled when current version 9_0_0`() {
    val project = projectRule.project
    val processor = BlockRenderScriptPropertyPresentRefactoringProcessor(project, AgpVersion.parse("9.0.0"), AgpVersion.parse("10.0.0"))
  ...
  fun `Refactoring not enabled when new version lower than 9_0_0-alpha01`() {
    val project = projectRule.project
    val processor = BlockRenderScriptPropertyPresentRefactoringProcessor(project, AgpVersion.parse("8.0.0"), AgpVersion.parse("8.1000.0"))
    assertThat(processor.isEnabled).isFalse()
  }

  private fun verifyBlockedReasons(from: String, expectedReasons: List<String>) {
    val processor = BlockRenderScriptPropertyPresentRefactoringProcessor(project, AgpVersion.parse(from), AgpVersion.parse("9.0.0"))
