...
class DevicesConnectionStep(
  ...
  private suspend fun showFirstPhase(
    phonePairingDevice: PairingDevice,
    phoneDevice: IDevice,
    wearPairingDevice: PairingDevice,
    wearDevice: IDevice,
  ) {
  ...
  private suspend fun showWaitForPairingSetup(
    phoneWearPair: PhoneWearPair,
    phoneDevice: IDevice,
    wearDevice: IDevice,
    ...
    // After some time we give up and show the manual retry ui. waitForPairingSuccessOnBackground()
    // will take care of success case
  ...
  private suspend fun IDevice.waitForPairingStatus() {
    if (hasPairingFeature(PairingFeature.GET_PAIRING_STATUS)) {
      ...
      // Give some time for Node/Cloud ID to load, but not too long, as it may just mean it never
      // paired before
  ...
  private suspend fun showUI(
    header: String = "",
    description: String = "",
    progressTopLabel: String = "",
    progressBottomLabel: String = "",
    ...
    withContext(uiThread(ModalityState.any())) {
      ...
      mainPanel.apply {
        ...
        if (header.isNotEmpty()) {
          add(
            JBLabel(header, UIUtil.ComponentStyle.LARGE).apply {
              name = "header"
        ...
        if (progressTopLabel.isNotEmpty()) {
          add(
            JBLabel(progressTopLabel).apply { border = empty(4, 0) },
        ...
        if (progressBottomLabel.isNotEmpty()) {
          add(
            JBLabel(progressBottomLabel).apply {
              border = empty(4, 0)
  ...
  private fun createScanningPanel(
    ...
    JPanel(GridBagLayout()).apply {
      ...
      if (buttonLabel.isNotBlank()) {
        add(
          JButton(buttonLabel).apply { addActionListener(buttonListener) },
  ...
  private suspend fun showUiLaunchingDevice(progressTopLabel: String, progressBottomLabel: String) =
    showUI(
  ...
  private suspend fun showUiInstallCompanionAppSuccess(phoneDevice: IDevice, wearDevice: IDevice) =
    showUiInstallCompanionApp(
      phoneDevice = phoneDevice,
      wearDevice = wearDevice,
  ...
  private suspend fun showUiInstallCompanionAppRetry(phoneDevice: IDevice, wearDevice: IDevice) =
    showUiInstallCompanionApp(
      phoneDevice = phoneDevice,
      wearDevice = wearDevice,
  ...
  private suspend fun showUiPairingNonInteractive(
    phoneWearPair: PhoneWearPair,
    phoneDevice: IDevice,
    wearDevice: IDevice,
    scanningLabel: String = message("wear.assistant.device.connection.pairing.auto.start"),
    buttonLabel: String = "",
    scanningLink: String = "",
    ...
    val successLabel =
      message(
        if (tapAndFinishWarning) {
          "wear.assistant.device.connection.pairing.success.skipandfinish"
        } else {
          "wear.assistant.device.connection.pairing.success.subtitle"
        },
        phoneName,
        watchName,
      )
    val svgLabel =
      JBLabel(successLabel).apply {
  ...
  private suspend fun showUiNeedsFactoryReset(wearDeviceName: String) {
    ...
    val warningPanel =
      createWarningPanel(message("wear.assistant.factory.reset.subtitle", wearDeviceName)).apply {
  ...
  private fun prepareErrorListener() {
    deviceStateListener.listenAll(model.selectedPhoneDevice, model.selectedWearDevice).withAndFire {
      val errorDevice =
        model.selectedPhoneDevice.valueOrNull.takeIf { it?.isOnline() == false }
          ?: model.selectedWearDevice.valueOrNull.takeIf { it?.isOnline() == false }
      if (errorDevice != null) {
        showDeviceError(
          header = currentUiHeader,
          description = currentUiDescription,
  ...
  private fun showGenericError(ex: Throwable) {
    ...
    showDeviceError(
      header = message("wear.assistant.connection.alert.cant.bridge.title"),
      description = " ",
  ...
  private fun goToNextStep() {
    ApplicationManager.getApplication()
      .invokeLater(
        {
          // The "Next" button changes asynchronously. Create a temporary property that will change
...
private suspend fun checkWearMayNeedFactoryReset(
  ...
  if (wearDevice.hasPairingFeature(PairingFeature.GET_PAIRING_STATUS)) {
    ...
    if (wearNodeId != null) {
      ...
      return wearPairingStatus.isNotEmpty() &&
        !wearPairingStatus[0].nodeId.isNullOrEmpty() &&
