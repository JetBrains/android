...
class DeviceExplorerControllerTest {
  ...
  @After
  fun cleanUp() {
    // Disconnect lingering devices after each test
  ...
  @Test
  fun testStartingControllerWithNoDeviceConnected() = runBlocking(AndroidDispatchers.uiThread) {
    // Prepare
    val controller = createController()
  ...
  @Test
  fun testStartingControllerWithDeviceConnected() = runBlocking(AndroidDispatchers.uiThread) {
    // Prepare
    val controller = createController()
  ...
  @Test
  fun testDeviceSelection() = runBlocking(AndroidDispatchers.uiThread) {
    // Prepare
    val controller = createController()
    connectDevice("test_device_01")
    ...
    // Assert
    pumpEventsAndWaitForFutures(fileController.activeDeviceTracker.consumeMany(1))
    pumpEventsAndWaitForFutures(monitorController.activeDeviceTracker.consumeMany(1))
    assertThat(view.viewComboBox().isVisible).isTrue()
    assertThat(view.viewTabPane().isVisible).isTrue()
    assertThat(view.viewComboBox().itemCount).isEqualTo(2)
    assertThat((view.viewComboBox().selectedItem as? DeviceHandle)?.state?.connectedDevice?.serialNumber).isEqualTo(newDeviceState.deviceId)
  }
