 ...
 * making any changes to the project itself. This is to avoid running Gradle sync for the same project multiple times and thus improve test
 * execution time.
 */
...
/**
 * A base class for a test case that applies multiple [SyncedProjectTestDef]s to multiple [TestProjectDefinition]s. See `SyncedProjectTest`
 * as an example.
 */
abstract class SyncedProjectTestBase<TestProject : TestProjectDefinition>(
  ...
  /**
   * if `true` activates a special mode that do not run actual tests but instead verify that the test case implementation implements test
   * methods for all declared projects. See [SyncedProjectTestSelfCheckBase] and its implementations.
   */
  ...
  protected fun testProject(testProject: TestProject) {
    ...
    val testDefinitions = testDefs.map(::transformTest).filter { it.isCompatible() }.groupBy { it.agpVersion }
    ...
    try {
      ...
      aggregateAndThrowIfAny {
        ...
        preparedProject.open(
          updateOptions = { it.copy(disableForcedAgpUpgradeDialog = true, disableKtsRelatedIndexing = true, reportProjectSizeUsage = true) }
        ) { project ->
          runBlocking {
            // Wait for ReportProjectSizeTask to finish if started since this runs async during
            // tests due to b/354210253
            project.getService(ReportProjectSizeTask::class.java).repostStatsJobs.forEach { it.join() }
          }
