...
@VisibleForTesting
data class SafePromptImpl(
  override val messages: List<SafePrompt.Message>
) : SafePrompt
...
class SafePromptBuilderImpl(private val project: Project): SafePromptBuilder {
  ...
  class MessageBuilderImpl(val makeMessage: (List<SafePrompt.Message.Chunk>) -> SafePrompt.Message): SafePromptBuilder.MessageBuilder {
    ...
    /**
     * Adds [code] as a formatted code block in the message,
     * with optional [language] specified.
     */
  ...
  override fun systemMessage(builderAction: SafePromptBuilder.MessageBuilder.() -> Unit) {
    if (messages.isNotEmpty()) {
      throw MalformedPromptException("Prompt can only contain one system message, and it must be the first message.")
    }
    messages.add(
      MessageBuilderImpl { SafePrompt.SystemMessage(it) }.apply(builderAction).build()
    )
  }

  override fun userMessage(builderAction: SafePromptBuilder.MessageBuilder.() -> Unit) {
    messages.add(
      MessageBuilderImpl { SafePrompt.UserMessage(it) }.apply(builderAction).build()
    )
  }

  override fun modelMessage(builderAction: SafePromptBuilder.MessageBuilder.() -> Unit) {
    messages.add(
      MessageBuilderImpl { SafePrompt.ModelMessage(it) }.apply(builderAction).build()
    )
  }
  ...
  private fun excludedFiles(): Set<VirtualFile> =
    messages
      ...
      .filter {
        StudioBot.getInstance().aiExcludeService().isFileExcluded(project, it)
      }
      .toSet()
  ...
  fun build(): SafePrompt {
    ...
    checkPromptFormat(messages.last() is SafePrompt.UserMessage, "Last message in prompt must be a user message.")
