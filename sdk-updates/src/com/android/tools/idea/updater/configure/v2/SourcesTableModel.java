...
class SourcesTableModel extends ListTableModel<SourcesTableModel.Row> implements EditableModel {
   ...
   */
  @NotNull
  private EditSourceDialog showEditDialog(RepositorySource source) {
    EditSourceDialog input = new EditSourceDialog(myHandler.getUserSourceProvider(myLogger), source);
    input.show();
    return input;
  }
  ...
  private void createSource(String url, String uiName) {
    RepositorySource newSource = new SimpleRepositorySource(url, uiName, true, ImmutableList
      .of(myHandler.getAddonModule(myLogger), myHandler.getSysImgModule(myLogger)), myHandler.getUserSourceProvider(myLogger));
    myHandler.getUserSourceProvider(myLogger).addSource(newSource);
    refreshUi();
  }
  ...
  public void removeRow(int idx) {
    myHandler.getUserSourceProvider(myLogger).removeSource(getRowValue(idx).mySource);
    refreshUi();
  }
  ...
  public void save(@NotNull ProgressIndicator progress) {
    if (isSourcesModified()) {
      myHandler.getUserSourceProvider(myLogger).save(new RepoProgressIndicatorAdapter(progress));
      reset();
    }
  }
