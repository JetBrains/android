...
internal class BuildInfo(
  ...
  val buildRootDirectory: File
    get() = buildFolderPaths.buildRootDirectory!!
}

/**
 * Implementation of the Android Gradle sync processes.
 *
 * [syncOptions] defines the sync variant (mode) to run: single-variant-sync (the regular sync), all-variant-sync (requested by the project
 * structure dialog) and native-variant-sync (supports run/debug with older AGP versions).
 *
 * The overall flow is: buildInfo -> List<BasicIncompleteGradleModule> via AndroidProjectResult for Android modules -> List<GradleModule>
 * Android build variant selection via SyncVariantResultCore -> SyncVariantResult -> List<GradleModule> (mutable properties populated) ->
...
internal class AndroidExtraModelProviderWorker(
  ...
  private val safeActionRunner = SyncActionRunner.create(controller, syncCounters, syncOptions.flags.studioFlagParallelSyncEnabled)
      ...
      val modelCollections: List<GradleModelCollection> =
        when (syncOptions) {
          is SyncProjectActionOptions -> {
            ...
            val shouldUseProjectGraph =
              GradleVersion.version(gradleVersion) >= GradleVersion.version("8.10-rc-1") &&
                v2AndroidGradleModules.all { it.modelVersions[ModelFeature.HAS_PROJECT_GRAPH_MODEL] }

            val configuredSyncActionRunner =
              safeActionRunner.enableParallelFetchForV2Models(
            ...
            val syncExecutionReport =
              IdeSyncExecutionReport(
    ...
    return safeActionRunner.runActions(
      buildInfo.projects
        .map { gradleProject ->
          ...
          val buildPath =
            buildInfo.buildIdMap[buildId] ?: error("Build not found: $buildId \n" + "available builds ${buildInfo.buildIdMap}")
          ActionToRun(
            fun(controller: BuildController): BasicIncompleteGradleModule {
              ...
              if (legacyV1AgpVersionModel != null)
                return BasicV1AndroidModuleGradleProject(gradleProject, buildPath, legacyV1AgpVersionModel.convert())
...
private fun verifyIncompatibleAgpVersionsAreNotUsedOrFailSync(modules: List<BasicIncompleteGradleModule>) {
  val agpVersionsAndGradleBuilds =
    modules.filterIsInstance<BasicIncompleteAndroidModule>().map { it.modelVersions.agpVersionAsString to it.buildPath }
...
/**
 * This is the only model-reading version check that should compare AGP versions directly. All other checks should use [ModelVersions.get]
 * with a [ModelFeature]
 */
...
fun Versions.convert(): ModelVersions {
  ...
  val minimumModelConsumer =
    versions[Versions.MINIMUM_MODEL_CONSUMER]?.let { version ->
      ...
      ModelConsumerVersion(
        version.major,
        version.minor,
        version.humanReadable ?: error("AGP that reports a MINIMUM_MODEL_CONSUMER version must have a human readable version"),
      )
    }
  val modelVersion =
    versions[Versions.MODEL_PRODUCER]?.let { version ->
      ModelVersion(
        version.major,
        version.major,
        version.humanReadable ?: error("AGP that reports a MODEL_PRODUCER version must have a human readable version"),
      )
    } ?: ModelVersion(Int.MIN_VALUE, Int.MIN_VALUE, agpVersion.toString())

  return ModelVersions(agp = agpVersion, modelVersion = modelVersion, minimumModelConsumer = minimumModelConsumer)
}
