...
open class AndroidSyncException(
  ...
  message: String,
  /**
   * The root directory of the current build where this exception originated. For included build in a composite build, this points to the
   * root directory(dir that hold settings.gradle) of the included build. For root build in a composite build, this points to the root
   * directory of root build.
   */
...
class AgpVersionTooNew(agpVersion: AgpVersion, latestSupportedVersion: AgpVersion) :
  AndroidSyncException(AGP_VERSION_TOO_NEW, generateMessage(agpVersion, latestSupportedVersion)) {
  companion object {
    private const val LEFT = "The project is using an incompatible version (AGP "
    private const val RIGHT = ") of the Android Gradle plugin. Latest supported version is AGP "

    private fun generateMessage(agpVersion: AgpVersion, latestSupportedVersion: AgpVersion) =
      "$LEFT$agpVersion$RIGHT$latestSupportedVersion"
...
class AgpVersionIncompatible(agpVersion: AgpVersion, latestSupportedVersion: AgpVersion) :
  AndroidSyncException(AGP_VERSION_INCOMPATIBLE, generateMessage(agpVersion, latestSupportedVersion)) {
  companion object {
...
class AgpVersionsMismatch(agpVersions: List<Pair<String, String>>) :
  AndroidSyncException(AGP_VERSIONS_MISMATCH, generateMessage(agpVersions)) {
  companion object {
