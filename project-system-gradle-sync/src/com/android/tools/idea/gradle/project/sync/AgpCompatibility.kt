...
import com.android.tools.idea.gradle.project.upgrade.checkAgpCompatibilityForIdeaAndroidProject
...
internal fun checkAgpVersionCompatibility(minimumModelConsumerVersion: ModelConsumerVersion?, agpVersion: AgpVersion, syncOptions: SyncActionOptions) {
  ...
  if (minimumModelConsumerVersion != null && syncOptions.flags.studioFlagSupportFutureAgpVersions) {
    return when {
      ...
      agpVersion < MINIMUM_SUPPORTED_AGP_VERSION -> {
        throw AgpVersionTooOld(agpVersion)
      }
      (minimumModelConsumerVersion > MODEL_CONSUMER_VERSION) -> {

        throw AgpVersionTooNew(agpVersion)
      }
  ...
  return when (computeAndroidGradlePluginCompatibility(agpVersion, LATEST_KNOWN_ANDROID_GRADLE_PLUGIN_VERSION)) {
    ...
    AndroidGradlePluginCompatibility.DIFFERENT_PREVIEW -> {
      ...
      if (syncOptions.flags.ideaFlagSupportFutureAgpVersions
          && checkAgpCompatibilityForIdeaAndroidProject(agpVersion, LATEST_KNOWN_ANDROID_GRADLE_PLUGIN_VERSION)) {
        Unit // Compatible
      }
      else {
