...
/**
 * AGP [current] and Studio's [latestKnown] can each be of "release class": alpha/beta, rc/release, or snapshot (-dev). Their major/minor
 * release number can be equal, or one major/minor release can precede the other. If they are of the same major/minor and non-snapshot
 * release class, then either can precede the other. (All snapshots of the same major/minor cycle compare equal, even if they reflect
 * completely different actual code...)
 *
 ...
 *
 * Some of the complexity below is handling the fact that the "natural" version comparison places -dev between rc and release, whereas we
 * want to treat rc and release equivalently.
 *
 ...
 *
 * There should be 9 cases with current: 7.1.0-alpha01, 9 with current: 7.1.0-rc01, 8 with current: 7.1.0-dev, one each of current:
 * 7.1.0-alpha02 and 7.1.0.
 *
 * There should be 3 cases with latestKnown: 7.0.0-alpha01, 3 with latestKnown: 7.0.0-rc01, 3 with latestKnown: 7.0.0-dev, 3 cases with
 * latestKnown: 7.2.0-alpha01, 3 with latestKnown: 7.2.0-rc01, 3 with latestKnown: 7.2.0-dev, 3 cases with latestKnown: 7.1.0-alpha01, 3
 * with latestKnown: 7.1.0-rc01, 2 with latestKnown: 7.1.0-dev, 1 with latestKnown: 7.1.0-alpha02, 1 with latestKnown: 7.1.0
 *
 * (Note that the above consistency check was written before the addition of the notion of the DEPRECATED class, which divides the previous
 * COMPATIBLE class in two: something is DEPRECATED if current is earlier than GRADLE_PLUGIN_NEXT_MINIMUM_VERSION and latestKnown is after,
 * the second of which conditions will always hold in production).
 */
fun computeAndroidGradlePluginCompatibility(current: AgpVersion, latestKnown: AgpVersion): AndroidGradlePluginCompatibility = run {
  val compatibleOrDeprecated =
    when {
  ...
  when {
    ...
    (!latestKnown.isPreview || latestKnown.previewType == "rc") &&
      (!current.isPreview || current.previewType == "rc") &&
    ...
    current.isSnapshot &&
      (latestKnown.previewType == "alpha" || latestKnown.previewType == "beta") &&
      (AgpVersion(latestKnown.major, latestKnown.minor) == AgpVersion(current.major, current.minor)) ->
      compatibleOrDeprecated // in practice presumably always compatible
