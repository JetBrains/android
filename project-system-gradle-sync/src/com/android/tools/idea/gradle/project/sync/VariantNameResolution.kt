...
fun buildVariantNameResolver(androidProject: IdeAndroidProject, v2Variants: Collection<IdeVariantCore>): AndroidVariantResolver {
  ...
  val dimensions =
    androidProject.flavorDimensions
      .filter { availableDimensions.contains(it) }
      // See: b/242856048 and b/242289523: `FAKE_DIMENSION` is not reported by the AGP in `androidProject.flavorDimensions`.
      .takeUnless { it.isEmpty() } ?: (if (availableDimensions == setOf(FAKE_DIMENSION)) availableDimensions else emptySet())

  val map = v2Variants.associate { variant -> variant.productFlavors.toList() + listOfNotNull(variant.buildType) to variant.name }
