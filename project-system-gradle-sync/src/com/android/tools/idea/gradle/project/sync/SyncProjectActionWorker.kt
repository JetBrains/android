...
internal class SyncProjectActionWorker(
  ...
  private val rootBuildModel: BuildModel
    get() = buildInfo.rootBuild
  ...
  fun populateAndroidModels(
    ...
    shouldUseProjectGraph: Boolean,
  ): List<GradleModelCollection> {
    ...
    syncCounters.variantAndDependencyResolutionPhase {
      val variantDiscovery =
        VariantDiscovery(actionRunner, androidModulesById, syncOptions, buildInfo, internedModels, ::resolveAndroidProjectPath)
      when (syncOptions) {
        is SingleVariantSyncActionOptions -> {
          // This section is for Single Variant Sync specific models if we have reached here we should have already requested
          // AndroidProjects
    ...
    syncCounters.additionalArtifactsPhase {
      // AdditionalClassifierArtifactsModel must be requested after AndroidProject and Variant model since it requires the library list in
      // dependency model.
      ...
      dynamicFeatureToBaseFeatureMap =
        modules
          ...
          .flatMap { androidModule ->
            ...
            androidModule.androidProject.dynamicFeatures.map { ModuleId(it, rootDir.path) }.map { it to androidModule.moduleId }
          }
          .toMap()
    )
  }

  private fun fetchGradleModulesAction(incompleteBasicModules: List<BasicIncompleteGradleModule>): List<GradleModule> {
    return actionRunner.runActions(incompleteBasicModules.map { it.getGradleModuleAction(internedModels, buildInfo) }.toList())
  }
}
