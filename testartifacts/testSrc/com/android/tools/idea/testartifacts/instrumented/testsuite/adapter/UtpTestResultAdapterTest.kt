...
class UtpTestResultAdapterTest {
  ...
  private val utpProtoFile: File by lazy { temporaryFolder.newFile() }

  @Test
  fun invalidProtobuf() {
    utpProtoFile.outputStream().write("Invalid".toByteArray(Charset.defaultCharset()))
    assertFailsWith<InvalidProtocolBufferException> { UtpTestResultAdapter(utpProtoFile).packageName }
  }
  ...
  fun importSuccessTestResults() {
    ...
    verifyInOrder
      ...
      .onTestCaseFinished(
        any(),
        any(),
        argThat { result == AndroidTestCaseResult.PASSED && retentionSnapshot == null && packageName == TEST_PACKAGE_NAME },
      )
    verifyInOrder.verify(mockListener).onTestCaseStarted(any(), any(), any())
    verifyInOrder
      .verify(mockListener)
    ...
    verifyInOrder.verify(mockListener).onTestSuiteFinished(any(), argThat { result == AndroidTestSuiteResult.PASSED })
  }
  ...
  fun importPartialFailedTestResults() {
    ...
    verifyInOrder.verify(mockListener).onTestSuiteStarted(any(), any())
    verifyInOrder.verify(mockListener).onTestCaseStarted(any(), any(), any())
    verifyInOrder.verify(mockListener).onTestCaseFinished(any(), any(), argThat { result == AndroidTestCaseResult.PASSED })
    verifyInOrder.verify(mockListener).onTestCaseStarted(any(), any(), any())
    verifyInOrder
      .verify(mockListener)
      .onTestCaseFinished(
        any(),
        any(),
        argThat { result == AndroidTestCaseResult.FAILED && retentionSnapshot == null && packageName == TEST_PACKAGE_NAME },
      )
    verifyInOrder.verify(mockListener).onTestSuiteFinished(any(), argThat { result == AndroidTestSuiteResult.FAILED })
  }
  ...
  fun importFailedTestResultsWithSnapshots() {
    ...
    val protobuf =
      TestSuiteResultProto.TestSuiteResult.newBuilder()
        .addTestResult(
          TestResultProto.TestResult.newBuilder()
            .setTestCase(
              TestCaseProto.TestCase.newBuilder().setTestClass(testClass).setTestPackage(TEST_PACKAGE_NAME).setTestMethod(testMethod1)
            )
            .setTestStatus(TestStatusProto.TestStatus.FAILED)
            .addOutputArtifact(TestArtifactProto.Artifact.newBuilder().setSourcePath(PathProto.Path.newBuilder().setPath(snapshotName1)))
        )
        .addTestResult(
          TestResultProto.TestResult.newBuilder()
            .setTestCase(
              TestCaseProto.TestCase.newBuilder().setTestClass(testClass).setTestPackage(TEST_PACKAGE_NAME).setTestMethod(testMethod2)
            )
            .setTestStatus(TestStatusProto.TestStatus.FAILED)
            .addOutputArtifact(TestArtifactProto.Artifact.newBuilder().setSourcePath(PathProto.Path.newBuilder().setPath(snapshotName2)))
        )
        .build()
    protobuf.writeTo(utpProtoFile.outputStream())
    val utpTestResultAdapter = UtpTestResultAdapter(utpProtoFile)
    ...
    verifyInOrder
      ...
      .onTestCaseFinished(
        any(),
        any(),
        argThat { result == AndroidTestCaseResult.FAILED && retentionSnapshot?.canonicalPath == failedSnapshot1.canonicalPath },
      )
    verifyInOrder.verify(mockListener).onTestCaseStarted(any(), any(), any())
    verifyInOrder
      .verify(mockListener)
      .onTestCaseFinished(
        any(),
        any(),
        argThat { result == AndroidTestCaseResult.FAILED && retentionSnapshot?.canonicalPath == failedSnapshot2.canonicalPath },
      )
    verifyInOrder.verify(mockListener).onTestSuiteFinished(any(), argThat { result == AndroidTestSuiteResult.FAILED })
  }
  ...
  fun importFailedTestResultWithoutSnapshot() {
      ...
      TestSuiteResultProto.TestSuiteResult.newBuilder()
        .addTestResult(
          TestResultProto.TestResult.newBuilder()
            ...
            .setTestStatus(TestStatusProto.TestStatus.FAILED)
            .addOutputArtifact(
              TestArtifactProto.Artifact.newBuilder().setSourcePath(PathProto.Path.newBuilder().setPath(unrelatedArtifact))
            )
        )
        .build()
    protobuf.writeTo(utpProtoFile.outputStream())
    ...
    verifyInOrder
      .verify(mockListener)
      .onTestCaseFinished(any(), any(), argThat { result == AndroidTestCaseResult.FAILED && retentionSnapshot == null })
    verifyInOrder.verify(mockListener).onTestSuiteFinished(any(), argThat { result == AndroidTestSuiteResult.FAILED })
  }
  ...
  fun importSkippedTestResults() {
    ...
    verifyInOrder
      ...
      .onTestCaseFinished(
        any(),
        any(),
        argThat { result == AndroidTestCaseResult.SKIPPED && retentionSnapshot == null && packageName == TEST_PACKAGE_NAME },
      )
    verifyInOrder.verify(mockListener).onTestSuiteFinished(any(), argThat { result == AndroidTestSuiteResult.PASSED })
  }
  ...
  fun importTestResultsWithLogcat() {
              ...
              TestArtifactProto.Artifact.newBuilder().apply {
                label =
                  LabelProto.Label.newBuilder()
                    ...
                    .build()
                sourcePath = PathProto.Path.newBuilder().apply { path = logcatFile.canonicalPath }.build()
              }
            )
        )
        .build()
    ...
    verify(mockListener).onTestCaseFinished(any(), any(), argThat { logcat == "test logs" })
  }
  ...
  fun importMultipleDevices() {
    ...
    AndroidTestDeviceInfoProto.AndroidTestDeviceInfo.newBuilder()
      ...
      .build()
      .writeTo(deviceInfoProtoFile1.outputStream())
    AndroidTestDeviceInfoProto.AndroidTestDeviceInfo.newBuilder()
      .apply {
      ...
      .build()
      .writeTo(deviceInfoProtoFile2.outputStream())
    ...
    TestSuiteResultProto.TestSuiteResult.newBuilder()
      .addTestResult(
        TestResultProto.TestResult.newBuilder()
          .setTestCase(
            TestCaseProto.TestCase.newBuilder().setTestClass(testClass1).setTestPackage(TEST_PACKAGE_NAME).setTestMethod(testMethod1)
          )
          .setTestStatus(TestStatusProto.TestStatus.PASSED)
          .addOutputArtifact(
            TestArtifactProto.Artifact.newBuilder().apply {
              label = labelBuilder.setLabel("device-info").setNamespace("android").build()
              sourcePath = sourcePathBuilder.setPath(deviceInfoProtoFile1.absolutePath).build()
            }
          )
      )
      .addTestResult(
        TestResultProto.TestResult.newBuilder()
          .setTestCase(
            TestCaseProto.TestCase.newBuilder().setTestClass(testClass2).setTestPackage(TEST_PACKAGE_NAME).setTestMethod(testMethod2)
          )
          .setTestStatus(TestStatusProto.TestStatus.FAILED)
          .addOutputArtifact(
            TestArtifactProto.Artifact.newBuilder().apply {
              label = labelBuilder.setLabel("device-info").setNamespace("android").build()
              sourcePath = sourcePathBuilder.setPath(deviceInfoProtoFile2.absolutePath).build()
            }
          )
      )
      .build()
      .writeTo(utpProtoFile.outputStream())
    val utpTestResultAdapter = UtpTestResultAdapter(utpProtoFile)
    utpTestResultAdapter.forwardResults(mockListener)
    val deviceMatcher1 =
      ArgumentMatcher<AndroidDevice> { device ->
        device?.deviceName == deviceName1 &&
          device.deviceType == AndroidDeviceType.LOCAL_PHYSICAL_DEVICE &&
          device.additionalInfo["Manufacturer"] == manufacturer1 &&
          device.additionalInfo["Model"] == model1
      }
    val deviceMatcher2 =
      ArgumentMatcher<AndroidDevice> { device ->
        device?.deviceName == deviceName2 &&
          device.deviceType == AndroidDeviceType.LOCAL_EMULATOR &&
          device.additionalInfo["Manufacturer"] == manufacturer2 &&
          device.additionalInfo["Model"] == model2
      }
    val testCaseMatcher1 =
      ArgumentMatcher<AndroidTestCase> { testCase ->
        testCase?.methodName == testMethod1 && testCase.className == testClass1 && testCase.packageName == TEST_PACKAGE_NAME
      }
    val testCaseMatcher2 =
      ArgumentMatcher<AndroidTestCase> { testCase ->
    ...
    verify(mockListener).onTestSuiteStarted(argThat(deviceMatcher1), argThat { testCaseCount == 1 })
    verify(mockListener).onTestSuiteStarted(argThat(deviceMatcher2), argThat { testCaseCount == 1 })
    ...
    verify(mockListener).onTestSuiteFinished(argThat(deviceMatcher1), argThat { result == AndroidTestSuiteResult.PASSED })
    verify(mockListener).onTestSuiteFinished(argThat(deviceMatcher2), argThat { result == AndroidTestSuiteResult.FAILED })
  }
  ...
  fun importManagedDevice() {
    ...
    AndroidTestDeviceInfoProto.AndroidTestDeviceInfo.newBuilder()
      ...
      .build()
      .writeTo(deviceInfoProtoFile.outputStream())
    ...
    TestSuiteResultProto.TestSuiteResult.newBuilder()
      .addTestResult(
        TestResultProto.TestResult.newBuilder()
          .setTestCase(
            TestCaseProto.TestCase.newBuilder().setTestClass(testClass).setTestPackage(TEST_PACKAGE_NAME).setTestMethod(testMethod)
          )
          .setTestStatus(TestStatusProto.TestStatus.PASSED)
          .addOutputArtifact(
            TestArtifactProto.Artifact.newBuilder().apply {
              label = labelBuilder.setLabel("device-info").setNamespace("android").build()
              sourcePath = sourcePathBuilder.setPath(deviceInfoProtoFile.absolutePath).build()
            }
          )
      )
      .build()
    ...
    verify(mockListener).onTestSuiteFinished(argThat(deviceMatcher), argThat { result == AndroidTestSuiteResult.PASSED })
  }

  @Test
  fun importBenchmarkTest() {
    val benchmarkMessageFile = temporaryFolder.newFile("benchmarkMessage.txt").apply { writeText("benchmarkMessage") }
    val benchmarkTraceFile = temporaryFolder.newFile("benchmarkTraceFile.trace").apply { writeText("benchmarkTrace") }
