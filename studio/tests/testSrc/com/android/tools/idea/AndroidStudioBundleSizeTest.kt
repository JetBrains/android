 ...
 *
 * $platform.files - total number of files $platform.total[.zip] - total uncompressed [ compressed ] size $platform.plugin[.zip].$name -
 * uncompressed [ compressed ] size per android plugin $platform.plugin_pt[.zip].$name - uncompressed [ compressed ] size per platform
 * plugin $platform.platform[.zip] - total uncompressed [ compressed ] size
 *
  ...
  fun testLoggingBinarySize(): Unit = runBlocking {
    ...
    val analyzer =
      WindowDeviationAnalyzer.Builder()
        ...
        .addMeanTolerance(WindowDeviationAnalyzer.MeanToleranceParams.Builder().setMeanCoeff(0.01).setStddevCoeff(0.0).build())
        .build()
