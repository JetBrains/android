    ...
    def test_version_matches_resources_jar(self):
        ...
        for platform, path in ides_map().items():
            ...
            with self.subTest(msg=platform):
                ...
                self.assertEqual(set(spec["jars"] + spec["jars_" + platform]), set(ide.platform_jars))
                plugins = defaultdict(set)
                for id, jars in list(spec["plugin_jars"].items()) + list(spec["plugin_jars_" + platform].items()):
                    if jars:
                        plugins[id] |= set(jars)
