...
/** Extract a number of statements about completion from the given text. Those statements should be asserted during test execution. */
object ExpectedCompletionUtils {
        ...
        companion object {
            ...
            val validKeys: Set<String> =
                setOf(
                    ...
                    PRESENTATION_TYPE_TEXT,
                    PRESENTATION_TAIL_TEXT,
    ...
    private val KNOWN_PREFIXES: List<String> =
        ImmutableList.of(
            "LANGUAGE_VERSION:",
            EXIST_LINE_PREFIX,
            ABSENT_LINE_PREFIX,
    ...
    fun assertNotContainsRenderedItems(unexpected: Array<CompletionProposal>, items: Array<LookupElement>) {
        ...
        for (unexpectedProposal in unexpected) {
            for (proposal in itemsInformation) {
                Assert.assertFalse("Unexpected '$unexpectedProposal' presented in\n$allItemsString", proposal.matches(unexpectedProposal))
            }
        }
    }
