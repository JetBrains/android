...
abstract class AbstractQuickFixMultiFileTest : LightJavaCodeInsightFixtureAdtTestCase() {
                    ...
                    try {
                        ...
                        if (actionShouldBeAvailable) {
                            ...
                            for (file in myFixture.file.containingDirectory.files) {
                                ...
                                if (fileName == myFixture.file.name || !fileName.startsWith(extraFileNamePrefix(myFixture.file.name)))
                                    continue
    ...
    companion object {
        private fun getActionsTexts(availableActions: List<IntentionAction>): List<String> = availableActions.map(IntentionAction::getText)
        ...
        fun doAction(
            ...
            val pattern =
                if (text.startsWith("/")) Pattern.compile(text.substring(1, text.length - 1))
                else Pattern.compile(StringUtil.escapeToRegexp(text))
