...
class ModuleTemplateDataBuilder(
  ...
  fun setBuildVersion(buildVersion: AndroidVersionsInfo.VersionItem, project: Project) {
    ...
    themesData =
      ThemesData(
        appName = getAppNameForTheme(project.name)
      ) // New modules always have a theme (unless its a library, but it will have no activity)
...
fun getExistingModuleTemplateDataBuilder(module: Module): ModuleTemplateDataBuilder {
  ...
  val projectStateBuilder =
    ProjectTemplateDataBuilder(false).apply {
