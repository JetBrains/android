...
/**
 * This class describes the logic of how build output Downloads info page [DownloadsInfoExecutionConsole] is updated. There should be
 * strictly 1 to 1 relation between the view and this model.
 */
...
data class DownloadRequestKey(val startTimestamp: Long, val url: String)
...
data class DownloadRequestItem(
  ...
  val failed: Boolean
    get() = !failureMessage.isNullOrBlank()
}

class RepositoryTableItem(val repository: DownloadsAnalyzer.Repository?) {
  private val requestsMap = mutableMapOf<DownloadRequestKey, DownloadRequestItem>()
  val requests: List<DownloadRequestItem>
    get() = requestsMap.values.toList()
  ...
  fun updateRequests(downloadRequests: List<DownloadRequestItem>) {
    downloadRequests.forEach { requestsMap[it.requestKey] = it }
    ...
    columnInfos =
      arrayOf(
        ...
        column("Requests", "Total number of requests.") {
          ...
          if (runningRequests > 0) "$totalRequests ($runningRequests running)" else totalRequests.toString()
        },
        ...
        column("Failed Requests Time", "Total amount of time taken to execute failed requests.") {
          StringUtil.formatDuration(it.timeOfFailed)
        },
      )
  }
...
class RequestsTableModel : ListTableModel<DownloadRequestItem>() {
  ...
  val fileNameColumn =
    object : ColumnInfo<DownloadRequestItem, String>("File") {
  ...
  val timeColumn =
    object : ColumnInfo<DownloadRequestItem, String>("Time") {
  ...
  val sizeColumn =
    object : ColumnInfo<DownloadRequestItem, String>("Size") {
  ...
  val speedColumn =
    object : ColumnInfo<DownloadRequestItem, String>("Avg Speed") {
  ...
  class Status(val text: String, val icon: Icon, val tooltip: String) {
    override fun toString(): String = text
  }

  val statusColumn =
    object : ColumnInfo<DownloadRequestItem, Status>("Status") {
      val columnCellRenderer =
        object : ColoredTableCellRenderer() {
          override fun customizeCellRenderer(table: JTable, value: Any?, selected: Boolean, hasFocus: Boolean, row: Int, column: Int) {
            if (value is Status) {
      ...
      override fun valueOf(item: DownloadRequestItem): Status =
        when {
  ...
  init {
    columnInfos = arrayOf(statusColumn, fileNameColumn, timeColumn, sizeColumn, speedColumn)
    isSortable = true
  }
