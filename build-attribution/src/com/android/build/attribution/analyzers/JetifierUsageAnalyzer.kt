...
fun checkJetifierResultFile(buildRequestData: GradleBuildInvoker.Request.RequestData): File =
  FileUtils.join(getAgpAttributionFileDir(buildRequestData), SdkConstants.FD_BUILD_ATTRIBUTION, "checkJetifierResult.json")

/** Minimal AGP version that supports running checkJetifier task. */
private val minAGPVersion = AgpVersion.parse("7.1.0-beta01")

class JetifierUsageAnalyzer(cachedCheckJetifierResultData: CheckJetifierResultData?) :
  BaseAnalyzer<JetifierUsageAnalyzerResult>(), PostBuildProcessAnalyzer {
  ...
  override fun calculateResult(): JetifierUsageAnalyzerResult {
    if (!shouldAnalyzerRun)
      return JetifierUsageAnalyzerResult(AnalyzerNotRun, checkJetifierResultData?.lastCheckJetifierBuildTimestamp, false)
    if (enableJetifierFlagState == true && useAndroidXFlagState == true) {
      return checkJetifierResultData?.checkJetifierResult?.let {
        if (it.isEmpty())
          JetifierUsageAnalyzerResult(JetifierCanBeRemoved, checkJetifierResultData?.lastCheckJetifierBuildTimestamp, isCheckJetifierBuild)
        else
          JetifierUsageAnalyzerResult(
            JetifierRequiredForLibraries(it),
            checkJetifierResultData?.lastCheckJetifierBuildTimestamp,
            isCheckJetifierBuild,
          )
...
data class CheckJetifierResultData(val lastCheckJetifierBuildTimestamp: Long?, val checkJetifierResult: CheckJetifierResult?)
