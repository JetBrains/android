...
data class StudioProvidedInfo(
  ...
  val isInConfigurationCacheTestFlow: Boolean
    get() = buildInvocationType == BuildInvocationType.CONFIGURATION_CACHE_TRIAL
  ...
  companion object {
    ...
    fun fromProject(project: Project, buildRequest: BuildRequestHolder, buildInvocationType: BuildInvocationType) =
      StudioProvidedInfo(
        ...
        configurationCachingGradlePropertyState =
    ...
    fun turnOnConfigurationCacheInProperties(project: Project, buildRequestData: RequestData, useStableFeatureProperty: Boolean) {
      getProjectProperties(project, buildRequestData, createIfNotExists = true)?.let { propertiesFile ->
        ...
        val property =
          WriteCommandAction.writeCommandAction(project, propertiesFile.containingFile).compute<IProperty, Throwable> {
            propertiesFile.findPropertyByKey(propertyName)?.apply { setValue("true") } ?: propertiesFile.addProperty(propertyName, "true")
          }
    ...
    private fun getUserPropertiesConfigurationCachePropertyState(project: Project): String? {
      return runCatching { PropertiesFiles.getProperties(GradleProjectSystemUtil.getUserGradlePropertiesFile(project)) }
        .getOrNull()
        ?.getConfigurationCachePropertyState()
    }

    private fun getProjectPropertiesConfigurationCachePropertyState(buildRequestData: RequestData): String? {
      return runCatching { PropertiesFiles.getProperties(buildRequestData.rootProjectPath.resolve(SdkConstants.FN_GRADLE_PROPERTIES)) }
        .getOrNull()
        ?.getConfigurationCachePropertyState()
    }

    private fun Properties.getConfigurationCachePropertyState() =
      sequence {
