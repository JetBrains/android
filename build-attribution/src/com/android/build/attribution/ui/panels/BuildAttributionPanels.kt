...
fun taskDetailsPage(taskData: TaskUiData, actionHandlers: ViewActionHandlers): JComponent =
  JPanel().apply {
    ...
    .apply {
      ...
      when {
        taskData.onLogicalCriticalPath ->
          this.add("This task frequently determines build duration because of dependencies")
            .newline()
            .add("between its inputs/outputs and other tasks.")
            .newline()
        taskData.onExtendedCriticalPath ->
          this.add("This task occasionally determines build duration because of parallelism constraints")
            .newline()
            .add("introduced by number of cores or other tasks in the same module.")
            .newline()
      }
      newline()
      addBold("Duration:")
        .addHtml("  ${taskData.executionTime.durationStringHtml()} / ${taskData.executionTime.percentageStringHtml()}")
        .newline()
...
private fun HtmlBuilder.createWarningsSection(taskData: TaskUiData, actionHandlers: ViewActionHandlers, linksHandler: HtmlLinksHandler) {
  ...
  if (taskData.issues.isEmpty() && taskData.relatedTaskCategoryIssues.isEmpty()) {
    ...
    if (taskData.sourceType != PluginSourceType.BUILD_SCRIPT && taskData.issues.isNotEmpty()) {
      val generateReportLink =
        linksHandler.actionLink("Generate report", "generateReport") { actionHandlers.generateReportClicked(taskData) }
    ...
    if (taskData.relatedTaskCategoryIssues.isNotEmpty()) {
      val redirectLink =
        linksHandler.actionLink(taskData.primaryTaskCategory.displayName(), taskData.primaryTaskCategory.name) {
          actionHandlers.redirectToTaskCategoryWarningsPage(taskData.primaryTaskCategory)
        }
      addTableRow(
        warnIconHtml,
        "This task is impacted by ${taskData.relatedTaskCategoryIssues.size.withPluralization("issue")} found in the $redirectLink category.",
      )
    }
