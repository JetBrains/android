...
interface WarningsDataPageModel {
  ...
  /**
   * Selects node in a tree to provided. Provided node object should be the one created in this model and exist in the trees it holds. Null
   * means changing to empty selection. Notifies listener if model state changes.
   */
...
class WarningsDataPageModelImpl(private val reportData: BuildAttributionReportUiData) : WarningsDataPageModel {
  ...
  override val treeHeaderText: String
    get() =
      treeStructure.treeStats.let { treeStats ->
  ...
  private val treeStructure = WarningsTreeStructure(reportData).apply { updateStructure(groupByPlugin, filter) }
  ...
  // TODO (mlazeba): this starts look wrong. Can we provide TreeModel instead of a root node? what are the pros and cons? what are the other
  // options?
  ...
  private var selectedPageId: WarningsPageId =
    when {
...
private class WarningsTreeStructure(val reportData: BuildAttributionReportUiData) {

  val pageIdToNode: MutableMap<WarningsPageId, WarningsTreeNode> = mutableMapOf()

  val treeStats: TreeStats = TreeStats()

  private fun treeNode(descriptor: WarningsTreePresentableNodeDescriptor) =
    WarningsTreeNode(descriptor).apply { pageIdToNode[descriptor.pageId] = this }
  ...
  fun updateStructure(groupByPlugin: Boolean, filter: WarningsFilter) {
    ...
    val taskWarnings = reportData.issues.asSequence().flatMap { it.issues.asSequence() }.filter { filter.acceptTaskIssue(it) }.toList()
    treeStats.filteredWarningsCount += taskWarnings.size

    if (groupByPlugin) {
      taskWarnings
        .groupBy { it.task.pluginName }
        .forEach { (pluginName, warnings) ->
      ...
      taskWarnings
        ...
        .forEach { (type, warnings) ->
          ...
          warnings
            ...
            .forEach { taskIssueNodeDescriptor -> warningTypeGroupingNode.add(treeNode(taskIssueNodeDescriptor)) }
        }
    }

    if (filter.showAnnotationProcessorWarnings) {
      val descriptors =
        reportData.annotationProcessors.nonIncrementalProcessors.asSequence().map { AnnotationProcessorDetailsNodeDescriptor(it) }.toList()
      if (descriptors.isNotEmpty()) {
        ...
        descriptors.forEach { annotationProcessorsRootNode.add(treeNode(it)) }
    ...
    if (filter.showConfigurationCacheWarnings && reportData.confCachingData.shouldShowWarning()) {
      ...
      warningsToAdd.add(
        treeNode(ConfigurationCachingRootNodeDescriptor(configurationCacheData, configurationDuration)).apply {
    ...
    warningsToAdd.sortedBy { it.descriptor }.forEach(treeRoot::add)
...
class WarningsTreeNode(val descriptor: WarningsTreePresentableNodeDescriptor) : DefaultMutableTreeNode(descriptor)
...
data class WarningsPageId(val pageType: WarningsPageType, val id: String) {
  companion object {
    ...
    fun annotationProcessor(annotationProcessorData: AnnotationProcessorUiData) =
      WarningsPageId(WarningsPageType.ANNOTATION_PROCESSOR_DETAILS, annotationProcessorData.className)
...
class TaskWarningTypeNodeDescriptor(val warningType: TaskIssueType, val presentedWarnings: List<TaskIssueUiData>) :
...
class TaskWarningDetailsNodeDescriptor(val issueData: TaskIssueUiData) : WarningsTreePresentableNodeDescriptor() {
...
class PluginGroupingWarningNodeDescriptor(val pluginName: String, val presentedTasksWithWarnings: Map<TaskUiData, List<TaskIssueUiData>>) :
  WarningsTreePresentableNodeDescriptor() {
...
class TaskCategoryWarningNodeDescriptor(val taskCategoryData: CriticalPathTaskCategoryUiData) : WarningsTreePresentableNodeDescriptor() {
  ...
  override val presentation: BuildAnalyzerTreeNodePresentation
    get() =
      BuildAnalyzerTreeNodePresentation(
        ...
        suffix =
          warningsCountString(taskCategoryData.getTaskCategoryIssues(TaskCategoryIssue.Severity.WARNING, forWarningsPage = true).size),
        rightAlignedSuffix = rightAlignedNodeDurationTextFromMs(executionTimeMs),
      )
...
/** Descriptor for the task warning page node. */
class TaskUnderPluginDetailsNodeDescriptor(val taskData: TaskUiData, val filteredWarnings: List<TaskIssueUiData>) :
  WarningsTreePresentableNodeDescriptor() {
...
class AnnotationProcessorsRootNodeDescriptor(val annotationProcessorsReport: AnnotationProcessorsReport) :
  WarningsTreePresentableNodeDescriptor() {
...
class AnnotationProcessorDetailsNodeDescriptor(val annotationProcessorData: AnnotationProcessorUiData) :
  WarningsTreePresentableNodeDescriptor() {
        ...
        suffix =
          when (data) {
            ...
            is IncompatiblePluginsDetected ->
              data.upgradePluginWarnings.size.let {
                when (it) {
                  0 -> ""
...
class ConfigurationCachingWarningNodeDescriptor(val data: IncompatiblePluginWarning, val projectConfigurationTime: TimeWithPercentage) :
  WarningsTreePresentableNodeDescriptor() {
...
class JetifierUsageWarningRootNodeDescriptor(val data: JetifierUsageAnalyzerResult) : WarningsTreePresentableNodeDescriptor() {
  ...
  override val presentation: BuildAnalyzerTreeNodePresentation
    get() = BuildAnalyzerTreeNodePresentation(mainText = "Jetifier")

  override val executionTimeMs = null
}

class WindowsDefenderWarningNodeDescriptor(val data: WindowsDefenderCheckService.WindowsDefenderWarningData) :
  WarningsTreePresentableNodeDescriptor() {
  ...
  override val presentation: BuildAnalyzerTreeNodePresentation
    get() = BuildAnalyzerTreeNodePresentation(mainText = "Windows Defender Active Scanning")

  override val executionTimeMs = null
}

private fun ConfigurationCachingCompatibilityProjectResult.warningsCount() =
  when (this) {
...
fun JetifierUsageAnalyzerResult.shouldShowWarning(): Boolean =
  when (this.projectStatus) {
...
private fun rightAlignedNodeDurationTextFromMs(timeMs: Long) = if (timeMs >= 100) "%.1fs".format(timeMs.toDouble() / 1000) else "<0.1s"
