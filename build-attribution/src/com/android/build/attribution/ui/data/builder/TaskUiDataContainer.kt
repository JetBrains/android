...
/**
 * This class holds [TaskUiData] representations for [TaskData] objects provided from build analyzers. Clients of this class may assume that
 * there is only one [TaskUiData] object for every [TaskData] object.
 */
class TaskUiDataContainer(
  ...
  fun getByTaskData(task: TaskData): TaskUiData =
    tasksCache.computeIfAbsent(task) {
      object : TaskUiData {
        override val pluginName: String = task.originPlugin.displayNameInProject(task.projectPath)
        override val sourceType: PluginSourceType =
          when {
        ...
        override val pluginUnknownBecauseOfCC: Boolean =
          task.originPlugin.pluginType == PluginData.PluginType.UNKNOWN && configurationCacheUsed
        ...
        override val relatedTaskCategoryIssues =
          taskCategoryIssuesContainer.issuesForCategory(task.primaryTaskCategory, TaskCategoryIssue.Severity.WARNING)
      }
    }
}
