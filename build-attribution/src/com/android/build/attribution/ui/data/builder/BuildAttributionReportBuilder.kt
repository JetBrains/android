...
/**
 * A Builder class for a data structure holding the data gathered by Gradle build analyzers. The data structure of the report is described
 * in UiDataModel.kt
 */
class BuildAttributionReportBuilder(
  ...
  private val taskUiDataContainer: TaskUiDataContainer =
    TaskUiDataContainer(buildAnalysisResult, issueUiDataContainer, taskCategoryIssueUiDataContainer, criticalPathDurationMs)
  ...
  fun build(): BuildAttributionReportUiData {
    ...
    return object : BuildAttributionReportUiData {
      ...
      override val criticalPathTaskCategories =
        if (showTaskCategoryInfo) {
  ...
  private fun createBuildSummary(pluginConfigurationTimeReport: ConfigurationUiData) =
    object : BuildSummary {
      override val buildFinishedTimestamp = this@BuildAttributionReportBuilder.buildAnalysisResult.getBuildFinishedTimestamp()
      override val totalBuildDuration =
        TimeWithPercentage(buildAnalysisResult.getTotalBuildTimeMs(), buildAnalysisResult.getTotalBuildTimeMs())
  ...
  private fun createCriticalPathTasks(criticalPathDuration: TimeWithPercentage) =
  ...
  private fun createCriticalPathPlugins(criticalPathDuration: TimeWithPercentage): CriticalPathPluginsUiData {
    ...
    return object : CriticalPathPluginsUiData {
      ...
      override val entries =
        buildAnalysisResult
          .getPluginsDeterminingBuildDuration()
          .map { createCriticalPathPluginUiData(taskByPlugin[it.plugin].orEmpty(), it, criticalPathDuration) }
  ...
  private fun createCriticalPathPluginUiData(
    ...
    object : CriticalPathPluginUiData {
      ...
      override val criticalPathTasks =
        criticalPathTasks.map { taskUiDataContainer.getByTaskData(it) }.sortedByDescending { it.executionTime }
  ...
  private fun createCriticalPathTaskCategories(criticalPathDuration: TimeWithPercentage): CriticalPathTaskCategoriesUiData {
    ...
    return object : CriticalPathTaskCategoriesUiData {
      ...
      override val miscStepsTime = criticalPathDuration.supplement()
      override val entries =
        taskCategoriesDeterminingBuildDuration
          .map { createCriticalPathTaskCategoryUiData(taskByTaskCategory[it.taskCategory].orEmpty(), it, criticalPathDuration) }
          .sortedByDescending { it.criticalPathDuration }
      override val warningCount = entries.sumOf { it.warningCount }
      override val infoCount = entries.sumOf { it.infoCount }
    }
  ...
  private fun createCriticalPathTaskCategoryUiData(
    ...
    object : CriticalPathTaskCategoryUiData {
      ...
      override val criticalPathDuration =
        TimeWithPercentage(taskCategoryCriticalPathBuildData.buildDuration, totalCriticalPathDuration.timeMs)
      override val criticalPathTasks =
        criticalPathTasks.map { taskUiDataContainer.getByTaskData(it) }.sortedByDescending { it.executionTime }
      ...
      override fun getTaskCategoryIssues(severity: TaskCategoryIssue.Severity, forWarningsPage: Boolean): List<TaskCategoryIssueUiData> {
        return taskCategoryIssueUiDataContainer.issuesForCategory(taskCategory, severity).filter {
