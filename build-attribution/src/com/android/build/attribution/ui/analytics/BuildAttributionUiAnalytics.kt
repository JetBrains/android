...
class BuildAttributionUiAnalytics(private val project: Project, private val uiSizeProvider: () -> Dimension?) {
  ...
  private val unknownPage: BuildAttributionUiEvent.Page =
    BuildAttributionUiEvent.Page.newBuilder().setPageType(BuildAttributionUiEvent.Page.PageType.UNKNOWN_PAGE).build()
  ...
  /**
   * Called when "Build Analyzer" tab becomes selected in Build toolwindow. If [registerOpenEventSource] was called just before this call
   * then this event will be reported using provided there value.
   */
  fun tabOpened() {
    val eventType =
      when (tabOpenEventSource) {
  ...
  /**
   * Registers what action was clicked to open Build Analyzer tab so next [tabOpened] call should report it's event as opened using that
   * action. This state will be cleared with any next event sent.
   */
  ...
  /**
   * Called when page selection changes and new page is shown to the user. Called from the action handler which should be aware of and
   * provide in the parameters what page user is navigation to and what method is used.
   */
  fun pageChange(
  ...
  fun helpLinkClicked(currentPageId: BuildAttributionUiEvent.Page, target: BuildAnalyzerBrowserLinks) =
    doLog(
      newUiEventBuilder()
  ...
  fun noGCSettingWarningSuppressed() =
    doLog(newUiEventBuilder().setEventType(BuildAttributionUiEvent.EventType.CONFIGURE_GC_WARNING_SUSPEND_CLICKED))

  fun runAgpUpgradeClicked() = doLog(newUiEventBuilder().setEventType(BuildAttributionUiEvent.EventType.UPGRADE_AGP_BUTTON_CLICKED))

  fun rerunBuildWithConfCacheClicked() =
    doLog(newUiEventBuilder().setEventType(BuildAttributionUiEvent.EventType.RERUN_BUILD_WITH_CONFIGURATION_CACHE_CLICKED))

  fun turnConfigurationCacheOnInPropertiesClicked() =
    doLog(newUiEventBuilder().setEventType(BuildAttributionUiEvent.EventType.TURN_ON_CONFIGURATION_CACHE_IN_PROPERTIES_LINK_CLICKED))

  fun updatePluginButtonClicked(duration: Duration) =
    doLog(
      newUiEventBuilder()
  ...
  fun runCheckJetifierTaskClicked(duration: Duration) =
    doLog(
      newUiEventBuilder()
  ...
  fun migrateToNonTransitiveRClassesClicked() =
    doLog(newUiEventBuilder().setEventType(BuildAttributionUiEvent.EventType.MIGRATE_NON_TRANSITIVE_R_CLASS_ACTION_CLICKED))

  fun turnJetifierOffClicked(duration: Duration) =
    doLog(
      newUiEventBuilder()
  ...
  fun findLibraryVersionDeclarationActionUsed(duration: Duration) =
    doLog(
      newUiEventBuilder()
  ...
  fun warningsFilterApplied(filter: WarningsFilter, duration: Duration) =
    doLog(
      newUiEventBuilder()
  ...
  fun tasksFilterApplied(filter: TasksFilter, duration: Duration) =
    doLog(
      newUiEventBuilder()
        .setEventType(BuildAttributionUiEvent.EventType.FILTER_APPLIED)
  ...
  fun toolWindowBalloonShown() = doLog(newUiEventBuilder().setEventType(BuildAttributionUiEvent.EventType.TOOL_WINDOW_BALLOON_SHOWN))
  ...
  private fun toPage(pageId: AnalyticsPageId): BuildAttributionUiEvent.Page = pagesVisited.computeIfAbsent(pageId) { registerPage(it) }
  ...
  /** Set new build id to be sent with the events. If previous session existed, send closing event for it. */
  ...
  fun getStateFromModel(model: BuildAnalyzerViewModel): BuildAttributionUiEvent.Page {
    return toPage(
      when (model.selectedData) {
                ...
                pageType =
                  when (model.tasksPageModel.selectedGrouping) {
                    ...
                    TasksDataPageModel.Grouping.BY_TASK_CATEGORY ->
                      BuildAttributionUiEvent.Page.PageType.TASK_CATEGORY_CRITICAL_PATH_TASKS_ROOT
                  },
                pageId = "",
              )
          }
        ...
        BuildAnalyzerViewModel.DataSet.DOWNLOADS -> AnalyticsPageId(BuildAttributionUiEvent.Page.PageType.DOWNLOADS_INFO, "")
      }
    )
  }
  ...
  data class AnalyticsPageId(val pageType: BuildAttributionUiEvent.Page.PageType, val pageId: String)

  private fun warningsFilterState(filter: WarningsFilter): List<BuildAttributionUiEvent.FilterItem> {
    return mutableListOf<BuildAttributionUiEvent.FilterItem>()
      .apply {
