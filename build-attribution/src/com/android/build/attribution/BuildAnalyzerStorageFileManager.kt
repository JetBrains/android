...
class BuildAnalyzerStorageFileManager(private val dataFolder: File) {
   ...
   * error during file storage, then an IOException is logged and False is returned. If the folder containing build results cannot be
   * resolved then False is returned and file storage is not attempted. If the process succeeds then True is returned.
   *
   * @return Boolean
   */
  @Slow
  ...
  fun getHistoricBuildResultByID(buildSessionID: String): HistoricBuildAnalysisResults {
    try {
      ...
      return BuildResultsProtoMessageConverter.convertBuildAnalysisResultsFromBytesToObject(message)
    } catch (e: Exception) {
