...
class ConfigurationCachingCompatibilityAnalyzerUnitTest(
  ...
  @Test
  fun `AGP compatible, no other plugins`() =
    test(
      TestCase(
  ...
  @Test
  fun `AGP incompatible, no other plugins`() =
    test(
      TestCase(
  ...
  @Test
  fun `No applied plugins, no buildscript dependencies, no known data`() =
    test(
      TestCase(
  ...
  @Test
  fun `No applied plugins, no buildscript dependencies`() =
    test(
      TestCase(
        pluginsApplied = listOf(),
        buildscriptDependenciesInfo = emptySet(),
  ...
  @Test
  fun `buildscript has dependency but no applied plugins, no plugins known`() =
    test(
      TestCase(
        pluginsApplied = listOf(),
  ...
  @Test
  fun `buildscript has dependency of known compatible but not applied plugin`() =
    test(
      TestCase(
        pluginsApplied = listOf(),
  ...
  @Test
  fun `buildscript has dependency of known incompatible but not applied plugin`() =
    test(
      TestCase(
        pluginsApplied = listOf(),
  ...
  @Test
  fun `buildscript has dependency of unknown applied plugin`() =
    test(
      TestCase(
  ...
  @Test
  fun `buildscript has dependency of known incompatible applied plugin`() =
    test(
      TestCase(
  ...
  @Test
  fun `buildscript has lower version dependency of known compatible applied plugin`() =
    test(
      TestCase(
        pluginsApplied = listOf(binaryPlugin("my.org.gradle.Plugin1")),
        buildscriptDependenciesInfo = setOf("my.org:plugin1-jar:0.1.0"),
  ...
  private fun test(testCaseData: TestCase) {
    ...
    whenever(pluginAnalyzer.result)
      .thenReturn(ProjectConfigurationAnalyzer.Result(emptyMap(), emptyList(), mapOf(":" to testCaseData.pluginsApplied)))
  ...
  fun testPluginAppliedInSeveralProjects() {
    ...
    Truth.assertThat(analyzer.result)
      .isEqualTo(
        IncompatiblePluginsDetected(
          incompatiblePluginWarnings = emptyList(),
          upgradePluginWarnings =
  ...
  fun testWhenCCFlagIsAlreadyOn() {
    ...
    whenever(pluginAnalyzer.result)
      .thenReturn(
        ProjectConfigurationAnalyzer.Result(emptyMap(), emptyList(), mapOf(":app" to listOf(binaryPlugin("my.org.gradle.Plugin1", ":app"))))
      )
  ...
  private fun binaryPlugin(pluginClassName: String, projectPath: String = ":", displayName: String = pluginClassName) =
    pluginContainer.getPlugin(
      object : BinaryPluginIdentifier {
