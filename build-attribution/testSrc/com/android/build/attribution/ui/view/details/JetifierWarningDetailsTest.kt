...
class JetifierWarningDetailsViewTest {
  ...
  private val mockHandlers =
    mock<ViewActionHandlers>().apply { whenever(createFindSelectedLibVersionDeclarationAction(any())).thenReturn(mock()) }

  @Test
  fun testCheckRequiredPageCreation() {
    val page =
      JetifierWarningDetailsView(JetifierUsageAnalyzerResult(JetifierUsedCheckRequired), mockHandlers, disposableRule.disposable).pagePanel
    ...
    TreeWalker(page)
      ...
      .single { it.name == "run-check-button" }
      .let {
    ...
    TreeWalker(page)
      .descendants()
      .filterIsInstance<JButton>()
      .single { it.name == "disable-jetifier-button" }
      .let {
    ...
    TreeWalker(page)
      ...
      .single { it.name == "outdated-results-banner" }
      .let { Truth.assertThat(it.isVisible).isFalse() }
    val declaredDependenciesList =
      TreeWalker(page).descendants().filterIsInstance<JBList<JetifierWarningDetailsView.DirectDependencyDescriptor>>().single()
    Truth.assertThat(declaredDependenciesList.isEmpty).isTrue()

    val header =
      TreeWalker(page).descendants().filterIsInstance<SimpleColoredComponent>().single { it.name == "declared-dependencies-header" }
  ...
  fun testJetifierCanBeRemovedPageCreation() {
    val jetifierUsageAnalyzerResult =
  ...
  fun testJetifierRequiredForLibsPageCreation() {
    ...
    fun treePresentation(dependenciesTree: Tree) =
      (dependenciesTree.model.root as DefaultMutableTreeNode)
        .preorderEnumeration()
        .asSequence()
        .drop(1) // Skip root.
    ...
    Truth.assertThat(treePresentation(dependenciesTree))
      .isEqualTo(
        """
  ...
  fun testResultsOutdatedWhenJetifierRequired() {
    ...
    TreeWalker(page)
      ...
      .filterIsInstance<JPanel>()
      .single { it.name == "outdated-results-banner" }
      .let { Truth.assertThat(it.isVisible).isTrue() }
    TreeWalker(page)
      .descendants()
      .filterIsInstance<JButton>()
      .single { it.name == "disable-jetifier-button" }
  ...
  fun testResultsOutdatedWhenJetifierCanBeRemoved() {
    val jetifierUsageAnalyzerResult =
      JetifierUsageAnalyzerResult(JetifierCanBeRemoved, lastCheckJetifierBuildTimestamp = 0, checkJetifierBuild = false)
    val page = JetifierWarningDetailsView(jetifierUsageAnalyzerResult, mockHandlers, disposableRule.disposable).pagePanel
    page.size = Dimension(600, 400)
    val ui = FakeUi(page)
    ui.layoutAndDispatchEvents()
