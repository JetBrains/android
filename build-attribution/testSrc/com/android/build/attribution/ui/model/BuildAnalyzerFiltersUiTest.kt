...
class BuildAnalyzerFiltersUiTest {
  ...
  fun testTasksFilterUiShortSummary() {
    expect.that(TasksFilter(emptySet(), showTasksWithoutWarnings = false).toUiText()).isEqualTo("No task types selected")
    expect.that(TasksFilter(emptySet(), showTasksWithoutWarnings = true).toUiText()).isEqualTo("No task types selected")

    expect.that(TasksFilter(setOf(ANDROID_PLUGIN), showTasksWithoutWarnings = true).toUiText()).isEqualTo("Android/Java/Kotlin tasks")
    expect.that(TasksFilter(setOf(BUILD_SCRIPT), showTasksWithoutWarnings = true).toUiText()).isEqualTo("Project customization tasks")
    expect.that(TasksFilter(setOf(THIRD_PARTY), showTasksWithoutWarnings = true).toUiText()).isEqualTo("Other tasks")

    expect
      .that(TasksFilter(setOf(ANDROID_PLUGIN), showTasksWithoutWarnings = false).toUiText())
      .isEqualTo("Android/Java/Kotlin tasks with warnings")
    expect
      .that(TasksFilter(setOf(BUILD_SCRIPT), showTasksWithoutWarnings = false).toUiText())
      .isEqualTo("Project customization tasks with warnings")
    expect.that(TasksFilter(setOf(THIRD_PARTY), showTasksWithoutWarnings = false).toUiText()).isEqualTo("Other tasks with warnings")

    expect
      .that(TasksFilter(setOf(ANDROID_PLUGIN, BUILD_SCRIPT), showTasksWithoutWarnings = true).toUiText())
      .isEqualTo("Android/Java/Kotlin, Project customization tasks")
    expect
      .that(TasksFilter(setOf(THIRD_PARTY, BUILD_SCRIPT), showTasksWithoutWarnings = true).toUiText())
      .isEqualTo("Project customization, Other tasks")
    expect
      .that(TasksFilter(setOf(ANDROID_PLUGIN, THIRD_PARTY, BUILD_SCRIPT), showTasksWithoutWarnings = true).toUiText())
      .isEqualTo("All tasks")

    expect
      .that(TasksFilter(setOf(ANDROID_PLUGIN, BUILD_SCRIPT), showTasksWithoutWarnings = false).toUiText())
      .isEqualTo("Android/Java/Kotlin, Project customization tasks with warnings")
    expect
      .that(TasksFilter(setOf(THIRD_PARTY, BUILD_SCRIPT), showTasksWithoutWarnings = false).toUiText())
      .isEqualTo("Project customization, Other tasks with warnings")
    expect
      .that(TasksFilter(setOf(ANDROID_PLUGIN, THIRD_PARTY, BUILD_SCRIPT), showTasksWithoutWarnings = false).toUiText())
