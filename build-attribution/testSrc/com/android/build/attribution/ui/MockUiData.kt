...
fun mockTask(
  ...
  TestTaskUiData(
    ...
    pluginName,
    taskCategory,
    TimeWithPercentage(executionTimeMs, criticalPathDurationMs),
    pluginUnknownBecauseOfCC,
  )
...
class MockUiData(
  ...
  fun mockBuildOverviewData(javaVersionUsed: Int? = null, isGarbageCollectorSettingSet: Boolean? = null) =
    object : BuildSummary {
      override val buildFinishedTimestamp =
  ...
  fun mockCriticalPathTasksUiData() =
    object : CriticalPathTasksUiData {
  ...
  fun mockCriticalPathTaskCategoriesUiData() =
    object : CriticalPathTaskCategoriesUiData {
      override val criticalPathDuration = TimeWithPercentage(criticalPathDurationMs, totalBuildDurationMs)
      override val miscStepsTime = criticalPathDuration.supplement()
      override val entries = tasksList.groupBy { it.primaryTaskCategory }.map { createTaskCategoryData(it.key, it.value) }
      override val warningCount = entries.sumOf { it.warningCount }
      override val infoCount = entries.sumOf { it.infoCount }
    }

  fun mockCriticalPathPluginsUiData() =
    object : CriticalPathPluginsUiData {
      override val criticalPathDuration = TimeWithPercentage(criticalPathDurationMs, totalBuildDurationMs)
      override val miscStepsTime = criticalPathDuration.supplement()
      override val entries: List<CriticalPathPluginUiData> = tasksList.groupBy { it.pluginName }.map { createPluginData(it.key, it.value) }
      override val warningCount: Int = entries.sumOf { it.warningCount }
      override val infoCount: Int = entries.sumOf { it.infoCount }
    }

  fun createTaskCategoryData(taskCategory: TaskCategory, tasks: List<TaskUiData>) =
      ...
      override fun getTaskCategoryIssues(severity: TaskCategoryIssue.Severity, forWarningsPage: Boolean): List<TaskCategoryIssueUiData> {
        return if (createTaskCategoryWarning && severity == TaskCategoryIssue.Severity.WARNING) {
          listOfNotNull(
              ...
              TaskCategoryIssue.JAVA_NON_INCREMENTAL_ANNOTATION_PROCESSOR.takeIf { !forWarningsPage && taskCategory == TaskCategory.JAVA },
            )
            .map {
              TaskCategoryIssueUiData(
                it,
                it.getWarningMessage(
                  annotationProcessors.nonIncrementalProcessors.map {
  ...
  fun createPluginData(name: String, tasks: List<TaskUiData>) =
  ...
  fun createIssuesGroup(type: TaskIssueType, issues: List<TaskIssueUiData>) =
    object : TaskIssuesGroup {
      override val type = type
      override val issues: List<TaskIssueUiData> = issues.sortedByDescending { it.task.executionTime }
      override val timeContribution = TimeWithPercentage(issues.map { it.task.executionTime.timeMs }.sum(), totalBuildDurationMs)
    }

  fun mockAnnotationProcessorsData() =
    object : AnnotationProcessorsReport {
      override val nonIncrementalProcessors =
        listOf(
...
fun mockDownloadsData(): DownloadsAnalyzer.ActiveResult {
  ...
  return DownloadsAnalyzer.ActiveResult(
    repositoryResults =
      listOf(
...
class TestTaskUiData(
  ...
  override val relatedTaskCategoryIssues: List<TaskCategoryIssueUiData>
    get() =
      if (primaryTaskCategory == TaskCategory.ANDROID_RESOURCES) {
        listOf(TaskCategoryIssue.NON_TRANSITIVE_R_CLASS_DISABLED).map {
          TaskCategoryIssueUiData(it, it.getWarningMessage(emptyList()), it.getLink())
        }
      } else {
        emptyList()
      }
