 ...
 *
 * The default [JTableHeader] using DarculaTableHeaderUI will unconditionally paint column divider lines between all columns. We only want
 * them where they are defined by the specified [ColumnInfo] instances.
 *
 * Draw a divider between the header and the table content see [paintBottomSeparator].
 *
 * The component keeps track of the hover column and will use the tooltipText and cursor for the component found under the mouse pointer.
 * Also: all mouse clicks will be forwarded.
 *
 * This implementation supports editing columns. However, only focus navigation (usually though the keyboard) will use column editors.
 * Currently, the component from the renderer is used as the editor. This only works as long as that component is not shared with other
 * column renderers.
 */
