...
interface FilteredPTableModel<P : PropertyItem> : PTableModel {
   ...
   * placed at the bottom the table. Adding an already existing item is a noop.
   *
 ...
 *
 * The [model] specifies where the items are retrieved from. Only the items satisfying the [itemFilter] are included in the table.
 *
 * The [refresh] method will repopulate the table with items from the available properties from the [model] applying the [itemFilter]. The
 * [deleteOperation] is applied when [removeItem] is called. If the model includes an item implementing [NewPropertyItem] that item will be
 * excluded if a corresponding matching item is found in the [model] except if [keepNewAfterFlyAway] is true, then the item will be included
 * at the end of the table after setting its name to null i.e. the new item line will be ready for the user to add another item to the
 * table. Use [allowEditing] to turn off editing completely in all cells. Use [valueEditable] to turn off editing for some values. Group
 * item and items implementing [NewPropertyItem] will be editable regardless. The [groups] specifies which item are grouped under a
 * specified group name. The items are sorted using [itemComparator].
 */
