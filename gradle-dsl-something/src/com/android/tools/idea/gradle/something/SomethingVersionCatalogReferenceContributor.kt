...
class SomethingVersionCatalogReferenceContributor : PsiReferenceContributor() {
  override fun registerReferenceProviders(registrar: PsiReferenceRegistrar) {
    registrar.registerReferenceProvider(PlatformPatterns.psiElement(SomethingProperty::class.java), SomethingVersionCatalogReferenceProvider())
  }
}

class SomethingVersionCatalogReferenceProvider : PsiReferenceProvider() {
  override fun getReferencesByElement(element: PsiElement, context: ProcessingContext): Array<PsiReference> {
    ...
    val identifiers: MutableList<SomethingIdentifier> = mutableListOf()
    element.accept(object : SomethingRecursiveVisitor() {
      override fun visitQualified(o: SomethingQualified) {
  ...
  class VersionCatalogFileReference(private val element: SomethingProperty, private val identifier: SomethingIdentifier) : PsiReference {
  ...
  class VersionCatalogTableReference(
    private val element: SomethingProperty,
    private val catalogIdentifier: SomethingIdentifier,
    private val identifier: SomethingIdentifier
  ): PsiReference {
    override fun getElement() = element
    override fun getRangeInElement(): TextRange = identifier.textRange.shiftLeft(element.textRange.startOffset)
  ...
  class VersionCatalogEntryReference(
    private val element: SomethingProperty,
    private val catalogIdentifier: SomethingIdentifier,
    private val tableIdentifier: SomethingIdentifier?,
    private val identifiers: List<SomethingIdentifier>
  ): PsiReference {
    override fun getElement() = element
    ...
    override fun handleElementRename(newElementName: String): PsiElement {
      ...
      return element.replace(SomethingPsiFactory(element.project).createProperty(string))
    }
    override fun bindToElement(newElement: PsiElement): PsiElement {
      if (newElement !is PsiNamedElement) return element
  ...
  companion object {
    ...
    fun SomethingIdentifier.getVersionCatalogFile(): PsiFile? =
