...
import com.android.tools.idea.gradle.something.psi.SomethingIdentifierOwner
...
class SomethingAnnotator : Annotator {
  ...
  override fun annotate(element: PsiElement, holder: AnnotationHolder) {
    ...
    if (parent is SomethingIdentifier) {
      ...
      val service = SchemaService.getInstance(element.project)
  ...
  private fun getPath(element: SomethingIdentifier): List<String> {
    val result = mutableListOf<String>()
    var current: PsiElement = element
    while (current.parent != null && current is SomethingElement) {
      if (current is SomethingIdentifierOwner) {
