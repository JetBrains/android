...
class SomethingCompletionContributor : CompletionContributor() {
  init {
    extend(CompletionType.BASIC, SOMETHING_IN_BLOCK_SYNTAX_PATTERN, createCompletionProvider())
  }
  ...
  private fun createCompletionProvider(): CompletionProvider<CompletionParameters> {
    return object : CompletionProvider<CompletionParameters>() {
      override fun addCompletions(parameters: CompletionParameters, context: ProcessingContext, result: CompletionResultSet) {
        ...
        val service = SchemaService.getInstance(project)
...
class EnableAutoPopupInSomethingCompletion : CompletionConfidence() {
  override fun shouldSkipAutopopup(contextElement: PsiElement, psiFile: PsiFile, offset: Int): ThreeState {
    return if (SOMETHING_IN_BLOCK_SYNTAX_PATTERN.accepts(contextElement)) ThreeState.NO
