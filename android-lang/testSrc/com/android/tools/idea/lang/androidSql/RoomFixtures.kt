...
private val roomAnnotationToClassBodyJava =
  mapOf(
    "Dao" to
      """
...
private val roomAnnotationToClassBodyKotlin =
  mapOf(
...
fun createStubRoomClassesInPath(codeInsightTestFixture: JavaCodeInsightTestFixture, path: String) {
  roomAnnotationToClassBodyJava.forEach { codeInsightTestFixture.addFileToProject(path + "/androidx/room/${it.key}.java", it.value) }
}
...
fun JavaCodeInsightTestFixture.findField(qualifiedClassName: String, fieldName: String, checkBases: Boolean = false): PsiField {
  return findClass(qualifiedClassName).findFieldByName(fieldName, checkBases)!!
}

fun JavaCodeInsightTestFixture.findMethod(qualifiedClassName: String, methodName: String, checkBases: Boolean = false): PsiMethod {
...
fun JavaCodeInsightTestFixture.addRoomEntity(qualifiedClassName: String, vararg fields: FieldDefinition): PsiClass {
  return addRoomEntity(qualifiedClassName, tableNameOverride = null, fields = *fields)
}

val JavaCodeInsightTestFixture.referenceAtCaret
  get() = file.findReferenceAt(caretOffset)!!
