...
interface LightBindingClassConfig {
  ...
  /** The exact class name that should be generated for this light binding class, potentially including an "Impl" suffix. */
  val className: String

  /** The exact qualified name that should be generated for this light binding class, potentially including an "Impl" suffix. */
   ...
   *
   * Note: Even if a binding is being generated for a target layout, its API may still include variables from alternate layouts in the
   * group. This is why we need to return the XML tags explicitly, as they may refer to a layout configuration outside of our own.
   */
  val variableTags: List<Pair<VariableData, XmlTag>>

  /**
   * Returns a list of all views with IDs that fields should be created for, e.g. `<Button android:id="@+id/example_button">` ->
   * "exampleButton", scoped by the binding layout they were defined in.
   *
  ...
  /**
   * Returns `true` if the generated light binding class should have a full method API, e.g. getters for its `<variable>` fields and other
   * static methods.
   */
  fun shouldGenerateGettersAndStaticMethods(): Boolean

  /** Returns `true` if we generate setters for a base class that should delegate to child implementation classes. */
  fun settersShouldBeAbstract(): Boolean
}

private fun BindingLayoutGroup.getAggregatedVariables(project: Project): List<Pair<VariableData, XmlTag>> {
 ...
 *
 * A "Binding" class should always be created. "BindingImpl"s should only be created if there are multiple layout configurations.
 */
data class BindingClassConfig(override val facet: AndroidFacet, private val group: BindingLayoutGroup) : LightBindingClassConfig {
  ...
  /** Returns the specialized root type if set consistently across all layout configurations, e.g. "LinearLayout", or "View" otherwise. */
 ...
 *
 * This config should only be used when there are alternate layouts defined in addition to the main one; otherwise, just use
 * [BindingClassConfig].
 */
data class BindingImplClassConfig(override val facet: AndroidFacet, private val group: BindingLayoutGroup, private val layoutIndex: Int) :
...
/**
 * Implementation of [LightBindingClassConfig] that requires all data to be known at the time of construction.
 *
 * This is being used to address b/330744400. The hope is that by evaluating this data at construction, the contained data will never be out
 * of date or fetched at the wrong time (ie, during indexing). If this works as intended, this class can be removed and the other config
