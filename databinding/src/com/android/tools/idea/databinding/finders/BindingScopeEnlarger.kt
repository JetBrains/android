...
private val LAST_RETURNED_LAYOUT_GROUPS_KEY =
  Key.create<Collection<BindingLayoutGroup>>(
...
private fun LayoutBindingModuleCache.getBindingLayoutGroupsDumbAware(
  ...
  // the groups are being fetched, so may loop in case of contention.
  while (true) {
    val storedBindingLayoutGroups = module.getUserData(LAST_RETURNED_LAYOUT_GROUPS_KEY)
    val newBindingLayoutGroups = bindingLayoutGroups

    synchronized(LAST_RETURNED_LAYOUT_GROUPS_KEY) {
      // Check that the stored groups haven't changed since we started, so that we're sure we're
      // always storing the latest value.
      if (module.getUserData(LAST_RETURNED_LAYOUT_GROUPS_KEY) == storedBindingLayoutGroups) {
        module.putUserData(LAST_RETURNED_LAYOUT_GROUPS_KEY, newBindingLayoutGroups)
