...
/**
 * This class compiles a real project with view binding that has multiple library modules with the same package as each other and the
 * layouts with the same name. The project compiles because only one of the two modules is added as a dependency. We want to make sure that
 * the IDE handles this scenario well, since the layout binding cache is project wide and was previously only keeping one class per fqcn
 * around.
 *
...
class ViewBindingSamePackageModulesTest {
  ...
  fun multipleBindingClassesCanHaveSameFqcn() {
    ...
    assertThat(bindingClassFinder.findClasses(libBindingClassName, GlobalSearchScope.everythingScope(project))).hasLength(6)
