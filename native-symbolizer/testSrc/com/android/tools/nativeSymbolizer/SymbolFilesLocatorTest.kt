...
/**
 * A specialized symbol source for these test. The thing that makes this symbol source "bad" is that it does nothing to ensure "good"
 * output. For example, by design it allows duplicate paths to be returned.
 */
...
class SymbolFilesLocatorTest {
  ...
  fun doesNotReturnDuplicateDirectories() {
    ...
    source = BadSymbolSource().add(getTestPath("arm")).add(getTestPath("arm"))
