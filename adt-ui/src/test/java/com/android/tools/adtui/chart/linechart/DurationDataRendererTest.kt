...
class DurationDataRendererTest {
  ...
  fun testCursorIsSetToDefaultWhenMouseIsOverLabel() {
    ...
    // Creates the DurationDataRenderer and forces an update, which calculates the DurationData's
    ...
    val clickRegionRect =
      durationDataRenderer.getScaledClickRegion(
        durationDataRenderer.clickRegionCache[0],
        200,
        50,
  ...
  fun testDurationDataPositioning() {
    ...
    val durationDataRenderer =
      DurationDataRenderer.Builder(durationData, Color.BLACK)
        .setIcon(mockIcon)
        .setHostInsets(Insets(5, 10, 15, 20))
    ...
    fun testPostScaled(hostWidth: Int, hostHeight: Int) =
      fun(i: Int, x: Float, y: Float) =
        validateRegion(
          durationDataRenderer.getScaledClickRegion(
            durationDataRenderer.clickRegionCache[i],
            hostWidth,
            hostHeight,
  ...
  fun testDashPhaseMatchingOnCustomConfig() {
    ...
    // updated.
    durationDataRenderer.renderLines(
      lineChart,
