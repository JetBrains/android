...
class ColorPaletteManagerTest(private val isDarkMode: Boolean) {
  ...
  /**
   * This test loads the production palette and if passes validates that the current data-colors.json is in a valid format. It is meant to
   * catch errors during presubmit when updating the palette json.
   */
  @Test
  ...
  fun validateFileHasSameNumberVariantsForEachColor() {
    assumeFalse(isDarkMode)
    colorPaletteManager.backgroundPalette.values.forEach { assertThat(it.size).isEqualTo(colorPaletteManager.numberOfTonesPerColor) }
    colorPaletteManager.foregroundPalette.values.forEach { assertThat(it.size).isEqualTo(colorPaletteManager.numberOfTonesPerColor) }
  }
  ...
  fun backgroundColorsByIndexHandlesOutOfBounds() {
    ...
    assertThat(colorPaletteManager.getBackgroundColor(-1)).isEqualTo(colorPaletteManager.getBackgroundColor(1))
    assertThat(colorPaletteManager.getBackgroundColor(numberOfColors * numberOfTones)).isEqualTo(colorPaletteManager.getBackgroundColor(0))
  }
  ...
  fun foregroundColorsByIndexHandlesOutOfBounds() {
    ...
    val numberOfColors = palette.size
    val numberOfTones = colorPaletteManager.numberOfTonesPerColor
    assertThat(colorPaletteManager.getForegroundColor(-1)).isEqualTo(colorPaletteManager.getForegroundColor(1))
    assertThat(colorPaletteManager.getForegroundColor(numberOfColors * numberOfTones)).isEqualTo(colorPaletteManager.getForegroundColor(0))
  }

  @Test
  fun getBackgroundColorByName() {
    assertThat(colorPaletteManager.getBackgroundColor("Gray", 0)).isEqualTo(colorPaletteManager.getBackgroundColor(0, 0, false))
  }
