...
class SplittingTabsStateManagerTest {
  ...
  fun getState_clientsWithState() {
    stateManager.registerToolWindow(
      createToolWindowWithStates("toolWindow1", 1, ContentInfo("tab-1-1", "state-1-1"), ContentInfo("tab-1-2", "state-1-2"))
    )
    stateManager.registerToolWindow(
      createToolWindowWithStates("toolWindow2", 0, ContentInfo("tab-2-1", "state-2-1"), ContentInfo("tab-2-2", "state-2-2"))
    )
    ...
    assertThat(state.toolWindows)
      .containsExactly(
        ToolWindowState(
          "toolWindow1",
          listOf(TabState("tab-1-1", PanelState("state-1-1")), TabState("tab-1-2", PanelState("state-1-2"))),
          selectedTabIndex = 1,
        ),
        ToolWindowState(
          "toolWindow2",
          listOf(TabState("tab-2-1", PanelState("state-2-1")), TabState("tab-2-2", PanelState("state-2-2"))),
          selectedTabIndex = 0,
        ),
      )
  }
