...
class FakeKeyboard {
  ...
  /**
   * Begins holding down the specified key. You may release it later using [release]. This method
   * will not generate `KEY_TYPED` events. For now those must be generated using [type].
   *
  ...
  fun release(keyCode: Int) {
    check(isPressed(keyCode)) {
      "Can't release key ${KeyEvent.getKeyText(keyCode)} as it's not pressed"
    }
   ...
   *   between key presses and typed characters isn't straightforward in some cases (the simplest
   *   being typing capital letters and a more complex example being the generation of e.g. chinese
   *   characters using an input method) this might be a significant undertaking to do correctly.
   */
  ...
  private fun performDownKeyEvent(keyCode: Int, event: Int) {
    check(!isPressed(keyCode)) {
      "Can't press key ${KeyEvent.getKeyText(keyCode)} as it's already pressed"
    }
