...
class FakeKeyboard {
  ...
  /**
   * Set (or clear) the component that will receive the key events. Note that if the focus is `null`, you can still press/release keys but
  ...
  /**
   * Begins holding down the specified key. You may release it later using [release]. This method will not generate `KEY_TYPED` events. For
   * now those must be generated using [type].
   *
   ...
   *
   * TODO: We should consider having these events be generated by [press], but as the mapping between key presses and typed characters isn't
   *   straightforward in some cases (the simplest being typing capital letters and a more complex example being the generation of e.g.
   *   chinese characters using an input method) this might be a significant undertaking to do correctly.
   */
  ...
  companion object {
    @JvmField val MENU_KEY_CODE = if (SystemInfo.isMac) KeyEvent.VK_META else KeyEvent.VK_CONTROL
  }
}
