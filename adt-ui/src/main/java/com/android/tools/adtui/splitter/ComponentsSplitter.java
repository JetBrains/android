...
public class ComponentsSplitter extends JPanel implements Disposable {
   ...
   */
  public ComponentsSplitter() {
    this(false);
  }

  public ComponentsSplitter(boolean vertical) {
    this(vertical, false);
  }

  public ComponentsSplitter(boolean vertical, boolean onePixelDividers) {
  ...
  private boolean lastVisible() {
    return !isNull(myLastComponent) && myLastComponent.isVisible();
  }

  private boolean innerVisible() {
    return !isNull(myInnerComponent) && myInnerComponent.isVisible();
  }

  private boolean firstVisible() {
    return !isNull(myFirstComponent) && myFirstComponent.isVisible();
  }
  ...
  private class Divider extends JPanel implements Disposable {
    ...
    protected void processMouseMotionEvent(MouseEvent e) {
      ...
      if (MouseEvent.MOUSE_DRAGGED == e.getID() && myWasPressedOnMe) {
        ...
        myPoint = SwingUtilities.convertPoint(this, e.getPoint(), ComponentsSplitter.this);
        final int size = getOrientation() ? ComponentsSplitter.this.getHeight() : ComponentsSplitter.this.getWidth();
        if (getOrientation()) {
        ...
        ComponentsSplitter.this.doLayout();
  ...
  private static boolean isNull(@Nullable Component component) {
    return NullableComponent.Check.isNull(component);
  }

  private static final Rectangle ourNullBounds = new Rectangle();

  private static void hideNull(@Nullable Component component) {
    if (component instanceof NullableComponent) {
      if (!component.getBounds().equals(ourNullBounds)) {
        component.setBounds(ourNullBounds);
        component.validate();
      }
    }
  }
