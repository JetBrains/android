...
class HorizontalSpinner<T> private constructor(_model: ListModel<T>) :
  ...
  companion object {
    ...
    fun forStrings(strings: Array<String>): HorizontalSpinner<String> {
      val model =
        object : AbstractListModel<String>() {
  ...
  private val leftButton = JButton(AllIcons.General.ArrowLeft).apply { name = "prev" }
  private val rightButton = JButton(AllIcons.General.ArrowRight).apply { name = "next" }
  ...
  private fun onModelUpdate() {
    ...
    listeners.filterIsInstance<ListSelectionListener>().forEach { it.valueChanged(e) }
  }
  ...
  private fun onNewModel() {
    ...
    // The model has changed so try call innerSetSelectedIndex to update the selected position and
    // the UI
