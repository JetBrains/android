...
class ColorPaletteManager(pallets: Array<ColorPalette>) {
  data class ColorPalette(
    val name: String,
    val light: List<ColorDefinition>,
    val dark: List<ColorDefinition>,
  )
  ...
  val foregroundPalette: LinkedHashMap<String, List<JBColor>> =
    pallets.associateByTo(LinkedHashMap(), ColorPalette::name) {
  ...
  @VisibleForTesting val numberOfTonesPerColor = pallets.first().light.size

  /**
   * Returns a color that is mapped to an index. Index values that are outside the total number of
   * colors are wrapped. The map of colors can be thought of as a table of colors by tones. Each row
   * is a new color each column is a new tone. We start at 0,0 and enumerate the table row first
   * until we run out of colors. At this point we increment our column and repeat. Example:
   * [COLORA_TONE0, COLORA_TONE1] [COLORB_TONE0, COLORB_TONE1]
   *
   * Index 0 maps to COLORA_TONE0 Index 1 maps to COLORB_TONE0 Index 2 maps to COLORA_TONE1 Index 3
   * maps to COLORB_TONE1.
   */
  fun getBackgroundColor(index: Int): JBColor = getBackgroundColor(index, false)

  fun getBackgroundColor(index: Int, isFocused: Boolean): JBColor =
    getBackgroundColor(index, 0, isFocused)
  ...
  fun getBackgroundColor(name: String, toneIndex: Int): JBColor =
    getBackgroundColor(name, toneIndex, false)

  fun getBackgroundColor(name: String, isFocused: Boolean): JBColor =
    getBackgroundColor(name, 0, isFocused)
  ...
  fun getForegroundColor(index: Int): JBColor =
    getColorFromPalette(index, 0, false, foregroundPalette)

  /** Returns the matching font color for the data color with the same name. */
  fun getForegroundColor(name: String): JBColor =
    getColorFromPalette(name, 0, false, foregroundPalette)

  /** Returns the computed grayscale version of the passed in color */
  fun toGrayscale(color: Color): Color {
    val avg =
      (0.3f * color.red / 255.0f) + (0.59f * color.blue / 255.0f) + (0.11f * color.green / 255.0f)
    return Color(avg, avg, avg)
  }

  private fun getColorFromPalette(
    index: Int,
    toneIndex: Int,
    isFocused: Boolean,
    palette: Map<String, List<JBColor>>,
  ): JBColor {
    val boundIndex =
      abs(index + numberOfTonesPerColor * toneIndex) % (palette.size * numberOfTonesPerColor)
