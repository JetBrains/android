...
class ColorPaletteManager(pallets: Array<ColorPalette>) {
  ...
  val foregroundPalette: LinkedHashMap<String, List<JBColor>> =
    pallets.associateByTo(LinkedHashMap(), ColorPalette::name) { loadColors(it, ColorDefinition::foreground) }

  @VisibleForTesting
  val backgroundPalette: LinkedHashMap<String, List<JBColor>> =
    pallets.associateByTo(LinkedHashMap(), ColorPalette::name) { loadColors(it, ColorDefinition::background) }
  ...
  /**
   * Returns a color that is mapped to an index. Index values that are outside the total number of colors are wrapped. The map of colors can
   * be thought of as a table of colors by tones. Each row is a new color each column is a new tone. We start at 0,0 and enumerate the table
   * row first until we run out of colors. At this point we increment our column and repeat. Example: [COLORA_TONE0, COLORA_TONE1]
   * [COLORB_TONE0, COLORB_TONE1]
   *
   * Index 0 maps to COLORA_TONE0 Index 1 maps to COLORB_TONE0 Index 2 maps to COLORA_TONE1 Index 3 maps to COLORB_TONE1.
   */
