...
fun <PropertyT : Any> ParsedValue<PropertyT>.getText(formatValue: PropertyT.() -> String) =
  when (this) {
...
val <T : Any> ParsedValue<T>.maybeValue: T?
  get() = (this as? ParsedValue.Set.Parsed)?.value
val <T : Any> ParsedValue<T>.maybeLiteralValue: T?
  get() = (this as? ParsedValue.Set.Parsed)?.takeIf { it.dslText === DslText.Literal }?.value
...
fun <T : Any> makeAnnotatedParsedValue(parsed: T?, dslText: DslText?, annotation: ValueAnnotation?): Annotated<ParsedValue<T>> =
  when {
...
fun <T : Any> makeParsedValue(parsed: T?, dslText: DslText?): ParsedValue<T> =
  when {
...
@Suppress("unused") fun ParsedValue.NotSet.annotated() = notSetAnnotated
