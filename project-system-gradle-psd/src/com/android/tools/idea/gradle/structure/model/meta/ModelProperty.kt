...
interface ModelPropertyCore<PropertyT : Any> : ModelPropertyParsedCore<PropertyT>, ModelPropertyResolvedCore<PropertyT> {
...
interface ModelProperty<in ModelT, PropertyT : Any, ValueT : Any, out PropertyCoreT> : ReadWriteProperty<ModelT, ParsedValue<PropertyT>> {
  /** A property description as it should appear in the UI. */
  @get:Nls val description: String
...
fun <T : Any> emptyKnownValues() =
  object : KnownValues<T> {
...
interface ModelPropertyContext<ValueT : Any> {
   ...
   * This is up to the parser to decide whether [value] is valid, invalid or is a DSL expression.
   */
  fun parseEditorText(text: String): Annotated<ParsedValue<ValueT>> =
    when {
      text.startsWith("\$\$") -> ParsedValue.Set.Parsed(value = null, dslText = DslText.OtherUnparsedDslText(text.substring(2))).annotated()
      text.startsWith("\$") -> ParsedValue.Set.Parsed<ValueT>(value = null, dslText = DslText.Reference(text.substring(1))).annotated()
 ...
 */
interface ModelSimpleProperty<in ModelT, PropertyT : Any> : ModelProperty<ModelT, PropertyT, PropertyT, ModelPropertyCore<PropertyT>>
...
interface ModelListPropertyCore<ValueT : Any> : ModelCollectionPropertyCore<List<ValueT>> {
...
fun <ValueT : Any> ModelPropertyContext<ValueT>.valueFormatter(): ValueT.() -> String = { this@valueFormatter.format(this) }
