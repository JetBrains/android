...
/**
 * A temporary node structure representing the desired tree structure. Implementing classes must override equals/hashCode methods to
 * implement structural equality support.
 */
...
val ShadowedTreeNode.childNodes: Sequence<ShadowedTreeNode>
  get() = children().asSequence().map { it as ShadowedTreeNode }

/** Initializes [node] which children of [from] and subscribes to change notifications from [from]. */
internal fun DefaultTreeModel.initializeNode(node: ShadowedTreeNode, from: ShadowNode) {
...
private fun DefaultTreeModel.updateChildrenOf(parentNode: ShadowedTreeNode, from: ShadowNode) {
  val children = from.getChildrenModels().toSet()
  val existing = parentNode.childNodes.map { it.shadowNode to it }.toMap()
  ...
  children.forEachIndexed { index, model ->
    ...
    when {
      existingNode != null -> {
        ...
        insertNodeInto(newNode.also { initializeNode(it, from = model) }, parentNode, index)
