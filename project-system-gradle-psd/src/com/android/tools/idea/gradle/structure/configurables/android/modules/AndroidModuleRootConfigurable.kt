...
class AndroidModuleRootConfigurable(context: PsContext, perspectiveConfigurable: BasePerspectiveConfigurable, module: PsAndroidModule) :
  ...
  override fun createPanel() = ModulePanel(context, module, signingConfigsModel)
...
private fun viewBindingPresent(context: PsContext, module: PsAndroidModule) =
  context.project.androidGradlePluginVersion.maybeValue?.let { AgpVersion.tryParse(it) }?.isAtLeastIncludingPreviews(3, 6, 0) ?: false

private fun dependenciesInfoPresent(context: PsContext, module: PsAndroidModule) =
  module.projectType == PsModuleType.ANDROID_APP &&
    context.project.androidGradlePluginVersion.maybeValue?.let { AgpVersion.tryParse(it) }?.isAtLeastIncludingPreviews(4, 0, 0) ?: false

fun androidModulePropertiesModel(context: PsContext, module: PsAndroidModule) =
  PropertiesUiModel(
    listOf(
      uiProperty(
        AndroidModuleDescriptors.compileSdkVersion,
        ::simplePropertyEditor,
        PSDEvent.PSDField.PROJECT_STRUCTURE_DIALOG_FIELD_MODULE_PROPERTIES_COMPILE_SDK_VERSION,
      ),
      uiProperty(
        AndroidModuleDescriptors.buildToolsVersion,
        ::simplePropertyEditor,
      ...
      uiProperty(
        AndroidModuleDescriptors.sourceCompatibility,
        ::simplePropertyEditor,
        PSDEvent.PSDField.PROJECT_STRUCTURE_DIALOG_FIELD_MODULE_PROPERTIES_SOURCE_COMPATIBILITY,
      ),
      uiProperty(
        AndroidModuleDescriptors.targetCompatibility,
        ::simplePropertyEditor,
...
fun defaultConfigPropertiesModel(isLibrary: Boolean) =
  PropertiesUiModel(
    listOfNotNull(
      if (!isLibrary)
        uiProperty(
          PsAndroidModuleDefaultConfigDescriptors.applicationId,
          ::simplePropertyEditor,
      ...
      uiProperty(
        PsAndroidModuleDefaultConfigDescriptors.versionCode,
        ::simplePropertyEditor,
        PSDEvent.PSDField.PROJECT_STRUCTURE_DIALOG_FIELD_MODULES_DEFAULTCONFIG_VERSION_CODE,
      ),
      uiProperty(
        PsAndroidModuleDefaultConfigDescriptors.versionName,
        ::simplePropertyEditor,
      ...
      uiProperty(
        PsAndroidModuleDefaultConfigDescriptors.targetSdkVersion,
        ::simplePropertyEditor,
        PSDEvent.PSDField.PROJECT_STRUCTURE_DIALOG_FIELD_MODULES_DEFAULTCONFIG_TARGET_SDK_VERSION,
      ),
      uiProperty(
        PsAndroidModuleDefaultConfigDescriptors.minSdkVersion,
        ::simplePropertyEditor,
      ...
      uiProperty(
        PsAndroidModuleDefaultConfigDescriptors.testInstrumentationRunner,
        ::simplePropertyEditor,
      ...
      uiProperty(
        PsAndroidModuleDefaultConfigDescriptors.testApplicationId,
        ::simplePropertyEditor,
