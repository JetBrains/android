...
class ParsedValueTest {
  ...
  fun parsedValueGetText() {
    ...
    assertThat(ParsedValue.Set.Parsed(null, DslText.OtherUnparsedDslText("fun1()")).getText(Any::testToString), equalTo("\$fun1()"))
  }

  @Test
  fun parsedValueGetText_unparsed() {
    assertThat(
      ParsedValue.Set.Parsed(value = null, dslText = DslText.OtherUnparsedDslText(text = "doSomething()")).getText(Any::testToString),
      equalTo("\$doSomething()"),
    )
  }
  ...
  fun makeParsedValue_parsedReference() {
    assertThat(makeParsedValue(1, DslText.Reference("var1")), equalTo<ParsedValue<*>>(ParsedValue.Set.Parsed(1, DslText.Reference("var1"))))
  }

  @Test
  fun makeParsedValue_parsedInterpolatedString() {
    assertThat(
      makeParsedValue("a and b", DslText.InterpolatedString("\$var1 and \$var2")),
  ...
  fun makeParsedValue_parsedUnparsed() {
    assertThat(
      makeParsedValue(null, DslText.OtherUnparsedDslText("doSomething()")),
  ...
  fun makeParsedValue_invalidLiteral() {
    assertThat(
      makeAnnotatedParsedValue(-1, DslText.Literal, ValueAnnotation.Error("bad")),
  ...
  fun makeParsedValue_invalidReference() {
    assertThat(
      makeAnnotatedParsedValue(null, DslText.Reference("var1"), ValueAnnotation.Error("annotation")),
      equalTo<Annotated<ParsedValue<*>>>(ParsedValue.Set.Parsed(null, DslText.Reference("var1")).annotateWithError("annotation")),
    )
  }
}
