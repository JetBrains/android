...
abstract class GmdDeviceCatalogService<T : GmdDeviceCatalogState>(
  ...
  /**
   * In the load state and get state functions, we are not guaranteed that myDeviceCatalogState is not used / modified in another thread.
   * While syncing device catalog, user can already invoke code completion (indexing is finished by then). Without the lock the
   * myDeviceCatalogState obtained in code completion contributor might be in an undefined state.
   */
  protected val myLock: Lock = ReentrantLock()

  /**
   * This function runs before Task starts in updateDeviceCatalog. Do not call from EDT as it makes expensive calls
   *
   * Return false to avoid running the updateDeviceCatalog function. Default is true
   */
  protected open fun shouldUpdate(project: Project): Boolean {
    return true
  }
  ...
  fun updateDeviceCatalog(project: Project) {
    object : Task.Backgroundable(project, "Syncing in $myServiceName", true) {
      ...
      .setCancelText("Device catalog sync stopped")
      .queue()
  }
  ...
  protected fun setState(state: T) {
    myLock.withLock { myDeviceCatalogState = state }
  }
}
