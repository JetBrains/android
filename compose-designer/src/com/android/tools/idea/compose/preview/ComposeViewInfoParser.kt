...
internal data class SourceLocationImpl(override val fileName: String, override val lineNumber: Int, override val packageHash: Int) :
  SourceLocation

/**
 * Parse the viewObject for ComposeViewAdapter. For now we use reflection to parse these information without much structuring. In the future
...
fun parseViewInfo(rootViewInfo: ViewInfo, logger: Logger): List<ComposeViewInfo> {
  try {
    ...
    // the
    // name mangling.
    val viewInfoField = viewObj::class.java.declaredMethods.single { it.name.contains("getViewInfos") }.also { it.isAccessible = true }
...
private fun parseBounds(elements: List<Any?>, logger: Logger): List<ComposeViewInfo> =
  elements.mapNotNull { item ->
    try {
      ...
      val packageHash =
        try {
          item.javaClass.getMethod("getLocation").invoke(item)?.let { it.javaClass.getMethod("getPackageHash").invoke(it) as Int }
        } catch (t: Throwable) {
          logger.warn(t)
          null
        } ?: -1
...
private fun getBound(viewInfo: Any): PxBounds {
  ...
  return PxBounds(left = getInt(leftPx), top = getInt(topPx), right = getInt(rightPx), bottom = getInt(bottomPx))
}
