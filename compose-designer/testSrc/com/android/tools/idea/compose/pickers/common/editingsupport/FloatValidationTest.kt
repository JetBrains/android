...
class FloatValidationTest {
  ...
  fun testFunctionValidateFloat() {
    ...
    // FloatValidator
    assertEquals(EDITOR_NO_ERROR, validateFloat(editedValue = "0", validateSuffix = false, canBeZero = true))
    assertEquals(EDITOR_NO_ERROR, validateFloat(editedValue = "0f", validateSuffix = false, canBeZero = true))
    assertEquals(EDITOR_NO_ERROR, validateFloat(editedValue = "500", validateSuffix = false, canBeZero = true, maxValueAllowed = 500f))
    assertEquals(EDITOR_NO_ERROR, validateFloat(editedValue = "400", validateSuffix = false, canBeZero = true, maxValueAllowed = 500f))
    assertEquals(ERROR_NOT_FLOAT, validateFloat(editedValue = "abc", validateSuffix = false, canBeZero = true))
    assertEquals(
      ERROR_GREATER_THAN_ZERO,
      validateFloat(editedValue = "-1", validateSuffix = false, canBeZero = false, maxValueAllowed = 500f),
    )
    assertEquals(
      ERROR_GREATER_THAN_ZERO,
      validateFloat(editedValue = "0", validateSuffix = false, canBeZero = false, maxValueAllowed = 500f),
    )
    val maxValue = 10f
    assertEquals(
