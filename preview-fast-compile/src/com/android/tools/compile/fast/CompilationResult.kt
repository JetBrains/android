...
sealed class CompilationResult {
  ...
  /**
   * An expected compilation error caused by normal user actions. For example, a syntax error would trigger this. This is recoverable by the
   * user updating the code.
   */
  data class CompilationError(override val e: Throwable? = null) : CompilationResult(), WithThrowable

  /**
   * An exception happened while trying to execute the request. This could have happened before the request arrive to the daemon. [e] will
   * contain the exception if available.
   */
