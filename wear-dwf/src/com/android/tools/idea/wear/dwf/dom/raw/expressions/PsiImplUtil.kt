...
/**
 * Retrieves the Declarative Watch Face [XmlFile] associated with the element. If the element is within an injected [WFFExpressionLanguage],
 * then the Declarative Watch Face file is the file the language is injected in. Otherwise, we attempt to use the current file.
 */
fun getWatchFaceFile(element: PsiElement): XmlFile? {
  val injectionHostFile = InjectedLanguageManager.getInstance(element.project).getInjectionHost(element)?.containingFile as? XmlFile
  return injectionHostFile ?: element.containingFile as? XmlFile
}

/** Returns the parent `<Complication>` tag from the Declarative Watch Face file the WFF Expression has been injected in, if any. */
fun getParentComplicationTag(element: PsiElement): XmlTag? {
  val injectedHost = InjectedLanguageManager.getInstance(element.project).getInjectionHost(element.containingFile) ?: return null
  return injectedHost.findParentInFile(withSelf = true) { (it as? XmlTag)?.name == TAG_COMPLICATION } as? XmlTag
}

/**
 * Creates a [UserConfigurationReference] for a given [literalExpr] and [watchFaceFile]. The reference will only be created if the
 * [literalExpr] is an ID or a data source.
 *
 * @see WFFExpressionLiteralExpr.isIdOrDataSource
 */
private fun userConfigurationReference(literalExpr: WFFExpressionLiteralExpr, watchFaceFile: XmlFile): UserConfigurationReference? {
  if (!literalExpr.isIdOrDataSource()) return null
  return UserConfigurationReference(literalExpr, watchFaceFile, literalExpr.text)
}

/**
 * Creates a [ReferenceTagReference] for a given [literalExpr] and [watchFaceFile]. The reference will only be created if the [literalExpr]
 * is an ID or a data source, the current WFF version is 4 or higher.`.
 *
 * @see WFFExpressionLiteralExpr.isIdOrDataSource
...
/**
 * Expressions in `<Parameter>` tags can reference a string resource. However, to do so, the whole expression must be the reference to the
 * string. If there are other elements in the expression, then the resource name will be used.
 *
 * This method returns a [TemplateParameterStringReference] if the given [literalExpr] is the only element in the expression, is inside a
 * `<Template>`'s `<Parameter>` tag and, is an ID or a string.
 *
 * @see <a href="https://developer.android.com/reference/wear-os/wff/group/part/text/formatter/template">Template</a>
 */
private fun templateParameterStringReference(literalExpr: WFFExpressionLiteralExpr): TemplateParameterStringReference? {
...
private fun isInjectedInTemplateParameterTag(element: WFFExpressionLiteralExpr): Boolean {
  val injectedHost = InjectedLanguageManager.getInstance(element.project).getInjectionHost(element.containingFile) ?: return false
...
/** Returns whether the given [WFFExpressionLiteralExpr] is an ID (i.e. a string), or a [WFFExpressionDataSource]. */
