...
class AndroidAppBundleExtractorTest {
  ...
  fun `test extract items`() = runTest {
    ...
    val extractedItems = extractor.extract(testDataPath.resolve("import/example.aab")).toList()
    ...
    assertThat(StringUtil.convertLineSeparators(extractedManifest.content))
      .isEqualTo(testDataPath.resolve("import/expected/AndroidManifest_extracted.xml").readText())
    ...
    assertThat(StringUtil.convertLineSeparators(rawWatchFace!!.text))
      .isEqualTo(testDataPath.resolve("import/expected/res/raw/watchface_extracted.xml").readText())
  }
}
