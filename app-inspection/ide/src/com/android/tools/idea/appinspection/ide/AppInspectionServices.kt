...
import com.android.tools.idea.appinspection.api.toDeployableFile
...
/**
 * Utility function extending [AppInspectionDiscovery] to help create an [AppInspectionJarCopier].
 */
fun AppInspectionDiscovery.createJarCopierForProcess(processDescriptor: ProcessDescriptor): AppInspectionJarCopier {
  val device = AndroidDebugBridge.getBridge()?.devices?.first {
    TransportServiceProxy.transportDeviceFromIDevice(it) == processDescriptor.stream.device
  } ?: throw IllegalStateException("Cannot find ADB device for process!")
