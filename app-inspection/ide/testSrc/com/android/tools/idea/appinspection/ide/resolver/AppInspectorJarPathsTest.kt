...
class AppInspectorJarPathsTest {
  ...
  @Test
  fun findJars() {
    val basePath = TestUtils.getWorkspaceFile("tools/adt/idea/app-inspection/ide/testData/cache").toPath()
    val jarPaths = AppInspectorJarPaths(basePath)
    ...
    assertThat(jarPaths.getInspectorJar(artifact1)).isEqualTo(
      AppInspectorJar("inspector.jar", artifact1DirPath, artifact1DirPath))
    assertThat(jarPaths.getInspectorJar(artifact2)).isEqualTo(
      AppInspectorJar("inspector.jar", artifact2DirPath, artifact2DirPath)
    )
