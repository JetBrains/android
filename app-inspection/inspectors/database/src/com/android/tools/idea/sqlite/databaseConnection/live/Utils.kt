...
fun ListenableFuture<SqliteInspectorProtocol.Response>.mapToColumns(executor: Executor) =
  transform(executor) { response ->
    response.query.columnNamesList.map { columnName -> ResultSetSqliteColumn(columnName, null, null, null) }
  }

/**
 * This method is used to handle synchronous errors from the on-device inspector. All errors are logged, except for recoverable errors.
 *
 * An on-device inspector can send an error as a response to a command (synchronous) or as an event (asynchronous). When detected,
 * synchronous errors are thrown as exceptions so that they become part of the usual flow for errors: they cause the futures to fail and are
 * shown in the views.
 *
...
internal fun getErrorMessage(errorContent: SqliteInspectorProtocol.ErrorContent): String {
   ...
   * 1. "Recoverable" errors are errors after which execution can continue as normal (e.g. typo in query).
