 ...
 *
 * The RowId column has different valid names: rowid, oid and _rowid_. Each one of these names stops being valid for the RowId column if the
 * table has a user-defined column using that name. e.g. CREATE TABLE t (oid TEXT), here oid is a user-defined column and can't be used to
 * get the RowId column.
 *
 * If all three names are used for user-define column it's not possible to get the RowId column, unless the table has an integer primary
 * key, in that case the name of the integer primary key column corresponds to the RowId column.
 */
...
/**
 * Returns a new [SqliteStatement], the text of which is obtained applying [func] to the text of the original [SqliteStatement]. The
 * parameters are the same of the original [SqliteStatement].
 */
