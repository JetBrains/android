...
open class OpenActionManager(private val wrapped: ActionManagerEx) : ActionManagerEx() {
  ...
  ): ActionToolbar = wrapped.createActionToolbar(place, group, horizontal, decorateButtons)
  override fun createActionToolbar(
    place: String,
    group: ActionGroup,
    horizontal: Boolean
  ): ActionToolbar = wrapped.createActionToolbar(place, group, horizontal)
  ...
  override fun registerAction(actionId: String, action: AnAction) =
    wrapped.registerAction(actionId, action)
  override fun registerAction(actionId: String, action: AnAction, pluginId: PluginId?) =
    wrapped.registerAction(actionId, action, pluginId)
  ...
  override fun replaceAction(actionId: String, newAction: AnAction) =
    wrapped.replaceAction(actionId, newAction)
  ...
  override fun tryToExecute(
    ...
    place: String?,
    now: Boolean
  ): ActionCallback = wrapped.tryToExecute(action, inputEvent, contextComponent, place, now)
  ...
  override fun addActionPopupMenuListener(
    listener: ActionPopupMenuListener,
  ...
  override fun getPluginActions(pluginId: PluginId): Array<String> =
    wrapped.getPluginActions(pluginId)
}
