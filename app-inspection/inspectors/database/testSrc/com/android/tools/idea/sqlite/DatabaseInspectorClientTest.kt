...
class DatabaseInspectorClientTest {
  ...
  @get:Rule val rule = RuleChain(projectRule, WaitForIndexRule(projectRule), disposableRule, EdtRule())

  private val openDatabaseFunction: (SqliteDatabaseId, DatabaseConnection) -> Unit = { _, _ -> openDatabaseInvoked = true }
  ...
  private val hasDatabasePossiblyChangedFunction: () -> Unit = { hasDatabasePossiblyChangedInvoked = true }
  private var hasDatabasePossiblyChangedInvoked = false

  private val handleDatabaseClosedFunction: (SqliteDatabaseId) -> Unit = { databaseId -> databaseClosedInvocations.add(databaseId) }
  ...
  fun testStartTrackingDatabaseConnectionSendsMessage_withAdditionalDatabase_notifiesError(): Unit = runBlocking {
    ...
    assertThat(ideServices.notifications)
      .containsExactly(TestNotification("Database Inspector", "Failed to inject tracker into Driver", ERROR))
  }
  ...
  fun testStartTrackingDatabaseConnectionSendsMessage_ignoreFrameworkApi() = runBlocking {
    ...
    val trackDatabasesCommand =
      Command.newBuilder().setTrackDatabases(TrackDatabasesCommand.newBuilder().setIgnoreFrameworkApi(true)).build()
  ...
  fun testDatabaseClosedCallsCallback() {
    ...
    assertThat(databaseClosedInvocations.first()).isEqualTo(SqliteDatabaseId.fromLiveDatabase("", 1))
  }

  @Test
  fun testKeepConnectionOpenSuccess() = runBlocking {
    // Prepare
    val keepDbsOpenResponse = Response.newBuilder().setKeepDatabasesOpen(SqliteInspectorProtocol.KeepDatabasesOpenResponse.newBuilder())
      ...
      Response.newBuilder()
        ...
        .setErrorOccurred(
          SqliteInspectorProtocol.ErrorOccurredResponse.newBuilder()
            .setContent(
              SqliteInspectorProtocol.ErrorContent.newBuilder()
                .setRecoverability(SqliteInspectorProtocol.ErrorRecoverability.newBuilder().setIsRecoverable(true))
                .setMessage("msg")
                .setStackTrace("stk")
            )
        )
  ...
  private data class TestNotification(val title: String, val content: String, val severity: Severity)

  private class TestIdeServices : AppInspectionIdeServicesAdapter() {
    val notifications = mutableListOf<TestNotification>()

    override fun showNotification(content: String, title: String, severity: Severity, action: AnAction?) {
