...
/** A class that facilitates two-way communication between ourselves (the host) and the app inspector that lives on the device. */
interface AppInspectorMessenger {
  /**
   * Sends a raw command using the provided [rawData]. Returns the raw response.
   *
   * This function can throw [AppInspectionConnectionException] which happens when App Inspection framework encounters an issue with the
   * underlying connection to the process. Clients must be able to handle it gracefully. An example would be the connection ended because
   * the app crashed.
   */
   ...
   *
   * Note: Once the inspector client is disposed by any means, collection won't be possible and will result in CancellationException being
   * thrown immediately.
   */
   ...
   *
   * Cancelling this scope has the side effect of disposing the inspector running on device. Likewise, exceptional events that happen to the
   * inspector (such as crash) will result in the cancellation of this scope. Therefore, calling join() on this scope is a reliable way to
   * find out when the inspector is disposed.
   */
