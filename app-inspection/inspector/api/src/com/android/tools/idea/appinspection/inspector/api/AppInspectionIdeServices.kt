 ...
 *
 * Note that this class, despite containing the word "IDE" in its name, does not belong in the `ide` module -- rather, we expect the `ide`
 * module to implement it. This interface is just an API for making requests that will make sense if we are running in the context of an
 * IDE, but can be ignored otherwise (for example, in tests).
 */
interface AppInspectionIdeServices {
   ...
   *
   * @param content Content text for this notification, which can contain html. If an `<a href=.../>` tag is present and the user clicks on
   *   it, the [hyperlinkClicked] parameter will be triggered.
   ...
   */
  @UiThread fun showNotification(content: String, title: String = "", severity: Severity = Severity.INFORMATION, action: AnAction? = null)
   ...
   *
   * This method may do expensive work to convert the [CodeLocation] to an actual navigation point before finally doing the navigation,
   * which is why it is a suspend function and should get launched on a background thread.
   */
...
open class AppInspectionIdeServicesAdapter : AppInspectionIdeServices {
  ...
  @UiThread override fun showNotification(content: String, title: String, severity: AppInspectionIdeServices.Severity, action: AnAction?) {}
