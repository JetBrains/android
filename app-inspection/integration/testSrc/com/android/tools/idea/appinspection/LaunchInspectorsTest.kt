...
class LaunchInspectorsTest {
   ...
   * after opening the App Inspection tool window.
   * <p>
   *   <pre>
   *   Test Steps:
   ...
   *   7. Open the App Inspection tool window again
   *   Verify:
   *   Project builds successfully and runs on the emulator
   *   Find specific UI components for all inspectors
   *   Find specific logs for all inspector deployments
   *   </pre>
   */
  @Test
    ...
    system.runAdb { adb ->
      system.runEmulator { emulator ->
        system.runStudio(project, watcher.dashboardName) { studio ->
          ...
          /**
           * Android Studio deploys agents to the device after opening the App Inspection tool window, which
           * may terminate the running process in RBE environment. To avoid the issue temporarily, we open
           * the tool window before launching the app.
           *
           * TODO (b/255808916): prevent process from being terminated after opening the app inspection tool window.
           */
          ...
          /**
           * The Run tool window pops up immediately after launching the application.
           * To prevent the Run tool window from covering the App Inspection tool window, we need to
           * wait until the Run tool window pops up before showing the App Inspection tool window again.
           */
          ...
          adb.runCommand("logcat") {
            // Log for Database Inspector: Accessing hidden method Landroid/database/sqlite/SQLiteDatabase
            waitForLog(".*Accessing hidden method.*SQLiteDatabase.*", 60.seconds)
            // Log for Background Task Inspector: Transformed class: android/os/PowerManager$WakeLock
