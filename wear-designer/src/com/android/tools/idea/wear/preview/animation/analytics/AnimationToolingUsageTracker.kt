...
/** Usage tracker implementation for the Wear Tile Animation tooling. */
interface AnimationToolingUsageTracker {
  ...
  companion object {
    ...
    private val MANAGER = DesignerUsageTrackerManager(::InternalAnimationToolingUsageTracker, NOP_TRACKER)
...
/** Represents a [WearTileAnimationToolingEvent] to be tracked, and uses the builder pattern to create it. */
class AnimationToolingEvent(type: WearTileAnimationToolingEvent.WearTileAnimationToolingEventType) {

  private val eventBuilder = WearTileAnimationToolingEvent.newBuilder().setType(type)
  ...
  fun build(): WearTileAnimationToolingEvent {
...
private fun AnimationToolingEvent.createAndroidStudioEvent(): AndroidStudioEvent.Builder {
  return AndroidStudioEvent.newBuilder()
    .setKind(AndroidStudioEvent.EventKind.WEAR_TILE_ANIMATION_TOOLING)
    .setWearTileAnimationToolingEvent(build())
}
