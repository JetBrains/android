...
class AdditionalClassifierArtifactsModelBuilder : ParameterizedToolingModelBuilder<AdditionalClassifierArtifactsModelParameter> {
      ...
      if (useArtifactViews) {
        ...
        componentsIds.map { componentId ->
          ...
          artifactsCollector.getAdditionalPaths(artifactId)?.let {
            if (it.javaDoc == null) {
              missingJavadoc.add(componentId)
            }
            if (it.sources.isEmpty()) {
              missingSources.add(componentId)
            }
          }
        }
  ...
  private fun getComponentIds(parameter: AdditionalClassifierArtifactsModelParameter) =
    parameter.artifactIdentifiers.map { DefaultModuleComponentIdentifier(ModuleIdentifierImpl(it.groupId, it.artifactId), it.version) }
  ...
  private fun getArtifacts(
    ...
    val resolvableConfiguration =
      project.configurations.detachedConfiguration().also {
    ...
    artifactIdentifiers.asDependencies(project).forEach { resolvableConfiguration.dependencies.add(it) }
  ...
  private fun getArtifactsWithArtifactResolutionQuery(
    ...
    val docQuery =
      project.dependencies.createArtifactResolutionQuery().forComponents(componentsIds).withArtifacts(JvmLibrary::class.java, artifactTypes)
  ...
  private fun getFile(result: ComponentArtifactsResult, clazz: Class<out Artifact>): File? {
    return result.getArtifacts(clazz).filterIsInstance(ResolvedArtifactResult::class.java).firstOrNull()?.file
  }
...
/**
 * Because of http://b/272214715, some AndroidX KMP libraries have their sources published as `"org.gradle.docstype=fake-sources"`. Except
 * for this attribute, these are valid source variants, and we should fetch them.
 */
