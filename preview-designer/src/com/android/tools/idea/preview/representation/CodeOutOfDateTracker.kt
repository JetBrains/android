...
interface CodeOutOfDateTracker : ModificationTracker {
  ...
  @TestOnly fun needsRefreshOnSuccessfulBuild(): Boolean

  @TestOnly fun buildWillTriggerRefresh(): Boolean
...
constructor(module: Module, parentDisposable: Disposable, needsRefreshCallback: () -> Unit) :
  ...
  @GuardedBy("previewFreshnessLock") private var needsRefreshOnSuccessfulBuild = true
  ...
  init {
    ...
    setupBuildListener(
      ...
      object : BuildListener {
        ...
        @GuardedBy("previewFreshnessLock") private var someConcurrentBuildFailed = false

        override fun buildStarted() {
          previewFreshnessLock.withLock {
            pendingBuildsCount = pendingBuildsCount.inc()
            // The modification count is updated here and not in 'buildSucceeded' so that the
            // changes
        ...
        override fun buildSucceeded() {
          ...
          previewFreshnessLock.withLock {
            // This build listener could be set in the middle of a build process, what could lead to
            // unintended behaviors.
            // Make sure to avoid problems related to this by keeping pendingBuildsCount
            // non-negative.
            ...
            if (pendingBuildsCount == 0) {
              // Only reset the need of refreshing the previews when every concurrent build
              // succeeded
  ...
  override fun invalidateSavedBuildStatus() {
    previewFreshnessLock.withLock { needsRefreshOnSuccessfulBuild = true }
  }
}
