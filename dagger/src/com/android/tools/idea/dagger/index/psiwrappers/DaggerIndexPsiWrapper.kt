 ...
 *
 * At indexing time, type resolution and other deeper analysis is not available. As such, the wrappers implementing this interface exist to
 * help enforce that they only use the operations that are available at indexing time. Additionally, the use of wrappers allows indexing to
 * be done in a common way for both Java and Kotlin.
 *
...
sealed interface DaggerIndexPsiWrapper {
  class KotlinFactory(ktFile: KtFile) {
    ...
    fun of(psiElement: KtAnnotationEntry): DaggerIndexAnnotationWrapper = KtAnnotationEntryWrapper(psiElement)
  ...
  class JavaFactory(psiJavaFile: PsiJavaFile) {
    ...
    fun of(psiElement: PsiAnnotation): DaggerIndexAnnotationWrapper = PsiAnnotationWrapper(psiElement)
    ...
    fun of(psiElement: PsiParameter): DaggerIndexParameterWrapper = PsiParameterWrapper(psiElement, importHelper)
