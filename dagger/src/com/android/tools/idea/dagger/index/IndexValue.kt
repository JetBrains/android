...
abstract class IndexValue {
   ...
   */
  protected abstract fun getResolveCandidates(project: Project, scope: GlobalSearchScope): Sequence<PsiElement>

  /**
   * Identifiers that search specifically for the types of [DaggerElement]s represented by this [IndexValue]. The identifiers are
   * responsible for validating that necessary conditions for defining a [DaggerElement] are met.
   *
   * As an example, this method would be responsible for checking that a Component has the correct `@Component` annotation, since the
   * results returned from the index may have had a different `@Component` annotation.
   */
