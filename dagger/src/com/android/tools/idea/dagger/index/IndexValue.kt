...
abstract class IndexValue(val dataType: DataType) {
   ...
   * @return a list of valid [DaggerElement]s, or the empty list when no elements can be found that match the given type.
   */
  fun resolveToDaggerElements(expectedPsiType: PsiType, project: Project, scope: GlobalSearchScope): List<DaggerElement> {
   ...
   *
   * The elements may or may not represent valid Dagger elements; it's not the responsibility of this method to determine that. Rather, this
   * method is just searching for elements based on whatever data it has from the index. This method is used by [resolveToDaggerElements] to
   * get a candidate set of elements, and that method will then filter based upon which elements are valid Dagger items.
   *
   * However, this method may choose to do further filtering of candidates if the information is quick and readily available. An example is
   * checking for an attribute on a [PsiClass]; that information is quickly available on the stub returned by [JavaPsiFacade], and so it's
   * okay for this method to check for a required annotation on a class and filter out candidates that don't have it.
   *
   * On the flip side, checking for an annotation on a [PsiMethod] method coming from Kotlin actually *can't* be done quickly, since it
   * requires resolving to the navigation element first. Therefore, that type of check should not be done here, in order to keep this method
   * as lightweight as possible.
   */
  protected abstract fun getResolveCandidates(project: Project, scope: GlobalSearchScope): List<PsiElement>

  /**
   * Identifiers that search specifically for the types of [DaggerElement]s represented by this [IndexValue].
   */
