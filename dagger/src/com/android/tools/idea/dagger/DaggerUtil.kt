...
private val injectedConstructorAnnotations =
  setOf(INJECT_ANNOTATION, DAGGER_VIEW_MODEL_INJECT_ANNOTATION, DAGGER_WORKER_INJECT_ANNOTATION)

/**
 * Searching for annotations is done behind an interface, allowing us to switch out implementations
...
private interface AnnotationSearchStrategy {
   ...
   * .
   */
  fun getDaggerBindsInstanceMethodsAndParametersForType(
    type: PsiType,
    scope: GlobalSearchScope,
    project: Project
  ): Collection<PsiModifierListOwner>

  /** Returns all @Inject-annotated fields of [type] within given [scope]. */
  fun getInjectedFieldsForType(
    type: PsiType,
    scope: GlobalSearchScope,
    project: Project
  ): Collection<PsiField>
  ...
  /**
   * Returns params of @Provides/@Binds/@Inject-annotated method or @Inject-annotated constructor
   * that have given [type] within given [scope].
   */
  fun getParamsOfDaggerProvidersForType(
    type: PsiType,
    scope: GlobalSearchScope,
    project: Project
  ...
  fun getDaggerProvidesMethodsForType(
    type: PsiType,
    scope: GlobalSearchScope,
    project: Project
  ...
  fun getDaggerBindsMethodsForType(
    type: PsiType,
    scope: GlobalSearchScope,
    project: Project
  ): Collection<PsiMethod>
  ...
  /**
   * Returns all the relevant methods from @AssistedFactory-annotated classes where the return type
   * matches [type], scope to the [module] and its dependents.
   */
...
private object CustomDaggerSearchStrategy : AnnotationSearchStrategy {
  override fun getClassesWithAnnotation(
    project: Project,
    annotationName: String,
    searchScope: SearchScope
  ) = DaggerAnnotatedElementsSearch.getInstance(project).searchClasses(annotationName, searchScope)

  override fun getDaggerBindsInstanceMethodsAndParametersForType(
    type: PsiType,
    scope: GlobalSearchScope,
    project: Project
  ): Collection<PsiModifierListOwner> {
  ...
  override fun getInjectedFieldsForType(
    type: PsiType,
    scope: GlobalSearchScope,
    project: Project
  ): Collection<PsiField> {
    val project = scope.project ?: return emptyList()
    return DaggerAnnotatedElementsSearch.getInstance(project)
      .searchFields(INJECT_ANNOTATION, scope, type.unboxed)
  }

  override fun getParamsOfDaggerProvidersForType(
    type: PsiType,
    scope: GlobalSearchScope,
    project: Project
  ): Collection<PsiParameter> {
    ...
    val project = scope.project ?: return emptyList()
    val search = DaggerAnnotatedElementsSearch.getInstance(project)
    return injectedConstructorAnnotations.flatMap {
      search.searchParameterOfMethodAnnotatedWith(it, scope, unboxedType)
    } +
  ...
  override fun getDaggerProvidesMethodsForType(
    ...
    val project = scope.project ?: return emptyList()
    return DaggerAnnotatedElementsSearch.getInstance(project)
      .searchMethods(DAGGER_PROVIDES_ANNOTATION, scope, type.unboxed)
  }

  override fun getDaggerBindsMethodsForType(
    type: PsiType,
    scope: GlobalSearchScope,
    project: Project
  ): Collection<PsiMethod> {
    val project = scope.project ?: return emptyList()
    return DaggerAnnotatedElementsSearch.getInstance(project)
      .searchMethods(DAGGER_BINDS_ANNOTATION, scope, type.unboxed)
  }

  override fun getAssistedFactoryMethodsForType(
    type: PsiType,
    module: Module
  ): Collection<PsiMethod> {
    val scope = module.moduleWithDependentsAndDependenciesScope()
    val psiClasses =
      DaggerAnnotatedElementsSearch.getInstance(module.project)
        .searchClasses(DAGGER_ASSISTED_FACTORY, scope)
...
private object BuiltInAnnotationSearchStrategy : AnnotationSearchStrategy {
  ...
  override fun getDaggerBindsInstanceMethodsAndParametersForType(
    ...
    return getMethodsWithAnnotation(DAGGER_BINDS_INSTANCE_ANNOTATION, scope, project).filter {
      it.returnType?.unboxed == type.unboxed
    } +
      getParametersWithAnnotation(DAGGER_BINDS_INSTANCE_ANNOTATION, scope, project).filter {
  ...
  override fun getInjectedFieldsForType(
    ...
    val annotationClass =
      JavaPsiFacade.getInstance(project).findClass(INJECT_ANNOTATION, scope) ?: return emptyList()
    return AnnotatedElementsSearch.searchPsiFields(annotationClass, scope).filter {
      it.type.unboxed == type.unboxed
    }
  }
  ...
  override fun getDaggerProvidesMethodsForType(
    ...
    return getMethodsWithAnnotation(DAGGER_PROVIDES_ANNOTATION, scope, project).filter {
  ...
  private fun getMethodsWithAnnotation(
    annotationName: String,
    scope: GlobalSearchScope,
    project: Project
  ): Query<PsiMethod> {
    val annotationClass =
      JavaPsiFacade.getInstance(project).findClass(annotationName, scope) ?: return EmptyQuery()
  ...
  private fun getParametersWithAnnotation(
    annotationName: String,
    scope: GlobalSearchScope,
    project: Project
  ): Query<PsiParameter> {
...
private val searchStrategy: AnnotationSearchStrategy
  get() =
    if (StudioFlags.DAGGER_BUILT_IN_SEARCH_ENABLED.get()) BuiltInAnnotationSearchStrategy
    else CustomDaggerSearchStrategy
...
/**
 * Returns all Dagger providers (@Provide/@Binds-annotated methods, @Inject-annotated constructors)
 * for [element].
 */
@WorkerThread
...
/**
 * Returns all Dagger consumers (see [isDaggerConsumer]) for given [type] with given [qualifierInfo]
 * within [scope].
 *
 * Null [qualifierInfo] means that binding has not qualifier or has more then one.
 */
private fun getDaggerConsumers(
  type: PsiType,
  qualifierInfo: QualifierInfo?,
  scope: GlobalSearchScope,
  project: Project
  ...
  return strategy.getInjectedFieldsForType(type, scope, project).filterByQualifier(qualifierInfo) +
    strategy
      .getParamsOfDaggerProvidersForType(type, scope, project)
      .filterByQualifier(qualifierInfo)
}
...
fun getDaggerConsumersFor(element: PsiElement): Collection<PsiVariable> {
  ...
  val javaxInjectProviderClass =
    JavaPsiFacade.getInstance(element.project).findClass(JAVAX_INJECT_PROVIDER, scope)
  if (javaxInjectProviderClass != null) {
    val javaxInjectProviderType =
      PsiElementFactory.getInstance(element.project).createType(javaxInjectProviderClass, type)
...
/**
 * Returns all the relevant methods from @AssistedFactory-annotated classes where the return type
 * matches class [constructor] belongs to.
 */
@WorkerThread
fun getDaggerAssistedFactoryMethodsForAssistedInjectedConstructor(
  ...
  // No need to check for qualifiers since AssistedFactories can only have a single abstract,
  // non-default method.
  return searchStrategy.getAssistedFactoryMethodsForType(type, module)
}

/**
 * Returns all the @AssistedInject-annotated constructors for a given @AssistedFactory abstract
 * method
 */
@WorkerThread
fun getDaggerAssistedInjectConstructorForAssistedFactoryMethod(
  element: PsiElement
): Collection<PsiMethod> {
...
private fun getDaggerInjectedConstructorsForType(type: PsiType): Collection<PsiMethod> {
  val clazz = (type as? PsiClassType)?.resolve() ?: return emptyList()
  return clazz.constructors.filter { constructor ->
    injectedConstructorAnnotations.any { constructor.hasAnnotation(it) }
  }
}

/** True if PsiField has @Inject annotation. */
private val PsiField.isInjected
  get() = hasAnnotation(INJECT_ANNOTATION)

/** True if KtProperty has @Inject annotation. */
private val KtAnnotated.isInjected
  get() = findAnnotation(FqName(INJECT_ANNOTATION)) != null
...
private val PsiElement?.isInjectedConstructor: Boolean
  get() =
    this is PsiMethod &&
      isConstructor &&
      injectedConstructorAnnotations.any { this.hasAnnotation(it) } ||
      this is KtConstructor<*> &&
        injectedConstructorAnnotations.any { this.findAnnotation(FqName(it)) != null }
 ...
 */
val PsiElement?.isDaggerProvider
  get() =
    isProvidesMethod ||
      isBindsMethod ||
      isInjectedConstructor ||
      isBindsInstanceMethodOrParameter ||
      isDaggerAssistedFactory

/** True if PsiElement is @AssistedInject-annotated constructor. */
val PsiElement?.isAssistedInjectedConstructor: Boolean
  get() =
    this is PsiMethod && isConstructor && this.hasAnnotation(DAGGER_ASSISTED_INJECT) ||
...
val PsiElement?.isDaggerConsumer: Boolean
  get() {
    return this is KtLightField && lightMemberOrigin?.originalElement?.isInjected == true ||
      ...
      this is KtParameter &&
        this.ownerFunction.isAssistedInjectedConstructor &&
        !this.isAssistedParameter ||
      this is PsiParameter &&
        declarationScope.isAssistedInjectedConstructor &&
        !this.isAssistedParameter
  }
...
internal fun PsiElement?.isClassOrObjectAnnotatedWith(annotationFQName: String): Boolean {
  return this is KtClass &&
    this !is KtEnumEntry &&
    findAnnotation(FqName(annotationFQName)) != null ||
...
internal val PsiElement?.isDaggerModule
  get() = isClassOrObjectAnnotatedWith(DAGGER_MODULE_ANNOTATION)

private val PsiMethod.isInstantiationMethod: Boolean
  get() = parameterList.isEmpty && returnType?.isSubcomponentCreatorType == false

private val KtNamedFunction.isInstantiationMethod
  get() = valueParameters.isEmpty() && psiType?.isSubcomponentCreatorType == false
...
internal val PsiElement?.isDaggerComponentInstantiationMethod: Boolean
  get() =
    this is PsiMethod && containingClass.isDaggerComponent && isInstantiationMethod ||
      this is KtNamedFunction && containingClass().isDaggerComponent && isInstantiationMethod ||
      this is KtProperty &&
        containingClass().isDaggerComponent &&
        psiType?.isSubcomponentCreatorType == false

internal val PsiElement?.isDaggerEntryPointInstantiationMethod: Boolean
  get() =
    this is PsiMethod && containingClass.isDaggerEntryPoint && isInstantiationMethod ||
      this is KtNamedFunction && containingClass().isDaggerEntryPoint && isInstantiationMethod ||
      this is KtProperty &&
        containingClass().isDaggerEntryPoint &&
        psiType?.isSubcomponentCreatorType == false

internal val PsiElement?.isDaggerComponent
  get() = isClassOrObjectAnnotatedWith(DAGGER_COMPONENT_ANNOTATION)

internal val PsiElement?.isDaggerEntryPoint
  get() = isClassOrObjectAnnotatedWith(DAGGER_ENTRY_POINT_ANNOTATION)

internal val PsiElement?.isDaggerSubcomponent
  get() = isClassOrObjectAnnotatedWith(DAGGER_SUBCOMPONENT_ANNOTATION)

internal val PsiElement?.isDaggerSubcomponentFactory
  get() = isClassOrObjectAnnotatedWith(DAGGER_SUBCOMPONENT_FACTORY_ANNOTATION)

internal val PsiElement?.isDaggerAssistedFactory
  get() = isClassOrObjectAnnotatedWith(DAGGER_ASSISTED_FACTORY)
...
private fun extractTypeAndQualifierInfo(element: PsiElement): Pair<PsiType, QualifierInfo?>? {
  var type: PsiType =
    when (element) {
      ...
      is PsiMethod ->
        if (element.isConstructor) element.containingClass?.let { toPsiType(it) }
        else element.returnType
      is KtFunction ->
        if (element is KtConstructor<*>) element.containingClass()?.toPsiType() else element.psiType
...
/**
 * Returns methods of interfaces annotated [DAGGER_COMPONENT_ANNOTATION] that return a type and have
 * a [QualifierInfo] as a [provider].
 */
@WorkerThread
fun getDaggerComponentMethodsForProvider(provider: PsiElement): Collection<PsiMethod> {
  ...
  val components =
    searchStrategy.getClassesWithAnnotation(
      provider.project,
      DAGGER_COMPONENT_ANNOTATION,
      provider.useScope
    )
...
/**
 * Returns methods of interfaces annotated [DAGGER_ENTRY_POINT_ANNOTATION] that return a type and
 * have a [QualifierInfo] as a [provider].
 */
@WorkerThread
...
/**
 * Returns interfaces annotated [DAGGER_COMPONENT_ANNOTATION] or [DAGGER_SUBCOMPONENT_ANNOTATION]
 * that are parents to a [subcomponent].
 *
 * A component is a parent of [subcomponent] if the [subcomponent] class is added to the
 * 'subcomponents' attribute of a @Module that the parent component installs.
 *
 * See [Dagger doc](https://dagger.dev/subcomponents.html).
 */
@WorkerThread
internal fun getDaggerParentComponentsForSubcomponent(
  subcomponent: PsiClass
): Collection<PsiClass> {

  val strategy = searchStrategy
  val components =
    strategy.getClassesWithAnnotation(
      ...
      strategy.getClassesWithAnnotation(
        subcomponent.project,
        DAGGER_SUBCOMPONENT_ANNOTATION,
        subcomponent.useScope
      )
 ...
 *
 * [component] is an interface annotated [DAGGER_COMPONENT_ANNOTATION] or
 * [DAGGER_SUBCOMPONENT_ANNOTATION] Subcomponents are classes in [SUBCOMPONENTS_ATTR_NAME] attribute
 * of a @Module that a [component] installs.
 *
 * See [Dagger doc](https://dagger.dev/subcomponents.html).
 */
@WorkerThread
internal fun getSubcomponents(component: PsiClass): Collection<PsiClass> {
  return getModulesForComponent(component).flatMap {
    it.getAnnotation(DAGGER_MODULE_ANNOTATION)?.getClassesFromAttribute(SUBCOMPONENTS_ATTR_NAME)
      ?: emptyList()
  }
}

/**
 * Returns classes annotated [DAGGER_MODULE_ANNOTATION] that in [SUBCOMPONENTS_ATTR_NAME] attribute
 * have subcomponents class.
 */
private fun getDaggerModulesForSubcomponent(subcomponent: PsiClass): Collection<PsiClass> {
  val modules =
    searchStrategy.getClassesWithAnnotation(
      subcomponent.project,
      DAGGER_MODULE_ANNOTATION,
      subcomponent.useScope
    )
...
/**
 * Returns true if an attribute with an [attrName] contains class with fully qualified name equals
 * [fqcn].
 *
...
/**
 * Returns Dagger-components and Dagger-modules that in a attribute “modules” and "includes"
 * respectively have a [module] class.
 *
 * Dagger-component is an interface annotated [DAGGER_COMPONENT_ANNOTATION] or
 * [DAGGER_SUBCOMPONENT_ANNOTATION]). Dagger-module is a class annotated [DAGGER_MODULE_ANNOTATION].
...
fun getUsagesForDaggerModule(module: PsiClass): Collection<PsiClass> {
  val strategy = searchStrategy
  val componentQuery =
    strategy.getClassesWithAnnotation(module.project, DAGGER_COMPONENT_ANNOTATION, module.useScope)
  val subComponentQuery =
    strategy.getClassesWithAnnotation(
      module.project,
      DAGGER_SUBCOMPONENT_ANNOTATION,
      module.useScope
  ...
  val moduleQuery =
    strategy.getClassesWithAnnotation(module.project, DAGGER_MODULE_ANNOTATION, module.useScope)
  val predicate: (PsiClass, String, String) -> Boolean =
    predicate@{ component, annotationName, attrName ->
      component
        .getAnnotation(annotationName)
        ?.isClassPresentedInAttribute(attrName, module.qualifiedName!!) == true
    }
...
/**
 * Return classes annotated [DAGGER_COMPONENT_ANNOTATION] that in a attribute
 * [DEPENDENCIES_ATTR_NAME] have a [component] class.
 */
@WorkerThread
...
private fun PsiAnnotation.getClassesFromAttribute(attrName: String): Collection<PsiClass> {
  ...
  return classes.mapNotNull {
    ((it as? PsiClassObjectAccessExpression)?.operand?.type as? PsiClassType)?.resolve()
  }
}

/** Tries to cast PsiElement to PsiClass. */
internal fun PsiElement.toPsiClass(): PsiClass? =
  when {
...
/**
 * Returns PsiClasses from value of "modules" attribute of DAGGER_COMPONENT_ANNOTATION or
 * DAGGER_SUBCOMPONENT_ANNOTATION annotations.
 */
@WorkerThread
internal fun getModulesForComponent(component: PsiClass): Collection<PsiClass> {
  val annotation =
    component.getAnnotation(DAGGER_COMPONENT_ANNOTATION)
      ?: component.getAnnotation(DAGGER_SUBCOMPONENT_ANNOTATION) ?: return emptyList()
 ...
 *
 * [this] is an interface annotated [DAGGER_COMPONENT_ANNOTATION] or
 * [DAGGER_SUBCOMPONENT_ANNOTATION].
 */
  ...
  return modules.any { module ->
    val subcomponents =
      module
        ...
        ?.getClassesFromAttribute(SUBCOMPONENTS_ATTR_NAME)
        ?: return@any false
...
private fun Module.moduleWithDependentsAndDependenciesScope(): GlobalSearchScope {
  return GlobalSearchScope.moduleWithDependentsScope(this)
