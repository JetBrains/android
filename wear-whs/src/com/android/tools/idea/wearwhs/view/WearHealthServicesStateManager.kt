...
internal interface WearHealthServicesStateManager {
  ...
  /**
   * Sets the overridden sensor value for the given capability. Null value here means there's no overridden value and WHS should use the
   * default value.
   */
  ...
  /**
   * State flow for the ongoing status updates, it's an instance of [WhsStateManagerStatus] which could be the syncing state, containing the
   * capability that is currently being synced across to the device, an error state, or an idle state.
   */
  val status: StateFlow<WhsStateManagerStatus>

  /**
   * State flow for the ongoing exercise status, emits a single boolean, true if there's an ongoing exercise on the device, false otherwise.
   */
  ...
  /**
   * Indicates whether the state is stale or not. The state is stale if a sync with the device hasn't succeeded for over a given time
   * period.
   */
 ...
 *
 * [STANDARD] corresponds to a basic set of sensors which are likely to be supported by most devices, e.g. heart rate, location. [ALL]
 * includes less common capabilities as well, such as elevation gain.
 */
...
/**
 * Progress state of the Wear Health Services tool window, tells the user what is currently being synced across to the device, if there's an
 * error, or if the state is idle.
 */
internal sealed class WhsStateManagerStatus(val idle: Boolean) {
  /** Initial state, when the tool window is first opened. State manager will wait until it's idle to execute commands. */
  object Initializing : WhsStateManagerStatus(idle = false)

  /** Busy state, non-user action is currently being performed. This could be connecting to the device, loading values from the device. */
...
/** Class representing a [CapabilityUIState] with the latest known [CapabilityState] without any user change. */
internal data class UpToDateCapabilityUIState(override val upToDateState: CapabilityState) : CapabilityUIState()

/** Class representing a [CapabilityUIState] containing pending user changes within [userState]. */
internal data class PendingUserChangesCapabilityUIState(val userState: CapabilityState, override val upToDateState: CapabilityState) :
  CapabilityUIState()
...
/**
 * Checks if a [CapabilityUIState] has any user changes that can be applied. When [ongoingExercise] is `true`, only a capability's
 * overrideable value can be changed. In this case the function will return true only if there is a pending user override value change. When
 * [ongoingExercise] is `false` only the capability availability will be considered changed by the user.
 */
