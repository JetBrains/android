...
private val WHS_STATE_KEY = Key<WearHealthServicesStateManager>(WearHealthServicesStateManager::class.java.name)
...
class OpenWearHealthServicesPanelAction : AbstractEmulatorAction(configFilter = {
  ...
  private fun showWearHealthServicesToolPopup(action: AnAction, event: AnActionEvent) {
    ...
    val stateManager = emulatorView.getOrPutUserData(WHS_STATE_KEY) {
    ...
    val workerScope: CoroutineScope = AndroidCoroutineScope(emulatorView, AndroidDispatchers.workerThread)
    val panel = createWearHealthServicesPanel(
      stateManager,
      uiScope,
      workerScope
    )
