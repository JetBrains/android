...
class ComposeClassNameCalculatorTest {
  ...
  fun setup() {
    ...
    projectRule.fixture.addFileToProject("src/Composable.kt", """
  ...
  fun testTopLevelComposableFunction_noAdditionalClassName() {
    val file = projectRule.fixture.addFileToProject("src/App.kt", """
  ...
  fun testComposableLambdaArgument_providesAdditionalClass() {
    val file = projectRule.fixture.addFileToProject("src/a/App.kt", """
      package a
      import androidx.compose.runtime.Composable
  ...
  fun testIntegrationWithExtensionPoint() {
    ...
    runReadAction {
      ...
      Assert.assertEquals(
        mapOf(
          file to "AppKt",
          file.findDescendantOfType<KtLambdaExpression>() to "ComposableSingletons\$AppKt\$lambda-1\$1"
        ),
        allClassNames
      )
    }
