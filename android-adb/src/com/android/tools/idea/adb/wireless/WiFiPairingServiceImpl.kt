...
class WiFiPairingServiceImpl(private val randomProvider: RandomProvider,
      ...
      .mapNotNull { line ->
        ...
        matchResult?.let {
          try {
            ...
            val ipAddress = withContext(diskIoThread) { InetAddress.getByName(it.groupValues[2]) }
  ...
  override suspend fun pairMdnsService(mdnsService: MdnsService, password: String): PairingResult {
    ...
    return matchResult?.let {
      try {
        val ipAddress = withContext(diskIoThread) { InetAddress.getByName(it.groupValues[1]) }
