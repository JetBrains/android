...
internal open class ScreenshotTask(
...
) : Task.Modal(project, AndroidAdbBundle.message("screenshot.action.title"), true) {
  ...
  override fun run(indicator: ProgressIndicator) {
    ...
    indicator.text = AndroidAdbBundle.message("screenshot.task.step.obtain")
    try {
      ...
      if (message == null) {
        AndroidAdbBundle.message("screenshot.error.generic", e.javaClass.name)
      }
      else {
        error = message
      }
