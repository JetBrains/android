...
/**
 * Given information about devices from three different sources (the operating system's USB system, the ADB command line, and ddmslib), this
 * method attempts to figure out which of these are referring to the same device and groups the data up into a list of
 * [DeviceCrossReference] objects, where each instance of [DeviceCrossReference] contains all the information about a single device.
 */
fun crossReference(
  usbDevices: Collection<UsbDevice>,
  ddmsDevices: Collection<IDevice>,
  adbDevices: Collection<AdbDevice>,
): List<DeviceCrossReference> {
  ...
  return (serialNumbers.map { DeviceCrossReference(snToUsb[it].orEmpty(), snToDdms[it].orEmpty(), snToAdb[it].orEmpty()) } +
