...
class LegacyAllocationCommandHandler(
  val device: IDevice,
  ...
  private fun enableAllocations(command: Commands.Command) {
    ...
    if (myInProgressTrackingInfo.containsKey(command.pid)) {
      ...
      if (success) {
        val newInfo =
          Memory.AllocationsInfo.newBuilder().setStartTime(requestTime).setEndTime(java.lang.Long.MAX_VALUE).setLegacy(true).build()
        ...
        val allocEvent =
          Common.Event.newBuilder()
            .apply {
              ...
              groupId = requestTime
              timestamp =
                requestTime // Not exactly the current time, but it's close enough and saves a query to get the actual device time.
              memoryAllocTracking = Memory.MemoryAllocTrackingData.newBuilder().apply { info = newInfo }.build()
            }
            .build()
    ...
    val statusEvent =
      Common.Event.newBuilder()
        .apply {
          ...
          memoryAllocTrackingStatus = Memory.MemoryAllocTrackingStatusData.newBuilder().apply { status = statusBuilder.build() }.build()
        }
        .build()
          ...
          Consumer { bytes ->
            val path =
              if (bytes == null || bytes.isEmpty()) {
                ...
                TransportServiceUtils.createTempFile("legacy_allocs_${lastInfo.startTime}", ".alloc", ByteString.copyFrom(bytes))
                  .absolutePath
              }
      ...
      val allocEvent =
        Common.Event.newBuilder()
          .apply {
            ...
            isEnded = true
            timestamp = requestTime // Not exactly the current time, but it's close enough and saves a query to get the actual device time.
            memoryAllocTracking = Memory.MemoryAllocTrackingData.newBuilder().apply { info = lastInfoBuilder.build() }.build()
          }
          .build()
      eventQueue.offer(allocEvent)
