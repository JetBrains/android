...
class AndroidProfilerToolWindowFactoryTest {
  ...
  fun isLibraryToolWindow() {
    val toolWindow =
      LibraryDependentToolWindow.EXTENSION_POINT_NAME.extensions.find { it.id == "Android Profiler" } ?: fail("Tool window not found")
  ...
  fun `createToolWindowContent implicitly opens the home and past recordings tab, with home tab selected`() {
    ...
    waitForCondition(5L, TimeUnit.SECONDS) { toolWindow.contentManager.contentCount == 2 }
  ...
  fun `opening the task tab with CPU task creates second tab with CPU stage`() {
    ...
    profilerToolWindow!!
      ...
      .taskHomeTabModel
      .processListModel
      .onDeviceSelection(TaskModelTestUtils.createDevice("FakeDevice", Common.Device.State.ONLINE, "13", 33))
