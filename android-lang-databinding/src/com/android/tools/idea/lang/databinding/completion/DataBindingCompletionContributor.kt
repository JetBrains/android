 ...
 */
class DataBindingCompletionContributor : CompletionContributor() {
  init {
  ...
  companion object {
    ...
    private fun autoCompleteUnqualifiedFunctions(result: CompletionResultSet) {
      result.addElement(attachTracker(LookupElementBuilder.create("safeUnbox")))
    }
    ...
    private fun populateFieldReferenceCompletions(referenceExpression: PsiElement, onlyValidCompletions: Boolean): List<LookupElement> {
      ...
      for (reference in childReferences) {
        ...
        for (psiModelField in resolvedType.allFields) {
          ...
          completionSuggestionsList.add(attachTracker(lookupBuilder))
        }
      }
      ...
      for (reference in childReferences) {
        if (reference is ModelClassResolvable) {
          ...
          for (psiModelMethod in resolvedType.allMethods) {
            ...
            val lookupBuilder = JavaLookupElementBuilder.forMethod(psiMethod, name, PsiSubstitutor.EMPTY, resolvedType.psiClass)
            completionSuggestionsList.add(attachTracker(lookupBuilder))
          }
        }
      }
    ...
    @VisibleForTesting
    fun attachTracker(element: LookupElementBuilder): LookupElement {
