 ...
 *
 * [Interaction] uses [InteractionEvent] as its event interface so all events can be used to start, update, end, or cancel. The
 * implementation of [Interaction] has to cast the received [InteractionEvent] to its acceptable event type such like [KeyTypedEvent],
 * [MousePressedEvent], [MouseWheelEvent], [DragEnterEvent], [DragOverEvent], or [DropEvent] to perform the [begin], [update], [commit], and
 * [cancel] operations. This makes [Interaction] able to handle more than one kind of event. For example, a drag interaction can also handle
 * the [KeyPressedEvent] during dragging. So it can apply different behavior when the modifiers are pressed or released during interacting.
 */
interface Interaction {

  /**
   * Returns a list of overlays, from bottom to top (where the later overlays are painted on top of earlier ones if they overlap). (The
   * first [Layer] is painted first, so the following [Layer]s may overwrite the previous [Layer]s).
   *
...
/**
 * Same as [InteractionEvent] but also provides [eventObject] field to access the original [EventObject] which is created by Swing
 * framework.
 *
...
/**
 * There is no real mouse wheel stop event in Swing framework. [GuiInputHandler] sent this event when mouse wheel is stopped scrolling for
 * [GuiInputHandler.SCROLL_END_TIME_MS] milliseconds.
 *
