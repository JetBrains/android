...
interface SceneComponentHierarchyProvider {
  /** Called by the [SceneManager] to create the initially [SceneComponent] hierarchy from the given [NlComponent]. */
  fun createHierarchy(manager: SceneManager, component: NlComponent): List<SceneComponent>

  /**
   * Call by the [SceneManager] to trigger a sync of the [NlComponent] to the given [SceneComponent]. This allows for the SceneComponent to
   * sync the latest data from the [NlModel] and update the UI representation. The method will be called when the [SceneManager] detects
   * that there is the need to sync. This could be after a render or after a model change, for example.
   */
  fun syncFromNlComponent(sceneComponent: SceneComponent)
}

/**
 * Default [SceneComponentHierarchyProvider]. It will create one [SceneComponent] per every given [NlComponent]. It will move the existing
 * components in the [SceneManager] to their correct position if they already existed.
 */
open class DefaultSceneManagerHierarchyProvider : SceneComponentHierarchyProvider {
  override fun createHierarchy(manager: SceneManager, component: NlComponent): List<SceneComponent> {
    val sceneComponent =
      manager.scene.getSceneComponent(component) ?: SceneComponent(manager.scene, component, manager.getHitProvider(component))
