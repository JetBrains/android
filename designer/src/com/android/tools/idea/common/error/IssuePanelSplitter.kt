...
class IssuePanelSplitter(
  ...
  init {
    Disposer.register(surface) {
      file?.let { IssuePanelService.getInstance(surface.project).unregisterFile(file) }
    }
    firstComponent = content
    secondComponent = null
    file?.let { IssuePanelService.getInstance(surface.project).registerFile(file, surface.name) }
  }
}
