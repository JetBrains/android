...
fun Configuration.toHtmlTooltip(): String {
  ...
  val device = device
  if (device != null) {
    tooltip.addBold("Hardware").newline().add("Device: ${device.displayName}").newline()
  ...
  tooltip
    .add("Locale: ${Locale.getLocaleLabel(locale, false)}")
    .newline()
    .add("Theme: $theme")
    .newline()
    .add("UI Mode: ${uiMode.shortDisplayValue}")
    .newline()
