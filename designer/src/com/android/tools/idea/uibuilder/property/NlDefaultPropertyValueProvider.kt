 ...
 *
 * Layoutlib will generate a map from each view object to the default value computed by each view class. Currently this is only done for a
 * few view classes example: TextView, but this map could be more extensive in the future.
 */
class NlDefaultPropertyValueProvider(private val sceneManager: SceneManager) : DefaultPropertyValueProvider {
  ...
  @GuardedBy("lookupPerformed") private val lookupPerformed = HashBasedTable.create<NlComponent, ResourceReference, ResourceValue>()

  // Store the default style values requested so far. Use this to detect default value changes.
  @GuardedBy("styleLookupPerformed") private val styleLookupPerformed = mutableMapOf<NlComponent, ResourceReference>()
  ...
  private fun lookup(component: NlComponent, reference: ResourceReference): ResourceValue? {
    val valueMap = (sceneManager as? LayoutlibSceneManager)?.renderResult?.defaultProperties?.get(component.snapshot) ?: return null
  ...
  private fun hasDefaultPropertyValuesChanged(): Boolean {
    synchronized(lookupPerformed) {
      for (cell in lookupPerformed.cellSet()) {
        val valueMap = (sceneManager as? LayoutlibSceneManager)?.renderResult?.defaultProperties?.get(cell.rowKey?.snapshot) ?: return true
  ...
  private fun hasDefaultStyleValuesChanged(): Boolean {
    synchronized(styleLookupPerformed) {
      for (entry in styleLookupPerformed) {
        val value = (sceneManager as? LayoutlibSceneManager)?.renderResult?.defaultStyles?.get(entry.key.snapshot) ?: return true
  ...
  private fun styleLookup(component: NlComponent): ResourceReference? {
    val value = (sceneManager as? LayoutlibSceneManager)?.renderResult?.defaultStyles?.get(component.snapshot) ?: return null
