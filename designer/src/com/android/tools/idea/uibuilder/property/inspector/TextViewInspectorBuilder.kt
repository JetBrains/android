 ...
 *
 * The attributes for a TextView is shown only if all the [REQUIRED_PROPERTIES] are available. The controls for [ATTR_TEXT_APPEARANCE] are
 * displayed in a collapsible section.
 *
 * There are 2 attributes that may or may not be present: [ATTR_FONT_FAMILY] and [ATTR_TEXT_ALIGNMENT] They are present if the minSdkVersion
 * is high enough or if AppCompat is used.
 */
