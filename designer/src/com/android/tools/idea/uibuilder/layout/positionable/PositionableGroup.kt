 ...
 */
class PositionableGroup(val content: List<PositionableContent>, val header: PositionableContent? = null) {
  /** If [PositionableGroup] has a [header]. */
  val hasHeader = header != null
}

val NO_GROUP_TRANSFORM: (Collection<PositionableContent>) -> List<PositionableGroup> = { listOf(PositionableGroup(it.toList())) }

/** Group [PositionableContent] into [PositionableGroup]. */
val GROUP_BY_BASE_COMPONENT: (Collection<PositionableContent>) -> List<PositionableGroup> = { contents ->
  ...
  groups.values
    .fold(Pair(mutableListOf<PositionableGroup>(), mutableListOf<PositionableContent>())) { temp, next ->
