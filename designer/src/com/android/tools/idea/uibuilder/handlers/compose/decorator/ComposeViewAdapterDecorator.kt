...
/**
 * [SceneDecorator] for the ComposeViewAdapter class, the class is used to preview Composables on
 * the DesignSurface and this [SceneDecorator] provides additional visual information, based on the
 * [DesignInfo] attached to the [SceneComponent].
 */
 ...
 *
 * The cache does not need to be updated since new [SceneComponent]s are generated every time the
 * Composable preview is updated.
 *
...
private fun addConstraints(
  ...
  // TODO(b/148788971): Find a way to relate SceneComponents to Views in this map (preferably during
  // parsing), this would allow us to
...
private fun addConnections(
  ...
  // TODO(b/148788971): Try to refactor ConstraintLayoutDecorator so that the logic can be used
  // here, using DesignInfo instead of NlModel
  //  and SceneComponents
  ...
  viewDescription.constraints.forEach { constraintInfo ->
    ...
    when (constraintInfo.targetAnchor) {
      // TODO(b/148788971): Consider making the baseline part of the [ViewDescription], for accurate
...
private fun getConnectionType(
  ...
  if (
    targetView.constraints.any {
      it.target == currentView.viewId && currentConstraint.targetAnchor == it.originAnchor
    }
  ) {
    // Connecting view connects back
    return DrawConnection.TYPE_CHAIN
  }
  val oppositeConstraint =
    currentView.constraints.firstOrNull {
      it.originAnchor == currentConstraint.originAnchor.toOppositeAnchor()
    }
...
/**
 * We currently don't have all helper information available, but we can make some assumptions to
 * offer some visual feedback.
 */
private fun drawHelperIfPossible(
  list: DisplayList,
  sceneContext: SceneContext,
  viewBox: PxBounds,
  rootBox: PxBounds
...
private fun PxBounds.toSwingRectangle(sceneContext: SceneContext) =
  Rectangle(
...
/**
 * Helpers usually only have one value set on one of their coordinates, with size zero, for better
 * constraint visualization, we extend their length to match the root view. This results in
 * constraints to the helper going towards the center of the layout instead of the edges.
 */
...
private fun Anchor.toOppositeAnchor() =
  when (this) {
...
private fun Anchor.toDrawDirection() =
  when (this) {
...
private fun Anchor.toOppositeDrawDirection() =
  when (this) {
