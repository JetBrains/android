...
class RelativeDropHandler(val myComponent: SceneComponent) {
  ...
  /**
   * Split [SdkConstants.ATTR_LAYOUT_MARGIN] and [SdkConstants.ATTR_LAYOUT_CENTER_IN_PARENT] to multiple attributes.<br> This makes the
   * determine function simple while dragging.
   */
  private fun preProcessAttribute(attributes: AttributesTransaction) {
    ...
    if (margin != null) {
      MARGINS_WITHOUT_RTL.flatMap { getProperAttributesForLayout(myComponent, it) }.forEach { attributes.setAndroidAttribute(it, margin) }
    }
  ...
  /** Calculate the align and margin on x axis. This functions should only be used when there is no Notch on X axis */
  ...
  /** Calculate the align and margin on y axis. This functions should only be used when there is no Notch on Y axis */
