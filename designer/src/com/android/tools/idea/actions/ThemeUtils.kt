...
/**
 * If the [theme] is called "Theme" or "android:Theme", returns "Theme". Otherwise, if the [theme] has prefix "android:Theme.", "Theme.", or
 * "@style/", removes it.
 */
...
fun createFilter(resolver: ThemeResolver, excludedNames: Set<String>, vararg baseThemes: StyleResourceValue): ThemeStyleFilter {
  if (baseThemes.isEmpty()) {
    ...
    return { style: ConfiguredThemeEditorStyle ->
      !excludedNames.contains(style.qualifiedName) && resolver.themeIsChildOfAny(style.styleResourceValue, *baseThemes)
    }
  }
}
...
fun getProjectThemeNames(themeResolver: ThemeResolver, filter: ThemeStyleFilter) = getFilteredNames(getProjectThemes(themeResolver), filter)
...
fun getRecentlyUsedThemes(project: Project, excludedNames: Set<String> = emptySet()) =
  PropertiesComponent.getInstance(project).getList(RECENTLY_USED_THEMES_PROPERTY)?.minus(excludedNames) ?: emptyList()
...
private fun getFilteredByPrefixSortedByName(
  ...
  themes.filter { theme -> excludedPrefixes.none({ prefix -> theme.name.startsWith(prefix) }) }.sortedBy { it.qualifiedName }
