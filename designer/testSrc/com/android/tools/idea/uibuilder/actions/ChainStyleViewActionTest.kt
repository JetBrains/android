...
class ChainStyleViewActionTest: SceneTest() {
  ...
  private fun verifyVerticalChainStyle(action: ChainStyleViewAction) {
    ...
    action.perform(
      ...
      mockHandler,
      button0,
      mutableListOf(button0, button3),
      0)

    myScreen.get("@+id/button0")
      .expectXml(
        """<Button
  ...
  /**
   * Create a model where button0 is both Horizontal && Vertical chain.
   * 0, 1, 2 are in horizontal chain
   * 0, 3 are in vertical chain
   *
