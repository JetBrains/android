...
class StyleFilterTest : AndroidTestCase() {
  ...
  override fun setUp() {
    super.setUp()
    filter = StyleFilter(myFacet, resolver)
  }
  ...
  fun testTextAppearances() {
    ...
    val styles = filter?.getStylesDerivedFrom(textAppearanceStyle) ?: failure("Missing filter")
    assertThat(styles).named(dump(styles)).containsExactly(
        resolve(auto, "Text2"),
        resolve(auto, "Text34"),
  ...
  fun testAppCompatThemes() {
    ...
    val styles = filter?.getStylesDerivedFrom(appCompat) ?: failure("Missing filter")
    assertThat(styles).named(dump(styles)).containsExactly(
  ...
  fun testTextViewStyles() {
    val styles = filter?.getWidgetStyles(SdkConstants.TEXT_VIEW) ?: failure("Missing filter")
    assertThat(styles).named(dump(styles)).containsExactly(
        resolve(auto, "Widget.AppCompat.TextView"),
        resolve(auto, "Widget.AppCompat.TextView.SpinnerItem"),
        resolve(android, "Widget.Material.TextView"),
        resolve(android, "Widget.TextView")).inOrder()
  }
