...
class CommonDragTargetTest : SceneTest() {
  ...
  fun testDragComponentInConstraintLayoutWithSnapping() {
    ...
    // The result would be 16dp due to snapped by TargetSnapper. In non-snapped case it would be
    // 10dp.
  ...
  private fun setAutoConnection(view: SceneView, component: SceneComponent, on: Boolean) {
    if (ToggleAutoConnectAction.isAutoconnectOn() != on) {
      ToggleAutoConnectAction()
        .perform(ViewEditorImpl(view), component.nlComponent.getLayoutHandler {}!!, component.parent!!.nlComponent, listOf(), 0)
    }
  }
      ...
      component(AndroidXConstants.CONSTRAINT_LAYOUT.newName())
        ...
        .children(
          ...
          component(SdkConstants.LINEAR_LAYOUT)
            ...
            .children(
              component(SdkConstants.BUTTON).withBounds(1400, 1400, 200, 200).id("@id/button").width("100dp").height("100dp"),
              component(SdkConstants.BUTTON).withBounds(1400, 1600, 200, 200).id("@id/button2").width("100dp").height("100dp"),
            ),
        ),
    )
  }
