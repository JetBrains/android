...
class CommandLatch(private val timeout: Long, private val unit: TimeUnit) {
   ...
   * Enable this latch.
   *
   * When this latch is enabled, incoming command responses will be delayed until [waitingCommand] is called. When this latch is disabled,
   * incoming command responses are received immediately.
   */
   ...
   *
   * If this latch is not [enabled] or a previous command is currently being delayed: the response will be returned immediately. Otherwise:
   * release a test that may already have called [waitForCommand] and wait for that operation to finish.
   */
  fun incomingCommand() {
