 ...
 *
 * It provides a very simple structure for handling command / response / event messages, as well as listeners for interception callbacks, so
 * tests can override behavior with finer detail.
 *
...
abstract class FakeInspector<C, R, E>(val connection: Connection<E>) {
  ...
  /**
   * A callback which is triggered with a command received by the inspector from the host, when it matches the specified [condition].
   *
   * Only the first interceptor that matches its condition is used; in other words, multiple overlapping interceptors are not allowed.
   *
   * The callback should fire any events as appropriate using the [connection] property and return an appropriate response.
   */
  ...
  /**
   * Handle a received command, mimicking code that would normally exist on the device in `Inspector.onReceiveCommand`.
   *
   * This method should only be called by internal test framework logic.
   */
  fun handleCommand(command: C): R {
    val interceptedResponse = interceptors.entries.firstOrNull { it.key(command) }?.let { entry -> entry.value(command) }
   ...
   *
   * This will be called with a target [command] unless an interceptor was already registered that handled it.
   */
