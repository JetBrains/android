...
class ResolutionElementEditor(
  ...
  private fun updateFromModel() {
    ...
    if (!hideLinkPanel) {
      ...
      classLocation?.let { linkPanel.add(SourceLocationLink(it, isSelected, false, null)) }
      when (locations.size) {
        0 -> {}
        1 -> linkPanel.add(SourceLocationLink(locations.first(), isSelected, isOverridden, null))
  ...
  private class ExpansionPanel(
    ...
    init {
      ...
      val expandIcon = JBLabel(if (isExtraPanelVisible) AllIcons.Ide.Notification.Collapse else AllIcons.Ide.Notification.Expand)
      expandIcon.addMouseListener(object : MouseAdapter() {
      ...
      mainPanel.add(SourceLocationLink(mainLocation, isSelected, isOverridden, null))
      mainPanel.add(expandIcon)

      // TODO: Replace StudioIcons.LayoutEditor.Palette.INCLUDE with a LayoutInspector icon for first indented source location:
      var icon = StudioIcons.LayoutEditor.Palette.INCLUDE
      for (index in 1 until locations.size) {
        extraPanel.add(SourceLocationLink(locations[index], isSelected, isOverridden, icon))
   ...
   */
  private class SourceLocationLink(location: SourceLocation, isSelected: Boolean, isOverridden: Boolean, icon: Icon?): JBLabel(icon) {
