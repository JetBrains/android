...
"""
      .trim()
      .replace("!", "") // This avoids the warning for a line intentional left with trailing spaces
  ...
  fun testGetAllWindowIds() {
    ...
    verify(launchMonitor)
      .updateProgress(DynamicLayoutInspectorErrorInfo.AttachErrorState.LEGACY_WINDOW_LIST_REQUESTED)
    verify(launchMonitor)
      .updateProgress(DynamicLayoutInspectorErrorInfo.AttachErrorState.LEGACY_WINDOW_LIST_RECEIVED)
  }
  ...
  fun testLoadComponentTree() {
    ...
    val expected =
      view(0x41673e3, width = 585, height = 804) {
    ...
    verify(legacyClient.launchMonitor)
      .updateProgress(DynamicLayoutInspectorErrorInfo.AttachErrorState.LEGACY_SCREENSHOT_RECEIVED)
  }
  ...
  fun testRefreshImages() = runBlocking {
    ...
    ImageDiffUtil.assertImageSimilar(
      "image1.png",
      image1,
      ViewNode.readAccess { window.root.drawChildren }
    ...
    val scaledImage1 =
      BufferedImage(image1.width / 2, image1.height / 2, BufferedImage.TYPE_INT_ARGB)
    ...
    val image2Bytes =
      TestUtils.resolveWorkspacePathUnchecked("$TEST_DATA_PATH/image2.png").readBytes()
