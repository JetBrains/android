    ...
    val eventStr =
      """
        ...
        children {
          ...
          """
        .trim() +
        (1..200).joinToString(separator = "") { "\\%1\$03o\\%1\$03o\\000\\777".format(it) } +
        """ "
        }
        children {
          id: 4
          ...
          """
          .trim() +
        (100..135).joinToString(separator = "") { "\\000\\%1\$03o\\%1\$03o\\777".format(it) } +
        """ "
        }
        children {
          id: 1
          ...
          """
          .trim() +
        (1..200).joinToString(separator = "") { "\\%1\$03o\\000\\%1\$03o\\777".format(it) } +
        """ "
        }"""
          .trim()
    val tree =
  ...
  fun testThreadSafety() {
    ...
    val shutdownThread = Thread {
      while (!hasErrors && numberOfViewTreeCalls < iterations) {
        // Starting a server is so slow that all the shutdown calls could finish without a single
