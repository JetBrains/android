...
private val PROFILE_SLOW_TASKS_TIMEOUT_MS = Integer.getInteger("layoutlib.thread.profile.timeoutms", 4000)
...
data class ThreadProfileSettings(
  ...
  companion object {
    val disabled = ThreadProfileSettings(maxSamples = 0, scheduledExecutorService = ScheduledThreadPoolExecutor(0), onSlowThread = {})
  }
}

/**
 * Interface for a single threaded executor service with additional functionality to allow for interrupting the thread and checking whether
 * is active or not.
 */
interface SingleThreadExecutorService : ExecutorService {
  ...
  /**
   * Returns the current stack thread for the thread executing commands. The stack trace might be empty if the thread is not currently
   * running.
   */
  ...
  companion object {
    /**
     * Creates a new single thread [ExecutorService] with optional profiling. The [threadName] will name the thread used by this executor.
     */
    fun create(threadName: String, threadProfileSettings: ThreadProfileSettings): SingleThreadExecutorService =
      ProfilingSingleThreadExecutorImpl(threadName, threadProfileSettings)
  }
}

private class ProfilingSingleThreadExecutorImpl(threadName: String, private val threadProfileSettings: ThreadProfileSettings) :
  ...
  /**
   * The thread factory allows us controlling when the new thread is created to we can keep track of it. This allows us to capture the stack
   * trace later.
   */
  ...
  private fun profileThread(remainingSamples: Int, profileThreadFuture: CompletableFuture<Unit>) {
    ...
    threadProfileSettings.onSlowThread(
      TimeoutException("Slow render action ${threadProfileSettings.maxSamples-remainingSamples}/${threadProfileSettings.maxSamples}")
        .also { slowException -> theThread.get()?.also { slowException.stackTrace = it.stackTrace } }
    )
    threadProfileSettings.scheduledExecutorService
      ...
      .also { scheduledFuture -> profileThreadFuture.whenComplete { _, _ -> scheduledFuture.cancel(true) } }
  }

  private var profileThreadFuture: CompletableFuture<Unit>? = null
  private val threadPoolExecutor =
    object : ThreadPoolExecutor(1, 1, 0, TimeUnit.MILLISECONDS, PriorityBlockingQueue(), threadFactory) {
  ...
  override fun hasSpawnedCurrentThread(): Boolean = renderThreadGroup.parentOf(Thread.currentThread().threadGroup)
  ...
  override fun awaitTermination(timeout: Long, unit: TimeUnit): Boolean = threadPoolExecutor.awaitTermination(timeout, unit)
}
