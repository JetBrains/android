 ...
 */
package com.android.tools.idea.gradle.dsl.helpers.something
...
class SomethingPairedBraceMatcherTest : BasePlatformTestCase() {
  @Language("Something")
  private val source =
    """
    androidApplication {
  ...
  fun testParentheses() {
    myFixture.configureByText(SomethingFileType.INSTANCE, source.insertCaret("foo<caret>(\"bar\")"))
  ...
  fun testBraces() {
    myFixture.configureByText(SomethingFileType.INSTANCE, source.insertCaret("block <caret>{ foo = \"bar\" }"))
    val offset = BraceMatchingUtil.getMatchedBraceOffset(myFixture.editor, true, myFixture.file)
