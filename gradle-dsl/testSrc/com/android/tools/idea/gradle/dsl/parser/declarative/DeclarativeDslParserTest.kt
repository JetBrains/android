...
class DeclarativeDslParserTest : LightPlatformTestCase() {
  ...
  fun testExpressionFunction() {
    val file =
      ...
      """
        .trimIndent()
    val expected = mapOf("plugins" to listOf(mapOf("id" to "org.example", "version" to "1"), mapOf("id" to "org.other", "version" to "2")))
  ...
  fun testMap() {
    ...
    val expected =
      mapOf(
        "androidApp" to
          mapOf("defaultConfig" to mapOf("mManifestPlaceholders" to mapOf("activityLabel1" to "newName1", "activityLabel2" to "newName2")))
      )
    doTest(file, expected)
  }
  ...
  private fun propertiesToMap(file: GradleDslFile): Map<String, Any> {
    ...
    file.properties.forEach { map[it] = populate(file.getElement(it)) }
    return map
  }
}
