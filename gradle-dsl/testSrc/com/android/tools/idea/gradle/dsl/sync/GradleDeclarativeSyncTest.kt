...
class GradleDeclarativeSyncTest : GradlePhasedSyncTestCase() {
  ...
  @Test
  @TargetVersions("8.9+")
  fun `test declarative model creation in the simple Gradle project`() {
  ...
  @Test
  @TargetVersions("8.9+")
  fun `test declarative model creation in the complex Gradle project`() {
    val projectRoot = projectRoot.toNioPath()
    val virtualFileUrlManager = project.workspaceModel.getVirtualFileUrlManager()
    ...
    Disposer.newDisposable().use { disposable ->
      ...
      whenResolveProjectInfoStarted(disposable) { _, storage ->
        declarativeContributorAssertion.trace {
          //TODO this fails because settings file parser ir faulty
      ...
      assertModuleModuleDeps("test-dcl.app.test", "test-dcl.app.main")
