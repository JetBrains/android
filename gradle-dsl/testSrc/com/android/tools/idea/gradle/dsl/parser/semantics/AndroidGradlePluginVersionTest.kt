...
class AndroidGradlePluginVersionTest {
  ...
  fun testInvalidParse() {
    invalid.forEach {
      ...
      expect
        .that(
          try {
            AndroidGradlePluginVersion.parse(it)
  ...
  fun testComparison() {
    ordered
      .map { AndroidGradlePluginVersion.parse(it) }
      .forEachIndexed { i1, v1 ->
        ordered
          .map { AndroidGradlePluginVersion.parse(it) }
          .forEachIndexed { i2, v2 -> expect.that(compareValues(v1, v2).sign).isEqualTo(compareValues(i1, i2).sign) }
      }
  }

  @Test
  fun testToString() {
    valid.forEach { (string, expected) -> expect.that(expected.toString()).isEqualTo(string) }
  }
}
