...
// The distance from the edge of the circular image in the pop action icon to the closest point on
// the associated action
val POP_ICON_DISTANCE = scaledAndroidLength(7f)

// The maximum distance from the starting point of the action to the closest point to the pop action
// icon
...
fun getCurvePoints(source: SwingRectangle, dest: SwingRectangle, scale: Scale): CurvePoints {
  ...
  return CurvePoints(
    startPoint,
    getControlPoint(scale, startPoint, endPoint, START_DIRECTION),
    getControlPoint(scale, endPoint, startPoint, destDirection),
    endPoint,
    destDirection,
  )
}
...
fun getAnyPoint(action: SceneComponent, context: SceneContext): SwingPoint? {
  ...
  when (actionNlComponent.getActionType(rootNlComponent)) {
    ...
    ActionType.EXIT,
    ActionType.GLOBAL -> return action.inlineDrawRect(context).center
    else -> return null
  }
}
...
fun getArrowPoint(
  ...
  return shiftPoint(getConnectionPoint(rectangle, direction), direction, shiftY * scale)
}
