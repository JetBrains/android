...
sealed class Destination(protected open val parent: NlComponent) : Comparable<Destination> {
  ...
  abstract class ScreenShapedDestination(parent: NlComponent) : Destination(parent) {
    ...
    override fun thumbnail(
      ...
      val result =
        UIUtil.createImage(
          component,
          thumbnailDimension.width + 2 * THUMBNAIL_BORDER_THICKNESS.toInt(),
          thumbnailDimension.height + 2 * THUMBNAIL_BORDER_THICKNESS.toInt(),
      ...
      val roundRect =
        RoundRectangle2D.Float(
          THUMBNAIL_BORDER_THICKNESS,
          THUMBNAIL_BORDER_THICKNESS,
          thumbnailDimension.width.toFloat(),
          thumbnailDimension.height.toFloat(),
          THUMBNAIL_INNER_RADIUS,
          THUMBNAIL_INNER_RADIUS,
        )
    ...
    abstract fun drawThumbnailContents(
      model: NlModel,
    ...
    protected fun drawBackground(thumbnailDimension: Dimension, graphics: Graphics2D) {
      ...
      graphics.fillRect(
        THUMBNAIL_BORDER_THICKNESS.toInt(),
        THUMBNAIL_BORDER_THICKNESS.toInt(),
  ...
  internal data class RegularDestination
  @JvmOverloads
  constructor(
    override val parent: NlComponent,
    val tag: String,
    private val destinationLabel: String? = null,
    val destinationClass: PsiClass,
    val idBase: String = destinationClass.name ?: tag,
    private val layoutFile: XmlFile? = null,
    override val inProject: Boolean = true,
    val dynamicModuleName: String? = null,
  ) : ScreenShapedDestination(parent) {

    override fun drawThumbnailContents(
      model: NlModel,
      thumbnailDimension: Dimension,
      graphics: Graphics2D,
      iconCallback: (VirtualFile, Dimension) -> ImageIcon,
      ...
      if (layoutFile != null) {
        ...
        StartupUiUtil.drawImage(
          graphics,
          IconUtil.toImage(icon, ScaleContext.create(graphics)),
          Rectangle(
            THUMBNAIL_BORDER_THICKNESS.toInt(),
            THUMBNAIL_BORDER_THICKNESS.toInt(),
            thumbnailDimension.width,
        ...
        graphics.drawString(
          unknownString,
          (thumbnailDimension.width - stringWidth) / 2 + THUMBNAIL_BORDER_THICKNESS,
    ...
    override val destinationOrder =
      parent.model.schema.getDestinationTypesForTag(tag).let {
        when {
  ...
  companion object {
    private val comparator =
      Comparator.comparing<Destination, Boolean> { it.inProject }
