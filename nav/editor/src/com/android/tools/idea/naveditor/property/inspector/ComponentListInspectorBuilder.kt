...
abstract class ComponentListInspectorBuilder(
  ...
  override fun attachToInspector(
    ...
    list.addListSelectionListener { onSelectionChanged(list) }
    ...
    if (textEditor != null) {
      ...
      // We won't be able to get the text editor if the nav editor is just being opened. Wait until
      // it's hooked up to try.
    ...
    list.addHierarchyListener {
      if (
        it.changeFlags and HierarchyEvent.PARENT_CHANGED.toLong() > 0 &&
  ...
  private class AddAction(
    private val builder: ComponentListInspectorBuilder,
