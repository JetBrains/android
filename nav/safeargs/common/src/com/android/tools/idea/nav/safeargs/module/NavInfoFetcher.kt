...
data class NavEntry(
  ...
  val backingXmlFile: XmlFile? by lazy { PsiManager.getInstance(facet.module.project).findFile(file) as? XmlFile }
}
...
class NavInfoFetcher(
  ...
  private val _listenersRegistered = AtomicBoolean(false)

  @VisibleForTesting
  val listenersRegistered
    get() = _listenersRegistered.get()

  private fun ensureListenersRegistered() {
    if (_listenersRegistered.getAndSet(true)) {
      // Already registered, or registering on another thread.
      return
    }
    ...
    module.project.messageBus.connect(parent).apply {
      ...
      subscribe(PROJECT_SYSTEM_SYNC_TOPIC, ProjectSystemSyncManager.SyncResultListener { invalidate(NavInfoChangeReason.GRADLE_SYNC) })
      subscribe(
  ...
  override val isEnabled: Boolean
    get() {
      ...
      return androidFacetIfEnabled != null
    }

  /**
   * The current modification count of the navigation resources.
   *
   * [NavInfo] objects generated by this [NavInfoFetcher] are only valid so long as their [NavInfo.modificationCount] property continues to
   * match this property.
   */
  ...
  companion object {
    private fun getAndroidFacetIfSafeArgsEnabled(module: Module, requiredMode: SafeArgsMode) =
      AndroidFacet.getInstance(module)?.takeIf { it.isSafeArgsEnabled() && it.safeArgsMode == requiredMode }

    fun isSafeArgsModule(module: Module, requiredMode: SafeArgsMode) = getAndroidFacetIfSafeArgsEnabled(module, requiredMode) != null
  }
}
