 ...
 *
 * A destination may have zero or more arguments. Actions may also have arguments, which act as default values for their destinations.
 *
 * In most cases, users will provide [type] explicitly, but if not specified, it can be inferred from [defaultValue]. If neither [type] nor
 * [defaultValue] are set, this argument data is invalid.
 */
...
/**
 * A navigation action, which allows the navigation framework to transition from one destination to another.
 *
 * A destination may have zero or more actions.
 *
 * An action itself should point to a single target destination, which is usually set by `destination`, but could also just be set by
 * `popUpTo` (which essentially means the user wants to navigate backwards to a destination that's in the back stack)
 */
...
interface NavNavigationData : MaybeNavDestinationData {
  ...
  /**
   * We can't predict all possible tag types, because navigation allows custom tags. Instead, we catch all remaining tags and assume they
   * are destinations, but even if they aren't, we will still successfully complete parsing (and callers can strip out invalid destinations
   * later by checking if [MaybeNavDestinationData.toDestination] returns null.
   */
  ...
  override fun toDestination(): NavDestinationData? {
    ...
    return object : NavDestinationData {
      override val id = id
      override val name = ".${id.toUpperCamelCase()}" // The prefix '.' means this class should be scoped in the
      // current module
...
/** Data class for storing the indexed content nav XML files, useful for generating relevant safe args classes. */
data class NavXmlData(val root: NavNavigationData) {
   ...
   */
  val resolvedDestinations: List<NavDestinationData> by lazy { root.traverse(emptyList(), mutableListOf()) }
  ...
  private fun NavNavigationData.traverse(
    ...
    this.potentialDestinations.mapNotNull { it.toDestination()?.withGlobalActions(newGlobalActions) }.let { allDestinations.addAll(it) }
