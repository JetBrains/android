...
class TestSafeArgsTracker(project: Project) : SafeArgsTracker(project) {
  override fun runSlowWork(block: () -> Unit) {
    // It's not production code, so we just run slow work on the current thread. This allows the
