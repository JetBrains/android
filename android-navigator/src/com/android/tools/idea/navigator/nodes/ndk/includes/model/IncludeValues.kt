...
object IncludeValues {
  ...
  /**
   * When there is an include folder that references a known kind of package, for example NDK STL, we don't want to show these as plain
   * include files since they are already showing in their individual package location.
   *
   * For this reason, we compute a list of folders (in order) along with a set of folders that should be excluded since they are already
   * present elsewhere.
   */
  private fun groupTopLevelIncludesIntoShadowExpression(includes: List<ClassifiedIncludeValue>): List<IncludeValue> {
