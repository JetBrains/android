...
class CaptureNodeDetailTable(
  ...
  private val tableModel =
    extendedCaptureNodes.asTableModel(
      getColumn = { it.getValue(captureRange) },
  ...
  init {
    if (initialPageSize < Int.MAX_VALUE) {
      ...
      table = JBTable(tableModel).apply { autoCreateRowSorter = true }
      component =
    ...
    table.apply {
      ...
      setColumnRenderers<Column> {
        when (it) {
          Column.START_TIME -> TimestampRenderer()
          Column.NAME -> CustomBorderTableCellRenderer()
          Column.WALL_DURATION,
          Column.WALL_SELF_TIME,
          Column.CPU_DURATION,
          Column.CPU_SELF_TIME -> DurationRenderer()
        }
      }
   ...
   */
  private enum class Column(val displayName: String, val type: Class<*>, val getValue: (Range) -> (ExtendedCaptureNode) -> Comparable<*>) {
    START_TIME(
      "Start Time",
