...
class CaptureNodeSummaryDetailsView(profilersView: StudioProfilersView, tabModel: CaptureNodeAnalysisSummaryTabModel) :
  ...
  @get:VisibleForTesting val dataTypeLabel = JLabel()
  ...
  private fun buildSelectedNodeTable() =
    JPanel(TabularLayout("*").setVGap(8)).apply {
  ...
  private fun buildAllOccurrencesSection(
      ...
      JPanel(FlowLayout(FlowLayout.LEFT)).apply {
        ...
        add(
          JPanel(BorderLayout()).apply {
            ...
            add(
              StatLabel(allOccurrencesStats.count, "Count", numFont = ProfilerFonts.H2_FONT, descFont = ProfilerFonts.H4_FONT).apply {
                isOpaque = false
              }
            )
          }
        ...
        fun buildDurationLabel(num: Long, desc: String) =
          StatLabel(
              ...
              descFont = ProfilerFonts.H4_FONT,
    ...
    val topOccurrencesLabel =
      JLabel("Longest running occurrences (select row to navigate)").apply {
        isOpaque = false
        font = ProfilerFonts.H3_FONT
      }
    val topOccurrencesTable =
      CaptureNodeDetailTable(tenLongestRunningOccurrences, tabModel.captureRange, profilersView.studioProfilers.stage.timeline.viewRange)

    val panel =
      JPanel(TabularLayout("*", "Fit,Fit,Fit").setVGap(8)).apply {
        isOpaque = false
