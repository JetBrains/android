...
class JankSummaryDetailsView(profilersView: StudioProfilersView, model: JankAnalysisModel.Summary) :
  ...
  init {
    ...
    addRowToCommonSection("Jank type", JBLabel(event.appJankType.getTitle()).apply { foreground = event.getActiveColor() })
    ...
    val (expectedPercent, actualPercent) =
      when {
    ...
    addRowToCommonSection(
      "Expected duration",
      FilledLabel(
        TimeFormatter.getSingleUnitDurationString(event.expectedDurationUs),
        ProfilerColors.CAPTURE_SPARKLINE,
        expectedPercent,
        maxDurationBarWidth,
      ),
    )
    addRowToCommonSection(
      "Actual duration",
      FilledLabel(
        TimeFormatter.getSingleUnitDurationString(event.actualDurationUs),
        event.getActiveColor(),
        actualPercent,
        maxDurationBarWidth,
      ),
    )
...
private class FilledLabel(text: String, private val barColor: Color, private val percent: Int, private val maxBarWidth: Int) :
  JBLabel(text) {
  init {
...
private object EventTable {
  fun of(capture: CpuCapture, vararg renderEventLists: Pair<List<CaptureNode>, String>): JComponent =
    renderEventLists
      ...
      .toMutableList()
      .asTableModel(Col::get, Col::type, Col::title)
      .let { model ->
        with(PaginatedTableView(model)) {
          with(table) {
            showVerticalLines = true
            showHorizontalLines = true
            setColumnRenderers<Col> {
              when (it) {
