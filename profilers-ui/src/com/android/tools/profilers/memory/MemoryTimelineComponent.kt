...
class MemoryTimelineComponent(stageView: MemoryProfilerStageView, timeAxis: JComponent)
  ...
  private val lineChart = makeLineChart()
  ...
  init {
    ...
    val overlay = OverlayComponent(rangeSelectionComponent)
    ...
    listOfNotNull<AbstractDurationDataRenderer>(
      ...
      lineChart.addCustomRenderer(it)
      overlay.addDurationDataRenderer(it)
    }
    ...
    val tooltip = RangeTooltipComponent(stage.timeline, stageView.tooltipPanel, stageView.profilersView.component) {
      rangeSelectionComponent.shouldShowSeekComponent() && !heapDumpRenderer.isMouseOverHeapDump
    }.apply {
    ...
    add(ProfilerScrollbar(stage.timeline, this), TabularLayout.Constraint(3, 0))
    add(timeAxis, TabularLayout.Constraint(2, 0))
    add(makeMonitorPanel(overlayPanel), TabularLayout.Constraint(1, 0))
  }

  private fun makeMonitorPanel(overlayPanel: JBPanel<*>) = transparentPanel(TabularLayout("*", "*")).apply {
  ...
  private fun makeGcDurationDataRenderer() =
  ...
  private fun makeLineChart() = stage.detailedMemoryUsage.let { memUsage ->
  ...
  private fun makeAllocationRenderer(model: DurationDataModel<CaptureDurationData<CaptureObject>>, tag: String) =
