...
@Composable
fun DeviceDropdownAndRestartButtons(deviceList: List<Common.Device>,
                                    selectedDevice: Common.Device,
                                    onDeviceSelection: (Common.Device) -> Unit,
                                    buildAndLaunch: (Boolean) -> Unit) {
  Row(modifier = Modifier.fillMaxWidth()) {
    Box(
      modifier = Modifier.weight(1f).fillMaxWidth().padding(DEVICE_SELECTION_DROPDOWN_CONTENT_PADDING_DP),
    ...
    Box(
      ...
      RestartActionDropdown(buildAndLaunch)
    }
  }
}