...
class LogFileProviderTest : TestCase() {
  ...
  fun testLogFileProvider() {
    ...
    assertThat(fileInfo.size).isEqualTo(6)
    ...
    assertThat(fileInfo[3].source).isEqualTo(threadDumpFile)
    assertThat(fileInfo[3].destination).isEqualTo(logDir.relativize(threadDumpFile))

    assertThat(fileInfo[4].source).isEqualTo(uiFreezeFile)
    assertThat(fileInfo[4].destination).isEqualTo(logDir.relativize(uiFreezeFile))

    assertThat(fileInfo[5].source).isEqualTo(vmOptionsFile)
    assertThat(fileInfo[5].destination).isEqualTo(testDirectoryPath.relativize(vmOptionsFile))
  }
}