...
public class IdeResourcesUtilTest extends AndroidTestCase {
  ...
  public void testCompareResourceFiles() {
    ...
    Collections.sort(list, IdeResourcesUtil::compareResourceFiles);
    StringBuilder sb1 = new StringBuilder();
    for (PsiFile file : list) {
      if (file.getParent() != null && IdeResourcesUtil.getFolderType(file) != null) {
    ...
    Collections.sort(list2, IdeResourcesUtil::compareResourceFiles);
    ...
    //   AndroidResourcesIdeUtil.compareResourceFiles
    for (int i = 0; i < list2.size(); i++) {
      for (int j = i; j < list2.size(); j++) { // j=i, not j=i+1 because we want to compare with self too!
        ...
        int result1 = IdeResourcesUtil.compareResourceFiles(e1, e2);
        int result2 = IdeResourcesUtil.compareResourceFiles(e2, e1);
        assertEquals(result1, -result2);

        VirtualFile e3 = list2.get(i);
        VirtualFile e4 = list2.get(j);
        result1 = IdeResourcesUtil.compareResourceFiles(e3, e4);
        result2 = IdeResourcesUtil.compareResourceFiles(e4, e3);
        assertEquals(result1, -result2);
      }
    }
  }
