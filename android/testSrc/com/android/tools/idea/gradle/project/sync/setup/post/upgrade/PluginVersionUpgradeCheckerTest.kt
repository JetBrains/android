...
class PluginVersionUpgradeCheckerTest : PlatformTestCase() {
  ...
  override fun setUp() {
    ...
    Mockito.`when`(projectInfo.isBuildWithGradle).thenReturn(true)
  ...
  fun testNoNotificationWhenProjectIsNotUpgradable() {
    ...
    checkAndShowNotification(myProject)
  ...
  private fun replaceUpgradeService(upgradable: Boolean) {
    ...
    Mockito.`when`(upgrade.isRecommendedUpgradable()).thenReturn(upgradable)
