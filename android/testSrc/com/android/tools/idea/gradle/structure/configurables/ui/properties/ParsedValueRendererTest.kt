...
class ParsedValueRendererTest {
  ...
  fun testBuildKnownValueRenderers_empty() {
    val renderers = buildKnownValueRenderers(listOf(), null)
  ...
  fun testBuildKnownValueRenderers_noDefault() {
    ...
    val renderers = buildKnownValueRenderers(valueDescriptors, null)
  ...
  fun testBuildKnownValueRenderers_describedDefault() {
    ...
    val renderers = buildKnownValueRenderers(valueDescriptors, 1)
    assertThat(renderers.size, equalTo(3))
  ...
  fun testRenderParsedValue_notSet() {
    ...
    assertThat(ParsedValue.NotSet.testRenderWith<Int>(buildKnownValueRenderers(listOf(ValueDescriptor(1, "One")), 1)),
  ...
  fun testRenderParsedValue_setValue() {
    ...
    assertThat(one.testRenderWith<Int>(buildKnownValueRenderers(listOf(), null)), equalTo("1"))
    assertThat(one.testRenderWith<Int>(buildKnownValueRenderers(listOf(ValueDescriptor(1, "One")), null)), equalTo("1 <comment>(One)"))
  }
  ...
  fun testRenderParsedValue_setOtherDsl() {
    ...
    assertThat(dsl.testRenderWith<String>(buildKnownValueRenderers(listOf(), null)),
  ...
  fun testRenderParsedValue_setInvalid() {
    ...
    assertThat(error.testRenderWith<String>(buildKnownValueRenderers(listOf(), null)),
...
private fun <T : Any> ParsedValue<T>.testRenderWith(valueToText: Map<T?, ValueRenderer>): String = let { value ->
  testRender { value.renderTo(this, valueToText) }
}
