...
class GoogleRepositoryBaseTest {
  ...
  fun searchByName() {
    Assert.assertThat(
      repository.search(SearchRequest("common-java8", null, 50, 0)).get(10, TimeUnit.SECONDS),
  ...
  fun searchByGroupId() {
    Assert.assertThat(
      repository.search(SearchRequest("", "com.android.support", 50, 0)).get(10, TimeUnit.SECONDS),
  ...
  fun searchByNameWildcard() {
    Assert.assertThat(
      repository.search(SearchRequest("common*", null, 50, 0)).get(10, TimeUnit.SECONDS),
  ...
  fun searchByGroupIdWildcard() {
    Assert.assertThat(
      repository.search(SearchRequest("", "com.android.*", 50, 0)).get(10, TimeUnit.SECONDS),
      CoreMatchers.equalTo(
        SearchResult(artifacts = listOf(LEANBACK, RECOMMENDATION, RECYCLER_VIEW_V7, SUPPORT_COMPAT, SUPPORT_VECTOR_DRAWABLE))))
  }
  ...
  fun searchByExactMatch() {
    Assert.assertThat(
      repository.search(SearchRequest("leanback-v17", "com.android.support", 50, 0)).get(10, TimeUnit.SECONDS),
  ...
  fun searchByWildcard() {
    Assert.assertThat(
      repository.search(SearchRequest("common-*", "android.*", 50, 0)).get(10, TimeUnit.SECONDS),
      CoreMatchers.equalTo(SearchResult(artifacts = listOf(COMMON_JAVA8))))
  }
}