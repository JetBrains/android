...
class LocalMavenRepositoryTest {
  ...
  fun searchByName() {
    assertThat(
      repository.search(SearchRequest(SearchQuery(null, "lib1"), 50, 0)).get(10, TimeUnit.SECONDS),
  ...
  fun searchByGroupIdWildcard() {
    assertThat(
      repository.search(SearchRequest(SearchQuery("com.example.j*", null), 50, 0)).get(10, TimeUnit.SECONDS),
      equalTo(SearchResult(artifacts = listOf(JLIB3, JLIB4))))
  }
  ...
  fun searchByExactMatch() {
    assertThat(
      repository.search(SearchRequest(SearchQuery("com.example.libs", "lib2"), 50, 0)).get(10, TimeUnit.SECONDS),
  ...
  fun searchByWildcard() {
    assertThat(
      repository.search(SearchRequest(SearchQuery("com.example.lib*", "lib*"), 50, 0)).get(10, TimeUnit.SECONDS),
