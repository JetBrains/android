...
public class PatchInstallerFactoryTest {
  ...
  private final LocalPackage PATCHER_4 = new FakeLocalPackage("patcher;v4", sdkRoot.resolve("patcher4"));
  private final LocalPackage PATCHER_2 = new FakeLocalPackage("patcher;v2", sdkRoot.resolve("patcher2"));
  ...
  public void setUp() {
    ...
    myRepoManager = new FakeRepoManager(sdkRoot, myRepositoryPackages);
  }

  @Test
  public void cantHandleLinuxUninstallWithPatcher() {
    assumeFalse(FileOpUtils.isWindows());
    LocalPackage p = new FakeLocalPackage("foo", sdkRoot.resolve("foo"));
  ...
  public void cantHandleWindowsUninstallWithLargeFile() {
    assumeTrue(FileOpUtils.isWindows());
    LocalPackage p = new FakeLocalPackage("foo", sdkRoot.resolve("foo"));
    InMemoryFileSystems.recordExistingFile(p.getLocation(), 0, new byte[100 * 1024 * 1024]);
    myRepositoryPackages.setLocalPkgInfos(ImmutableList.of(p, PATCHER_4));
    assertFalse(myInstallerFactory.canHandlePackage(p, myRepoManager));
  }
  ...
  public void canHandleOnLinux() {
    ...
    FakeLocalPackage local = new FakeLocalPackage("foo", sdkRoot.resolve("foo"));
  ...
  public void createPatchUninstaller() {
    assumeTrue(FileOpUtils.isWindows());
    FakeLocalPackage p = new FakeLocalPackage("foo", sdkRoot.resolve("foo"));
    myRepositoryPackages.setLocalPkgInfos(ImmutableList.of(p, PATCHER_4));
  ...
  public void createInstallerWithPatch() {
    ...
    myRepositoryPackages.setLocalPkgInfos(ImmutableList.of(local, PATCHER_4));
    myRepositoryPackages.setRemotePkgInfos(ImmutableList.of(remote));
    Installer installer = myInstallerFactory.createInstaller(remote, myRepoManager, new FakeDownloader(sdkRoot.getRoot().resolve("tmp")));
