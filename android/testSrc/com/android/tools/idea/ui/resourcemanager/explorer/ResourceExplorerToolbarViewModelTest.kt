...
class ResourceExplorerToolbarViewModelTest {
  ...
  fun getData() {
    ...
    runInEdtAndWait {
      val bgtDataProvider = viewModel.getData(PlatformCoreDataKeys.BGT_DATA_PROVIDER.name) as DataProvider
      assertThat(bgtDataProvider.getData(CommonDataKeys.PSI_ELEMENT.name) as PsiElement).isEqualTo(psiDrawableDir) }
  }
