...
internal class SelectedDevicesErrorDialogTest {
  ...
  fun testErrorDevice() {
    ...
    createModalDialogAndInteractWithIt({ dialog.show() }) { dialogWrapper ->
      ...
      val message = treeWalker.descendants().filterIsInstance<JLabel>().find { it.text == "error message" }
  ...
  fun testWarningDevice() {
    ...
    createModalDialogAndInteractWithIt({ dialog.show() }) { dialogWrapper ->
      ...
      assertThat(title).isNotNull()
      val message = treeWalker.descendants().filterIsInstance<JLabel>().find { it.text == "warning message" }
      assertThat(message).isNotNull()
      val buttons = treeWalker.descendants().filterIsInstance<JButton>()
  ...
  fun testMultipleDevices() {
    ...
    createModalDialogAndInteractWithIt({ dialog.show() }) { dialogWrapper ->
      ...
      val title = treeWalker.descendants().filterIsInstance<JLabel>().find { it.text == "Error" }
      assertThat(title).isNotNull()
      val warning = treeWalker.descendants().filterIsInstance<JLabel>().find { it.text == "warning message" }
      assertThat(warning).isNotNull()
      val error = treeWalker.descendants().filterIsInstance<JLabel>().find { it.text == "error message" }
