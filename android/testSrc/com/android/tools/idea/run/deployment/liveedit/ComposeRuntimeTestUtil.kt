...
/**
 * Path to the compose-runtime jar. Note that unlike all other dependencies, we don't need to load that into the test's runtime classpath.
 * Instead, we just need to make sure it is in the classpath input of the compiler invocation Live Edit uses. Aside from things like
 * references to the @Composable annotation, we actually don't need anything from that runtime during the compiler. The main reason to
 * include that is because the compose compiler plugin expects the runtime to be path of the classpath or else it'll throw an error.
 */
private val composeRuntimePathForK2 =
  TestUtils.resolveWorkspacePath("tools/adt/idea/compose-ide-plugin/testData/lib/compose-runtime-desktop-1.7.0.jar").toString()
val composeRuntimePath
  get() = composeRuntimePathForK2
...
private val composeExtensionStorage by lazy {
  ...
  with(pluginRegistrar) { storage.registerExtensions(configurationWithComposeSpecificOptions) }
  storage
}
...
private val composeCompilerPluginProviderForTest by lazy {
  object : KotlinCompilerPluginsProvider {
    ...
    override fun isPluginOfTypeRegistered(module: KaModule, pluginType: KotlinCompilerPluginsProvider.CompilerPluginType): Boolean = false
  }
}
...
/**
 * Loads the Compose runtime into the project class path. This allows for tests using the compiler (Live Edit/FastPreview) to correctly
 * invoke the compiler as they would do in prod.
 *
...
fun <T : CodeInsightTestFixture> setUpComposeInProjectFixture(
  projectRule: AndroidProjectRule.Typed<T, Nothing>,
  exceptModuleNames: Set<String> = emptySet(),
) {
