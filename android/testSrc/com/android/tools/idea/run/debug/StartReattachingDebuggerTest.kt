...
class StartReattachingDebuggerTest {
  ...
  @Before
  fun setUp() {
    testDisposable = Disposer.newDisposable("StartReattachingDebuggerTest_disposable")
    StudioFlags.NEW_EXECUTION_FLOW_FOR_JAVA_DEBUGGER.override(true)
    runnableClientsService = RunnableClientsService(testDisposable)
  ...
  fun testStartReattachingDebuggerForOneClient() {
    ...
    val firstSession = startJavaReattachingDebugger(project, mockDevice, MASTER_PROCESS_NAME, listOf(APP_ID), executionEnvironment)
  ...
  fun testStartReattachingDebuggerForFewClients() {
    ...
    val latch = CountDownLatch(ADDITIONAL_CLIENTS)
    ...
    startJavaReattachingDebugger(project, mockDevice, MASTER_PROCESS_NAME, listOf(APP_ID), executionEnvironment).blockingGet(10,
                                                                                                                             TimeUnit.SECONDS)

    for (i in 0 until ADDITIONAL_CLIENTS) {
    ...
    // 2 and not 3 because we don't show tab for runnableClient1, because it's responsibility of the caller to show it.
    verify(runContentManagerImplMock, times(2))
      .showRunContent(eq(DefaultDebugExecutor.getDebugExecutorInstance()), any())
  }
  ...
  fun testStopping() {
    ...
    val sessionImpl = startJavaReattachingDebugger(project, mockDevice, MASTER_PROCESS_NAME, listOf(APP_ID),
                                                   executionEnvironment).blockingGet(10, TimeUnit.SECONDS)
