...
import com.android.tools.idea.resourceExplorer.ResourcePasteProviderTest.Companion.projectRule
...
class PresenterTest {
  ...
  fun generateFilesTest() {
    val presenter = SketchImporterPresenter(AndroidTestBase.getTestDataPath() + "/sketch/parser_generateFiles.sketch")
    val filesMap = presenter.generateFiles(projectRule.project)!!

    var lightVirtualFileList = filesMap.values.first()
    assertEquals(
    ...
    lightVirtualFileList = filesMap.values.last()
    assertEquals(
