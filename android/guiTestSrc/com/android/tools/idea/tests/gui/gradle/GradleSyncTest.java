...
public class GradleSyncTest extends GuiTestCase {
  ...
  public void testJavaModelSerialization() throws IOException {
    IdeFrameFixture projectFrame = importProject("MultipleModuleTypes");
  ...
  private static String getGradleHomeFromSystemProperty(@NotNull String propertyName, @NotNull String gradleVersion) {
    String unsupportedGradleHome = System.getProperty(propertyName);
    if (isEmpty(unsupportedGradleHome)) {
      fail("Please specify the path of a local, Gradle " + gradleVersion + " distribution using the system property "
           + quote(propertyName));
    }
