...
public class LibJavaUnitTest {
    @Test
    public void referenceProductionCode() {
        assertEquals("LibJavaClass", new LibJavaClass().getName());
    }

    @Test
    public void referenceProductionKotlinCode() {
        assertEquals("LibKotlinClass", new LibKotlinClass().getName());
    }
    ...
    public void javaResourcesOnClasspath() throws Exception {
        URL url = getClass().getClassLoader().getResource("lib_test_resource_file.txt");
        assertNotNull(url);

        InputStream stream = getClass().getClassLoader().getResourceAsStream("lib_test_resource_file.txt");
    ...
    public void prodJavaResourcesOnClasspath() throws Exception {
        URL url = getClass().getClassLoader().getResource("lib_resource_file.txt");
        assertNotNull(url);

        InputStream stream = getClass().getClassLoader().getResourceAsStream("lib_resource_file.txt");
        assertNotNull(stream);
        byte[] line = new byte[1024];
        assertTrue("Expected >0 bytes read from input stream", stream.read(line) > 0);
        String s = new String(line, "UTF-8").trim();
    ...
    public void workingDir() {
        assertTrue(new File("").getAbsolutePath().endsWith("lib"));
    }
