...
class GoogleRepositoryBaseTest {
  ...
  private val testSupportVersionSet = setOf(Version.parse("25.3.1"), Version.parse("26.0.0-beta1"))

  private val testArchVersionSet =
    setOf(
  ...
  private val SUPPORT_COMPAT =
    FoundArtifact(
      repositoryName = "Google",
      groupId = "com.android.support",
      name = "support-compat",
      unsortedVersions = testSupportVersionSet,
    )
  ...
  private val SUPPORT_VECTOR_DRAWABLE =
    FoundArtifact(
      repositoryName = "Google",
      groupId = "com.android.support",
  ...
  private val COMMON =
    FoundArtifact(repositoryName = "Google", groupId = "android.arch.lifecycle", name = "common", unsortedVersions = testArchVersionSet)

  private val COMMON_JAVA8 =
    FoundArtifact(
      repositoryName = "Google",
      groupId = "android.arch.lifecycle",
      name = "common-java8",
      unsortedVersions = testArchVersionSet,
    )
  ...
  fun searchByName() {
    Assert.assertThat(
      repository.search(SearchRequest(ArbitraryModulesSearchQuery(null, "common-java8"), 50, 0)).get(10, TimeUnit.SECONDS).clearStats(),
  ...
  fun searchByModule() {
    // search in name
    Assert.assertThat(
      repository.search(SearchRequest(ArbitraryModulesSearchByModuleQuery("*common*"), 50, 0)).get(10, TimeUnit.SECONDS).clearStats(),
      CoreMatchers.equalTo(SearchResult(artifacts = listOf(COMMON, COMMON_JAVA8))),
    )

    // search in group
    Assert.assertThat(
      repository.search(SearchRequest(ArbitraryModulesSearchByModuleQuery("*support*"), 50, 0)).get(10, TimeUnit.SECONDS).clearStats(),
  ...
  fun searchByNameWildcard() {
    Assert.assertThat(
      repository.search(SearchRequest(ArbitraryModulesSearchQuery("", "common*"), 50, 0)).get(10, TimeUnit.SECONDS).clearStats(),
      CoreMatchers.equalTo(SearchResult(artifacts = listOf(COMMON, COMMON_JAVA8))),
    )
  }
  ...
  fun searchByGroupIdWildcard() {
    Assert.assertThat(
      repository.search(SearchRequest(ArbitraryModulesSearchQuery("com.android.*", null), 50, 0)).get(10, TimeUnit.SECONDS).clearStats(),
      CoreMatchers.equalTo(
        SearchResult(artifacts = listOf(LEANBACK, RECOMMENDATION, RECYCLER_VIEW_V7, SUPPORT_COMPAT, SUPPORT_VECTOR_DRAWABLE))
      ),
    )
  ...
  fun searchByWildcard() {
    Assert.assertThat(
      repository.search(SearchRequest(ArbitraryModulesSearchQuery("android.*", "common-*"), 50, 0)).get(10, TimeUnit.SECONDS).clearStats(),
      CoreMatchers.equalTo(SearchResult(artifacts = listOf(COMMON_JAVA8))),
    )
  }
}

class IdeStubGoogleMavenRepository
@JvmOverloads
constructor(
