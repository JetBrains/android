...
class PresenterTest {
  ...
  companion object {
    @ClassRule @JvmField val projectRule = AndroidProjectRule.onDisk()
  }

  @Before
  fun setUp() {
    val dir = projectRule.fixture.tempDirFixture.findOrCreateDir("res")
    WriteCommandAction.runWriteCommandAction(projectRule.project) { dir.children.forEach { it.delete(this) } }
  }
  ...
  fun drawableFiles() {
    val sketchFile =
      com.android.tools.idea.ui.resourcemanager.sketchImporter.parser.SketchParser.read(
        AndroidTestBase.getTestDataPath() + "/sketch/presenter.sketch"
      )!!
    val view = SketchImporterView()
    val presenter =
      SketchImporterPresenter(view, sketchFile, DesignAssetImporter(), projectRule.module.androidFacet!!, StubAssetPreviewManager())
