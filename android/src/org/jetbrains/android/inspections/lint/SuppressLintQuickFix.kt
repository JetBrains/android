...
import org.jetbrains.android.inspections.lint.AndroidLintInspectionBase.LINT_INSPECTION_PREFIX
...
class SuppressLintQuickFix(private val id: String, element: PsiElement? = null) : SuppressQuickFix {
  ...
  fun applyFix(element: PsiElement) {
    when (element.language) {
      ...
      StdLanguages.XML -> handleXml(element)
  ...
  private fun handleBinary(element: PsiElement) {
    ...
    if (virtualFile != null) {
      ...
      if (module != null) {
        val dir = LintIdeProject.getLintProjectDirectory(module, AndroidFacet.getInstance(module))
        if (dir != null) {
          val project = element.project
          val client = LintIdeClient(project)
  ...
  companion object {
    ...
    private fun addSuppressAttribute(file: XmlFile, element: XmlTag, id: String) {
      ...
      AndroidResourceUtil.ensureNamespaceImported(file, SdkConstants.TOOLS_URI, null)
    ...
    fun displayName(element: PsiElement?, inspectionId: String): String {
      ...
      return when (element?.language) {
        StdLanguages.XML -> AndroidBundle.message("android.lint.fix.suppress.lint.api.attr", id)
        JavaLanguage.INSTANCE, KotlinLanguage.INSTANCE -> AndroidBundle.message("android.lint.fix.suppress.lint.api.annotation", id)
