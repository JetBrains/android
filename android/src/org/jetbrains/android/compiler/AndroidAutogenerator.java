...
public class AndroidAutogenerator {
  ...
  public static void run(@NotNull AndroidAutogeneratorMode mode,
    ...
    final Set<String> obsoleteFiles = new HashSet<>(autogenerating.getAutogeneratedFiles(mode));

    switch (mode) {
      case AAPT:
        runAapt(facet, autogenerating, context, force);
        break;
      case AIDL:
  ...
  private static void runAapt(@NotNull AndroidFacet facet,
                              @NotNull ModuleSourceAutogenerating autogenerating,
                              @NotNull CompileContext context,
                              boolean force) {
    final Module module = facet.getModule();
    ...
    final AptAutogenerationItem item = ApplicationManager.getApplication().runReadAction(new Computable<AptAutogenerationItem>() {
      ...
      public AptAutogenerationItem compute() {
        ...
        final Map<String, String> genFilePath2Package = new HashMap<>();
    ...
    if (force) {
      final Set<VirtualFile> filesToCheck = new HashSet<>();
  ...
  private static void removeAllFilesWithSameName(@NotNull final Module module, @NotNull final File file, @NotNull String directoryPath) {
    ...
    final List<VirtualFile> filesToDelete = new ArrayList<>();
  ...
  private static void runAidl(@NotNull AndroidFacet facet,
    ...
    final List<IdlAutogenerationItem> items = new ArrayList<>();
