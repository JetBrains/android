...
public class AndroidRootUtil {
  ...
  public static VirtualFile getManifestFile(@NotNull AndroidFacet facet) {
    return getFileByRelativeModulePath(facet.getModule(), facet.getConfiguration().getState().MANIFEST_FILE_RELATIVE_PATH, true);
  }

  @Nullable
  public static VirtualFile getCustomManifestFileForCompiler(@NotNull AndroidFacet facet) {
    return getFileByRelativeModulePath(facet.getModule(), facet.getConfiguration().getState().CUSTOM_COMPILER_MANIFEST, false);
  }
  ...
  public static VirtualFile getManifestFileForCompiler(@NotNull AndroidFacet facet) {
    return facet.getConfiguration().getState().USE_CUSTOM_COMPILER_MANIFEST
  ...
  public static VirtualFile getResourceDir(@NotNull AndroidFacet facet) {
    String resRelPath = facet.getConfiguration().getState().RES_FOLDER_RELATIVE_PATH;
  ...
  private static String suggestResourceDirPath(@NotNull AndroidFacet facet) {
    ...
    return root.getPath() + facet.getConfiguration().getState().RES_FOLDER_RELATIVE_PATH;
  }
  ...
  public static VirtualFile getAssetsDir(@NotNull AndroidFacet facet) {
    return getFileByRelativeModulePath(facet.getModule(), facet.getConfiguration().getState().ASSETS_FOLDER_RELATIVE_PATH, false);
  }

  @Nullable
  public static VirtualFile getProguardCfgFile(@NotNull AndroidFacet facet) {
    return getFileByRelativeModulePath(facet.getModule(), facet.getConfiguration().getState().PROGUARD_CFG_PATH, false);
  }

  @Nullable
  public static VirtualFile getLibsDir(@NotNull AndroidFacet facet) {
    return getFileByRelativeModulePath(facet.getModule(), facet.getConfiguration().getState().LIBS_FOLDER_RELATIVE_PATH, false);
  }
      ...
      public void run() {
        ...
        for (OrderEntry entry : manager.getOrderEntries()) {
          ...
          if (libraries != null && entry instanceof LibraryOrderEntry) {
            ...
            final boolean libraryProject = facet != null && facet.getConfiguration().getState().LIBRARY_PROJECT;
  ...
  public static VirtualFile[] getResourceOverlayDirs(@NotNull AndroidFacet facet) {
    List<String> overlayFolders = facet.getConfiguration().getState().RES_OVERLAY_FOLDERS;
  ...
  public static String getAptGenSourceRootPath(@NotNull AndroidFacet facet) {
    String path = facet.getConfiguration().getState().GEN_FOLDER_RELATIVE_PATH_APT;
  ...
  public static String getAidlGenSourceRootPath(@NotNull AndroidFacet facet) {
    String path = facet.getConfiguration().getState().GEN_FOLDER_RELATIVE_PATH_AIDL;
    if (path.length() == 0) return null;
    String moduleDirPath = getModuleDirPath(facet.getModule());
    return moduleDirPath != null ? moduleDirPath + path : null;
  }
  ...
  public static String getApkPath(@NotNull AndroidFacet facet) {
    String path = facet.getConfiguration().getState().APK_PATH;
    if (path.length() == 0) {
