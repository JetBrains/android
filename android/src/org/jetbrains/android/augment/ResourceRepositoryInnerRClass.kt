...
class ResourceRepositoryInnerRClass(
   ...
   * [AndroidLightClassBase.findFieldByName] for everything else. The `super` implementation relies on first creating the [PsiField]s for
   * *all* resources and then searching for the requested field. In addition, the logic for determining whether a field should be final or
   * not relies on a very expensive Manifest class creation `AndroidCompileUtil.findCircularDependencyOnLibraryWithSamePackage`. Both of
   * those operations are avoided for the common case in this implementation.
   */
  ...
  private fun scenarioUnsupported(name: String) =
    resourceType == ResourceType.STYLEABLE || resourcesSource.fieldModifier == AndroidLightField.FieldModifier.FINAL || name.contains("_")
