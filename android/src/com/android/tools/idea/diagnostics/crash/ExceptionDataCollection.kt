...
class ExceptionDataCollection {
  ...
  companion object {
    ...
    private val THROWABLE_CLASSES_TO_TRACK_MESSAGES =
      ImmutableList.of(
    ...
    private val LOGGER_CLASSES = ImmutableList.of(Logger::class.java.name, IdeaLogger::class.java.name)
    ...
    private fun removeLoggerErrorFrames(stackTraceElements: Array<StackTraceElement>): Array<StackTraceElement> {
      ...
      while (
        firstNonLoggerFrame < stackTraceElements.size &&
    ...
    fun calculateSignature(t: Throwable): String {
      ...
      stackTrace.slice(1 until min(6, stackTrace.size)).joinTo(sb, "") {
        ...
        if (matchResult != null) methodName = matchResult.groupValues[1]

        return@joinTo if (methodName != "") {
          val classLoaderName = it.classLoaderName?.let { "$it/" } ?: ""
          var modulePart =
            it.moduleName?.let { moduleName -> if (it.moduleVersion != null) "$moduleName@${it.moduleVersion}/" else "$moduleName/" } ?: ""
      ...
      val modulePart =
        stackTrace[0].moduleName?.let { moduleName ->
          if (stackTrace[0].moduleVersion != null) "$moduleName@${stackTrace[0].moduleVersion}/" else "$moduleName/"
        } ?: ""
  ...
  @TestOnly val registeredAppenders: List<TrackingAppender>
  ...
  private fun initializeLog(): List<TrackingAppender> {
    ...
    val needsToReconfigureAppenders =
  ...
  private fun registerTrackingAppenders(): MutableList<TrackingAppender> {
    ...
    for ((name, config) in configs) {
      ...
      if (!action.hasLogFilter() || action.logFilter.maxMessageCount <= 0) {
        continue
      }
  ...
  class TrackingAppender(val logger: java.util.logging.Logger, formatter: Formatter, private val logBuffer: LogBuffer) : Handler() {
    init {
  ...
  fun getExceptionUploadFields(t: Throwable, forceExceptionMessage: Boolean, includeLogs: Boolean): UploadFields {
    try {
      ...
      return UploadFields(description = getDescription(if (includeFullStack) t else cause, !includeMessage, includeFullStack), logs = logs)
    } catch (t: Throwable) {
  ...
  private fun getExceptionUploadConfigurations(t: Throwable): Map<String, ExceptionConfiguration> {
    ...
    return configs.filterValues { config ->
      ...
      if (!config.hasExpirationDate()) return@filterValues false
      ...
      if (now.after(expirationDate)) return@filterValues false
      ...
      if (filter.framePatternsCount > 0) {
        ...
        match =
          match &&
            filter.framePatternsList.all { frameToMatch ->
  ...
  /**
   * Returns an exception description (similar to [Throwables.getStackTraceAsString]}) with the exception message removed in order to strip
   * off any PII. The exception message is include for some specific exceptions where we know that the message will not have any PII.
   *
  ...
  fun getDescription(t: Throwable, stripMessage: Boolean, includeFullStack: Boolean): String {
    return StringBuilder().also { sb -> getDescription(t, stripMessage, includeFullStack, null, 1, sb) }.toString()
  }
  ...
  /**
   * Logger.error messages show up as java.lang.Throwable exceptions with a top frame being Logger.error. This method will detect if the
   * exception string is from such method. If so, it changes its type to com.android.diagnostic.LoggerErrorMessage and removes the top frame
   * (Logger.error frame).
   */
  private fun fixStackTraceStringForLoggerErrorMethod(s: String): String {
    ...
    if (lines[0] != throwableText && !lines[0].startsWith(throwableColonText)) return s
    ...
    if (indexOfFirstFrame == -1) return s

    var indexOfFirstNonLoggerFrame = indexOfFirstFrame
    while (
      indexOfFirstNonLoggerFrame < lines.size &&
    ...
    if (indexOfFirstNonLoggerFrame == lines.size || indexOfFirstNonLoggerFrame == indexOfFirstFrame) return s
   ...
   *
   * For example, Kotlin [CompilerInternalError]s may contain messages such as "java.lang.IllegalStateException: Failed to compile". In that
   * case, this method will return "org.jetbrains.kotlin.idea.gradle.diagnostic.CompilerInternalError_java.lang.IllegalStateException".
   * Note:
