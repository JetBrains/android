...
class IdeLevel2LibraryFactory {
  static Library create(Library library, @NotNull ModelCache modelCache) {
    if (library.getType() == LIBRARY_ANDROID) {
      ...
      return new IdeLevel2JavaLibrary(library, modelCache);
    }
    else if (library.getType() == LIBRARY_MODULE) {
      return new IdeLevel2ModuleLibrary(library, modelCache);
    }
