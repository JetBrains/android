...
class IdeLevel2LibraryFactory {
  ...
  static Library create(@NotNull Library library, @NotNull ModelCache modelCache) {
    if (library.getType() == LIBRARY_ANDROID) {
      ...
      return new IdeLevel2AndroidLibrary(library.getArtifactAddress(), library.getFolder(), library.getManifest(), library.getJarFile(),
                                         library.getResFolder(), library.getAssetsFolder(), library.getLocalJars(), library.getJniFolder(),
                                         library.getAidlFolder(), library.getRenderscriptFolder(), library.getProguardRules(),
                                         library.getLintJar(), library.getExternalAnnotations(), library.getPublicResources(),
                                         library.getSymbolFile(), modelCache, library, artifactFile);
    }
    else if (library.getType() == LIBRARY_JAVA) {
      return new IdeLevel2JavaLibrary(library.getArtifactAddress(), library.getArtifact(), modelCache, library);
    }
    else if (library.getType() == LIBRARY_MODULE) {
  ...
  static Library create(@NotNull AndroidLibrary androidLibrary, @NotNull ModelCache modelCache) {
    if (androidLibrary.getProject() != null) {
      ...
      return new IdeLevel2AndroidLibrary(computeAddress(androidLibrary),
                                         ...
                                         androidLibrary.getSymbolFile().getPath(),
                                         modelCache,
                                         androidLibrary,
                                         null);
    }
  ...
  static Library create(@NotNull JavaLibrary javaLibrary, @NotNull ModelCache modelCache) {
    if (javaLibrary.getProject() != null) {
      ...
      return new IdeLevel2ModuleLibrary(computeAddress(javaLibrary), javaLibrary.getJarFile(), javaLibrary.getProject(), null, modelCache,
                                        javaLibrary);
    }
    else {
