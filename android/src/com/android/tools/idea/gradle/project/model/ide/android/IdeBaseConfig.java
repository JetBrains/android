...
public class IdeBaseConfig implements BaseConfig, Serializable {
  ...
  public IdeBaseConfig(@NotNull BaseConfig config) {
    ...
    Map<String, ClassField> configFields = config.getBuildConfigFields();
    myBuildConfigFields = new HashMap<>(configFields.size());
    for (Map.Entry<String, ClassField> entry : configFields.entrySet()) {
      myBuildConfigFields.put(entry.getKey(), new IdeClassField(entry.getValue()));
    }
