...
public final class GradleUtil {
  ...
  /**
   * Checks if the given folder contains sources generated by aapt. When the IDE uses light R classes, these folders are not marked as
   * sources of the module.
   */
  public static boolean isGeneratedRFolder(@NotNull File folder, @NotNull AndroidModuleModel androidModuleModel) {
    try {
  ...
  public static Collection<File> getGeneratedSourceFoldersToUse(@NotNull IdeBaseArtifact artifact, @NotNull AndroidModuleModel model) {
    if (StudioFlags.IN_MEMORY_R_CLASSES.get()) {
      return artifact.getGeneratedSourceFolders()
        .stream()
        .filter(folder -> !isGeneratedRFolder(folder, model))
