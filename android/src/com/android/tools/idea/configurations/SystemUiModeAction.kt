...
class SystemUiModeAction(private val renderContext: ConfigurationHolder,
  ...
  override fun actionPerformed(event: AnActionEvent) {
    ...
    val menu = JPopupMenu().apply { border = BorderFactory.createEmptyBorder(POPUP_VERTICAL_BORDER, 0, POPUP_VERTICAL_BORDER, 0) }
    ...
    renderContext.configuration?.nightMode.let { currentNightMode ->
      enumValues<NightMode>().forEach { mode ->
        ...
        val item = ActionMenuItem(action, action.templatePresentation, ActionPlaces.POPUP, event.dataContext,
                                  false, true, false, false)
        item.border = BorderFactory.createEmptyBorder(2, sideBorderWidth.get(), 2, sideBorderWidth.get())
        gbc.gridy += 1
        menu.add(item, gbc)
      }
    }
