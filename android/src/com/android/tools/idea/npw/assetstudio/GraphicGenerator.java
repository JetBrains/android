...
public abstract class GraphicGenerator {
   ...
   */
  @Nullable
  public GeneratedIcon generateIcon(@NonNull GraphicGeneratorContext context, @NonNull Options options, @NonNull String name,
                                    @NonNull IconCategory category) {
    BufferedImage image = generate(context, options);
    return new GeneratedImageIcon(getIconName(options, name), Paths.get(getIconPath(options, name)), category, options.density, image);
  }
  ...
  protected String getIconName(@NonNull Options options, @NonNull String name) {
    ...
    return name + SdkConstants.DOT_PNG;
  }
  ...
  @Nullable
  private static BufferedImage getTrimmedAndPaddedImage(@Nullable ListenableFuture<BufferedImage> imageFuture, boolean isTrimmed,
                                                        int paddingPercent) {
    if (imageFuture == null) {
      return null;
    }
