...
public final class PathValidationResult {
  ...
  public static PathValidationResult validateLocation(@Nullable String projectLocation,
    ...
    if (projectLocation == null || projectLocation.isEmpty()) {
      return error(PathValidationResult.Message.NO_LOCATION_SPECIFIED, fieldName);
    }
    // Check the separators
    if ((File.separatorChar == '/' && projectLocation.contains("\\")) || (File.separatorChar == '\\' && projectLocation.contains("/"))) {
      return error(PathValidationResult.Message.BAD_SLASHES, fieldName);
    }
    ...
    while (testFile != null) {
      ...
      if (!CharMatcher.ASCII.matchesAllOf(filename)) {
        if (SystemInfo.isWindows) {
          ...
          warningResult = warn(PathValidationResult.Message.NON_ASCII_CHARS_WARNING, fieldName);
        }
      }
    ...
    if (file.isFile()) {
      return error(PathValidationResult.Message.PROJECT_LOC_IS_FILE, fieldName);
    }
    if (file.getParent() == null) {
      return error(PathValidationResult.Message.PROJECT_IS_FILE_SYSTEM_ROOT, fieldName);
    }
    if (file.getParentFile().exists() && !file.getParentFile().isDirectory()) {
      return error(PathValidationResult.Message.PARENT_NOT_DIR, fieldName);
    }
    ...
    if (installLocation != null && FileUtil.isAncestor(new File(installLocation), file, false)) {
      ...
      return error(PathValidationResult.Message.INSIDE_ANDROID_STUDIO, fieldName, applicationName);
    }

    if (checkEmpty && file.exists() && FileOpUtils.create().listFiles(file).length > 0) {
      return warn(PathValidationResult.Message.NON_EMPTY_DIR, fieldName);
    }
