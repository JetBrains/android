...
public class NavigationEditor implements FileEditor {
    ...
    for (ActivityState state : activityStates) {
          ...
          // Look for menu bindings
          {
            ...
            if (methods.length == 1) {
              ...
              for (PsiStatement s : statements) {
                MultiMatchResult multiMatchResult = installMenuItemOnGetMenuItemAndLaunchActivityMacro.match(s.getFirstChild());
      ...
      for (FragmentEntry fragment : fragments) {
        ...
        if (methods.length == 1) {
          ...
          for (PsiStatement s : statements) {
            MultiMatchResult multiMatchResult = installItemClickAndCallMacro.match(s.getFirstChild());
            if (multiMatchResult != null) {
              ...
              fragmentClass.accept(new JavaRecursiveElementVisitor() {
                ...
                public void visitAssignmentExpression(PsiAssignmentExpression expression) {
                  ...
                  if (expression.getLExpression().getText().equals($target.getText())) {
                    ...
                    MultiMatchResult multiMatchResult1 = defineInnerClassToLaunchActivityMacro.match(rExpression);
  ...
  static class MultiMatch {
    ...
    MultiMatch(PsiMethod macro) {
      this.macro = macro;
    }

    void addSubMacro(String name, PsiMethod macro) {
      subMacros.put(name, macro);
    }

    @Nullable
    MultiMatchResult match(PsiElement element) {
      Map<String, PsiElement> bindings = NavigationEditor.match(macro, element);
      ...
      for (Map.Entry<String, PsiMethod> entry : subMacros.entrySet()) {
        ...
        Map<String, PsiElement> subBinding = NavigationEditor.match(template, bindings.get(name));
        if (subBinding == null) {
          return null;
        }
