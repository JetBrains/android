...
interface FastPreviewTrackerManager {
  companion object {
    fun getInstance(project: Project): FastPreviewTrackerManager = project.getService(FastPreviewTrackerManager::class.java)
  }
  ...
  interface Request {
    ...
    /**
     * Called when the files were compiled successfully. The compilation took [compilationDurationMs]. Optionally, if [refreshTimeMs] is not
     * -1, it will be logged as the time that took to refresh the previews after compilation.
     */
    fun compilationSucceeded(compilationDurationMs: Long, compiledFiles: Int, refreshTimeMs: Long = -1)

    /**
     * Called when the fast preview refresh was cancelled, for example due to a new change in the code that happened while a refresh was
     * being done, or because the user manually cancelled it. [compilationCompleted] indicates whether the cancellation happened during
...
internal class FastPreviewTrackerManagerImpl(private val project: Project) : FastPreviewTrackerManager {
  private fun newStudioEvent(): AndroidStudioEvent.Builder =
    AndroidStudioEvent.newBuilder().setKind(AndroidStudioEvent.EventKind.FAST_PREVIEW_EVENT).withProjectId(project)

  override fun userEnabled() {
    UsageTracker.log(newStudioEvent().setFastPreviewEvent(FastPreviewEvent.newBuilder().setType(FastPreviewEvent.Type.USER_ENABLED)))
  }

  override fun userDisabled() {
    UsageTracker.log(newStudioEvent().setFastPreviewEvent(FastPreviewEvent.newBuilder().setType(FastPreviewEvent.Type.USER_DISABLED)))
  }
  ...
  override fun daemonStartFailed() {
    UsageTracker.log(
      newStudioEvent()
        .setFastPreviewEvent(
          FastPreviewEvent.newBuilder()
            .setType(FastPreviewEvent.Type.COMPILE)
            .setCompilationResult(
              FastPreviewEvent.CompilationResult.newBuilder().setStatus(FastPreviewEvent.CompilationResult.Status.DAEMON_START_ERROR)
            )
        )
    )
  }

  override fun trackRequest(): FastPreviewTrackerManager.Request =
    object : FastPreviewTrackerManager.Request {
      override fun daemonStartFailed() {
          ...
          newStudioEvent()
            .setFastPreviewEvent(
              FastPreviewEvent.newBuilder()
                .setType(FastPreviewEvent.Type.COMPILE)
                .setCompilationResult(
                  FastPreviewEvent.CompilationResult.newBuilder()
              ...
              FastPreviewEvent.newBuilder()
                ...
                .setCompilationResult(
                  FastPreviewEvent.CompilationResult.newBuilder()
                    ...
                    .setCompileDurationMs(compilationDurationMs)
                    .setCompiledFiles(compiledFiles.toLong())
                    .also { if (refreshTimeMs != -1L) it.refreshDurationMs = refreshTimeMs }
                )
            )
        )
      }
