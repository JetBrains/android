...
private class DocumentChangeListener(
  private val onDocumentsUpdated: (Collection<Document>, Long) -> Unit,
  private val documentManager: PsiDocumentManager,
  private val lookupManager: LookupManager,
  private val mergeQueue: MergingUpdateQueue,
  private val timeNanosProvider: () -> Long,
) : DocumentListener {
  val aggregatedEventsLock = ReentrantLock()

  @GuardedBy("aggregatedEventsLock") val aggregatedEvents = mutableSetOf<DocumentEvent>()

  @GuardedBy("aggregatedEventsLock") var lastUpdatedNanos = 0L

  /** [LookupListener] that is added to the active lookup to detect when the completion is closed. */
  private val lookupListener =
  ...
  private fun onDocumentChanged(events: Set<DocumentEvent>) {
    val documents = events.map { it.document }.distinct()
  ...
  private fun queueDocumentUpdates() {
    ...
    mergeQueue.queue(
      object : Update("document change") {
        override fun run() {
          documentManager.performLaterWhenAllCommitted {
            mergeQueue.queue(
              object : Update("handle changes") {
                override fun run() {
...
fun setupChangeListener(
  ...
  mergeQueue: MergingUpdateQueue =
    MergingUpdateQueue("Document change queue", TimeUnit.SECONDS.toMillis(1).toInt(), true, null, parentDisposable, null, false)
...
fun setupChangeListener(
  ...
  document.addDocumentListener(
    DocumentChangeListener(
      { _, lastUpdatedNanos -> onDocumentUpdated(lastUpdatedNanos) },
      documentManager,
      lookupManager,
      mergeQueue,
      timeNanosProvider,
...
/**
 * Sets up a save listener for the given [psiFile]. When the file is saved, [onSave] will be called. The listener might be called in any
 * thread.
 *
 * The given [parentDisposable] will be used to set the life cycle of the listener. When disposed, the listener will be disposed too.
 */
fun setupOnSaveListener(
  ...
  parentDisposable: Disposable,
  mergeQueue: MergingUpdateQueue =
    MergingUpdateQueue("Document save queue", TimeUnit.SECONDS.toMillis(1).toInt(), true, null, parentDisposable, null, false)
      .setRestartTimerOnAdd(true),
) {
...
/**
 * Returns a new flow that gets all the document changes for the given [psiFile]. The flow is cancelled if the [parentDisposable] is
 * disposed. [onReady] will be called when the flow is ready to start processing events from the document.
 */
