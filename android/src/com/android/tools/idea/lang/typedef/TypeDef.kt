...
internal data class TypeDef(val annotation: PsiElement, val values: List<PsiElement>, val type: Type) {
  ...
  /**
   * Returns a decorated and prioritized (via wrapping with [PrioritizedLookupElement]) version of
   * the given [LookupElement], or `null` if it does not correspond to a value of this [TypeDef] or
   * required information is missing.
   */
  fun decorateAndPrioritize(delegate: LookupElement): LookupElement? {
    val completionElement = delegate.psiElement?.navigationElement
    if (completionElement !is PsiNamedElement || completionElement !in valuesSet) return null
    val fqName = completionElement.getFqName() ?: return null
    val annotationName = getName() ?: return null
    val lookupStrings = completionElement.getLookupStrings() ?: return null
    val icon = completionElement.getIcon(/* flags= */ 0)
    val decorated = TypeDefLookupElementDecorator(delegate, fqName, annotationName, lookupStrings, icon)
    return PrioritizedLookupElement.withPriority(decorated, HIGH_PRIORITY)
  }
