...
import com.android.tools.idea.lang.db3.psi.DbTokenTypes;
...
public class DbSyntaxHighlighterFactory extends SyntaxHighlighterFactory {
  ...
  private static class DbSyntaxHighlighter extends SyntaxHighlighterBase {
    ...
    private static final TokenSet ourKeywords = TokenSet
      .create(BOOLEAN_KEYWORD, BYTE_KEYWORD, CHAR_KEYWORD, SHORT_KEYWORD, INT_KEYWORD, LONG_KEYWORD, FLOAT_KEYWORD, DOUBLE_KEYWORD,
              VOID_KEYWORD, CLASS_KEYWORD, INSTANCEOF_KEYWORD, DEFAULT_KEYWORD);
    private static final TokenSet ourBrackets = TokenSet.create(LBRACKET, RBRACKET);
    private static final TokenSet ourParentheses = TokenSet.create(LPARENTH, RPARENTH);
    private static final TokenSet ourNumber = TokenSet.create(INTEGER_LITERAL, FLOAT_LITERAL, LONG_LITERAL, DOUBLE_LITERAL);
    private static final TokenSet ourOperators = TokenSet
      .create(EQEQ, NE, LE, LTLT, LT, GTGTGT, GTGT, GTEQ, GT, EQ, EXCL, TILDE, QUESTQUEST, QUEST, COLON, PLUS, MINUS, ASTERISK, DIV, ANDAND,
              AND, OROR, OR, XOR);

    static {
      fillMap(sAttributes, ourKeywords, KEYWORD);
      fillMap(sAttributes, ourBrackets, BRACKETS);
      fillMap(sAttributes, ourParentheses, PARENTHESES);
      fillMap(sAttributes, ourNumber, NUMBER);
      fillMap(sAttributes, ourOperators, OPERATION_SIGN);
      sAttributes.put(STRING_LITERAL, STRING);
