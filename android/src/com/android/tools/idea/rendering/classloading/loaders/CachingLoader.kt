...
/**
 * A [DelegatingClassLoader.Loader] that caches the loaded data. This class accepts a pre-configured [CacheBuilder] to tweak the caching
 * behaviour.
 */
class CachingLoader constructor(private val delegate: DelegatingClassLoader.Loader, val cacheBuilder: CacheBuilder<String, ByteArray>) :
  DelegatingClassLoader.Loader {

  constructor(
    delegate: DelegatingClassLoader.Loader,
  ...
  override fun loadClass(fqcn: String): ByteArray? = cache.get(fqcn)?.takeUnless { it == NULL_VALUE }
  ...
  companion object {
    ...
    private fun createCacheBuilder(expireAfterWriteMs: Long, maxSizeInBytes: Long) =
