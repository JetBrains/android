...
private fun String.isSystemPrefix(): Boolean =
  startsWith("java.") || startsWith("javax.") || startsWith("kotlin.") || startsWith(INTERNAL_PACKAGE) || startsWith("sun.")

/**
 * A [CachingClassLoaderLoader] that loads the classes from a given IntelliJ [Module]. It relies on the given [findClassContent] to find the
 * [ClassContent] mapping to a given FQCN.
 */
class ProjectSystemClassLoader(private val findClassContent: (String) -> ClassContent?) : CachingClassLoaderLoader {
  /**
  ...
  @TestOnly fun getClassCache() = classCache.filter { entry -> entry.value.get() != null }.toMap()

  override fun isUpToDate(): Boolean = classCache.values.mapNotNull { it.get() }.all { it.isUpToDate() }
  ...
  private fun getClassContentForFqcn(fqcn: String): ClassContent? {
    ...
    val classContent =
      SlowOperations.knownIssue("b/326953914")
        .use { findClassContent(fqcn) }
        ?.also {
