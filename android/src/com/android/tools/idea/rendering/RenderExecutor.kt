 ...
 */
class RenderExecutor private constructor(maxQueueingTasks: Int,
  ...
  private val renderingExecutor: ExecutorService = if (maxQueueingTasks > 0)
    EvictingExecutor(executorProvider(threadFactory), maxQueueingTasks)
  else
    executorProvider(threadFactory)
  ...
  override fun <T> runAsyncAction(timeout: Long, unit: TimeUnit, callable: Callable<T>): CompletableFuture<T> {
    ...
    val timeoutFuture = timeoutExecutor.schedule(
      {
        val message = """
        Preview timed out (${unit.toMillis(timeout)}ms) while the layout.
        This typically happens when there is an infinite loop or unbounded recursion in one of the custom views.
      """.trimIndent()
  ...
  @get:TestOnly
  val accumulatedTimeouts: Int get() = accumulatedTimeoutExceptions.get()
