...
public final class JHandler implements GfxinfoHandler {
  ...
  public void sample(@NotNull IDevice device, @NotNull ClientData data, @NotNull TimelineData timeline) throws Exception {
    ...
    if (myReceiver.getLogSize() > 0) {
      if (myDelimiterAdded) {
        timeline.add(myLastSampleTime, DeviceSampler.TYPE_DATA, 0.0f, 0.0f, 0.0f);
      ...
      if (!myIgnoreInitialSamples) {
        for (int i = 0; i < myReceiver.getLogSize(); ++i) {
          ...
          timeline
            .add(time, DeviceSampler.TYPE_DATA, myReceiver.getDrawTime(i), myReceiver.getPrepareTime(i), myReceiver.getExecuteTime(i));
        }
      }
      ...
      timeline.add(currentTime, DeviceSampler.TYPE_DATA, 0.0f, 0.0f, 0.0f);
    }
  ...
  public TimelineData createTimelineData() {
    return new TimelineData(3, SAMPLE_BUFFER_SIZE);
  }
  ...
  /**
   * Output receiver for "dumpsys gfxinfo <pid>" in API 16 or later.
   */
      ...
      for (int i = profileSectionIndex; i < myOutput.size(); ++i) {
        ...
        if (timings.length == 3) {
          try {
            ...
            myExecuteTimes.add(Float.parseFloat(timings[2]));
          }
