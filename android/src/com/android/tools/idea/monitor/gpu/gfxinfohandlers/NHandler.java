...
/**
 * Sampler for API 25 or higher.
 */
public final class NHandler implements GfxinfoHandler {
  public static final int MIN_API_LEVEL = 25;
  ...
  private static final class ProcessStatReceiver extends MultiLineReceiver {
    ...
    private TLongArrayList mySyncQueuedTimes = new TLongArrayList();
    ...
    public long getSyncTime(int index) {
      return mySyncQueuedTimes.get(index) + mySyncTimes.get(index);
    }
      ...
      for (String line : lines) {
        ...
        if (timings.length == 16) {
          try {
            ...
            long endTime = Long.parseLong(timings[13]);
            if (flags == 0 && endTime > myLastSampleEndTime) {
              ...
              long syncQueued = Long.parseLong(timings[9]);
              long syncStart = Long.parseLong(timings[10]);
              long commandIssueStart = Long.parseLong(timings[11]);
              long swapStart = Long.parseLong(timings[12]);
              ...
              myDrawTimes.add(syncQueued - drawStart);
              mySyncQueuedTimes.add(syncStart - syncQueued);
