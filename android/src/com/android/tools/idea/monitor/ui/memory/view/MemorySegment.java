...
public class MemorySegment extends BaseLineChartSegment {
  ...
  public void populateSeriesData(@NotNull LineChart lineChart) {
    // Data population is handled inside toggleView
  }
  ...
  public void toggleView(boolean isExpanded) {
    ...
    if (isExpanded) {
      ...
      addLine(SeriesDataType.MEMORY_OBJECT_COUNT, mRightAxisRange, new LineConfig(MEMORY_COUNT_COLOR));
    } else {
  ...
  private void addMemoryLevelLine(SeriesDataType type, Color color) {
    addLine(type, mLeftAxisRange, new LineConfig(color).setFilled(true).setStacked(true));
  }

  private void addLine(SeriesDataType type, Range yRange, LineConfig config) {
    DataStoreContinuousSeries series = new DataStoreContinuousSeries(mSeriesDataStore, type);
    mLineChart.addLine(new RangedContinuousSeries(type.toString(), mXRange, yRange, series), config);
  }
