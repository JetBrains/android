...
import static com.android.tools.idea.lint.LintIdeUtils.SUPPRESS_LINT_FQCN;
import static com.android.tools.idea.lint.LintIdeUtils.SUPPRESS_WARNINGS_FQCN;
 ...
 * </ul>
 */
public class LintIdeApiDetector extends ApiDetector {
  @SuppressWarnings("unchecked")
  static final Implementation IMPLEMENTATION = new Implementation(
    LintIdeApiDetector.class,
  ...
  private class ApiCheckVisitor extends JavaRecursiveElementVisitor {
    ...
    public void visitMethod(PsiMethod method) {
      ...
      if (method.getModifierList().hasExplicitModifier(PsiModifier.DEFAULT)) {
        ...
        if (!isSuppressed(api, method, minSdk)) {
          Location location = LintIdeUtils.getLocation(myContext.file, method);
      ...
      if (api > buildSdk && buildSdk != -1) {
        if (mySeenSuppress &&
            LintIdeUtils.isSuppressed(method, OVERRIDE)) {
          return;
        }
        ...
        Location location = LintIdeUtils.getLocation(myContext.file, locationNode);
    ...
    public void visitClass(PsiClass aClass) {
          ...
          for (PsiAnnotation annotation : modifierList.getAnnotations()) {
            ...
            if ("java.lang.annotation.Repeatable".equals(name)) {
              ...
              if (!isSuppressed(api, aClass, minSdk)) {
                Location location = LintIdeUtils.getLocation(myContext.file, annotation);
      ...
      for (PsiClassType type : aClass.getSuperTypes()) {
        String signature = LintIdeUtils.getInternalName(type);
        if (signature == null) {
          continue;
        }
        ...
        if (mySeenSuppress && LintIdeUtils.isSuppressed(aClass, UNSUPPORTED)) {
          continue;
        }
        ...
        if (type instanceof PsiClassReferenceType) {
          ...
          location = LintIdeUtils.getLocation(myContext.file, element);
        } else {
          location = LintIdeUtils.getLocation(myContext.file, aClass);
        }
    ...
    public void visitReferenceExpression(PsiReferenceExpression expression) {
      ...
      if (resolved != null) {
        if (resolved instanceof PsiField) {
          ...
          Location location = LintIdeUtils.getLocation(myContext.file, expression);
    ...
    public void visitTryStatement(PsiTryStatement statement) {
      ...
      if (resourceList != null) {
        ...
        Location location = LintIdeUtils.getLocation(myContext.file, resourceList);
    ...
    private void checkCatchTypeElement(@NonNull PsiTryStatement statement,
      ...
      if (resolved != null) {
        String signature = LintIdeUtils.getInternalName(resolved);
        if (signature == null) {
          return;
        }
        ...
        if (mySeenSuppress && LintIdeUtils.isSuppressed(statement, UNSUPPORTED)) {
          return;
        }

        Location location;
        location = LintIdeUtils.getLocation(myContext.file, typeElement);
    ...
    private boolean isSuppressed(int api, PsiElement element, int minSdk) {
      ...
      if (mySeenSuppress &&
          (LintIdeUtils.isSuppressed(element, UNSUPPORTED) || LintIdeUtils.isSuppressed(element, INLINED))) {
        return true;
      }
    ...
    private void checkCast(@NotNull PsiElement node, @NotNull PsiClassType classType, @NotNull PsiClassType interfaceType) {
      ...
      String classTypeInternal = LintIdeUtils.getInternalName(classType);
      String interfaceTypeInternal = LintIdeUtils.getInternalName(interfaceType);
      ...
      Location location = LintIdeUtils.getLocation(myContext.file, node);
    ...
    public void visitForeachStatement(PsiForeachStatement statement) {
      ...
      if (type instanceof PsiClassType) {
        String expressionOwner = LintIdeUtils.getInternalName((PsiClassType)type);
        if (expressionOwner != null) {
          ...
          Location location = LintIdeUtils.getLocation(myContext.file, value);
    ...
    public void visitCallExpression(PsiCallExpression expression) {
      ...
      if (method != null) {
        ...
        if (expression instanceof PsiMethodCallExpression) {
          ...
          if (qualifier != null && !(qualifier instanceof PsiThisExpression) && !(qualifier instanceof PsiSuperExpression)) {
            ...
            while (cls != null) {
              ...
              String expressionOwner = LintIdeUtils.getInternalName(cls);
              if (expressionOwner == null) {
                break;
              }
              int specificApi = mApiDatabase.getCallVersion(expressionOwner, name, desc);
        ...
        PsiElement locationNode = LintIdeUtils.getCallName(expression);
        if (locationNode == null) {
