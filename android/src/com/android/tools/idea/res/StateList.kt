...
class StateList(val fileName: String, val dirName: String) {
  ...
  val folderType: ResourceFolderType
    get() = ResourceFolderType.getFolderType(dirName)!!

  val states: List<StateListState>
    get() = myStates
  ...
  val disabledStates: ImmutableList<StateListState>
    get() {
      ...
      // But we can use {a : true} as a representation for the set of all object states that have
      // true assigned to a.
      // Since we do not know a priori how many different flags there are, that is how we are going
      // to represent a set of object states.
      // We are using a set of maps, where each map represents a set of object states, and the
      // overall set is their union.
      // For example, the set S = { {a : true} , {b : true, c : false} } is the union of two sets of
      // object states.
      // The first one, described by {a : true} contains 4 object states, and the second one,
      // described by {b : true, c : false} contains 2.
      // Overall this set S represents: {a : true, b : true, c : true}, {a : true, b : true, c :
      ...
      // remainingObjects is going to represent all the object states that have not been matched in
      // the state list until now.
      // So before we start we want to initialise it to represents all possible object states. One
      // easy way to do so is to pick a flag
      // and make two representations {flag : true} and {flag : false} and take their union. We pick
      // "state_enabled" as that flag but any
      // flag could have been used.
      // We now go through the state list state by state.
      // For each state list state, we ask the question : does there exist an object state that
      // could reach this state list state,
      // and match it, and have "state_enabled = true"? If that object state exists, it has to be
      // represented in remainingObjectStates.
      // if there is no such object state, then all the object states that would match this state
      // list state would have
      // "state_enabled = false", so this state list state is considered disabled.
      // Before looking at the next state list state, we recompute remainingObjectStates so that it
      // does not represent any more
    ...
    for (attribute in stateAttributes.keys) {
      for (allowedState in allowedStates) {
        if (!allowedState.containsKey(attribute)) {
          // This allowed state does not have a constraint for attribute. So it represents object
          // states that can take either value
          // for it. We restrict this representation by adding to it explicitly the opposite
          // constraint to the one in the state list state
          // so that we remove from this representation all the object states that match the state
          // list state while keeping all the ones
          // that do not.
          ...
          // None of the object states represented by allowedState match the state list state. So we
          // keep them all by keeping
...
class StateListState(var value: String?, val attributes: Map<String, Boolean>, var alpha: String?) {
  ...
  internal fun matchesWithEnabledObjectState(
    ...
    for (allowedAttributes in allowedObjectStates) {
      ...
      if (match) {
        // There is one object state represented in allowedAttributes, that has state_enabled =
        // true, and that matches this
        // state list state.
        return true
      }
    }
