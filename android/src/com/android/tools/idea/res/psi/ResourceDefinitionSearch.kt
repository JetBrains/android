...
class ResourceDefinitionSearch :
  QueryExecutorBase<PsiElement, DefinitionsScopedSearch.SearchParameters>(
    /* requireReadAction = */ true
  ) {
  ...
  override fun processQuery(
    ...
    val repository =
      resourceRepositoryManager.getResourcesForNamespace(resourceReference.namespace) ?: return
    var minQualifiers = Int.MAX_VALUE
    for (resource in
      repository.getResources(resourceReference).filterDefinitions(project).sortedBy {
        it.configuration.qualifiers.size
      }) {
