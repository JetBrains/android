...
/**
 * [CompletionContributor] base class for Kotlin vs. Java-agnostic transformations and filtering of resources.
 *
 * Currently, does the following:
 * * Decorates [LookupElement]s for `Drawable` resources with an [Icon] rendered from the `Drawable`.
 * * Decorates [LookupElement]s for `Color` resources with an [Icon] of the appropriate color, and with tail text showing the hex code of
 *   the color.
 * * Filters out [LookupElement]s for private resources, unless they are qualified by a package that is not the current package.
 */
sealed class ResourceCompletionContributor : CompletionContributor() {
  override fun fillCompletionVariants(parameters: CompletionParameters, resultSet: CompletionResultSet) {
    ...
    val filterPrivateResources = facet?.let { shouldFilterPrivateResources(parameters.position, it) } ?: false
    val transformContext = TransformContext(facet, parameters.position.containingFile, filterPrivateResources)
    resultSet.runRemainingContributors(parameters) { it.transform(transformContext)?.let(resultSet::passResult) }
  }

  /** Returns whether we should filter private resources in the current [position]. */
  protected abstract fun shouldFilterPrivateResources(position: PsiElement, facet: AndroidFacet): Boolean
}
...
class JavaResourceCompletionContributor : ResourceCompletionContributor() {
  override fun shouldFilterPrivateResources(position: PsiElement, facet: AndroidFacet): Boolean {
    ...
    val r =
      (position.parent as? PsiReferenceExpression)?.qualifierReferenceExpression?.qualifierReferenceExpression?.takeIf {
        it.referenceName == SdkConstants.R_CLASS
      } ?: return false
...
class KotlinResourceCompletionContributor : ResourceCompletionContributor() {
  override fun shouldFilterPrivateResources(position: PsiElement, facet: AndroidFacet): Boolean {
    ...
    // R classes are assumed to know what they're doing. So if R is unqualified or is qualified by
    // the package or test package name, then filter.
    val rQualifierName = (r as? KtDotQualifiedExpression)?.receiverExpression?.text?.filterNot(Char::isWhitespace) ?: return true
    return facet.getModuleSystem().let { it.getPackageName() == rQualifierName || it.getTestPackageName() == rQualifierName }
  }
}

private data class TransformContext(val facet: AndroidFacet?, val containingFile: PsiFile, val filterPrivateResources: Boolean)
...
private fun CompletionResult.transform(context: TransformContext): CompletionResult? {
  ...
  if (context.facet != null && context.filterPrivateResources && isForPrivateResource(this, context.facet)) {
...
private fun decorateColor(resourceLightField: ResourceLightField, original: LookupElement): LookupElement? {
...
private fun computeColor(resourceLightField: ResourceLightField): Color? {
  ...
  return resourceLightField.getResourceItems(module)?.firstNotNullOfOrNull { it.resourceValue?.value?.let(::parseColor) }
}
 ...
 *
 * If the [Icon] for the Drawable is already available, this will be a [FastDrawableResourceLookupElement], otherwise it will be a
 * [SlowDrawableResourceLookupElement] that actually renders the [Icon] on a background thread via [LookupElement.getExpensiveRenderer].
 */
private fun decorateDrawable(resourceLightField: ResourceLightField, psiFile: PsiFile, original: LookupElement): LookupElement? {
  val module = ModuleUtilCore.findModuleForPsiElement(resourceLightField) ?: return null
  val file = resourceLightField.getResourceItems(module)?.firstNotNullOfOrNull { it.getSourceAsVirtualFile() } ?: return null
  ...
  val resolver = AndroidAnnotatorUtil.pickConfiguration(psiFile.originalFile, facet)?.resourceResolver ?: return null
...
private fun ResourceLightField.getResourceItems(module: Module) =
  StudioResourceRepositoryManager.getInstance(module)?.appResources?.getResources(ResourceNamespace.RES_AUTO, resourceType, resourceName)
...
private fun isForPrivateResource(result: CompletionResult, facet: AndroidFacet): Boolean {
  ...
  val resourceType =
    psiField.containingClass
      ...
      ?.name
      ?.let { ResourceType.fromClassName(it) } ?: return false // If there's no type, we can't look it up, so return false.
