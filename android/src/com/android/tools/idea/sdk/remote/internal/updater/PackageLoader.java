    ...
    try {
      ...
      mUpdaterData.getTaskFactory().start("Loading Sources", new ITask() {
        ...
        public void run(ITaskMonitor monitor) {
          ...
          Package[] localPkgs = mUpdaterData.getInstalledPackages(monitor.createSubMonitor(1));
          if (localPkgs == null) {
            ...
            if (sources != null && sources.length > 0) {
              ...
              for (SdkSource source : sources) {
                ...
                if (pkgs == null || overrideExisting) {
                  source.load(getDownloadCache(), subMonitor.createSubMonitor(1), forceHttp);
    ...
    loadPackages(false /*overrideExisting*/, new ISourceLoadedCallback() {
      ...
      private void installArchives(final List<Archive> archivesToInstall) {
        ...
        runOnUiThread(new Runnable() {
          @Override
          public void run() {
            List<Archive> archives = mUpdaterData.updateOrInstallAll_WithGUI(archivesToInstall, true /* includeObsoletes */, installFlags);
        ...
        final Package[] localPkgs = mUpdaterData.getInstalledPackages(new NullTaskMonitor(mUpdaterData.getSdkLog()));
          ...
          for (Package localPkg : localPkgs) {
            if (localPkg.canBeUpdatedBy(pkg) == UpdateInfo.NOT_UPDATE) {
              ...
              if (localArchive.length == 1 && localArchive[0].isCompatible()) {
                mInstallPaths.put(localPkg, new File(localArchive[0].getLocalOsPath()));
              }
            }
          }
        }
