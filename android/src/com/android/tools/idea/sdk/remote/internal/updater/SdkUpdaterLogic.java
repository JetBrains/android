...
public class SdkUpdaterLogic {
  ...
  public List<ArchiveInfo> getAllRemoteArchives(SdkSources sources, Package[] localPkgs, boolean includeAll) {
    ...
    for (Archive a : archives) {
      insertArchive(a, result, archives, remotePkgs, remoteSources, localArchives, false /*automated*/);
    }
   ...
   * those that can be updated and compute dependencies too.
   */
  public List<ArchiveInfo> computeUpdates(Collection<Archive> selectedArchives,
                                          SdkSources sources,
                                          Package[] localPkgs,
                                          boolean includeAll) {
    ...
    if (selectedArchives == null) {
      selectedArchives = findUpdates(localArchives, remotePkgs, remoteSources, includeAll);
    }
    ...
    for (Archive a : selectedArchives) {
      insertArchive(a, archives, selectedArchives, remotePkgs, remoteSources, localArchives, false /*automated*/);
    }
    ...
    // these dependencies.
    fixMissingLocalDependencies(archives, selectedArchives, remotePkgs, remoteSources, localArchives);
  ...
  public void addNewPlatforms(Collection<ArchiveInfo> archives, SdkSources sources, Package[] localPkgs, boolean includeAll) {
    ...
    for (Package p : remotePkgs) {
      ...
      if (p instanceof BuildToolPackage) {
        ...
        shouldAdd = !currentExtraScore.containsKey(key) || score > currentExtraScore.get(key).doubleValue();
      }
      else if (p instanceof DocPackage) {
      ...
      if (shouldAdd) {
        ...
        for (Archive a : p.getArchives()) {
          if (a.isCompatible()) {
            insertArchive(a, archives, null /*selectedArchives*/, remotePkgs, remoteSources, localArchives, true /*automated*/);
          }
        }
      }
  ...
  private Collection<Archive> findUpdates(ArchiveInfo[] localArchives,
    ...
    for (ArchiveInfo ai : localArchives) {
      ...
      for (Package remotePkg : remotePkgs) {
        // Only look for non-obsolete updates unless requested to include them
        if ((includeAll || !remotePkg.isObsolete()) && localPkg.canBeUpdatedBy(remotePkg) == UpdateInfo.UPDATE) {
          // Found a suitable update. Only accept the remote package
          // if it provides at least one compatible archive
   ...
   */
  private void fixMissingLocalDependencies(Collection<ArchiveInfo> outArchives,
    ...
    for (ArchiveInfo ai : localArchives) {
      ...
      ArchiveInfo[] deps = findDependency(p, outArchives, selectedArchives, remotePkgs, remoteSources, localArchives);
  ...
  private ArchiveInfo insertArchive(Archive archive,
    ...
    if (ai == null) {
      ai = new ArchiveInfo(archive,        //newArchive
  ...
  private ArchiveInfo[] findDependency(Package pkg,
    ...
    if (pkg instanceof IPlatformDependency) {
      ArchiveInfo ai =
        findPlatformDependency((IPlatformDependency)pkg, outArchives, selectedArchives, remotePkgs, remoteSources, localArchives);
    ...
    if (pkg instanceof IMinToolsDependency) {
      ...
      ArchiveInfo ai =
        findToolsDependency((IMinToolsDependency)pkg, outArchives, selectedArchives, remotePkgs, remoteSources, localArchives);
    ...
    if (pkg instanceof IMinPlatformToolsDependency) {
      ...
      ArchiveInfo ai =
        findPlatformToolsDependency((IMinPlatformToolsDependency)pkg, outArchives, selectedArchives, remotePkgs, remoteSources,
                                    localArchives);
    ...
    if (pkg instanceof IMinApiLevelDependency) {
      ...
      ArchiveInfo ai =
        findMinApiLevelDependency((IMinApiLevelDependency)pkg, outArchives, selectedArchives, remotePkgs, remoteSources, localArchives);
    ...
    if (pkg instanceof IExactApiLevelDependency) {
      ...
      ArchiveInfo ai =
        findExactApiLevelDependency((IExactApiLevelDependency)pkg, outArchives, selectedArchives, remotePkgs, remoteSources, localArchives);
   ...
   */
  public ArchiveInfo findToolsDependency(IMinToolsDependency pkg,
                                         Collection<ArchiveInfo> outArchives,
                                         Collection<Archive> selectedArchives,
                                         Collection<Package> remotePkgs,
                                         SdkSource[] remoteSources,
      ...
      for (Archive a : selectedArchives) {
        ...
        if (p instanceof ToolPackage) {
          if (((ToolPackage)p).getRevision().compareTo(rev) >= 0) {
            ...
            return insertArchive(a, outArchives, selectedArchives, remotePkgs, remoteSources, localArchives, true /*automated*/);
          }
        }
      }
    }
    ...
    if (localArch != null) {
      return insertArchive(localArch, outArchives, selectedArchives, remotePkgs, remoteSources, localArchives, true /*automated*/);
   ...
   * Returns the archive info dependency, if any.
   */
  public ArchiveInfo findPlatformToolsDependency(IMinPlatformToolsDependency pkg,
                                                 Collection<ArchiveInfo> outArchives,
                                                 Collection<Archive> selectedArchives,
                                                 Collection<Package> remotePkgs,
                                                 SdkSource[] remoteSources,
    ...
    if (selectedArchives != null) {
      ...
      if (localAMax != null) {
        if (findMax) {
          ...
          // It's not already in the list of things to install, so add it now
          return insertArchive(localAMax, outArchives, selectedArchives, remotePkgs, remoteSources, localArchives, true /*automated*/);
        }
      }
    }
    ...
    if (findMax) {
      if (aMax != null) {
        return insertArchive(aMax, outArchives, selectedArchives, remotePkgs, remoteSources, localArchives, true /*automated*/);
      }
      else if (aiMax != null) {
        return aiMax;
      }
   ...
   * Returns the archive info dependency, if any.
   */
  public ArchiveInfo findPlatformDependency(IPlatformDependency pkg,
                                            Collection<ArchiveInfo> outArchives,
                                            Collection<Archive> selectedArchives,
                                            Collection<Package> remotePkgs,
                                            SdkSource[] remoteSources,
        ...
        if (expectedClass.isInstance(p)) {
          if (v.equals(((IAndroidVersionProvider)p).getAndroidVersion())) {
            if (addonVendor != null && addonTag != null && p instanceof AddonPackage) {
              if (!((AddonPackage)p).getVendorId().equals(addonVendor.getId()) || !((AddonPackage)p).getNameId().equals(addonTag.getId())) {
                continue;
              }
            }
            // We found one already installed.
   ...
   * Returns the archive info dependency, if any.
   */
  protected ArchiveInfo findMinApiLevelDependency(IMinApiLevelDependency pkg,
                                                  Collection<ArchiveInfo> outArchives,
                                                  Collection<Archive> selectedArchives,
                                                  Collection<Package> remotePkgs,
                                                  SdkSource[] remoteSources,
    ...
    if (foundArchive != null) {
      // It's not already in the list of things to install, so add it now
      return insertArchive(foundArchive, outArchives, selectedArchives, remotePkgs, remoteSources, localArchives, true /*automated*/);
    }
   ...
   * Returns the archive info dependency, if any.
   */
  public ArchiveInfo findExactApiLevelDependency(IExactApiLevelDependency pkg,
                                                 Collection<ArchiveInfo> outArchives,
                                                 Collection<Archive> selectedArchives,
                                                 Collection<Package> remotePkgs,
                                                 SdkSource[] remoteSources,
  ...
  private static class MissingPlatformArchiveInfo extends ArchiveInfo {
    ...
    public String getShortDescription() {
      return String.format("Missing SDK Platform Android%1$s, API %2$d", mVersion.isPreview() ? " Preview" : "", mVersion.getApiLevel());
    }
  }
  ...
  private static class MissingArchiveInfo extends ArchiveInfo {
    ...
    @Override
    public String getShortDescription() {
      return String.format("Missing Android SDK %1$s, revision %2$s", mTitle, mRevision.toShortString());
    }
  }
}
