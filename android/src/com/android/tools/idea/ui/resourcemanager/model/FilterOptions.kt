...
internal constructor(
  ...
  var isShowModuleDependencies: Boolean by
    Delegates.observable(moduleDependenciesInitialValue) { _, old, new -> if (new != old) refreshResourcesCallback() }

  /** If true, the resources from the dependent libraries will be shown. */
  var isShowLibraries: Boolean by Delegates.observable(librariesInitialValue) { _, old, new -> if (new != old) refreshResourcesCallback() }

  /** If true, the Android Framework resources will be shown. */
  var isShowFramework: Boolean by Delegates.observable(showAndroidResources) { _, old, new -> if (new != old) refreshResourcesCallback() }

  /** If true, Theme Attributes resources will be shown. */
  var isShowThemeAttributes: Boolean by
    Delegates.observable(showThemeAttributes) { _, old, new -> if (new != old) refreshResourcesCallback() }
  ...
  companion object {
    ...
    fun create(isShowResourcesChanged: () -> Unit, searchStringChanged: (String) -> Unit, initialParams: FilterOptionsParams) =
      FilterOptions(
...
class TypeFiltersModel {
  ...
  /**
   * Set the state for a given [TypeFilter] if it exists for the given [ResourceType]. If it results in value change Eg: false -> true.
   * Triggers the [valueChangedCallback] function.
   */
...
private fun linkedMapOf(vararg typeFilterPairs: Pair<TypeFilter, Boolean>): LinkedHashMap<TypeFilter, Boolean> {
  return LinkedHashMap<TypeFilter, Boolean>().apply { putAll(typeFilterPairs) }
}
 ...
 *
 * Eg: A [TypeFilter] defined by [kind] = [TypeFilterKind.XML_TAG] and [value] = `vector` is a filter that looks for the `vector` root tag
 * in an XML resource file.
 */
