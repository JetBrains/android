...
private val ASSET_CARD_WIDTH
  get() = JBUI.scale(150)

private val SEPARATOR_BORDER
  get() = JBUI.Borders.empty(2, 4)

private val HEADER_PANEL_BORDER
  get() = BorderFactory.createCompoundBorder(JBUI.Borders.customLine(JBColor.border(), 0, 0, 1, 0), JBUI.Borders.empty(6, 5))

private val BACK_BUTTON_SIZE
  get() = JBUI.size(20)

/**
 * A [JPanel] displaying the [DesignAsset]s composing the provided [designAssetSet]. When double clicking on the [DesignAsset], it opens the
 * corresponding file.
 *
 * @param viewModel an existing instance of [ResourceExplorerListViewModel]
 * @param backCallback a callback that will be called to remove this view and show the previous one. The callback receives this view as a
 *   parameter to allow the parent view to remove it.
 */
class ResourceDetailView(
  ...
  private val backAction =
    object : AnAction(AllIcons.Actions.Back) {
      init {
  ...
  private val sharedActionMap =
    ActionMap().apply {
      put(
        backAction,
        object : AbstractAction("VK_ESCAPE") {
  ...
  private val header =
    JPanel(HorizontalLayout(0, SwingConstants.CENTER)).apply {
  ...
  private val content =
    JPanel(WrappedFlowLayout(FlowLayout.LEFT)).apply {
  ...
  init {
    ...
    add(JBScrollPane(content, VERTICAL_SCROLLBAR_AS_NEEDED, HORIZONTAL_SCROLLBAR_NEVER).apply { border = null })
  }

  /** Register focus request on mouse click and invocation of [backCallback] when the ESC key is pressed. */
  ...
  private val focusRequestMouseAdapter =
    object : MouseAdapter() {
  ...
  /** Create a [SingleAssetCard] representing the [DesignAsset]. The thumbnail is populated asynchronously. */
  private fun createAssetCard(asset: DesignAsset) =
    SingleAssetCard().apply {
      ...
      val assetIcon = AssetIcon(previewManager, asset, thumbnailSize.width, thumbnailSize.height)
