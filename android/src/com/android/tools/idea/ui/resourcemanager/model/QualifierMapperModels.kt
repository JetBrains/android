...
/**
 * A Mapper returns a [ResourceQualifier] given a provided value that was matched using the regex [pattern]. It can also optionally provide
 * a default qualifier that will be used if the [pattern] was not matched to ensure that a qualifier is always supplied. For example if we
 * want files with no Density qualifier defined by their path, we can apply the medium density qualifier by default.
 */
interface Mapper<out T : ResourceQualifier> {

  /** The implementing class should return a [ResourceQualifier] and can use [value] to customize the return [ResourceQualifier] */
  ...
  /**
   * If the implementing class is using capturing group with [pattern] or need to customize the value used to called [getQualifier], it can
   * override this class
   */
 ...
 *
 * It will compile a regex in the form `(string1|string2...)` where `stringN` is a key in [matchers] and return the captured string when
 * calling [getValue]. Each value in the capturing group is mapped with a [ResourceQualifier] using [matchers].
 *
 * For example if we have the following files: file@2x.png, file@3x.png, we can define an [StaticStringMapper] with matchers {@2x ->
 * DensityQualifier(XHDPI), @3x -> DensityQualifier(XXHDPI)}
 *
 * If [defaultParam] is not defined, it will try to find an empty string key in [matchers] to use as the default parameter when calling
 * [getQualifier].
 */
class StaticStringMapper(val matchers: Map<String, ResourceQualifier>, private val defaultParam: ResourceQualifier? = matchers[""]) :
  ...
  constructor(matcherEntries: List<MatcherEntry>) : this(matchers = matcherEntries.associate { it.matchingString to it.matchedQualifier })
