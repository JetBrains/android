...
/**
 * Interface to extend the rendering capabilities of the resources explorer for arbitrary type of files. Use the
 * [DesignAssetRendererManager] to get a instance of a renderer for a given [VirtualFile].
 */
interface DesignAssetRenderer {
  ...
  /**
   * Implementing class should return an image corresponding to the [file] with dimension optimized to be displayed at the provided
   * [dimension] (this usually means equals or smaller).
   *
...
class DesignAssetRendererManager private constructor() {
  ...
  /** Returns a renderer for the given [file]. If not renderer is found, it falls back on a renderer that always returns a null image. */
...
/** A renderer that always returns null and used as a fallback when no renderer are available for a given file. */
