 ...
 */
class ImageCache private constructor(mergingUpdateQueue: MergingUpdateQueue?, private val objectToImage: Cache<AssetKey, CachedImage>) :
  Disposable {
  companion object {
    private val objectToImageCache by lazy { createObjectToImageCache(5, CACHE_WEIGHT_BYTES) }
     ...
     */
    fun createImageCache(parentDisposable: Disposable, mergingUpdateQueue: MergingUpdateQueue? = null) =
  ...
  @Async.Schedule
  private fun runOrQueue(asset: Asset, executeImmediately: Boolean = false, runnable: () -> Unit) {
  ...
  override fun dispose() {
    synchronized(pendingFutures) { pendingFutures.values.forEach { it?.cancel(true) } }
  }
  ...
  /**
   * Return the value identified by [AssetKey] in the cache if it exists, otherwise returns the [placeholder] image and gets the image from
   * the [CompletableFuture] returned by [computationFutureProvider].
   *
   * If [forceComputation] is true, the [CompletableFuture] will be ran even if a value is present in the cache.
   *
   * Note that if a value is present in the cache and [forceComputation] is true, the returned [BufferedImage] will be the value from the
   * cache.
   *
   * Once the image is cached, [onImageCached] is invoked on [executor] (or the EDT if none is provided)
   */
  fun computeAndGet(
    @Async.Schedule asset: Asset,
    ...
    if (
      (cachedImage == null || cachedImage.modificationStamp != asset.modificationStamp || forceComputation) &&
        !pendingFutures.containsKey(asset)
    ) {
      val executeImmediately = cachedImage == null // If we don't have any image, no need to wait.
      runOrQueue(asset, executeImmediately) { startComputation(computationFutureProvider, asset, onImageCached, executor) }
    }
  ...
  private fun startComputation(
    ...
    val future =
      computationFutureProvider().thenAccept { image: BufferedImage? ->
        synchronized(pendingFutures) { pendingFutures.remove(asset) }
        if (image != null) {
