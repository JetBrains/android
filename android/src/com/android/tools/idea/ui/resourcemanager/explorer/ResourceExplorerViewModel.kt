...
private constructor(
  ...
  var facet: AndroidFacet by
    Delegates.observable(defaultFacet) { _, oldFacet, newFacet ->
      if (newFacet != oldFacet) {
        contextFileForConfiguration = null // AndroidFacet changed, optional Configuration file is not valid.
        selectedModuleName = newFacet.getProductionAndroidModule().name
      ...
      .thenApplyAsync(
        Function { resourceResolver ->
          ResourceExplorerListViewModelImpl(
              ...
              summaryImageCache,
              selectAssetAction,
              { assetSet -> updateResourceCallback?.invoke(assetSet.getHighestDensityAsset().resourceItem) },
            )
            .also {
              listViewModel = it
  ...
  private fun subscribeListener(facet: AndroidFacet) {
    resourceNotificationManager.addListener(resourceNotificationListener, facet, null, null)
  }

  private fun unsubscribeListener(oldFacet: AndroidFacet) {
    resourceNotificationManager.removeListener(resourceNotificationListener, oldFacet, null, null)
  }
...
private class ViewModelState(
  ...
  var selectedResourceType: ResourceType by
    Delegates.observable(defaultSelectedResourceType) { _, _, newValue ->
      saveParams?.let {
  ...
  var selectedModuleName: String? by
    Delegates.observable(defaultSelectedModuleName) { _, _, newValue ->
      if (newValue != null) {
        saveParams?.let {
...
private class ViewModelStateSaveParams(val project: Project, val preferencesKey: String)
...
private fun getConfiguration(facet: AndroidFacet, contextFile: VirtualFile? = null): CompletableFuture<Configuration?> =
  CompletableFuture.supplyAsync(
    Supplier {
      ...
      contextFile?.let { configuration = configManager.getConfiguration(contextFile) }
      if (configuration == null) {
        runReadAction { facet.getPrimaryManifestXml() }
          ?.let { manifestFile -> configuration = configManager.getConfiguration(manifestFile.virtualFile) }
      }
...
private fun getResourceResolver(
  ...
  return configurationFuture.thenApplyAsync<ResourceResolver>(
    Function { configuration ->
      configuration?.let {
        return@Function it.resourceResolver
      }
      ...
      return@Function configurationManager.resolverCache.getResourceResolver(
        target,
        theme,
