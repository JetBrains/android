 ...
 *
 * This action replaces the [org.intellij.images.actions.ShowThumbnailsAction] when the file is an Android Res directory, otherwise it
 * delegates the event to the [org.intellij.images.actions.ShowThumbnailsAction]
 */
class ShowFileInResourceManagerAction : DumbAwareAction("Show In Resource Manager", "Display selected file in the Resource Manager", null) {
  ...
  override fun actionPerformed(e: AnActionEvent) {
    ...
    if (project != null && file != null && isSupportedInResManager(file, project)) {
  ...
  private fun getPsiDir(file: VirtualFile, project: Project): PsiDirectory? =
    if (file.isDirectory) {
