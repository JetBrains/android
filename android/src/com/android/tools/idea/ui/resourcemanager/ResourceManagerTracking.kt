...
object ResourceManagerTracking {
  ...
  fun logShowLibrariesToggle(facet: AndroidFacet, state: Boolean) = log(facet, if (state) Kind.LIBRARIES_SHOWN else Kind.LIBRARIES_HIDDEN)

  /** Called when the user toggles the 'android resources' filter */
  fun logShowFrameworkToggle(facet: AndroidFacet, state: Boolean) = log(facet, if (state) Kind.FRAMEWORK_SHOWN else Kind.FRAMEWORK_HIDDEN)
  ...
  /** Called when users drop or paste a resource from the Resource Manager onto a blank area in an XML file. */
  ...
  private fun createEvent(
    ...
    AndroidStudioEvent.newBuilder()
      ...
      .setResourceManagerEvent(ResourceManagerEvent.newBuilder().setKind(kind).setResourceType(type))
  ...
  private fun ResourceType?.toEventType() =
    when (this) {
