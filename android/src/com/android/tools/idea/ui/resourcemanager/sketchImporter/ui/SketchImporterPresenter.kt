...
private fun LightVirtualFile.toAsset(name: String) =
  ResourceAssetSet(name, listOf(DesignAsset(this, listOf(DensityQualifier(Density.ANYDPI)), ResourceType.DRAWABLE)))
 ...
 * user interface.
 */
class SketchImporterPresenter(
  private val sketchImporterView: SketchImporterView,
  ...
  private val presenters: MutableList<ResourcesPresenter> =
    sketchFile.pages
      .mapNotNull { page ->
      ...
      .toMutableList()
  private val drawableFileGenerator =
    com.android.tools.idea.ui.resourcemanager.sketchImporter.converter.builders.ResourceFileGenerator(project)
  ...
  fun importFilesIntoProject() {
    val drawables = presenters.flatMap { presenter -> presenter.getSelectedDrawables() }
    designAssetImporter.importDesignAssets(drawables, facet)

    val colors = presenters.flatMap { presenter -> presenter.getSelectedColors() }
    generateSketchColorsFile(colors)
  }

  /** Add all displayed resources to the project. */
  fun importAllFilesIntoProject() {
    val drawables = presenters.flatMap { presenter -> presenter.getDisplayableDrawables() }
    designAssetImporter.importDesignAssets(drawables, facet)

    val colors = presenters.flatMap { presenter -> presenter.getDisplayableColors() }
    generateSketchColorsFile(colors)
  }
  ...
  private fun generateSketchColorsFile(colors: List<Pair<Color, String>>) {
    ...
    val resFolder =
      sourceProvider?.resDirectories?.firstOrNull()
...
abstract class ResourcesPresenter(project: Project, val assetPreviewManager: AssetPreviewManager) {
  ...
  protected abstract val filesToDrawableAssets:
    Map<ResourceAssetSet, com.android.tools.idea.ui.resourcemanager.sketchImporter.converter.models.DrawableAssetModel>
  protected abstract val colorsToColorAssets:
    Map<Pair<Color, String>, com.android.tools.idea.ui.resourcemanager.sketchImporter.converter.models.ColorAssetModel>
  ...
  protected fun generateDrawableFiles() =
    resources.drawableAssets?.associate { drawableFileGenerator.generateDrawableFile(it).toAsset(it.name) to it } ?: emptyMap()
  ...
  protected fun generateColorPairs() = resources.colorAssets?.associate { (it.color to it.name) to it } ?: emptyMap()
  ...
  fun getAsset(file: ResourceAssetSet): com.android.tools.idea.ui.resourcemanager.sketchImporter.converter.models.AssetModel? =
    filesToDrawableAssets[file]
}

class PagePresenter(
  private val sketchPage: com.android.tools.idea.ui.resourcemanager.sketchImporter.parser.pages.SketchPage,
  val project: Project,
  ...
  override val resources: com.android.tools.idea.ui.resourcemanager.sketchImporter.converter.models.StudioResourcesModel =
    getResources(sketchPage, library)
...
class DocumentPresenter(
  sketchDocument: com.android.tools.idea.ui.resourcemanager.sketchImporter.parser.document.SketchDocument,
  val project: Project,
  library: com.android.tools.idea.ui.resourcemanager.sketchImporter.converter.SketchLibrary,
  assetPreviewManager: AssetPreviewManager,
) : ResourcesPresenter(project, assetPreviewManager) {
  override val resources: com.android.tools.idea.ui.resourcemanager.sketchImporter.converter.models.StudioResourcesModel =
    getResources(sketchDocument, library)
  override var filesToDrawableAssets = generateDrawableFiles()
  override val colorsToColorAssets = generateColorPairs()
