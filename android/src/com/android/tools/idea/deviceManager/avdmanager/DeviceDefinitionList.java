...
import com.android.tools.idea.deviceManager.avdmanager.actions.CloneDeviceAction;
import com.android.tools.idea.deviceManager.avdmanager.actions.CreateDeviceAction;
import com.android.tools.idea.deviceManager.avdmanager.actions.DeleteDeviceAction;
import com.android.tools.idea.deviceManager.avdmanager.actions.DeviceUiAction;
import com.android.tools.idea.deviceManager.avdmanager.actions.EditDeviceAction;
import com.android.tools.idea.deviceManager.avdmanager.actions.ExportDeviceAction;
import com.android.tools.idea.deviceManager.avdmanager.actions.ImportDevicesAction;
...
public class DeviceDefinitionList extends JPanel implements ListSelectionListener, DocumentListener, DeviceUiAction.DeviceProvider {
  ...
  private static final DecimalFormat OUR_DECIMAL_FORMAT = new DecimalFormat(".##");
  ...
  private final List<DeviceDefinitionSelectionListener> myListeners = Lists.newArrayList();
  private final List<DeviceCategorySelectionListener> myCategoryListeners = Lists.newArrayList();
  ...
  public static String getDiagonalSize(@NotNull Device device) {
    return OUR_DECIMAL_FORMAT.format(device.getDefaultHardware().getScreen().getDiagonalLength()) + '"';
  }
  ...
  private static class PlayStoreColumnInfo extends ColumnInfo<Device, Icon> {
    ...
    public static final Icon HIGHLIGHTED_PLAY_STORE_ICON = ColoredIconGenerator.INSTANCE.generateWhiteIcon(StudioIcons.Avd.DEVICE_PLAY_STORE);

    private static final TableCellRenderer OUR_ICON_RENDERER = new DefaultTableCellRenderer() {
      @Override
      public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
        ...
        if (table.getSelectedRow() == row) {
          ...
          label.setOpaque(true);
          if (theIcon != null) {
            label.setIcon(HIGHLIGHTED_PLAY_STORE_ICON);
          }
        }
        return label;
      }
