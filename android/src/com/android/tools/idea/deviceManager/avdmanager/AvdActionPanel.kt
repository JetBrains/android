 ...
 */
class AvdActionPanel(
  override val avdInfo: AvdInfo, numVisibleActions: Int, private val refreshProvider: AvdRefreshProvider
) : JPanel(), AvdUiAction.AvdInfoProvider {
  ...
  private val actions: List<AvdUiAction>
    get() = sequence {
      val provider = this@AvdActionPanel
      yield(RunAvdAction(provider))
      yield(ExploreAvdAction(provider))
      yield(EditAvdAction(provider))
      yield(DuplicateAvdAction(provider))
      // yield(new ExportAvdAction(provider)); // TODO: implement export/import
      yield(WipeAvdDataAction(provider))
      val fastBootSupported = EmulatorAdvFeatures.emulatorSupportsFastBoot(
        AndroidSdks.getInstance().tryToChooseSdkHandler(),
        StudioLoggerProgressIndicator(AvdActionPanel::class.java),
        LogWrapper(logger<AvdManagerConnection>())
      )
      if (fastBootSupported) {
        yield(ColdBootNowAction(provider))
      }
      yield(ShowAvdOnDiskAction(provider))
      yield(AvdSummaryAction(provider))
      yield(DeleteAvdAction(provider))
      yield(StopAvdAction(provider))
    }.toList()
  ...
  init {
    ...
    if (avdInfo.status != AvdInfo.AvdStatus.OK) {
      var action: AvdUiAction? = null
      if (AvdManagerConnection.isSystemImageDownloadProblem(avdInfo.status)) {
        action = InstallSystemImageAction(this)
      }
      else if (AvdManagerConnection.isAvdRepairable(avdInfo.status)) {
        action = RepairAvdAction(this)
      }
      if (action != null) {
        FocusableHyperlinkLabel(action.text, action.icon, action.isEnabled).apply {
          addHyperlinkListener(action)
          addVisibleHyperlinkLabel(this)
        }
      }
      else {
        add(JBLabel("Failed to load", AllIcons.General.BalloonError, SwingConstants.LEADING))
      }
      numVisibleActions = 0
      errorState = true
    }
