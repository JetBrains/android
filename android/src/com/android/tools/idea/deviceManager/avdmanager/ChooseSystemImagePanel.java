...
import com.android.tools.idea.deviceManager.avdmanager.SystemImagePreview.ImageRecommendation;
...
public class ChooseSystemImagePanel extends JPanel
  ...
  static SystemImageClassification getClassificationForDevice(@NotNull SystemImageDescription image, @Nullable Device theDevice) {
    SystemImageClassification classification = getClassificationFromParts(
      Abi.getEnum(image.getAbiType()), image.getVersion().getApiLevel(), image.getTag()
    );
  ...
  public static boolean systemImageMatchesDevice(@Nullable SystemImageDescription image, @Nullable Device device) {
    ...
    if (device.getId().equals("13.5in Freeform")) {
      ...
      if ("R".equals(image.getVersion().getCodename())) {
        image.getRevision();
        if (image.getRevision().compareTo(new Revision(2, 0, 0)) <= 0) {
          return false;
        }
      }
    }
