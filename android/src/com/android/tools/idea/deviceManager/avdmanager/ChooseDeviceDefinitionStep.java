...
/**
 * {@link ModelWizardStep} for selecting a device definition from the devices declared in the SDK and defined by the user.
 */
    ...
    myValidatorPanel.registerValidator(getModel().device(), new Validator<Optional<Device>>() {
      ...
      public Result validate(@NotNull Optional<Device> value) {
        return (value.isPresent())
               ? Result.OK
               : new Result(Severity.ERROR, "A hardware profile must be selected to continue.");
      }
    });

    myDeviceDefinitionPanel.addDeviceListener(device -> getModel().device().setNullableValue(device));
  }
