      ...
      try {
        ...
        while (myVisitQueue.isNotEmpty()) {
          ...
          for (clazz in classes) {
            ...
            ReferencesSearch.search(clazz, manifestScope).asIterable().forEach { reference ->
        ...
        for (clazz in classReferences) {
          ReferencesSearch.search(clazz, globalSearchScope).asIterable().forEach { reference ->
          ...
          if (clazz is KtClass) {
            clazz.companionObjects.forEach { companion ->
              ReferencesSearch.search(companion, globalSearchScope).asIterable().forEach { reference ->
                myGraphBuilder.markReferencedOutsideScope(clazz)
                LOGGER.debug("$clazz referenced from ${reference.element.containingFile}")
              }
            }
        ...
        for (item in resourceReferences) {
          ...
          if (seenResources.add(ref)) {
            ...
            for (field in fields) {
              ReferencesSearch.search(field, globalSearchScope).asIterable().forEach { reference ->
