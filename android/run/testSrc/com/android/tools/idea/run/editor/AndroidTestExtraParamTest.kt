...
class AndroidTestExtraParamTest {
  ...
  @get:Rule val projectRule: AndroidProjectRule = AndroidProjectRule.testProject(AndroidCoreTestProject.RUN_CONFIG_RUNNER_ARGUMENTS)

  @Test
  fun testParseFromString() {
    assertThat(parseFromString("-e key1 value1 -e key2 value2").toList())
      .containsExactly(AndroidTestExtraParam("key1", "value1"), AndroidTestExtraParam("key2", "value2"))

    assertThat(parseFromString("-e key1_no_value -e key2 value2 -e key3_no_value").toList())
      .containsExactly(
        AndroidTestExtraParam("key1_no_value", ""),
    ...
    assertThat(parseFromString("-e key1 value1 with space -e key2 value2 with space").toList())
      .containsExactly(AndroidTestExtraParam("key1", "value1 with space"), AndroidTestExtraParam("key2", "value2 with space"))

    // Leading and trailing whitespace should be removed.
    assertThat(parseFromString("    -e    key1     value1    -e    key2    value2     ").toList())
      .containsExactly(AndroidTestExtraParam("key1", "value1"), AndroidTestExtraParam("key2", "value2"))

    assertThat(parseFromString("-e k-ey1 valu-e1").toList()).containsExactly(AndroidTestExtraParam("k-ey1", "valu-e1"))
  ...
  fun testMerge() {
    ...
    var seq1 = sequenceOf(AndroidTestExtraParam("key1", "value1"), AndroidTestExtraParam("key2", "value2"))
    var seq2 = sequenceOf(AndroidTestExtraParam("key3", "value3"))
    assertThat(seq1.merge(seq2))
      .containsExactly(
    ...
    seq1 = sequenceOf(AndroidTestExtraParam("key1", "value1"), AndroidTestExtraParam("key2", "value2"))
    seq2 = sequenceOf(AndroidTestExtraParam("key1", "value3"))
    assertThat(seq1.merge(seq2)).containsExactly(AndroidTestExtraParam("key1", "value3"), AndroidTestExtraParam("key2", "value2"))
    ...
    seq1 =
      sequenceOf(
        AndroidTestExtraParam("key1", "value1", "value1", AndroidTestExtraParamSource.GRADLE),
        AndroidTestExtraParam("key2", "value2"),
      )
    seq2 = sequenceOf(AndroidTestExtraParam("key1", "value3"))
    assertThat(seq1.merge(seq2))
  ...
  fun testGetAndroidTestExtraParamsFromAndroidModuleModel() {
    assertThat(projectRule.module.androidFacet.getAndroidTestExtraParams().toList())
      .containsExactly(
