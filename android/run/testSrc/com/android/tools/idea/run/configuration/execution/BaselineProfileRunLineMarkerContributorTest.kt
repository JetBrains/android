...
import com.android.tools.idea.run.configuration.BaselineProfileRunLineMarkerContributor
...
class BaselineProfileRunLineMarkerContributorTest {

  @get:Rule
  val projectRule = AndroidProjectRule.inMemory().onEdt()

  private val expectedGenerateBaselineProfileAction by lazy {
  ...
  @Test
  @RunsInEdt
  fun `when class does NOT have BaselineProfileRule, it should NOT show contributor`() {
    val sourceFile = addBaselineProfileGeneratorToProject("""
        $SRC_FILE_HEADER
  ...
  @Test
  @RunsInEdt
  fun `when class has BaselineProfileRule, it should show contributor`() {
    val sourceFile = addBaselineProfileGeneratorToProject("""
        $SRC_FILE_HEADER
        class BaselineProfileGenerator {
    ...
    assertContributorInfo(sourceFile.funNamed("test"))
  }

  @Test
  @RunsInEdt
  fun `when inner class has BaselineProfileRule, outer class methods should NOT show contributor`() {
    val sourceFile = addBaselineProfileGeneratorToProject("""
        $SRC_FILE_HEADER
        class BaselineProfileGenerator {
  ...
  @Test
  @RunsInEdt
  fun `when outer class has BaselineProfileRule, inner class methods should show contributor`() {
    val sourceFile = addBaselineProfileGeneratorToProject("""
        $SRC_FILE_HEADER
        class BaselineProfileGenerator {
  ...
  private fun addBaselineProfileGeneratorToProject(content: String): BaselineProfileGeneratorSourceFile =
    BaselineProfileGeneratorSourceFile(
  ...
  private class BaselineProfileGeneratorSourceFile(private val psiFile: PsiFile) {
