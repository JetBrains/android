...
class DeclarativeFormatBlock(private val node: ASTNode, private val indent: Indent?, private val ctx: DeclarativeFormatContext) : ASTBlock {
  ...
  private fun buildChildren(): List<Block> {
    return node
      ...
      .map { childNode: ASTNode -> DeclarativeFormatBlock(node = childNode, indent = computeIndent(childNode), ctx = ctx) }
  }

  override fun getSpacing(child1: Block?, child2: Block): Spacing? = ctx.spacingBuilder.getSpacing(this, child1, child2)
    ...
    return when (node.elementType) {
      ...
      BLOCK_GROUP ->
        when (child.elementType) {
          ...
          SIMPLE_FACTORY,
          BLOCK,
          LINE_COMMENT -> Indent.getNormalIndent()
