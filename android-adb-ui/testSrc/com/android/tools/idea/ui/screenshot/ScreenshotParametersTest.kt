...
class ScreenshotParametersTest {
  ...
  fun testGetFramingOptionsAutomotiveMatchingAspectRatio() {
    ...
    val screenshotImage =
      ScreenshotImage(image, 0, screenshotParameters.deviceType, "Supercar", PRIMARY_DISPLAY_ID, Dimension(960, 1280), 180)
    val framingOptions = screenshotParameters.getFramingOptions(screenshotImage)
  ...
  fun testGetFramingOptionsAutomotiveGeneric() {
    ...
    val screenshotImage =
      ScreenshotImage(image, 0, screenshotParameters.deviceType, "Supercar", PRIMARY_DISPLAY_ID, Dimension(1280, 768), 180)
    val framingOptions = screenshotParameters.getFramingOptions(screenshotImage)
    assertThat(framingOptions.map(FramingOption::displayName)).containsExactly("Generic Tablet")
    assertThat(screenshotParameters.getDefaultFramingOption()).isEqualTo(0)
  }
  ...
  fun testGetFramingOptionsTv() {
    ...
    val screenshotImage = ScreenshotImage(image, 0, screenshotParameters.deviceType, "TV", PRIMARY_DISPLAY_ID, Dimension(1920, 1080), 480)
    val framingOptions = screenshotParameters.getFramingOptions(screenshotImage)
  ...
  fun testGetFramingOptionsWatch() {
    ...
    val screenshotImage =
      ScreenshotImage(
        image,
        0,
        screenshotParameters.deviceType,
        "Watch",
        PRIMARY_DISPLAY_ID,
        Dimension(384, 384),
        200,
  ...
  fun constructFromAvdFolder_missingDeviceName() {
    val avdFolder = Path.of("test.avd")
    val avdManager = TestAvdManagerConnection(mapOf("skin.path" to "/skins/my-skin"))
  ...
  fun constructFromAvdFolder() {
    val avdFolder = Path.of("test.avd")
    val avdManager = TestAvdManagerConnection(mapOf("avd.ini.displayname" to "My Device", "skin.path" to "/skins/my-skin"))
