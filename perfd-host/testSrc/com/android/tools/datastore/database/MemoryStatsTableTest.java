...
public class MemoryStatsTableTest {
  ...
  public void testInsertAndGetData() throws Exception {
    ...
    myStatsTable.insertMemory(VALID_PID, VALID_SESSION, Collections.singletonList(memSample));
    myStatsTable.insertAllocStats(VALID_PID, VALID_SESSION, Collections.singletonList(allocStatsSample));
    myStatsTable.insertGcStats(VALID_PID, VALID_SESSION, Collections.singletonList(gcStatsSample));
    myStatsTable.insertOrReplaceHeapInfo(VALID_PID, VALID_SESSION, finishedHeapSample);
    myStatsTable.insertOrReplaceHeapInfo(VALID_PID, VALID_SESSION, ongoingHeapSample);
    myStatsTable.insertOrReplaceAllocationsInfo(VALID_PID, VALID_SESSION, ongoingAllocSample);
    myStatsTable.insertOrReplaceAllocationsInfo(VALID_PID, VALID_SESSION, finishedAllocSample);

    // Perform a sequence of queries to ensure we are getting startTime-exclusive and endTime-inclusive data.
    MemoryData result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(VALID_SESSION).setProcessId(VALID_PID).setStartTime(-1).setEndTime(0).build());
    verifyMemoryDataResultCounts(result, 0, 0, 0, 0, 0);

    result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(VALID_SESSION).setProcessId(VALID_PID).setStartTime(0).setEndTime(1).build());
    verifyMemoryDataResultCounts(result, 1, 0, 0, 0, 0);
    assertEquals(memSample, result.getMemSamples(0));

    result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(VALID_SESSION).setProcessId(VALID_PID).setStartTime(1).setEndTime(2).build());
    verifyMemoryDataResultCounts(result, 0, 1, 0, 0, 0);
    assertEquals(allocStatsSample, result.getAllocStatsSamples(0));

    result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(VALID_SESSION).setProcessId(VALID_PID).setStartTime(2).setEndTime(3).build());
    verifyMemoryDataResultCounts(result, 0, 0, 0, 1, 0);
    assertEquals(ongoingHeapSample, result.getHeapDumpInfos(0));

    result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(VALID_SESSION).setProcessId(VALID_PID).setStartTime(3).setEndTime(4).build());
    ...
    result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(VALID_SESSION).setProcessId(VALID_PID).setStartTime(4).setEndTime(5).build());
    verifyMemoryDataResultCounts(result, 0, 0, 0, 2, 0);
    assertTrue(result.getHeapDumpInfosList().contains(ongoingHeapSample));
    assertTrue(result.getHeapDumpInfosList().contains(finishedHeapSample));
    ...
    result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(VALID_SESSION).setProcessId(VALID_PID).setStartTime(5).setEndTime(6).build());
    ...
    result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(VALID_SESSION).setProcessId(VALID_PID).setStartTime(6).setEndTime(7).build());
    ...
    result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(VALID_SESSION).setProcessId(VALID_PID).setStartTime(7).setEndTime(8).build());
    ...
    result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(VALID_SESSION).setProcessId(VALID_PID).setStartTime(8).setEndTime(9).build());
    verifyMemoryDataResultCounts(result, 0, 0, 0, 1, 1);
    assertEquals(ongoingHeapSample, result.getHeapDumpInfos(0));
    assertEquals(ongoingAllocSample, result.getAllocationsInfo(0));
    ...
    result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(VALID_SESSION).setProcessId(INVALID_PID).setStartTime(0).setEndTime(9).build());
    verifyMemoryDataResultCounts(result, 0, 0, 0, 0, 0);

    // Test that querying for an invalid session returns no data.
    result =
      myStatsTable
        .getData(MemoryRequest.newBuilder().setSession(INVALID_SESSION).setProcessId(VALID_PID).setStartTime(0).setEndTime(9).build());
    verifyMemoryDataResultCounts(result, 0, 0, 0, 0, 0);
  }
  ...
  public void testHeapDumpQueriesAfterInsertion() throws Exception {
    ...
    myStatsTable.insertOrReplaceHeapInfo(VALID_PID, VALID_SESSION, sample);

    // Test that Status is set to NOT_READY and dump data is null
    assertEquals(DumpDataResponse.Status.NOT_READY, myStatsTable.getHeapDumpStatus(VALID_PID, VALID_SESSION, sample.getStartTime()));
    assertNull(myStatsTable.getHeapDumpData(VALID_PID, VALID_SESSION, sample.getStartTime()));
    ...
    assertEquals(DumpDataResponse.Status.SUCCESS, myStatsTable.getHeapDumpStatus(VALID_PID, VALID_SESSION, sample.getStartTime()));
    assertTrue(Arrays.equals(rawBytes, myStatsTable.getHeapDumpData(VALID_PID, VALID_SESSION, sample.getStartTime())));

    // Test that querying for the invalid app id returns NOT FOUND
    assertEquals(DumpDataResponse.Status.NOT_FOUND, myStatsTable.getHeapDumpStatus(INVALID_PID, VALID_SESSION, sample.getStartTime()));
    assertNull(myStatsTable.getHeapDumpData(INVALID_PID, VALID_SESSION, sample.getStartTime()));

    // Test that querying for the invalid session returns NOT FOUND
    assertEquals(DumpDataResponse.Status.NOT_FOUND, myStatsTable.getHeapDumpStatus(VALID_PID, INVALID_SESSION, sample.getStartTime()));
    assertNull(myStatsTable.getHeapDumpData(VALID_PID, INVALID_SESSION, sample.getStartTime()));
  }
  ...
  public void testLegacyAllocationsQueriesAfterInsertion() throws Exception {
    ...
    myStatsTable.insertOrReplaceAllocationsInfo(VALID_PID, VALID_SESSION, sample);

    // Tests that the info has been inserted into table, but the event response + dump data are still null
    assertEquals(sample, myStatsTable.getAllocationsInfo(VALID_PID, VALID_SESSION, sample.getStartTime()));
    ...
    myStatsTable.updateLegacyAllocationDump(VALID_PID, VALID_SESSION, sample.getStartTime(), rawBytes);
    assertTrue(Arrays.equals(rawBytes, myStatsTable.getLegacyAllocationDumpData(VALID_PID, VALID_SESSION, sample.getStartTime())));

    // Test that querying for the invalid app id returns null
    assertNull(myStatsTable.getAllocationsInfo(INVALID_PID, VALID_SESSION, sample.getStartTime()));
    assertNull(myStatsTable.getLegacyAllocationData(INVALID_PID, VALID_SESSION, sample.getStartTime()));
    assertNull(myStatsTable.getLegacyAllocationDumpData(INVALID_PID, VALID_SESSION, sample.getStartTime()));

    // Test that querying for the invalid session returns null
    assertNull(myStatsTable.getAllocationsInfo(VALID_PID, INVALID_SESSION, sample.getStartTime()));
    assertNull(myStatsTable.getLegacyAllocationData(VALID_PID, INVALID_SESSION, sample.getStartTime()));
    assertNull(myStatsTable.getLegacyAllocationDumpData(VALID_PID, INVALID_SESSION, sample.getStartTime()));
  }
