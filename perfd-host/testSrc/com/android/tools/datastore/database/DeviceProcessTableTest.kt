...
class DeviceProcessTableTest : DatabaseTest<DeviceProcessTable>() {
  companion object {
    val FAKE_DEVICE_ID = StreamId.of(1)
  }
  ...
  override fun getTableQueryMethodsForVerification(): List<Consumer<DeviceProcessTable>> {
    return mutableListOf(
      ...
      (Consumer { it.insertOrUpdateProcess(StreamId.of(-1), Common.Process.getDefaultInstance()) }),
      (Consumer {
        it.updateAgentStatus(StreamId.of(-1), Common.Process.getDefaultInstance(), Common.AgentData.getDefaultInstance())
      }))
  }
