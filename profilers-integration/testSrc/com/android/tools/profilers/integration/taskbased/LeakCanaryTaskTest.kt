...
class LeakCanaryTaskTest : ProfilersTaskTestBase() {
  ...
  override fun verifyTaskStarted(studio: AndroidStudio) {
    ...
    verifyIdeaLog(".*Entering\\s+LeakCanary\\s+stage", 120)
  }

  override fun verifyTaskStopped(studio: AndroidStudio) {
    verifyIdeaLog(".*PROFILER\\:\\s+Session\\s+stopped.*support\\s+level\\s+\\=DEBUGGABLE\$", 120)
  }

  override fun verifyUIComponents(studio: AndroidStudio) {
    studio.waitForComponentByClass("TooltipLayeredPane")
  }
