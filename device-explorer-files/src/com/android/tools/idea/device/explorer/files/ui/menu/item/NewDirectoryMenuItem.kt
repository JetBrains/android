...
class NewDirectoryMenuItem(listener: DeviceFileExplorerActionListener, private val context: MenuContext) :
  SingleSelectionTreeMenuItem(listener) {
  override fun getText(nodes: List<DeviceFileEntryNode>): String = if (context == MenuContext.Toolbar) "New Directory" else "Directory"

  override val icon: Icon
    get() = if (context == MenuContext.Toolbar) AllIcons.Actions.NewFolder else AllIcons.Nodes.Folder
