...
abstract class SingleSelectionTreeMenuItem(listener: DeviceFileExplorerActionListener) : NonToggleMenuItem(listener) {
  override fun isEnabled(nodes: List<DeviceFileEntryNode>): Boolean = super.isEnabled(nodes) && nodes.size == 1

  override fun isVisible(nodes: List<DeviceFileEntryNode>): Boolean = super.isVisible(nodes) && nodes.size == 1
