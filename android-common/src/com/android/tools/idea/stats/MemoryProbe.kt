 ...
 *
 * Specify any classes that the utility should include with [includedClasses] and the [includedPackagePrefixes], but ignore all classes in
 * [excludedClasses]. All arrays are included by default.
 */
open class MemoryProbe(
  ...
  fun check(root: Any): Long = check(mapOf(root to "Root"))
  ...
  fun check(startRoots: Map<Any, String>): Long {
    reset()
    DebugReflectionUtil.walkObjects(10000, startRoots, Object::class.java, { include(it) }, { obj, ref -> add(obj, ref) })
    return size
  }
  ...
  private fun arraySize(obj: Any): Int {
    ...
    val componentSize =
      when (obj.javaClass.componentType) {
