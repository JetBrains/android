...
class FlowableCollectionTest {
  ...
  fun testMap() {
    ...
    assertThat(flowableCollection.map { "A$it" }.asCollection()).containsExactly("A1", "A2", "A3", "A4")
  ...
  fun testEmptyIsMappedToEmpty() {
    val emptyFlowableCollection = FlowableCollection.Present<Int>(emptyList()).map { it + 1 }
    assertTrue(emptyFlowableCollection is FlowableCollection.Present)
  }

  @Test
  fun testFlatMap() {
    val originalCollection = listOf(listOf(1), listOf(2, 3), listOf(4))

    val flowableCollection = FlowableCollection.Present(originalCollection)
    assertThat(flowableCollection.flatMap { it.asSequence() }.map { "A$it" }.asCollection()).containsExactly("A1", "A2", "A3", "A4")
  ...
  fun testFilter() {
    ...
    val flowableCollection = FlowableCollection.Present(originalCollection)
    assertEquals(FlowableCollection.Present(listOf(2, 4)), flowableCollection.filter { it % 2 == 0 })
  }
