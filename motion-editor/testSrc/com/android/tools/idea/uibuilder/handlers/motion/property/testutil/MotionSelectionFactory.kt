...
class MotionSelectionFactory(private val nlModel: NlModel, sceneFile: XmlFile) {
  ...
  private val motionScene =
    MotionSceneTag.parse(
      motionLayout,
      nlModel.project,
      sceneFile.virtualFile,
      sceneFile,
    )
  private val meModel =
    MeModel(motionScene, convertToNlComponentTag(motionLayout), null, null)
  ...
  private fun findConstraintSet(id: String): MotionSceneTag {
    return motionScene
      .getChildTags(MotionSceneAttrs.Tags.CONSTRAINTSET)
      .firstOrNull {
  ...
  private fun findTransition(start: String, end: String): MotionSceneTag {
    return motionScene
      .getChildTags(MotionSceneAttrs.Tags.TRANSITION)
      .firstOrNull {
        start ==
          Utils.stripID(
            it.getAttributeValue(
              MotionSceneAttrs.Transition.ATTR_CONSTRAINTSET_START
            )
          ) &&
          end ==
            Utils.stripID(
              it.getAttributeValue(
                MotionSceneAttrs.Transition.ATTR_CONSTRAINTSET_END
              )
            )
      } as? MotionSceneTag
  ...
  private fun findKeyFrameSet(start: String, end: String): MotionSceneTag {
    ...
    return transition
      .getChildTags(MotionSceneAttrs.Tags.KEY_FRAME_SET)
      ?.single() as? MotionSceneTag
      ?: throw error("KeyFrameSet not found: start=$start, end=$end")
  }

  private fun findConstraint(
    constraintSet: MotionSceneTag,
    id: String,
  ): MotionSceneTag? {
    return constraintSet
      .getChildTags(MotionSceneAttrs.Tags.CONSTRAINT)
      .firstOrNull {
        id == Utils.stripID(it.getAttributeValue(SdkConstants.ATTR_ID))
      } as? MotionSceneTag
  }
  ...
  private fun findKeyFrame(
    ...
    return keyFrameSet.getChildTags(keyType).singleOrNull {
      framePosition.toString() ==
        it.getAttributeValue(MotionSceneAttrs.Key.FRAME_POSITION) &&
        target ==
          Utils.stripID(
            it.getAttributeValue(MotionSceneAttrs.Key.MOTION_TARGET)
          )
    } as? MotionSceneTag
