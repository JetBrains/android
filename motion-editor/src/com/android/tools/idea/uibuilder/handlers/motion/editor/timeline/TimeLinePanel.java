...
public class TimeLinePanel extends JPanel {
  ...
  public void notifyTimeLineListeners(MotionEditorSelector.TimeLineCmd cmd, Float value) {
    for (MotionEditorSelector.TimeLineListener listener : mTimeLineListeners) {
  ...
  private void refreshCycling(boolean cycle) {
    ...
    if (cycle) {
      ...
      myMouseDownTimer = new Timer(MS_PER_FRAME, e -> {
        notifyTimeLineListeners(MotionEditorSelector.TimeLineCmd.MOTION_PROGRESS, mMotionProgress);
      });
  ...
  public void resetMotionProgress() {
    mMotionProgress = 0;
    notifyTimeLineListeners(MotionEditorSelector.TimeLineCmd.MOTION_PROGRESS, mMotionProgress);
    notifyTimeLineListeners(MotionEditorSelector.TimeLineCmd.MOTION_STOP, mMotionProgress);
    repaint();
  }
  ...
  private void performCommand(TimelineCommands e, int mode) {
    ...
    switch (e) {
      case PLAY:
        ...
        createTimer();
        notifyTimeLineListeners(MotionEditorSelector.TimeLineCmd.MOTION_PLAY, mMotionProgress);
        mIsPlaying = true;
        break;
      case SPEED:
    ...
    switch (myYoyo) {
      case 0:
        ...
        if (mMotionProgress > 1f) {
          notifyTimeLineListeners(MotionEditorSelector.TimeLineCmd.MOTION_PROGRESS, 1f);
      ...
      case 1:
        ...
        if (mMotionProgress < 0f) {
          notifyTimeLineListeners(MotionEditorSelector.TimeLineCmd.MOTION_PROGRESS, 0f);
  ...
  private void processMouseDrag(MouseEvent e) {
    ...
    switch (e.getID()) {
      ...
      case MouseEvent.MOUSE_PRESSED: {
        ...
        if (mMouseDown) {
          notifyTimeLineListeners(MotionEditorSelector.TimeLineCmd.MOTION_SCRUB, mMotionProgress);
        }
        repaint();
      }
      break;
    ...
    if ((e.getModifiersEx() & MouseEvent.BUTTON1_DOWN_MASK) != 0) {
      ...
      if (progress >= 0.0f && progress <= 1.0f) {
        ...
        notifyTimeLineListeners(MotionEditorSelector.TimeLineCmd.MOTION_PROGRESS, progress);
        repaint();
      }
