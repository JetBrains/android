...
class MessageProcessorTest {
  ...
  private val messageBatches = mutableListOf<List<String>>()
  private val messageFormatter = ::formatMessages
  private val messageAppender = ::appendMessages
  ...
  fun appendMessages_batchesJoined(): Unit = runBlocking {
    val messageProcessor = MessageProcessor(disposable, messageFormatter, messageAppender)
    val batch1 = listOf(
    ...
    messageProcessor.onIdle {
      assertThat(messageBatches).containsExactly((batch1 + batch2).mapMessages())
    }
  }

  @Test
  fun appendMessages_batchesSplitOnMaxMessagesPerBatch() = runBlocking {
    val messageProcessor = MessageProcessor(disposable, messageFormatter, messageAppender, maxMessagesPerBatch = 1)
    val batch1 = listOf(
      LogCatMessage(LogCatHeader(WARN, 1, 2, "app1", "tag1", timestamp), "message1"),
      LogCatMessage(LogCatHeader(WARN, 1, 2, "app1", "tag2", timestamp), "message2"),
    )
  ...
  fun appendMessages_batchesSplitOnMaxTimePerBatchMs() = runBlocking {
    ...
    val messageProcessor = MessageProcessor(disposable, messageFormatter, messageAppender, maxTimePerBatchMs = 500, clock = mockClock)
  ...
  @Suppress("RedundantSuspendModifier") // Needs to be "suspend" in order to be used by MessageProcessor
