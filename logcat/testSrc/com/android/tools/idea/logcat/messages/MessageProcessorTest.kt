...
class MessageProcessorTest {
  ...
  fun appendMessages_batchesSplitOnMaxMessagesPerBatch() = runBlocking {
    val messageProcessor =
      messageProcessor(fakeLogcatPresenter, maxMessagesPerBatch = 1, autoStart = false)
    val batch1 =
      listOf(
        LogcatMessage(LogcatHeader(WARN, 1, 2, "app1", "", "tag1", timestamp), "message1"),
        LogcatMessage(LogcatHeader(WARN, 1, 2, "app1", "", "tag2", timestamp), "message2"),
  ...
  // We have to mock the clock ahead of time because of the asynchronous nature of the tested code.
  // If we set it up before each call to
  // appendMessages, they will all be executed immediately and the tested code will only see the
  // final value.
  @Test
  fun appendMessages_batchesSplitOnMaxTimePerBatchMs() = runBlocking {
    ...
    // setting up the mock with (1000, 2000, 3000) but that's not an accurate representation of what
    // actually happens where the first 2
    // calls to clock.millis() happen almost at the same time.
    val batch1 =
      listOf(LogcatMessage(LogcatHeader(WARN, 1, 2, "app1", "", "tag1", timestamp), "message1"))
    val batch2 =
      listOf(LogcatMessage(LogcatHeader(WARN, 1, 2, "app2", "", "tag2", timestamp), "message2"))
    val batch3 =
      listOf(LogcatMessage(LogcatHeader(WARN, 1, 2, "app2", "", "tag2", timestamp), "message3"))
    ...
    messageProcessor.onIdle {
      @Suppress("ConvertLambdaToReference") // Calling inOrder() confuses IDEA.
      assertThat(fakeLogcatPresenter.lineBatches)
        .containsExactly((batch1 + batch2).mapMessages(), batch3.mapMessages())
        .inOrder()
    }
  }
  ...
  fun appendMessages_emptyMessages() = runBlocking {
    ...
    val messageProcessor = messageProcessor(fakeLogcatPresenter)
    val batch = listOf(message1, message2)
    messageProcessor.logcatFilter =
      StringFilter("no-such-line", LINE, matchCase = true, EMPTY_RANGE)
    messageProcessor.appendMessages(batch)

    messageProcessor.onIdle { assertThat(fakeLogcatPresenter.lineBatches).isEmpty() }
  }
