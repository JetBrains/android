...
class MessageProcessorTest {
  ...
  fun appendMessages_batchesJoined(): Unit = runBlocking {
    ...
    messageProcessor.onIdle { assertThat(fakeLogcatPresenter.lineBatches).containsExactly((batch1 + batch2).mapMessages()) }
  }
  ...
  fun appendMessages_batchesSplitOnMaxTimePerBatchMs() = runBlocking {
    ...
    messageProcessor.onIdle {
      @Suppress("ConvertLambdaToReference") // Calling inOrder() confuses IDEA.
      assertThat(fakeLogcatPresenter.lineBatches).containsExactly((batch1 + batch2).mapMessages(), batch3.mapMessages()).inOrder()
    }
  }
  ...
  fun appendMessages_filters() = runBlocking {
    ...
    messageProcessor.onIdle { assertThat(fakeLogcatPresenter.lineBatches).containsExactly(listOf(message2).mapMessages()) }
  }
