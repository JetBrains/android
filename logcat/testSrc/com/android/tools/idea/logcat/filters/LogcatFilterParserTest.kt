...
class LogcatFilterParserTest(private val matchCase: Boolean) {
  ...
  fun parse_regexKey() {
    for ((key, field) in keys) {
      ...
      assertThat(logcatFilterParser().parse("$key~:Foo", matchCase)).isEqualTo(RegexFilter("Foo", field, matchCase, "$key~:Foo".asRange()))
    }
  }
  ...
  fun getUsageTrackingEvent_operators() {
    assertThat(logcatFilterParser().getUsageTrackingEvent("(foo | bar) & (for | boo)", matchCase)?.build())
      .isEqualTo(LogcatFilterEvent.newBuilder().setImplicitLineTerms(4).setAndOperators(1).setOrOperators(2).setParentheses(2).build())
  }
  ...
  fun parse_name_quoted() {
    ...
    assertThat(filter)
      .isEqualTo(AndLogcatFilter(NameFilter("Name1", query.rangeOf("name:'Name1'")), NameFilter("Name2", query.rangeOf("name:\"Name2\""))))
  }
