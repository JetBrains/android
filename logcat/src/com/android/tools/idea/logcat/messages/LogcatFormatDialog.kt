 ...
 */
internal open class LogcatFormatDialog(private val project: Project, formattingOptions: FormattingOptions) : Disposable {
  ...
  // This is lazy because it calls createPanel() which in turn calls the non-final createComponents() method which we don't want to call
  // from the constructor.
  val dialogWrapper by lazy {
    dialog(
      project = project,
      ...
      Disposer.register(disposable, this@LogcatFormatDialog)
      onComponentsChanged()
      pack()
    }
  }
  ...
  private fun createPanel(formattingOptions: FormattingOptions): DialogPanel {
    val panel = panel {
