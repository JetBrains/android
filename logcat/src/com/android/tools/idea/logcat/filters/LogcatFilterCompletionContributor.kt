...
private val IS_LOOKUPS
  get() = IS_VALUES.map { (value, hint) -> createLookupElement("$IS_KEY$value ", hint) }

private val IS_VALUE_LOOKUPS
  get() = IS_VALUES.map { (value, hint) -> createLookupElement("$value ", hint) }
...
private val KEY_LOOKUPS
  get() = BASE_KEY_LOOKUPS + LEVEL_LOOKUP + IS_LOOKUP
private val ALL_KEY_LOOKUPS
  get() =
    BASE_KEY_LOOKUPS +
      LEVEL_LOOKUPS +
      IS_LOOKUPS +
      ...
      object : CompletionProvider<CompletionParameters>() {
        override fun addCompletions(
          ...
          if (PsiTreeUtil.findSiblingBackward(parameters.position, ERROR_ELEMENT, null) != null) {
            // Do not complete a key if there is an error in the current level of the tree. This
            // happens when we are in an
