...
sealed class TargetsToBuild() {
  ...
  data class TargetGroup(override val targets: Set<Label>) : TargetsToBuild() {
    override val displayLabel: String
      get() = targets.joinToString(", ", limit = 3)
  ...
  data class UnknownSourceFile(private val sourceFile: Path) : TargetsToBuild() {
    ...
    override val targets: Set<Label>
      get() = emptySet()
  }
  ...
  companion object {
    @JvmStatic fun targetGroup(targets: Collection<Label>): TargetGroup = TargetGroup(targets.toSet())

    @JvmStatic fun forSourceFile(targets: Collection<Label>, sourceFile: Path): SourceFile = SourceFile(targets.toSet(), sourceFile)

    @JvmStatic fun forUnknownSourceFile(sourceFile: Path): UnknownSourceFile = UnknownSourceFile(sourceFile)
  }
}
