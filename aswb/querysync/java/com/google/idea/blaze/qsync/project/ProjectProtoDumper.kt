...
inline fun <reified T : FormattableModel> T.formatTo(to: OutputStream) {
  PrintStream(BufferedOutputStream(to)).use { formatToAs(it, T::class) }
}
...
@JvmName("formatTo") fun ProjectProto.Project.formatTo_javaShim(to: OutputStream) = formatTo(to)

fun FormattableModel.formatToAs(to: PrintStream, klass: KClass<*>) {
  val printer = PrinterImpl(to)
  @Suppress("UNCHECKED_CAST") (Dumpers.dumper(klass) as ValueFormatter<FormattableModel>).formatTo(this, printer)
}
...
private val KType.isSimple: Boolean
  get() = (this.classifier as? KClass<*>)?.typeParameters?.isEmpty() ?: false

private object Dumpers {
  private val registered: MutableMap<KClass<*>, ValueFormatter<*>> =
    mutableMapOf(
  ...
  object SealedValueFormatter : ValueFormatter<Any> {
    override fun formatTo(v: Any?, printer: ValuePrinter) {
      if (v == null) return
      @Suppress("UNCHECKED_CAST") val dumper = dumper(v::class) as ValueFormatter<Any>
...
private class AutoValueFormatter<T : FormattableModel>(clazz: KClass<T>) : ValueFormatter<T> {
  ...
  sealed class Case<in T, in V>(private val property: KProperty<V>, private val getter: (T) -> V?) {
    fun printTo(receiver: T?, printer: NestedValuePrinter) {
      ...
      printer.prop(property) { printTo(v) }
    }

    abstract fun ValuePrinter.printTo(value: V?)
  }

  class ListCase<T, V>(property: KProperty<Collection<V>>, getter: (T) -> Collection<V>?, private val valueFormatter: ValueFormatter<V>) :
  ...
  class MapCase<T, V>(property: KProperty<Map<Any, V>>, getter: (T) -> Map<Any, V>?, private val valueFormatter: ValueFormatter<V>) :
  ...
  class SimpleCase<T, V>(property: KProperty<V>, getter: (T) -> V?, val valueFormatter: ValueFormatter<V>) :
  ...
  init {
    ...
    printFunction =
      fun(v: T?, printer: ValuePrinter) {
        printer.value(valueFunction?.invoke(v).orEmpty()) {
          cases.drop(if (valueFunction != null) 1 else 0).forEach { case -> case.printTo(v, this@value) }
        }
      }
  }
