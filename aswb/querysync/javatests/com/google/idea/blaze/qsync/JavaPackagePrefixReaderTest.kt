...
class JavaPackagePrefixReaderTest {
  ...
  fun readPrefixesSinglePackage() {
    runBlocking<Unit> {
      ...
      val result = reader.readPrefixes(NoopContext(), PackageSet.of(Path.of("java/com/example")), ImmutableList.of(file))
  ...
  fun readPrefixesWithSubpackages() {
    runBlocking<Unit> {
      ...
      val reader = JavaPackagePrefixReaderImpl(workspaceRoot, packageReader, parallelPackageReader, fileExistenceCheck = { true })
      val result = reader.readPrefixes(NoopContext(), PackageSet.of(Path.of("java/com/example")), ImmutableList.of(file1, file2))
      assertThat(result).containsExactly(Path.of("java/com/example"), "com.example")
    }
  }
  ...
  fun readPrefixesNestedPackages() {
    runBlocking<Unit> {
      ...
      assertThat(result).containsExactly(Path.of("java/com/example"), "com.example", Path.of("java/com/example/sub"), "com.example.sub")
    }
  }
  ...
  fun readPrefixesEmptyPackage() {
    runBlocking {
      ...
      pkg.toFile().mkdirs()
      val reader = JavaPackagePrefixReaderImpl(workspaceRoot, packageReader, parallelPackageReader, fileExistenceCheck = { true })
      val result = reader.readPrefixes(NoopContext(), PackageSet.of(Path.of("java/com/example")), ImmutableList.of())
    ...
    runBlocking<Unit> {
      ...
      val reader =
        JavaPackagePrefixReaderImpl(
          workspaceRoot,
          capturingPackageReader,
          parallelPackageReader,
          fileExistenceCheck = { it in existingFiles },
        )

      val result = reader.readPrefixes(NoopContext(), PackageSet.of(Path.of("java/com/example")), ImmutableList.of(file1, file2, file3))
  ...
  fun readPrefixesTwoPackageRootsInSameBazelPackage() {
    runBlocking<Unit> {
      ...
      assertThat(result).containsExactly(Path.of("java/com/example1"), "com.example1", Path.of("java/com/example2"), "com.example2")
    }
  }
  ...
  fun readPrefixesMismatchedDirectoryAndPackageNames() {
    runBlocking<Unit> {
      ...
      assertThat(result).containsExactly(Path.of("java/com/actual1"), "com.declared1", Path.of("java/com/actual2"), "com.declared2")
    }
  }
  ...
  fun readPrefixesFileInRootPackageDir() {
    runBlocking<Unit> {
      ...
      assertThat(result).containsExactly(Path.of("a"), "a")
    }
  }
}
