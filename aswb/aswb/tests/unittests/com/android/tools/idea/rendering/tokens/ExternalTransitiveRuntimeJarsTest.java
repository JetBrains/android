...
public final class ExternalTransitiveRuntimeJarsTest {
  ...
  private final BlazeContext blazeContext = BlazeContext.create();

  @Test
  public void buildBasicLib() {
    // Arrange
    rule.runTest(rule.prepareTestProject(InBazelTestProjects.SIMPLE_COMPOSE), bazelTestProjectContext -> {
      ...
      assertEquals(expectedJars, toListOfArtifactPaths(output.getExternalTransitiveRuntimeJars()));
    });
  }

  @Test
  public void buildCc1() {
    // Arrange
    rule.runTest(rule.prepareTestProject(InBazelTestProjects.CC1), bazelTestProjectContext -> {
      var builder = bazelTestProjectContext.getQuerySyncProjectInternalServices().dependencyBuilder();

      // Act
      var output = builder.build(blazeContext, Set.of(Label.of("//cc1/project:cc1")),
                                 EnumSet.of(OutputGroup.CC_GEN_HEADERS, OutputGroup.CC_INFO_FILE));

      // Assert
      var expectedHeaders = pathListOf(
        "bazel-out/k8-fastbuild/bin/cc1/project/gen/TestClass-g.h"
      );

      assertEquals(expectedHeaders, toListOfArtifactPaths(output.getCcGeneratedHeaders()));

      var expectedFiles = pathListOf(
        "bazel-out/k8-fastbuild/bin/cc1/project/cc1.cc-info.txt",
        "bazel-out/k8-fastbuild/bin/external/rules_cc+/cc/current_cc_toolchain.local.txt"
      );

      assertEquals(expectedFiles, toListOfArtifactPaths(output.getCcInfoFile()));
    });
  }

  private static Collection<Path> pathListOf(String... paths) {
    return Arrays.stream(paths)
      .map(Path::of)
      .toList();
  }

  private static Collection<Path> toListOfArtifactPaths(Collection<OutputArtifact> artifacts) {
