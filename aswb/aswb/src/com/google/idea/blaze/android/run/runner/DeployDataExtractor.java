...
final class DeployDataExtractor {
  ...
  public static DeployData extract(
    ...
    try {
      ...
      deployInfoProto =
          fetchAndParseDeployInfo(infoBuildArtifacts, deployInfoSuffix, context, project);
    } catch (IOException e) {
    ...
    String mergedManifestPath =
        getBazelOutRelativePath(deployInfoProto.getMergedManifest().getExecRootPath());
    OutputArtifact manifestArtifact =
        infoBuildArtifacts.stream()
            .filter(
                a ->
                    mergedManifestPath.equals(
                        getBazelOutRelativePath(a.getArtifactPath().toString())))
            .collect(MoreCollectors.onlyElement());

    try {
      return DeployData.create(
          label, parseManifest(manifestArtifact, context, project), apkArtifacts);
    } catch (IOException e) {
  ...
  private static AndroidDeployInfo fetchAndParseDeployInfo(
      Collection<OutputArtifact> bepOutput,
      String deployInfoSuffix,
      BlazeContext context,
      Project project)
      throws IOException {
    OutputArtifact deployInfoArtifact =
        bepOutput.stream()
            .filter(a -> a.getArtifactPath().toString().endsWith(deployInfoSuffix))
            .collect(MoreCollectors.onlyElement());
    try (BufferedInputStream inputStream =
        ArtifactFetchers.downloadArtifact(deployInfoArtifact, context, project)) {
      return AndroidDeployInfo.parseFrom(inputStream);
    }
  }

  private static ParsedManifest parseManifest(
      OutputArtifact mergedManifestArtifact, BlazeContext context, Project project)
      throws IOException {
    try (InputStream is =
        ArtifactFetchers.downloadArtifact(mergedManifestArtifact, context, project)) {
