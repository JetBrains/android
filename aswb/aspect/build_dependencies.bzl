...
def merge_dependencies_info(
    ...
    merged = create_dependencies_info(
        ...
        external_transitive_runtime_jars = java_dep_info.external_transitive_runtime_jars if java_dep_info else None,
...
def _collect_own_and_dependency_java_artifacts(
    ...
    return struct(
        ...
        external_transitive_runtime_jars = _get_external_transitive_runtime_jars(not target_is_within_project_scope, own_files.transitive_runtime_jars, dependency_infos),
    )

def _get_external_transitive_runtime_jars(is_external, transitive_runtime_jars, dependency_infos):
    return transitive_runtime_jars if is_external else depset(transitive = [dep.external_transitive_runtime_jars for dep in dependency_infos.values()])
