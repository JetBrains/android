 ...
 *
 * This class uses a tree to store the set of targets so that finding all the child targets of a given directory is fast.
 */
...
data class TargetTreeImpl private constructor(private val sortedLabels: List<Label>) : TargetTree {
  ...
  override fun getDirectTargets(pkg: Path): Sequence<Label> =
    sortedLabels.subList(getPackageStartIndex(workspace = "", pkg), sortedLabels.size).asSequence().takeWhile {
      it.workspace == "" && it.getBuildPackagePath() == pkg
    }

  override fun getSubpackages(pkg: Path): Sequence<Label> =
    sortedLabels.subList(getPackageStartIndex(workspace = "", pkg), sortedLabels.size).asSequence().takeWhile {
      it.workspace == "" && it.getBuildPackagePath().startsWithRespectingEmpty(pkg)
    }
  ...
  companion object {
    @JvmField val EMPTY: TargetTreeImpl = TargetTreeImpl(sortedLabels = emptyList())

    fun create(targets: Collection<Label>): TargetTree {
      return TargetTreeImpl(
        sortedLabels =
          targets.sortedWith { a, b ->
