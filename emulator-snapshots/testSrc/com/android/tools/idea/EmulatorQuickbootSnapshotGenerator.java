...
public class EmulatorQuickbootSnapshotGenerator {
  ...
  public static Path getPackageDirectory(String[] files) throws IOException {
    for (String file : files) {
      if (file.endsWith("source.properties")) {
        // eg. ../+_repo_rules9+system_image_android-31_default_arm64/source.properties
        Path sourceProperties = Paths.get(file);
        // extract system image directory name
        // +_repo_rules9+system_image_android-31_default_arm64
        Path systemImageDirName = sourceProperties.subpath(1, sourceProperties.getNameCount() - 1);
        // system images are served under canonical repositories
        // eg. @system_image_android-31_default_arm64//:arm64-android-31-images
        // bazel serves all the non-local dependencies under 'external/' directory
        return Paths.get("external").resolve(systemImageDirName).toRealPath();
      }
    }
