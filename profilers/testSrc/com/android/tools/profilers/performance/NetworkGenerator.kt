...
class NetworkGenerator(connection: Connection) : DataGenerator(connection) {
  ...
  private fun generateDataPacket(timestamp: Long, properties: GeneratorProperties) {
    val data =
      NetworkProfiler.NetworkProfilerData.newBuilder()
        .setEndTimestamp(timestamp)
        .setSpeedData(NetworkProfiler.SpeedData.newBuilder().setReceived(Math.abs(random.nextLong())).setSent(Math.abs(random.nextLong())))
        .setConnectionData(NetworkProfiler.ConnectionData.newBuilder().setConnectionNumber(random.nextInt()))
        .build()
  ...
  private fun generateHttpRequestData(timestamp: Long, properties: GeneratorProperties) {
    val defaultData =
      NetworkProfiler.HttpDetailsResponse.newBuilder()
        .setRequest(
          NetworkProfiler.HttpDetailsResponse.Request.newBuilder()
            .setFields("HEADER")
            .setMethod("POST")
            .setTraceId("0")
        ...
        .build()
    val connection =
      NetworkProfiler.HttpConnectionData.newBuilder()
