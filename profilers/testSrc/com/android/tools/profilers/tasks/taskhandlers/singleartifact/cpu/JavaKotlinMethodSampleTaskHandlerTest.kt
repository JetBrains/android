...
class JavaKotlinMethodSampleTaskHandlerTest(private val myExposureLevel: ExposureLevel) {
  ...
  fun testStartTaskInvokedOnEnterWithAliveSession() {
    ...
    val javaKotlinMethodSampleSessionArtifact = SessionArtifactUtils.createCpuCaptureSessionArtifactWithConfig(myProfilers,
                                                                                                               ...
                                                                                                               100L,
                                                                                                               createDefaultArtSampleTraceConfiguration())
    val cpuTaskArgs = CpuTaskArgs(false, javaKotlinMethodSampleSessionArtifact)
  ...
  fun testStartTaskWithSetStage() {
    ...
    myJavaKotlinMethodSampleTaskHandler.startTask(CpuTaskArgs(false, null))
  ...
  fun testLoadTaskWithNullTaskArgsArtifact() {
    ...
    val exception = assertFailsWith<Throwable> {
      myJavaKotlinMethodSampleTaskHandler.loadTask(CpuTaskArgs(false, null))
    }

    assertThat(exception.message).isEqualTo(
      "There was an error with the Java/Kotlin Method Sample (legacy) task. Error message: The task arguments (CpuTaskArgs) supplied " +
      "do not contains a valid artifact to load.")
  ...
  fun testCreateArgsSuccessfully() {
    ...
    val cpuTaskArgs = myJavaKotlinMethodSampleTaskHandler.createArgs(false, sessionIdToSessionItems, selectedSession)
  ...
  fun testCreateArgsFailsToFindArtifact() {
    ...
    assertThrows(IllegalStateException::class.java) {
      myJavaKotlinMethodSampleTaskHandler.createArgs(false, sessionIdToSessionItems, selectedSession)
    }
  }
  ...
  fun testSupportsDeviceAndProcess() {
    ...
    assertThat(myJavaKotlinMethodSampleTaskHandler.supportsDeviceAndProcess(minVersionDevice, process)).isTrue()
  }
