...
class SystemTraceSurfaceflingerManagerTest {
  ...
  private companion object {
    ...
    private val SF_MAIN_THREAD_MODE =
      ThreadModel(
        SF_PID,
        SF_PID,
        "surfaceflinger",
        listOf(createEvent(3000, 5000), createEvent(7000, 15000), createEvent(15000, 20000)),
        listOf(),
        listOf(),
      )

    private val VSYNC_COUNTER =
      CounterModel("VSYNC-app", sortedMapOf(5000L to 0.0, 10000L to 1.0, 15000L to 0.0, 20000L to 1.0, 25000L to 0.0))

    private val BUFFER_QUEUE_COUNTER =
      CounterModel("com.example.app/mainActivity#0", sortedMapOf(1000L to 0.0, 2000L to 1.0, 3000L to 2.0, 4000L to 1.0, 5000L to 0.0))

    private val SF_PROCESS_MODEL =
      ProcessModel(
        SF_PID,
        "/system/bin/surfaceflinger",
        mapOf(SF_PID to SF_MAIN_THREAD_MODE),
        mapOf("VSYNC-app" to VSYNC_COUNTER, "com.example.app/mainActivity#0" to BUFFER_QUEUE_COUNTER),
      )
  ...
  fun vsyncCounterValues() {
    ...
    assertThat(vsyncValues)
      .containsExactly(SeriesData(5000, 0L), SeriesData(10000, 1L), SeriesData(15000, 0L), SeriesData(20000, 1L), SeriesData(25000, 0L))
      .inOrder()
  }
  ...
  fun bufferQueueValues() {
    val sfManager = SystemTraceSurfaceflingerManager(MODEL, MAIN_PROCESS_NAME)
    assertThat(sfManager.bufferQueueValues)
      .containsExactly(SeriesData(1000, 0L), SeriesData(2000, 1L), SeriesData(3000, 2L), SeriesData(4000, 1L), SeriesData(5000, 0L))
      .inOrder()
  }

  @Test
  fun surfaceViewBufferQueue() {
    val surfaceViewBufferQueueCounters =
      CounterModel(
        "SurfaceView - com.example.app/mainActivity#0",
        sortedMapOf(1000L to 2.0, 2000L to 1.0, 3000L to 0.0, 4000L to 1.0, 5000L to 2.0),
      )
    val model =
      TestModel(
        listOf(
          ProcessModel(
            ...
            "/system/bin/surfaceflinger",
            mapOf(SF_PID to SF_MAIN_THREAD_MODE),
            mapOf("VSYNC-app" to VSYNC_COUNTER, "SurfaceView - com.example.app/mainActivity#0" to surfaceViewBufferQueueCounters),
          )
        )
      )
    val surfaceViewSfManager = SystemTraceSurfaceflingerManager(model, MAIN_PROCESS_NAME)
    assertThat(surfaceViewSfManager.bufferQueueValues)
      .containsExactly(SeriesData(1000, 2L), SeriesData(2000, 1L), SeriesData(3000, 0L), SeriesData(4000, 1L), SeriesData(5000, 2L))
      .inOrder()
  }
  ...
  fun noMatchingBufferQueueValuesForTxCounters() {
    val txCounters =
      CounterModel("TX - com.example.app/mainActivity#0", sortedMapOf(1000L to 0.0, 2000L to 1.0, 3000L to 0.0, 4000L to 1.0, 5000L to 0.0))
    val model =
      TestModel(
        listOf(
          ProcessModel(
            ...
            "/system/bin/surfaceflinger",
            mapOf(SF_PID to SF_MAIN_THREAD_MODE),
            mapOf("VSYNC-app" to VSYNC_COUNTER, "TX - com.example.app/mainActivity#0" to txCounters),
          )
        )
      )
    val surfaceViewSfManager = SystemTraceSurfaceflingerManager(model, MAIN_PROCESS_NAME)
  ...
  private class TestModel(private val processes: List<ProcessModel>) : SystemTraceModelAdapter {
    ...
    override fun getAndroidFrameTimelineEvents(): List<AndroidFrameTimelineEvent> =
