...
class SystemTraceCpuCaptureBuilderTest {
  ...
  fun `buildThreadStateData - termination state present`() {
    val sleepingThread =
      ThreadModel(
        1,
        1,
        "ENDS_WITH_SLEEPING_STATE_THREAD",
        listOf(),
    ...
    val runningThread =
      ThreadModel(
        2,
        2,
        "ENDS_WITH_RUNNING_STATE_THREAD",
        listOf(),
    ...
    val waitingThread =
      ThreadModel(
        3,
        3,
        "ENDS_WITH_WAITING_STATE_THREAD",
        listOf(),
    ...
    val unknownThread =
      ThreadModel(
        5,
        5,
        "ENDS_WITH_UNKNOWN_STATE_THREAD",
        listOf(),
  ...
  fun `buildThreadStateData - termination state not present`() {
    val noActivityThread =
      ThreadModel(
        1,
        1,
        "ENDS_WITH_NO_ACTIVITY_STATE_THREAD",
        listOf(),
        listOf(
        ...
        listOf(ThreadStateModel(ThreadState.RUNNING_CAPTURED, 0L, 5L), ThreadStateModel(ThreadState.NO_ACTIVITY, 5L, 10L)),
      )
    val emptyThread = ThreadModel(2, 2, "NO_STATE_THREAD", listOf(), listOf(), listOf())

    val processes = mapOf(1 to ProcessModel(1, "Process", mapOf(1 to noActivityThread, 2 to emptyThread), mapOf()))
  ...
  fun `buildThreadStateData - main thread name not present`() {
    ...
    val processes = mapOf(1 to ProcessModel(1, "Main Process", mapOf(1 to mainThread, 2 to nonMainThread), mapOf()))
  ...
  fun `buildCpuStateData - thread states`() {
    ...
    assertThat(systemTraceData.getCpuThreadSliceInfoStates(0).map { "${it.x}-${it.value.processId}-${it.value.durationUs}" })
      .containsExactly("0-1-40", "40-0-0", "45-0-10", "55-0-0", "60-1-30", "90-0-0", "120-1-60", "200-0-0")
      .inOrder()
  ...
  fun `buildCpuStateData - utilization`() {
    ...
    assertThat(systemTraceData.cpuUtilizationSeries.map { "${it.x}-${it.value}" })
      .containsExactly("0-30", "50000-20")
      .inOrder() // First bucket = 30% utilization, Second bucket = 20% utilization.
  }
  ...
  fun buildCpuCounters() {
    ...
    assertThat(systemTraceData.cpuCount).isEqualTo(2)
    assertThat(systemTraceData.cpuCounters[0])
      .containsExactly(
        "cpufreq",
        listOf(SeriesData(1, 0L), SeriesData(2, 1000L)),
        "cpuidle",
        listOf(SeriesData(2, 0L), SeriesData(3, 4294967295L)),
      )
    assertThat(systemTraceData.cpuCounters[1])
      .containsExactly("cpufreq", listOf(SeriesData(10, 2000L)), "cpuidle", listOf(SeriesData(20, 0L)))
  }
  ...
  fun buildMainProcessMemoryCountersData() {
    ...
    assertThat(systemTraceData.memoryCounters)
      .containsExactly(
        "mem.locked",
        listOf(SeriesData(1, 1L), SeriesData(4, 2L), SeriesData(7, 1L)),
        "mem.rss",
        listOf(SeriesData(1, 5L), SeriesData(4, 6L), SeriesData(7, 4L)),
      )
      .inOrder()
  ...
  fun buildPowerCountersDataCumulativeView() {
    ...
    val batteryDrain =
      listOf(CounterModel("foo", sortedMapOf(1L to 100.0, 2L to 200.0)), CounterModel("bar", sortedMapOf(3L to 300.0, 4L to 400.0)))
    ...
    assertThat(systemTraceData.powerRailCounters["Memory"]!!.cumulativeData)
      .containsExactly(SeriesData(3, 500L), SeriesData(4, 600L))
      .inOrder()

    assertThat(systemTraceData.powerRailCounters["CPU Big"]!!.cumulativeData)
      .containsExactly(SeriesData(5, 500L), SeriesData(6, 600L))
      .inOrder()

    assertThat(systemTraceData.batteryDrainCounters)
      .containsExactly("bar", listOf(SeriesData(3, 300L), SeriesData(4, 400L)), "foo", listOf(SeriesData(1, 100L), SeriesData(2, 200L)))
      .inOrder()
  }
  ...
  fun buildPowerCountersDataDeltaView() {
    ...
    assertThat(systemTraceData.powerRailCounters["Memory"]!!.deltaData).containsExactly(SeriesData(4, 100L)).inOrder()

    assertThat(systemTraceData.powerRailCounters["CPU Big"]!!.deltaData).containsExactly(SeriesData(6, 100L)).inOrder()
  ...
  fun buildBlastBufferQueueCounterData() {
      ...
      mapOf(
        1 to
          ProcessModel(
            ...
            mapOf(1 to ThreadModel(1, 1, "Thread", listOf(), listOf(), listOf())),
            mapOf(
              "QueuedBuffer - ViewRootImpl[MainActivity]BLAST#0" to
                CounterModel("PendingBuffer - ViewRootImpl[MainActivity]BLAST#0", sortedMapOf(1L to 1.0, 4L to 2.0, 7L to 3.0))
            ),
          )
      )
    val model = TestModel(processes, emptyMap(), listOf(), listOf(), listOf())
    ...
    assertThat(systemTraceData.bufferQueueCounterValues).containsExactly(SeriesData(1, 1L), SeriesData(4, 2L), SeriesData(7, 3L)).inOrder()
  }
  ...
  fun `trace gives right render events for frame`() {
    val mainThread =
      ThreadModel(
        ...
        "main",
        listOf(
          TraceEventModel("${SystemTraceCpuCapture.MAIN_THREAD_EVENT_PREFIX} 42", 0, 500, 500, listOf()),
    ...
    val renderThread =
      ThreadModel(
        ...
        listOf(
          TraceEventModel("${SystemTraceCpuCapture.RENDER_THREAD_EVENT_PREFIX} 42", 500, 1500, 1000, listOf()),
    ...
    val gpuThread =
      ThreadModel(
        3,
        3,
        CpuThreadInfo.GPU_THREAD_NAME,
        listOf(TraceEventModel("${SystemTraceCpuCapture.GPU_THREAD_EVENT_PREFIX} 123", 1500, 2000, 500, listOf())),
        listOf(),
        listOf(),
      )
    val processes = mapOf(1 to ProcessModel(1, "Process", mapOf(1 to mainThread, 2 to renderThread, 3 to gpuThread), mapOf()))
    val frame1 =
  ...
  class TestModel(
    ...
    private val timelineEvents: List<AndroidFrameTimelineEvent> = listOf(),
