...
class CpuTreeNodeTest {
  class TopDownTest {
    ...
    fun testTreeMerger() {
      assertTree(
        id("A"),
        assertTree(id("B"), assertTree(id("D")), assertTree(id("E")), assertTree(id("G"))),
        assertTree(id("C"), assertTree(id("F"))),
      )(Aggregate.TopDown.rootAt(createTree()))
    }
    ...
    fun testTreeMergeWithFilter() {
      ...
      assertTree(
        id("A") and matched,
        assertTree(id("B") and matched, assertTree(id("D") and matched), assertTree(id("E") and matched)),
        assertTree(id("C") and matched, assertTree(id("F") and matched)),
        assertTree(id("B") and unmatched, assertTree(id("E") and unmatched), assertTree(id("G") and unmatched)),
      )(Aggregate.TopDown.rootAt(root))
    }
    ...
    fun testTreeTime() {
      ...
      val topDown =
        CpuTreeNode.of(Aggregate.TopDown.rootAt(root), ClockType.GLOBAL, Range(root.start.toDouble(), root.end.toDouble()), null)
    ...
    fun testThreadTime() {
      ...
      root.addChild(newNode("E", 7, 9))
      val topDown =
        CpuTreeNode.of(Aggregate.TopDown.rootAt(root), ClockType.THREAD, Range(root.start.toDouble(), root.end.toDouble()), null)
    ...
    companion object {
      ...
      fun newNode(method: String, start: Long, end: Long) = newNode(SingleNameModel(method), start, end, ClockType.GLOBAL)
  ...
  class BottomUpNodeTest {
    ...
    fun testDirectRecursion() {
      ...
      // Construct the tree
      val root = newNode("main", 0, 20)
      addChainSubtree(root, newNode("A", 0, 15), newNode("B", 3, 13), newNode("B", 5, 10), newNode("B", 5, 7))
      traverseAndCheck(root, expectedNodes)
    }
    ...
    fun testIndirectRecursion() {
      ...
      addChainSubtree(root, newNode("A", 5, 25), newNode("B", 5, 20), newNode("A", 10, 20), newNode("B", 15, 16))
      traverseAndCheck(root, expectedNodes)
    }
    ...
    fun testStaticRecursion() {
      ...
      addChainSubtree(root, newNode("A", 0, 25), newNode("B", 0, 20), newNode("C", 5, 15))
      addChainSubtree(root, newNode("D", 30, 40), newNode("C", 30, 35), newNode("B", 30, 33))
      traverseAndCheck(root, expectedNodes)
    }
    ...
    fun testPartialRangeWithMixedTwoMethods() {
      ...
      addChainSubtree(root, newNode("A", 0, 100), newNode("A", 0, 40), newNode("A", 0, 20))
      addChainSubtree(root.children[0], newNode("B", 45, 100), newNode("A", 50, 70), newNode("B", 55, 65))
      addChainSubtree(root.children[0].children[0], newNode("B", 21, 40), newNode("A", 25, 28))
      val node = Aggregate.BottomUp.rootAt(root)

      CpuTreeNode.of(node.children.first { it.id == "A" }, ClockType.GLOBAL, Range(0.0, 100.0), null).let { nodeA ->
        assertThat(nodeA.total).isWithin(EPS).of(100.0)
        assertThat(nodeA.childrenTotal).isWithin(EPS).of(61.0)
      }

      CpuTreeNode.of(node.children.first { it.id == "A" }, ClockType.GLOBAL, Range(21.0, 40.0), null).let { nodeA ->
        assertThat(nodeA.total).isWithin(EPS).of(19.0)
        assertThat(nodeA.childrenTotal).isWithin(EPS).of(16.0)
      }

      CpuTreeNode.of(node.children.first { it.id == "A" }, ClockType.GLOBAL, Range(66.0, 71.0), null).let { nodeA ->
    ...
    fun testWithUnmatchedNodes() {
      val root = newNode("main", 0, 100)
      addChildren(root, newNode("A", 0, 50, true), newNode("A", 51, 100, false))
      addChildren(root.getChildAt(0), newNode("B", 0, 50, true))
      addChildren(root.getChildAt(1), newNode("B", 51, 75), newNode("B", 76, 99, true))
      val expectedNodes =
        listOf(
    ...
    companion object {
      ...
      private fun traverseAndCheck(root: CaptureNode, expectedNodes: List<ExpectedNode>) {
        ...
        traverse(
          CpuTreeNode.of(Aggregate.BottomUp.rootAt(root), ClockType.GLOBAL, Range(root.start.toDouble(), root.end.toDouble()), null),
          traverseOrder,
        )
      ...
      fun newNode(method: String, start: Long, end: Long) = newNode(method, start, end, false)
