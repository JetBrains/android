...
// This class copies over from [com.android.tools.property.testing.ApplicationRule]
// and modifies the rules for running threads
open class ProfilersApplicationRule : ExternalResource() {
  ...
  private class TestApplication(disposable: Disposable, val name: String): MockApplication(disposable) {
    override fun invokeLater(runnable: Runnable) = runnable.run()
    override fun invokeAndWait(runnable: Runnable) = runnable.run()
    override fun executeOnPooledThread(action: Runnable): Future<*> = trivialFuture.also { action.run() }
    override fun toString() = "TestApplication@$name"

    private val trivialFuture = object: Future<Unit> {
