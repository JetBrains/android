...
abstract class SingleArtifactTaskHandler<T : TaskStage>(sessionsManager: SessionsManager) : ProfilerTaskHandler(sessionsManager) {
  ...
  /**
   * For a single artifact task handler, stopping the task functionally is equivalent to stopping the capture of the artifact. This method
   * is expected to be idempotent across multiple calls for the same task instance.
   */
  ...
  override fun canStop(): Boolean {
    return when (val currentStage = stage) {
      ...
      is LeakCanaryModel -> currentStage.isRecording.value
      else -> false
    }
  }
