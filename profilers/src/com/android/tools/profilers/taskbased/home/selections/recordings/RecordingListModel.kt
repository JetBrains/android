...
/**
 * This class serves as the model for the list of past and imported recordings. The user can select any of the listed recordings to launch a
 * task from.
 *
...
class RecordingListModel(
  ...
  private val openProfilerTask: () -> Unit,
) : AspectObserver() {
  ...
  init {
    profilers.sessionsManager.addDependency(this).onChange(SessionAspect.SESSIONS) { sessionItemsUpdated() }
  }
  ...
  fun isSelectedRecordingExportable() =
    _selectedRecording.value.let {
      it != null && it.containsExactlyOneArtifact() && (it.getChildArtifacts().firstOrNull()?.canExport ?: false)
    }

  val exportableArtifact
    get() = if (isSelectedRecordingExportable()) selectedRecording.value!!.getChildArtifacts().first() else null
  ...
  private fun sessionItemsUpdated() {
    val sessionItems =
      profilers.sessionsManager.sessionArtifacts.filterIsInstance<SessionItem>().filter {
