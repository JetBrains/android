...
class TraceProcessorModel(builder: Builder) : SystemTraceModelAdapter, Serializable {
  ...
  companion object {
    ...
    @JvmStatic val serialVersionUID = -2228053132990163476L
  }
  ...
  init {
    ...
    for (process in builder.processById.values) {
      ...
      val counterMap = builder.processToCounters.getOrDefault(process.id, listOf()).associateBy { it.name }
    ...
    // Build cpuCores
    cpuCores =
      (0 until builder.cpuCoresCount).map {
        val cpuCountersMap = builder.coreToCpuCounters.getOrDefault(it, listOf()).associateBy { counter -> counter.name }
  ...
  class Builder {
    ...
    companion object {
      ...
      fun getRunningTimeUsInRange(rangeStartUs: Long, rangeEndUs: Long, schedulingEvents: List<SchedulingEventModel>): Long {
        ...
        if (firstSchedIndexToCheck < 0) firstSchedIndexToCheck = -(firstSchedIndexToCheck + 1)
        var lastSchedIndexToCheck = schedulingEvents.binarySearch { sched -> sched.startTimestampUs.compareTo(rangeEndUs) }
        if (lastSchedIndexToCheck < 0) lastSchedIndexToCheck = -(lastSchedIndexToCheck + 1)
        return schedulingEvents
          .subList(firstSchedIndexToCheck /* inclusive */, min(lastSchedIndexToCheck + 1, schedulingEvents.size) /* exclusive */)
    ...
    fun addProcessMetadata(processMetadataResult: TraceProcessor.ProcessMetadataResult) {
      for (process in processMetadataResult.processList) {
        processById[process.id.toInt()] =
          ProcessModel(
            process.id.toInt(),
            process.name,
            process.threadList
              .associate { t -> t.id.toInt() to ThreadModel(t.id.toInt(), process.id.toInt(), t.name, listOf(), listOf(), listOf()) }
              .toSortedMap(),
            mapOf(),
          )
      }
      ...
      for (thread in traceEventsResult.threadList) {
        ...
        for (event in thread.traceEventList) {
          ...
          if (event.durationNanoseconds == -1L) {
            ...
            cpuTimeUs =
              getRunningTimeUsInRange(startTimestampUs, endTimestampUs, threadToScheduling.getOrDefault(thread.threadId.toInt(), listOf()))
          }

          eventPerId[event.id] = TraceEventModel(event.name, startTimestampUs, endTimestampUs, cpuTimeUs, listOf())
        }
    ...
    private fun reconstructTraceTree(
      rootIds: Set<Long>,
      eventToChildrenIds: Map<Long, List<Long>>,
      eventPerId: Map<Long, TraceEventModel>,
      incompleteEventToThreadId: Map<Long, Int>,
    ): Map<Long, TraceEventModel> {
      ...
      while (eventIdStack.isNotEmpty()) {
        ...
        val children =
          eventToChildrenIds
            .getOrDefault(eventId, mutableListOf())
        ...
        val updatedCpuTimeUs =
          if (event.cpuTimeUs != -1L) event.cpuTimeUs
          else
            getRunningTimeUsInRange(
              event.startTimestampUs,
              updatedEndTimestampUs,
              threadToScheduling.getOrDefault(incompleteEventToThreadId[eventId], listOf()),
            )
        val updatedEvent = event.copy(endTimestampUs = updatedEndTimestampUs, cpuTimeUs = updatedCpuTimeUs, childrenEvents = children)
    ...
    fun addCpuCounters(result: TraceProcessor.CpuCoreCountersResult) {
      ...
      result.countersPerCoreList.forEach { countersPerCore ->
        coreToCpuCounters[countersPerCore.cpu] =
          countersPerCore.counterList.map { counter ->
            CounterModel(counter.name, counter.valueList.associate { convertToUs(it.timestampNanoseconds) to it.value }.toSortedMap())
          }
      }
    }

    fun addProcessCounters(counters: TraceProcessor.ProcessCountersResult) {
      processToCounters[counters.processId.toInt()] =
    ...
    fun addThreadStates(states: TraceProcessor.ThreadStatesResult) {
      ...
      states.stateEventList
        .groupBy { it.threadId }
        .forEach { (tid, events) ->
          events.forEach {
            val threadStateEvent =
              ThreadStateModel(
                if (it.state.running) ThreadState.RUNNING_CAPTURED else convertSchedulingState(it.state.nonRunning),
                convertToUs(it.timestampNanoseconds),
                convertToUs(it.timestampNanoseconds + it.durationNanoseconds),
              )
            perThreadState.getOrPut(tid.toInt()) { mutableListOf() }.apply { add(threadStateEvent) }
          }
        }
    ...
    fun addAndroidFrameTimelineEvents(frameTimelineResult: TraceProcessor.AndroidFrameTimelineResult) {
      ...
      frameTimelineResult.actualSliceList.forEach { actualSlice ->
        expectedSlicesBySurfaceFrame[actualSlice.surfaceFrameToken]?.let { expectedSlice ->
          ...
          androidFrameTimelineEvents.add(
            AndroidFrameTimelineEvent(
              expectedSlice.displayFrameToken,
    ...
    fun indexSurfaceflingerFrameTimelineEvents(frameTimelineResult: TraceProcessor.AndroidFrameTimelineResult) {
      surfaceflingerDisplayTokenToEndNs =
        frameTimelineResult.actualSliceList.associate { it.displayFrameToken to it.timestampNanoseconds + it.durationNanoseconds }
    }
    ...
    private fun parsePresentType(presetTypeStr: String) =
      when (presetTypeStr) {
...
private fun List<Layer>.renumbered(
  timelineEvents: List<AndroidFrameTimelineEvent>,
  surfaceflingerDisplayTokenToEndNs: Map<Long, Long>,
): List<Layer> =
  mapFrameNumber(this, timelineEvents, surfaceflingerDisplayTokenToEndNs).let { frameNumberMap ->
    ...
    fun transformFrame(evt: FrameEvent): FrameEvent? = timelineNumberOf(evt.frameNumber)?.let { evt.toBuilder().setFrameNumber(it).build() }
...
fun List<Layer>.groupedByPhase(): List<Phase> =
  asSequence()
    .flatMap(Layer::getPhaseList)
    .groupingBy(Phase::getPhaseName)
    // for each phase, accumulate event list and max depth
    .fold({ _, _ -> mutableListOf<FrameEvent>() to 0 }) { phaseName, (eventsAcc, depthAcc), phase ->
      when (phaseName) {
