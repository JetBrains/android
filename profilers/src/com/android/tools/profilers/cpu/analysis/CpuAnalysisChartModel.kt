...
/**
 * This class is the base model for any [CpuAnalysisTabModel]'s that are backed by a [CaptureDetails]. The title is used as both the tab
 * title, and the tab tool tip.
 */
class CpuAnalysisChartModel<T>(
  tabType: Type,
  ...
  @get:VisibleForTesting val captureConvertedRange = Range()
  ...
  val isCaptureDualClock: Boolean
    get() = capture.isDualClock

  val dualClockDisabledMessage: String?
    get() = capture.dualClockDisabledMessage

  val clockTypes: List<ClockType>
    get() = listOf(ClockType.GLOBAL, ClockType.THREAD)
   ...
   */
  fun applyFilterAndCreateDetails(filter: Filter): CaptureDetailsWithFilterResult =
    collectCaptureNodes().let { nodes ->
      val combinedResult = nodes.fold(FilterResult.EMPTY_RESULT) { res, node -> res.combine(node.applyFilter(filter)) }
      CaptureDetailsWithFilterResult(detailsType.build(clockType, captureConvertedRange, nodes, capture, runModelUpdate), combinedResult)
    }

  /**
   * When using ClockType.THREAD, we need to scale the selection to actually select a relevant range in the capture. That happens because
   * selection is based on wall-clock time, which is usually way greater than thread time. As the two types of clock are synced at start
   * time, making a selection starting at a time greater than (start + thread time length) will result in no feedback for the user, which is
   * wrong. Therefore, we scale the selection so we can provide relevant thread time data as the user changes selection.
   */
  private fun updateCaptureConvertedRange() =
    captureConvertedRange.update(clampedSelectionRange, { it.threadGlobalRatio() }, { it.startThread }, { it.startGlobal })
  ...
  private fun updateSelectionRange() =
    clampedSelectionRange.update(captureConvertedRange, { 1 / it.threadGlobalRatio() }, { it.startGlobal }, { it.startThread })

  private inline fun Range.update(
    that: Range,
    ratio: (CaptureNode) -> Double,
    myStart: (CaptureNode) -> Long,
    theirStart: (CaptureNode) -> Long,
  ) =
  ...
  /**
   * Each thread has its own ClockType.GLOBAL to ClockType.THREAD mapping, so it's flawed to one fixed ratio when there are multiple threads
   * involved. As a best effort, we choose the one with the maximum threadGlobalRatio to cover every thread.
   *
   * For example, if the GLOBAL range is 0 to 10 seconds. Thread 1 runs for 2 seconds, implying a threadGlobalRatio of 0.2; Threads 2 runs
   * for 5 seconds and its threadGlobalRatio is 0.5. updateCaptureConvertedRange() is being called for the entire GLOBAL range. If choosing
   * Thread 1's ratio 0.2, the converted range would be 2 seconds in the context of thread time. It's shorter than Thread 2's 5 seconds, so
   * some of Thread 2's data would NOT be considered selected even though the entire GLOBAL range is selected. If choosing Thread 2's ratio
   * 0.5, the converted range would be 5 seconds of thread time and can fully cover data of the two threads.
   *
