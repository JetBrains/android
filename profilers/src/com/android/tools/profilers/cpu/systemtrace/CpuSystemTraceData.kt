...
interface CpuSystemTraceData {
  ...
  val cpuCount: Int
    get() = cpuUtilizationSeries.size

  /**
   * The memory counters for the process selected for this capture.
   *
   * For each memory counter (those that starts with "mem.", like "mem.locked", "mem.rss", "mem.rss.anon", "mem.rss.file", "mem.rss.shmem",
   * "mem.rss.watermark", "mem.swap" and "mem.virt") present in the main process, the retuned map will contain the corresponding data series
   * for that counter.
   */
   ...
   *
   * For each power rail counter (those that starts with "power.rails.", like "power.rails.modem") present in the system. The returned map
   * will contain the corresponding data series for that counter.
   */
   ...
   *
   * For each battery drain counter (those that starts with "batt.", like "batt.charge") present in the system. The returned map will
   * contain the corresponding data series for that counter.
   */
   ...
   *
   * Each element of the list is a map of counter name to counter values for one CPU core. Currently supported counters are "cpufreq" and
   * "cpuidle".
   */
  val cpuCounters: List<Map<String, List<SeriesData<Long>>>>

  /**
   * If the capture is potentially missing data. For example, on a ATrace or Perfetto capture, due to the capture buffer being a ring
   * buffer.
   */
