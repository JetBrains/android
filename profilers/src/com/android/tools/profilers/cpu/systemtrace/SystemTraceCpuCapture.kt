...
:
  BaseCpuCapture(
    traceId,
    model.getSystemTraceTechnology(),
    false,
    null,
  ...
  override val cpuCount
    get() = cpuThreadSliceInfoStates.size

  override val surfaceflingerEvents
    get() = surfaceflingerManager.surfaceflingerEvents

  override val vsyncCounterValues
    get() = surfaceflingerManager.vsyncCounterValues

  override val renderThreadId
    get() = frameManager.renderThreadId
  ...
  val frameRenderSequence: (AndroidFrameTimelineEvent) -> RenderSequence =
    CachedFunction(CappedLRUMap(10)) { event ->
  ...
  override val bufferQueueCounterValues
    get() =
      when {
  ...
  /**
   * The thread states are computed from the sched_switch trace line reported by an atrace capture. Atrace reports a sched_switch event each
   * time the thread state changes, because of this the thread states reported here are more accurate than the ones sampled via perfd.
   */
