...
class BufferQueueTooltip(val timeline: Timeline,
  ...
  private fun updateValue() {
    val series = bufferQueueValues.getSeriesForRange(timeline.tooltipRange)
    val newBufferQueueValue = if (series.isEmpty()) 0L else series[0].value
    if (newBufferQueueValue != activeBufferQueueValue) {
      activeBufferQueueValue = newBufferQueueValue
