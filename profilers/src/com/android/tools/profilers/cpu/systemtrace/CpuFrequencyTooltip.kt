...
class CpuFrequencyTooltip(val timeline: Timeline,
  ...
  private fun updateValue() {
    val series = cpuFrequencyValues.getSeriesForRange(timeline.tooltipRange)
    val newFrequencyKHz = if (series.isEmpty()) 0L else series[0].value
    if (newFrequencyKHz != activeFrequencyKHz) {
      activeFrequencyKHz = newFrequencyKHz
