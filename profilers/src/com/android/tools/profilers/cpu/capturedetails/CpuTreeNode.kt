...
class CpuTreeNode<T : Aggregate<T>>(
  val base: T,
  val total: Double,
  val childrenTotal: Double,
  val childrenDelegate: Lazy<List<CpuTreeNode<T>>>,
) : TreeNode {
  val self
    get() = total - childrenTotal

  /**
   * Return a tree for `newRange`, assuming current range is `oldRange`. New tree preserves original one's shape, i.e. uninitialized nodes
  ...
  private fun withRange(clockType: ClockType, newRange: Range, diffs: List<Range>, order: Comparator<CpuTreeNode<T>>?): CpuTreeNode<T> =
    base.totalOver(clockType, newRange).let { (total, totalChildren) ->
      when {
        ...
        else -> CpuTreeNode(base, total, totalChildren, fresh(clockType, newRange, order))
      }
    }
  ...
  internal fun withOrder(order: Comparator<CpuTreeNode<T>>, clockType: ClockType, range: Range): CpuTreeNode<T> =
    CpuTreeNode(
      ...
      childrenTotal,
      when {
        childrenDelegate.isInitialized() -> children.map { it.withOrder(order, clockType, range) }.sortedWith(order).asLazy()
  ...
  private fun fresh(clockType: ClockType, range: Range, order: Comparator<CpuTreeNode<T>>?) =
    ...
    baseChildren ->
      lazy(LazyThreadSafetyMode.NONE) { of(baseChildren, clockType, range, order) }
    }

  val children: List<CpuTreeNode<T>>
    get() = childrenDelegate.value
  ...
  companion object {
    ...
    internal fun <T : Aggregate<T>> of(
      bases: List<T>,
      clockType: ClockType,
      range: Range,
      order: Comparator<CpuTreeNode<T>>?,
    ): List<CpuTreeNode<T>> =
      bases
