...
open class CaptureNode(val data: CaptureNodeModel, var clockType: ClockType) : HNode<CaptureNode> {
  ...
  @VisibleForTesting constructor(data: CaptureNodeModel) : this(data, ClockType.GLOBAL) {}
  ...
  var endThread = 0L
  @JvmField protected val childrenList = mutableListOf<CaptureNode>()
  ...
  /**
   * Aspect model for the node [Aspect]. Only root nodes provide aspect changes so it is lazily initialized to avoid the overhead of its
   * instantiation.
   */
  ...
  fun findRootNode(): CaptureNode {
    tailrec fun find(node: CaptureNode): CaptureNode =
  ...
  fun getTopKNodes(
    ...
    val nameMatchedNodesStream =
      ...
      if (nameToNodes.contains(nodeName)) nameToNodes[nodeName]!!.stream()
      // Otherwise, compute the stream using a filter
      else descendantsStream.parallel().filter { it.data.fullName == nodeName }

    // Get the top K longest duration nodes with the matching name
    return nameMatchedNodesStream.sorted(comparator.reversed()).limit(k.toLong()).collect(Collectors.toList())
  }
  ...
  /**
   * Return a new tree like this one, but with all uninteresting nodes collapsed into the given abbreviation. In the returned abbreviated
   * tree:
  ...
  /**
   * Accumulates value of type T over the capture tree. This is a non-standard version of fold that instead of taking a single `combine`
   * function that combines sub-results, mirroring CaptureNode's shape, allows more incremental accumulation of the result.
   *
  ...
  private fun clonedWithData(data: CaptureNodeModel) =
    CaptureNode(data, clockType).also { clone ->
      clone.copyFrom(this, CaptureNode::startGlobal, CaptureNode::endGlobal, CaptureNode::startThread, CaptureNode::endThread)
    }
