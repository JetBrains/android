...
internal class HeapDumpInstanceObject
@JvmOverloads
constructor(
  ...
  private val valueType =
    when {
      precomputedValueType != null -> precomputedValueType
        ...
        instance.classObj.let { classObj ->
          when {
  ...
  override fun getValueText(): String {
    val name =
      if (valueType == ValueObject.ValueType.CLASS) {
  ...
  override fun getToStringText() =
    when (valueType) {
      ValueObject.ValueType.STRING ->
        when (val text = (instance as ClassInstance).getAsString(MAX_VALUE_TEXT_LENGTH)) {
          null -> INVALID_STRING_VALUE
          else -> {
            val content =
              when (val textLength = text.length) {
  ...
  override fun getAllocationCallStack() =
    when (val st = instance.stack) {
      null -> null
      else -> {
  ...
  fun extractReferences(): List<ReferenceObject> {
    ...
    val sortedReferences = instance.hardReverseReferences.sortedWith(order) + instance.softReverseReferences.sortedWith(order)
    return sortedReferences
      .mapNotNull { reference ->
        // Note that each instance can have multiple references to the same object.
        val referencingFieldNames =
          when (reference) {
            is ClassInstance ->
              reference.values.mapNotNull { ref -> ref.field.name.takeIf { ref.field.type == Type.OBJECT && ref.value === instance } }
            is ArrayInstance -> {
              assert(reference.arrayType == Type.OBJECT)
              reference.values.withIndex().mapNotNull { ref -> ref.index.toString().takeIf { ref.value === instance } }
            }
            is ClassObj ->
              reference.staticFieldValues.mapNotNull { (key, value) -> key.name.takeIf { key.type == Type.OBJECT && value === instance } }
            else -> listOf()
          }
