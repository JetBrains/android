...
sealed class Classifier {
  ...
  class Join<T : Any>(
    ...
    override val classifierSetSequence
      get() = cache.values.asSequence() + rest.classifierSetSequence
  }

  /** If this Classifier is a terminal classifier, and instances will not be further classified. */
  val isTerminalClassifier: Boolean
    get() = this is Id

  val filteredClassifierSets
    get() = classifierSetSequence.filter { !it.isFiltered }.toList()

  internal abstract val classifierSetSequence: Sequence<ClassifierSet>

  /**
   * Retrieve the next-level ClassifierSet that the given `instance` belongs to. If none exists and `createIfAbsent` is true, then create
   * and return the new ClassiferSet.
   */
  abstract fun getClassifierSet(instance: InstanceObject, createIfAbsent: Boolean): ClassifierSet?

  /**
   * Partitions [InstanceObject]s in `snapshotInstances` and `myDeltaInstances` according to the current [ClassifierSet]'s strategy. This
   * will consume the instances from the input.
   */
  ...
  companion object {
    @JvmStatic fun <T : Any> of(key: (InstanceObject) -> T?, classify: (T) -> ClassifierSet) = Join(key, classify, Id)
  }
}
