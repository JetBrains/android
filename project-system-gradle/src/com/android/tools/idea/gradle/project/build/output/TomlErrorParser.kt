  ...
  override fun parse(line: String, reader: BuildOutputInstantReader, messageConsumer: Consumer<in BuildEvent>): Boolean {
    ...
    while (true) {
      ...
      if (reasonPosition == null) {
        val reasonLineMatcher = REASON_POSITION_PATTERN.matcher(descriptionLine)
        if (reasonLineMatcher.matches()) {
          reasonPosition = reasonLineMatcher.group(1).toIntOrNull() to reasonLineMatcher.group(2).toIntOrNull()
        }
      }
    ...
    val buildIssue = object : BuildIssue {
      ...
      override fun getNavigatable(project: Project): Navigatable? {
        val tomlFile = project.baseDir?.findChild("gradle")?.findChild("$catalog.versions.toml") ?: return null
