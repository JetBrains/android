  ...
  nest(projectRoot, "PROJECT") {
    with(ideModelDumper(this)) {
      if (!forSnapshotComparison) {
        ...
        GradleHolderProjectPath(projectRoot.canonicalPath, ":").resolveIn(project)?.let { dump(it) }
      }
          ...
          GradleAndroidModel.get(module)?.let { it ->
            ...
            nest {
              if (it is GradleAndroidDependencyModel) {
                it.variantsWithDependencies
                  .filter { variant -> forSnapshotComparison || variant.name == it.selectedVariantName }
                  .forEach { dump(it) }
              } else {
                it.variants.filter { variant -> forSnapshotComparison || variant.name == it.selectedVariantName }.forEach { dump(it) }
              }
            }
          }

          NdkModuleModel.get(module)?.let { it -> dumpNdkModuleModel(it) }

          kotlinModels(module)?.let { dump(it) }

          kaptModels(module)?.let { dump(it) }

          mppModels(module)?.let { dump(it) }

          externalProjects(module)?.let { dump(it) }
        }
      }
    }
  }
...
fun ProjectDumper.dumpAllVariantsSyncAndroidModuleModel(
  project: Project,
  ...
  nest(File(projectPath), "PROJECT") {
    with(ideModelDumper(this)) {
      gradleAndroidModel.let { gradleAndroidModel ->
        ...
        // Dump all the fetched Ide variants.
        head("IdeVariants")
        nest { gradleAndroidModel.variantsWithDependencies.forEach { ideVariant -> dump(ideVariant) } }
      }
    }
  }
}

private val jbModelDumpers =
  listOf(
    SpecializedDumper(property = CommonCompilerArguments::pluginOptions),
    SpecializedDumper<IdeaKotlinDependency> { dependency -> prop(propertyName, dependency.coordinates.toString()) },
    ...
    SpecializedDumper(property = KotlinGradleModel::compilerArgumentsBySourceSet) { _, compilerArgumentsBySourceSet ->
      ...
      nest {
        compilerArgumentsBySourceSet.forEach { (sourceSet, compilerArguments) ->
          head(sourceSet)
          nest { prop("compilerArguments", parseCommandLineArguments<K2JVMCompilerArguments>(compilerArguments)) }
        }
      }
    },
    ...
    SpecializedDumper<DefaultGradleSourceSetModel> { gradleSourceSet ->
      ...
      nest {
        ...
        if (gradleSourceSet.configurationArtifacts.values.any { it.isNotEmpty() }) {
          ...
          nest { gradleSourceSet.configurationArtifacts.filter { it.value.isNotEmpty() }.toSortedMap().forEach { prop(it.key, it.value) } }
    ...
    SpecializedDumper(property = KotlinMPPGradleModel::dependencies) { holder, dependencies ->
      ...
      nest {
        holder.sourceSetsByName.keys.forEach { sourceSet ->
          head(sourceSet)
          nest { prop("dependency", dependencies[sourceSet].sortedBy { it.coordinates.toString() }) }
        }
      }
    },
    ...
    SpecializedDumper(property = KotlinMPPGradleModel::kotlinGradlePluginVersion) { _, kgpVersion ->
      head(propertyName)
      nest { prop("versionString", kgpVersion.versionString.replaceKotlinVersionForTests()) }
    },
  )

/**
 * Kotlin version used in AGP integration tests and sync tests.
 *
    ...
    object {
      fun dump(ideAndroidModel: IdeAndroidProject) {
        ...
        if (ideAndroidModel.testSuites.isNotEmpty()) {
          head("TestSuites")
          nest { ideAndroidModel.testSuites.forEach { dump(it) } }
        }
        ...
        ideAndroidModel.multiVariantData?.defaultConfig?.let { defaultConfig ->
          ...
          nest {
            head("ProductFlavor")
            nest { dump(defaultConfig) }
            dump(ideAndroidModel.defaultSourceProvider)
          }
        }
          ?: run {
            head("DefaultSourceProvider")
            nest { dump(ideAndroidModel.defaultSourceProvider) }
          }
        ideAndroidModel.multiVariantData
          ?.buildTypes
          ?.takeIf { it.isNotEmpty() }
          ?.let { buildTypes ->
            head("BuildTypes")
            nest { buildTypes.forEach { dump(it) } }
          }
        ideAndroidModel.multiVariantData
          ?.productFlavors
          ?.takeIf { it.isNotEmpty() }
          ?.let { productFlavors ->
            head("ProductFlavors")
            nest { productFlavors.forEach { dump(it) } }
          }
        head("SigningConfigs")
        nest { ideAndroidModel.signingConfigs.forEach { dump(it) } }

        head("VariantBuildInformation")
        nest { ideAndroidModel.variantsBuildInformation.forEach { dump(it) } }
      }
      ...
      fun dump(ideTestSuite: IdeTestSuite) {
        ...
        nest {
          ...
          if (ideTestSuite.sources.isNotEmpty()) {
            head("Sources")
            nest { ideTestSuite.sources.forEach { dump(it) } }
          }
          head("JUnitEngineInfo")
          nest {
            head("IncludedEngines")
            ideTestSuite.junitEngineInfo.includedEngines.forEach { prop("- includedEngine:") { it } }
          }
          if (ideTestSuite.targetedVariants.isNotEmpty()) {
            head("TargetedVariants")
            ideTestSuite.targetedVariants.forEach { prop("- targetedVariant:") { it } }
          }
        }
      }
      ...
      fun dump(ideTestSuiteSource: IdeTestSuiteSource) {
        ...
        nest {
          ...
          head("SourceProvider")
          nest { dump(ideTestSuiteSource.sourceProvider) }
        }
      }
      ...
      fun dump(ideVariant: IdeVariantCore) {
        ...
        nest {
          ...
          if (ideVariant.manifestPlaceholders.isNotEmpty()) {
            ...
            nest { ideVariant.manifestPlaceholders.forEach { (key, value) -> prop(key) { value } } }
          }
          ...
          if (ideVariant.testInstrumentationRunnerArguments.isNotEmpty()) {
            ...
            nest { ideVariant.testInstrumentationRunnerArguments.forEach { (key, value) -> prop(key) { value } } }
          }
          head("MainArtifact")
          nest { dump(ideVariant.mainArtifact) }
      ...
      fun dumpLibraryTable(project: Project) {
        ...
        DataNodeCaches.getInstance(project).cachedProjectData?.let { projectData ->
          ...
          if (kmpLibraryTable != null) {
            ...
            nest { dump(kmpLibraryTable.data) }
          }
        }
      }

      fun dump(libraryTable: IdeResolvedLibraryTable) {
        val libraryComparator =
          compareBy<IdeLibrary?> { it?.toLibraryType() }
        ...
        libraryTable.libraries
          ...
          .forEach { library -> modelDumper.dumpModel(projectDumper, "library", library) }
      }
      ...
      fun dump(resValues: Map<String, IdeClassField>) {
        ...
        nest {
          resValues.entries
            .sortIfComparingSnapshots { (key, value) -> key + value.name }
            .forEach { (key, value) -> prop(key) { "(${value.type}, ${value.name}, ${value.value})" } }
        }
      }
      ...
      private fun dump(ideTestSuiteVariantTarget: IdeTestSuiteVariantTarget) {
        ...
        if (ideTestSuiteVariantTarget.targets.isNotEmpty()) {
          head("Targets")
          nest { ideTestSuiteVariantTarget.targets.forEach { dump(it) } }
        }
      }
      ...
      private fun dump(ideTestSuiteTarget: IdeTestSuiteTarget) {
        ...
        nest {
          ...
          if (ideTestSuiteTarget.targetedDevices.isNotEmpty()) {
            ...
            nest { ideTestSuiteTarget.targetedDevices.forEach { prop("- device") { it } } }
          }
        }
      }
      ...
      private fun dump(property: String, ideDependencies: IdeDependencies) {
        ...
        nest {
          ideDependencies.unresolvedDependencies.forEach { dependency ->
            ideDependencies.resolver.resolve(dependency).forEach { prop(it.toLibraryType()) { it.toDisplayString() } }
            if (!projectDumper.forSnapshotComparison) {
              nest {
                ...
                dependency.dependencies
                  ?.map { ideDependencies.lookup(it) }
                  ?.forEach { nestedDependency ->
      ...
      private fun dump(ideBaseArtifact: IdeBaseArtifactCore) {
        ...
        ideBaseArtifact.generatedSourceFolders.map { it.path.toPrintablePath() }.sorted().forEach { prop("GeneratedSourceFolders") { it } }
        ideBaseArtifact.classesFolder.map { it.path.toPrintablePath() }.sorted().forEach { prop("ClassesFolder") { it } }
        ideBaseArtifact.generatedClassPaths.map { it.value.toPrintablePath() }.sorted().forEach { prop("GeneratedClassPaths") { it } }
        ...
        ideBaseArtifact.bytecodeTransforms?.let { transforms ->
          if (transforms.isNotEmpty()) {
            prop("BytecodeTransforms") { transforms.map { it.type.toString() }.sorted().joinToString(",") }
          }
        }
        ...
        val providedDependencies =
          ideBaseArtifact.compileClasspath.libraries.filterIsInstance<IdeArtifactLibrary>().filter { it.name !in runtimeNames }
        if (providedDependencies.isNotEmpty()) {
          head("ProvidedDependencies")
          nest {
            providedDependencies.sortedBy { it.name }.forEach { prop("- provided") { it.name.replaceKnownPatterns().replaceKnownPaths() } }
          }
        }
        ...
        if (runtimeOnlyClasses.isNotEmpty()) {
          ...
          nest { runtimeOnlyClasses.map { it.toPrintablePath() }.sorted().forEach { prop("- class") { it } } }
        }
      }
      ...
      private fun dump(ideProductFlavorContainer: IdeProductFlavorContainer) {
        head("ProductFlavor")
        nest { dump(ideProductFlavorContainer.productFlavor) }
        head("SourceProvider")
        nest { dump(ideProductFlavorContainer.sourceProvider) }
        head("ExtraSourceProviders")
        nest { ideProductFlavorContainer.extraSourceProviders.forEach { dump(it) } }
      }
      ...
      private fun dump(ideBaseConfig: IdeBaseConfig) {
        ...
        if (ideBaseConfig.manifestPlaceholders.isNotEmpty()) {
          head("ManifestPlaceholders")
          nest { ideBaseConfig.manifestPlaceholders.forEach { (key, value) -> prop(key) { value } } }
        }
      }
      ...
      private fun dump(ideProductFlavor: IdeProductFlavor) {
        ...
        if (ideProductFlavor.testInstrumentationRunnerArguments.isNotEmpty()) {
          head("TestInstrumentationRunnerArguments")
          nest { ideProductFlavor.testInstrumentationRunnerArguments.forEach { (key, value) -> prop(key) { value } } }
        }
        ideProductFlavor.vectorDrawables?.let {
          head("VectorDrawables")
          nest { prop("UseSupportLibrary") { it.useSupportLibrary?.toString() } }
        }
        ideProductFlavor.matchingFallbacks.takeIf { !it.isNullOrEmpty() }?.let { prop("MatchingFallbacks") { it.toString() } }
        ideProductFlavor.missingDimensionStrategy
          .takeIf { !it.isNullOrEmpty() }
          ?.let {
            head("MissingDimensionStrategy")
            nest { it.forEach { (key, value) -> prop(key) { value.toString() } } }
          }
      }

      private fun dump(ideSourceProviderContainer: IdeSourceProviderContainer) {
        head("SourceProvider")
        nest { dump(ideSourceProviderContainer.sourceProvider) }
        head("ExtraSourceProviders")
        nest { ideSourceProviderContainer.extraSourceProviders.forEach { dump(it) } }
      }

      private fun dump(ideSourceProvider: IdeSourceProvider?) {
        if (ideSourceProvider == null) return
        prop("Name") { ideSourceProvider.name }
        ideSourceProvider.manifestFile?.let { prop("Manifest") { it.path.toPrintablePath() } }
      ...
      private fun dump(extraSourceProvider: IdeExtraSourceProvider) {
        ...
        nest {
          ...
          head("SourceProvider")
          nest { dump(extraSourceProvider.sourceProvider) }
        }
      }
      ...
      private fun dump(ideBuildTypeContainer: IdeBuildTypeContainer) {
        ...
        nest {
          ...
          ideBuildTypeContainer.buildType.matchingFallbacks
            .takeIf { !it.isNullOrEmpty() }
            ?.let { prop("MatchingFallbacks") { it.toString() } }
        }
        head("SourceProvider")
        nest { dump(ideBuildTypeContainer.sourceProvider) }
      ...
      private fun dump(aaptOptions: IdeAaptOptions) {
        head("AaptOptions")
        nest { prop("NameSpacing") { aaptOptions.namespacing.toString() } }
      }
      ...
      private fun dump(lintOptions: IdeLintOptions) {
        ...
        nest {
          ...
          if (lintOptions.severityOverrides.orEmpty().isNotEmpty()) {
            ...
            nest {
              lintOptions.severityOverrides
                ?.entries
                ?.sortIfComparingSnapshots { it.key }
                ?.forEach { (key, value) -> prop(key) { value.toString() } }
            }
          }
        }
      }
      ...
      private fun dump(viewBindingOptions: IdeViewBindingOptions) {
        ...
        nest { prop("Enabled") { viewBindingOptions.enabled.toString() } }
      }
      ...
      fun IdeLibrary.toDisplayString(): String =
        when (this) {
      ...
      fun IdeLibrary.toLibraryType(): String =
        when (this) {
...
class DumpProjectIdeModelAction : InternalDumpAction("IDE Models") {
  ...
  override fun actionPerformed(e: AnActionEvent) {
    ...
    Logger.getInstance(DumpProjectIdeModelAction::class.java).info("Android IDE models dumped to file: " + outputFile.toURI().toURL())
  }
}

fun Module.hasDifferentGradleModuleModelThanHolder() =
  takeIf { it.isLinkedAndroidModule() }
    ?.getHolderModule()
    ?.let { holderModule -> gradleModuleModel?.copy(moduleNameField = holderModule.name) != holderModule.gradleModuleModel } == true
