...
fun GradleAndroidModel.format(): String = "\n" + """
    ...
    selectedMainCompileLevel2Dependencies = ${selectedMainCompileDependencies.format()}
    ...
    testFixturesSourceProviders = ${testFixturesSourceProviders.format()}
    ...
    buildTypeNames = ${getBuildTypeNames().format()}
    productFlavorNames = ${getProductFlavorNames().format()}
    variantNames = ${variantNames.format()}
    javaLanguageLevel = ${getJavaLanguageLevel()}
    overridesManifestPackage = ${overridesManifestPackage()}
    artifactForAndroidTest = ${getArtifactForAndroidTest()?.format()}
    artifactForTestFixtures = ${getArtifactForTestFixtures()?.format()}
    testExecutionStrategy = ${getTestExecutionStrategy()}
