...
class FilteringJavacOutputParser : BuildOutputParser {
  ...
  override fun parse(line: String, reader: BuildOutputInstantReader, messageConsumer: Consumer<in BuildEvent>): Boolean {
    if (
      line.startsWith(BuildOutputParserUtils.BUILD_COMPLETED_WITH_FAILURES_LINE) ||
        line.startsWith(BuildOutputParserUtils.BUILD_FAILED_WITH_EXCEPTION_LINE)
    ) {
      buildFailedWithExceptionLineSeen = true
    }
    val wrappedConsumer =
      Consumer<BuildEvent> { event ->
        if (
          buildFailedWithExceptionLineSeen &&
            JavaLanguageLevelDeprecationOutputParser.notSupportedMessagePattern.matcher(event.message).matches()
        ) {
          return@Consumer
        }
        ...
        when (event) {
          is FileMessageEventImpl ->
            messageConsumer.accept(
              object :
                FileMessageEventImpl(event.parentId!!, event.kind, event.group, event.message, event.description, event.filePosition),
                DuplicateMessageAware {}
            )
          is MessageEventImpl ->
            messageConsumer.accept(
              object :
                MessageEventImpl(event.parentId!!, event.kind, event.group, event.message, event.description), DuplicateMessageAware {}
            )
