...
abstract class AbstractCompileGradleModuleAction :
  ...
  final override fun doPerform(e: AnActionEvent, project: Project) {
    ...
    getInstance(project).compileJava(modules)
  }
  ...
  companion object {
    ...
    private fun updatePresentation(e: AnActionEvent, project: Project, moduleNames: List<String?>) {
      ...
      if (moduleCount > 0) {
        var text = StringBuilder("Compile")
        if (moduleCount > 1) {
          text.append("s")
        }
        for (i in 0 until moduleCount) {
          if (text.length > 30) {
            text = StringBuilder("Compile Selected Modules")
            break
          }
        ...
        presentationText = "Compile (No Modules Selected)"
      }
