...
data class PluginInsertionConfig(
  ...
  object BuildscriptClasspathInsertionStep : PluginInsertionStep() {
    ...
    override fun getUpdateLazyCall(
      ...
      require(classpathInfo != null) { "classpathDependency and classpathMatcher name must not be null for BUILDSCRIPT_CLASSPATH" }
      helper
        .updateDependencyVersion(classpathInfo.dependency) { projectBuildModel -> projectBuildModel.buildscript().dependencies() }
        .also { helper.maybeAddRepoToBuildscript(config, version, it) { model -> model.buildscript() } }
    }
  }
  ...
  object BuildscriptClasspathWithVariableInsertionStep : PluginInsertionStep() {
    override fun getAddLazyCall(
      ...
      helper
        .tryAddClasspathDependencyWithVersionVariable(classpathInfo.dependency, config.variableName, listOf(), classpathInfo.matcher)
        .also { helper.maybeAddRepoToBuildscript(config, version, it) { model -> model.buildscript() } }
    }
  ...
  object PluginManagementInsertionStep : PluginInsertionStep() {
    ...
    override fun getUpdateLazyCall(
      ...
      helper
        .updatePluginForPluginsBlock(pluginId, version) { projectModel: ProjectBuildModel ->
          projectModel.projectSettingsModel?.pluginManagement()?.plugins()
        }
        .also { helper.maybeAddRepoToPluginManagement(config, version, it) { model -> model.projectSettingsModel?.pluginManagement() } }
    }
  }
  ...
  object PluginBlockInsertionStep : PluginInsertionStep() {
    ...
    override fun getUpdateLazyCall(
      ...
      helper
        .updatePluginForBuildModel(pluginId, version) { projectModel: ProjectBuildModel -> projectModel.projectBuildModel }
        .also { helper.maybeAddRepoToPluginManagement(config, version, it) { model -> model.projectSettingsModel?.pluginManagement() } }
    }
  }
  ...
  companion object {
    fun defaultInsertionConfig(): PluginInsertionConfig {
      ...
      steps.addAll(listOf(PluginManagementInsertionStep, PluginBlockInsertionStep, BuildscriptClasspathInsertionStep))
      return PluginInsertionConfig(steps, MatchedStrategy.DO_NOTHING)
    }
  }
}
