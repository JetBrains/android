 ...
 *
 * This class is added to suppress top-level failures generated from build exceptions that instead should be attributed to failed tasks.
 * This filter stops exception from being converted to the Failure object and shown as a build error on the top level in Build Output.
 *
...
class AGPMessagesNotificationExtension : ExternalSystemNotificationExtension {
  ...
  override fun isInternalError(error: Throwable): Boolean {
    return isXmlProcessorError(error) || isJavaLanguageLevelDeprecationError(error) || isCompilationError(error)
  }
