...
class VersionCatalogDocumentationProvider : DocumentationProvider {
  ...
  private val plugins = mapOf("id" to "Plugin identifier \"<b>com.android.application</b><small>:8.4.0</small>\"") + all

  override fun getQuickNavigateInfo(element: PsiElement, originalElement: PsiElement): String? = generate(element, originalElement)

  override fun generateHoverDoc(element: PsiElement, originalElement: PsiElement?): String? = generate(element, originalElement)

  override fun generateDoc(element: PsiElement, originalElement: PsiElement?): String? = generate(element, originalElement)
