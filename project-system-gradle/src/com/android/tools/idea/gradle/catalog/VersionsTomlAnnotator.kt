...
class VersionsTomlAnnotator : Annotator {
  ...
  override fun annotate(element: PsiElement, holder: AnnotationHolder) {
    ...
    // for aliases
    if (element is TomlKey && element.parent is TomlKeyValue && grandParent is TomlTable && greatGrandParent is TomlFile) {
      checkDependencyAliases(element, grandParent, holder)
    }

    // table
    if (element is TomlKey && element.parent is TomlTableHeader && grandParent is TomlTable && greatGrandParent is TomlFile) {
      checkTableAliases(element, grandParent, holder)
    }

    // alias with literal
    if (element is TomlLiteral && element.parent is TomlKeyValue && grandParent is TomlTable && greatGrandParent is TomlFile) {
    ...
    if (name == "libraries" && element.text.split(":").size != 2)
      holder
        .newAnnotation(
          ...
          "Make sure that the module coordinates consist of 2 parts separated by colons, eg: my.group:artifact",
        )
        .create()
  }
  ...
  private fun checkDependencyLiteral(element: TomlLiteral, holder: AnnotationHolder) {
    ...
    when (name) {
      "plugins" ->
        if (element.text.split(":").size != 2)
          holder
            .newAnnotation(
              HighlightSeverity.ERROR,
              "Make sure that the coordinates consist of 2 parts separated by colons, eg: my_plugin:1.2",
            )
            .create()
      ...
      "libraries" ->
        if (element.text.split(":").size < 2)
          holder
            .newAnnotation(
              HighlightSeverity.ERROR,
              "Make sure that the coordinates consist of 2 parts with BOM and 3 without BOM that are separated by colons.",
            )
            .create()
  ...
  private fun checkDependencyAliases(element: TomlKey, table: TomlTable, holder: AnnotationHolder) {
    ...
    if (!"[a-z]([a-zA-Z0-9_\\-])+".toRegex().matches(text)) {
      holder
        .newAnnotation(
          ...
          "Invalid alias `${text}`. It must start with a lower-case letter, contain at least 2 characters " +
            "and be made up of letters, digits and the symbols '-' or '_' only",
        )
        .create()
    } else if (".+[_\\-][0-9]".toRegex().find(text) != null) {
      holder
        .newAnnotation(
          if (table.header.key?.text in dependencyTables) HighlightSeverity.ERROR else HighlightSeverity.WARNING,
          "Invalid alias `${text}`. There must be letter after '-' or '_ delimiter.",
        )
        .create()
    } else if ((text.endsWith("_") || text.endsWith("-"))) {
      holder.newAnnotation(HighlightSeverity.ERROR, "Invalid alias `${text}`. It cannot end with '-' or '_'").create()
    } else if ("[_\\-]{2,}".toRegex().find(text) != null) {
      holder.newAnnotation(HighlightSeverity.ERROR, "Invalid alias `${text}`. Cannot have more than one consecutive '-' or '_'").create()
    } else if (isInLibrariesTable(table) && "^((plugins)|(bundles)|(versions))[_\\-]?".toRegex().find(text) != null) {
      holder
        .newAnnotation(
          HighlightSeverity.ERROR,
          "Invalid alias `${text}`. It cannot start with 'plugins', 'bundles' or 'versions' as will interfere " + "with gradle naming",
        )
        .create()
      ...
      holder
        .newAnnotation(
          HighlightSeverity.ERROR,
          "Invalid alias `${text}`. Alias 'class' is a reserved name in Gradle which prevents generation of accessors.",
        )
        .create()
    } else {
  ...
  private fun isInLibrariesTable(element: TomlTable): Boolean = element.header.key?.segments?.firstOrNull()?.name == "libraries"
  ...
  private fun checkTableAliases(key: TomlKey, table: TomlTable, holder: AnnotationHolder) {
    ...
    if (text !in tables) {
      holder
        .newAnnotation(HighlightSeverity.ERROR, "Invalid table name `${text}`. It must be one of: ${tables.joinToString(", ")}")
        .create()
  ...
  private fun checkAliasDuplication(element: TomlKey, holder: AnnotationHolder) {
    ...
    if (same.isNotEmpty()) {
      ...
      holder
        .newAnnotation(HighlightSeverity.ERROR, "Duplicated alias name. Effectively same as ${same.take(2).joinToString(", ")}" + suffix)
        .create()
    }
  }
  ...
  private fun checkTableDuplication(element: TomlTable, holder: AnnotationHolder) {
    ...
    val hasDuplicates =
      element.parent.children
        ...
        .filter { it != element }
        .any { it.header.key?.firstSegmentNormalizedText() == name }
    if (hasDuplicates) {
  ...
  private fun PsiElement.isFirstElement(): Boolean {
    ...
    while (currentElement.firstChild != null) currentElement = currentElement.firstChild
