...
sealed class ProcessedDeviceSpec {
  ...
  sealed class SingleDeviceSpec : ProcessedDeviceSpec() {
    ...
    class TargetDeviceSpec(val deviceSpec: AndroidDeviceSpec) : SingleDeviceSpec() {
      fun writeToJsonTempFile(writeLanguages: Boolean, moduleAgpVersions: List<AgpVersion> = emptyList()): File {
  ...
  class MultipleDeviceSpec(val deviceSpecs: List<AndroidDeviceSpec>) : ProcessedDeviceSpec() {
    fun writeToMultipleJsonTempFiles(writeLanguages: Boolean, moduleAgpVersions: List<AgpVersion> = emptyList()): List<File> {
...
data class AndroidDeviceSpecImpl
@JvmOverloads
constructor(
  /**
...
/**
 * Creates an [AndroidDeviceSpec] instance from a list of [devices][AndroidDevice], or `null` if the list of devices is empty.
 *
 * If the [fetchLanguages] parameter is `true`, a request is made to the device to retrieve the list of installed languages using the
 * [getLanguages] method. The [timeout] and [unit] parameters are used in that case to ensure each request has a timeout.
 */
...
@JvmOverloads
fun createTargetDeviceSpec(
  devices: List<AndroidDevice>,
  timeout: Long = DEVICE_SPEC_TIMEOUT_SECONDS,
  unit: TimeUnit = TimeUnit.SECONDS,
  notification: (title: String, message: String) -> Unit = { _, _ -> },
): ProcessedDeviceSpec.SingleDeviceSpec {
  if (devices.isEmpty()) {
  ...
  val version =
    if (hasUnknownVersions) null else versions.singleOrNull() // Find the minimum value of the build API level for making other decisions
 ...
 *
 * Note: We should be able to cache the result of this call to improve performance, i.e. to avoid issuing an ADB call every time, but we
 * first need to define a reasonable cache validation policy, which is not trivial. See b/78452155.
 */
...
/**
 * message DeviceSpec { // Supported ABI architectures in the order of preference. // The values should be the string as reported by the
 * platform, e.g. // "armeabi-v7a" or "x86_64". repeated string supported_abis = 1;
 *
 * // All installed locales represented as BCP-47 strings. repeated string supported_locales = 2;
 *
 * // List of device features returned by the package manager utility. repeated string device_features = 3;
 *
 * // List of OpenGL extension strings supported by the device. repeated string gl_extensions = 4;
 *
 * // Screen dpi. uint32 screen_density = 5;
 *
...
private fun AndroidDeviceSpec.writeJson(writeLanguages: Boolean, out: Writer, moduleAgpVersions: List<AgpVersion>) {
  JsonWriter(out).use { writer ->
    ...
    commonVersion?.let {
      ...
      it.codename?.let { codename -> writer.name("codename").value(codename) }
    }
    density?.let {
      if (it.dpiValue > 0) {
    ...
    if (writeLanguages) {
      if (!languages.isEmpty()) {
        ...
        writer.beginArray()
        languages.forEach { writer.value(it) }
        writer.endArray()
      }
    }
    writer.endObject()
