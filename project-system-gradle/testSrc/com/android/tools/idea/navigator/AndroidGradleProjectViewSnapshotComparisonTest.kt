 ...
 *
 * These tests convert the Android project view to a stable text format which does not depend on local environment (and ideally should not
 * depend on the versions of irrelevant libraries) and compare them to pre-recorded golden results.
 *
...
data class AndroidProjectViewSnapshotComparisonTestDef(
  ...
  override fun runTest(root: File, project: Project) {
    ...
    val snapshotContext =
      SnapshotContext(testProject.projectName + suffix, agpVersion, "tools/adt/idea/android/testData/snapshots/projectViews")
    snapshotContext.assertIsEqualToSnapshot(text)
  }

  companion object {
    val tests: List<AndroidProjectViewSnapshotComparisonTestDef> =
      listOf(
