...
class PluginsHelperTest {
  ...
  fun testAddToBuildscriptWithCatalog() {
    doTest(
      ...
      },
      {
        ...
        assertThat(project.getTextForFile("build.gradle")).contains("classpath libs.lib2")
      },
    )
  }
  ...
  fun testAddAndroidKotlinMultiplatformPlugin() {
    ...
    doTest(
      ...
      },
      {
        ...
        assertThat(catalog)
          .contains("android-kotlin-multiplatform-library = { id = \"com.android.kotlin.multiplatform.library\", version.ref = \"agp\" }")
      },
    )
  }
  ...
  fun testAddToClasspathNoCatalog() {
    doTest(
      ...
      { assertThat(project.getTextForFile("build.gradle")).contains("classpath \'com.example.libs:lib2:1.0\'") },
    )
  }
  ...
  fun testSimpleAddPluginNoCatalogWithExistingPlugin() {
    ...
    val version = env.gradlePluginVersion
    doTest(
      SIMPLE_APPLICATION_PLUGINS_DSL,
      { projectBuildModel, moduleModel, helper ->
        val projectModel = projectBuildModel.projectBuildModel
        assertThat(projectModel).isNotNull()
        val changed =
  ...
  fun testSmartAddPluginNoCatalog() {
    ...
    doTest(
      ...
      { projectBuildModel, moduleModel, helper ->
        ...
        val changed =
          helper.addPluginOrClasspath("com.google.gms.google-services", "com.google.gms:google-services", "4.3.14", listOf(moduleModel))
        assertThat(changed.size).isEqualTo(2)
      },
      {
        val projectBuildContent = project.getTextForFile("build.gradle")
  ...
  fun testSmartUpdatePluginVersionInClasspath() {
    doTest(
      ...
      { projectBuildModel, moduleModel, helper ->
        ...
        // add first
        val changed = helper.addPluginOrClasspath(pluginId, classpathModule, "4.3.14", listOf(moduleModel))
        assertThat(changed.size).isEqualTo(2)

        // then update
        val config = PluginInsertionConfig.defaultInsertionConfig().copy(whenFoundSame = MatchedStrategy.UPDATE_VERSION)
        val changed2 = helper.addPluginOrClasspath(pluginId, classpathModule, "4.3.15", listOf(moduleModel), config = config)
  ...
  fun testSmartUpdatePluginVersionInPluginManagement() {
    doTest(
      SIMPLE_APPLICATION_PLUGINS_DSL,
      {
        ...
        FileUtil.writeToFile(File(project.basePath, "settings.gradle"), newContent)
      },
      { projectBuildModel, moduleModel, helper ->
        val projectModel = projectBuildModel.projectBuildModel
        assertThat(projectModel).isNotNull()
  ...
  fun testSmartAddPluginWithCatalogWithExistingPlugin() {
    ...
    doTest(
      SIMPLE_APPLICATION_VERSION_CATALOG,
      {
        ...
        FileUtil.writeToFile(settingsFile, settingsFileText.replace("pluginManagement {", "pluginManagement {\n  plugins {}"))
      },
      { projectBuildModel, moduleModel, helper ->
        val projectModel = projectBuildModel.projectBuildModel
        assertThat(projectModel).isNotNull()
  ...
  fun testFindPlaceAndAddPluginNoCatalogPluginsBlock() {
    doTest(
      SIMPLE_APPLICATION_PLUGINS_DSL,
      { _, moduleModel, helper ->
        val changed = helper.findPlaceAndAddPlugin("com.google.gms.google-services", "4.3.14", listOf(moduleModel))
        assertThat(changed.size).isEqualTo(2)
      },
      {
        val projectBuildContent = project.getTextForFile("build.gradle")
  ...
  fun testSmartAddPluginWithCatalog() {
    doTest(
      ...
      },
      {
        ...
        assertThat(project.getTextForFile("app/build.gradle")).contains("alias(libs.plugins.google.gms.google.services)")
  ...
  fun testSimpleAddWithCatalogIgnoreExistingDeclaration() {
    doTest(
      ...
      },
      {
        ...
        assertThat(project.getTextForFile("app/build.gradle")).contains("alias(libs.plugins.exampleLibs)")
        assertThat(project.getTextForFile("build.gradle")).contains("alias(libs.plugins.exampleLibs) apply false")
      },
    )
  }
  ...
  private fun doTest(
    ...
    val moduleModel: GradleBuildModel? =
      if (project.hasModule("app")) projectBuildModel.getModuleBuildModel(project.findModule("app"))
      else projectBuildModel.projectBuildModel
