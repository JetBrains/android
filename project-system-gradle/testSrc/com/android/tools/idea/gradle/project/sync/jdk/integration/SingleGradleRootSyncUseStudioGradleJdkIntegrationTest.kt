...
class SingleGradleRootSyncUseStudioGradleJdkIntegrationTest {
  ...
  fun `Given valid STUDIO_GRADLE_JDK env variable and invalid jdkTable entry When import project Then sync used the STUDIO_GRADLE_JDK`() =
    jdkIntegrationTest.run(
      project = SimpleApplication(ideaGradleJdk = "invalid jdk"),
      environment = TestEnvironment(environmentVariables = mapOf(JDK_LOCATION_ENV_VARIABLE_NAME to JDK_17_PATH)),
    ) {
      syncWithAssertion(expectedGradleJdkName = "invalid jdk", expectedProjectJdkName = JDK_17, expectedProjectJdkPath = JDK_17_PATH)
    }
