...
private fun getProjectSpecificIdeModelResyncIssues(testProject: TestProject) =
  when (testProject.template) {
...
fun ModuleDumpWithType.filterOutKnownResyncIssues(testProject: TestProject): ModuleDumpWithType {
  ...
  return copy(
    projectStructure =
      androidEntries.filter { line -> (projectSpecificIssues).none { line.contains(it) } }.asSequence() +
        rest.filter { line -> (projectSpecificIssues + setOf("/TEST_MODULE_PROPERTIES", "/Classes")).none { line.contains(it) } },
    ideModels = ideModels.filter { line -> getProjectSpecificIdeModelResyncIssues(testProject).none { line.contains(it) } },
  )
}

private fun fullResyncIssues(testProject: TestProject) =
  when (testProject) {
...
private fun ModuleDumpWithType.filterOutFullResyncIssues(testProject: TestProject) =
  copy(projectStructure = projectStructure.filter { line -> (fullResyncIssues(testProject)).none { line.contains(it) } })
...
class PhasedSyncResyncTests(val testProject: TestProject) : PhasedSyncSnapshotTestBase(checkObjectIdentity = true) {
  ...
  companion object {
    ...
    fun testParameters(): Collection<*> =
      phasedSyncTestProjects.filterNot {
        setOf(
