...
class KotlinCompilationErrorIntegrationTest : BuildOutputIntegrationTestBase() {
  ...
  @Test fun testBrokenCompilationWithoutStacktrace() = testBrokenCompilation(false)

  @Test fun testBrokenCompilationWithStacktrace() = testBrokenCompilation(true)

  private fun testBrokenCompilation(withStacktrace: Boolean) {
    val preparedProject = projectRule.prepareTestProject(testProject())

    val brokenFile =
      preparedProject.root.resolve("mylibrary/src/test/java/com/example/mylibrary/ExampleUnitTest.kt").apply {
  ...
  private fun testProject(): TemplateBasedTestProject =
    testProjectTemplateFromPath(TestProjectPaths.KOTLIN_LIB, "tools/adt/idea/android/testData")
}
