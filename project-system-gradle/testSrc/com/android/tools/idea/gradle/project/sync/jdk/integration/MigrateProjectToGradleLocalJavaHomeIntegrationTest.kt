...
class MigrateProjectToGradleLocalJavaHomeIntegrationTest {
  ...
  fun `Given invalid Gradle JDK configuration When sync project Then no migration happened`() =
    jdkIntegrationTest.run(project = SimpleApplication(ideaGradleJdk = USE_JAVA_HOME, ideaProjectJdk = "any")) {
      sync(
        assertOnDiskConfig = {
  ...
  fun `Given gradleJdk #GRADLE_LOCAL_JAVA_HOME pointing to valid JDK When sync project Then no migration happened`() =
    jdkIntegrationTest.run(
      project = SimpleApplication(ideaGradleJdk = USE_GRADLE_LOCAL_JAVA_HOME, gradleLocalJavaHome = JDK_EMBEDDED_PATH),
      environment = TestEnvironment(studioFlags = StudioFeatureFlags(true)),
    ) {
      syncWithAssertion(
  ...
  fun `Given gradleJdk #JAVA_HOME pointing to valid JDK When sync project Then no migration happened`() =
    jdkIntegrationTest.run(
      ...
      environment = TestEnvironment(environmentVariables = mapOf(JAVA_HOME to JDK_11_PATH), studioFlags = StudioFeatureFlags(true)),
    ) {
      syncWithAssertion(
  ...
  fun `Given gradleJdk #JAVA_INTERNAL pointing to valid JDK When sync project Then no migration happened`() =
    jdkIntegrationTest.run(
      project = SimpleApplication(ideaGradleJdk = USE_INTERNAL_JAVA),
      environment = TestEnvironment(studioFlags = StudioFeatureFlags(true)),
    ) {
      syncWithAssertion(
  ...
  fun `Given valid gradle JDK configuration and daemon JVM criteria When sync project Then gradleJdk got unconfigured`() =
    jdkIntegrationTest.run(
      project = SimpleApplication(ideaGradleJdk = "valid-entry", gradleDaemonToolchain = GradleDaemonToolchain(JDK_EMBEDDED_VERSION)),
      environment = TestEnvironment(jdkTable = listOf(Jdk("valid-entry", JDK_11_PATH)), studioFlags = StudioFeatureFlags(true)),
    ) {
      ...
      syncWithAssertion(
        expectedGradleRoots =
          mapOf(
            ...
            "project_root2" to
              ExpectedGradleRoot(ideaGradleJdk = USE_JAVA_HOME, gradleExecutionDaemonJdkPath = JDK_11_PATH, gradleLocalJavaHome = null),
            "project_root3" to
              ExpectedGradleRoot(
