...
class AndroidGradlePluginSnapshotRepositoryProviderTest {
  ...
  @get:Rule val testLogCollector = TestLogCollector()
  ...
  class TestAndroidGradlePluginSnapshotRepositoryProvider(cacheDir: Path, private val content: Map<String, String>) :
  ...
  fun testGetLatestSnapshot() {
    ...
    val setupRepository =
      TestAndroidGradlePluginSnapshotRepositoryProvider(
        cacheDir,
        mapOf(
          "studio/builds" to
            """
    ...
    assertThat(setupRepository.getLatestSnapshot())
      .isEqualTo(
        AndroidGradlePluginSnapshotRepositoryProvider.SnapshotRepository(
          setOf(AgpVersion.parse("8.6.0-dev")),
          URL("https://androidx.dev/studio/builds/11833616/artifacts/artifacts/repository"),
        )
      )
  }
  ...
  fun testInvalidPage() {
    ...
    assertThat(setupRepository.getLatestSnapshot()).isNull()
    assertThat(testLogCollector.logger.warnings)
      .containsExactly(
  ...
  fun testMissingMavenMetadataPath() {
    val cacheDir = temporaryFolder.newFolder().toPath()
    val setupRepository =
      TestAndroidGradlePluginSnapshotRepositoryProvider(cacheDir, mapOf("studio/builds" to "/studio/builds/11833616/artifacts"))
    assertThat(setupRepository.getLatestSnapshot()).isNull()
