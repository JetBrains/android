...
class GradleDependencyCompatibilityAnalyzerTest : AndroidTestCase() {
  ...
  private val testDataDir: Path =
    Paths.get(AndroidTestBase.getTestDataPath()).resolve("../../project-system-gradle/testData/repoIndex").normalize()

  /** This test is using a fake Maven Repository where we control the available artifacts and versions. */
  private val mavenRepository =
    object : GoogleMavenRepository(cacheDir = testDataDir, cacheExpiryHours = Int.MAX_VALUE, useNetwork = false) {
  ...
  fun testGetAvailableDependency_fallbackToPreview() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.ANDROIDX_NAVIGATION_RUNTIME.getDependency("+")))
  ...
  fun testGetAvailableDependency_returnsLatestStable() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.CONSTRAINT_LAYOUT.getDependency("+")))
        .get(TIMEOUT, TimeUnit.SECONDS)
  ...
  fun testGetAvailableDependency_returnsNullWhenNoneMatches() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.PLAY_SERVICES_ADS.getDependency("+")))
        .get(TIMEOUT, TimeUnit.SECONDS)
  ...
  fun testAddSupportDependencyWithMatchInSubModule() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.SUPPORT_RECYCLERVIEW_V7.getDependency("+")))
        .get(TIMEOUT, TimeUnit.SECONDS)
  ...
  fun testProjectWithIncompatibleDependencies() {
    ...
    setupProject(
      additionalAppResolvedDependencies =
        listOf(ideAndroidLibrary("androidx.appcompat:appcompat:2.0.0"), ideAndroidLibrary("androidx.appcompat:appcompat:1.2.0")),
      additionalAppDeclaredDependencies = listOf("androidx.appcompat:appcompat:2.0.0", "androidx.appcompat:appcompat:1.2.0"),
    )

    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.ANDROIDX_FRAGMENT.getDependency("+")))
        .get(TIMEOUT, TimeUnit.SECONDS)
  ...
  fun testProjectWithIncompatibleIndirectDependencies() {
    setupProject(
      appDependOnLibrary = true,
      additionalAppResolvedDependencies =
        listOf(ideAndroidLibrary("androidx.appcompat:appcompat:2.0.0"), ideAndroidLibrary("androidx.core:core:1.0.0")),
  ...
  fun testTwoArtifactsWithConflictingDependencies() {
    setupProject(
      additionalAppResolvedDependencies = listOf(ideAndroidLibrary("com.google.android.material:material:1.3.0")),
      additionalAppDeclaredDependencies = listOf("com.google.android.material:material:1.3.0"),
    )

    val (found, missing, warning) =
      analyzer.analyzeDependencyCompatibility(listOf(Dependency.parse("com.acme.pie:pie:+"))).get(TIMEOUT, TimeUnit.SECONDS)
    ...
    assertThat(missing).isEmpty()
    assertThat(found).containsExactly(Dependency.parse("com.acme.pie:pie:+"), Component.parse("com.acme.pie:pie:1.0.0-alpha1"))
  }
  ...
  fun testPreviewsAreAcceptedIfNoStableExists() {
    setupProject(additionalLibrary1DeclaredDependencies = listOf("androidx.appcompat:appcompat:2.0.0"))
  ...
  fun testNewestSameMajorIsChosenFromExistingIndirectDependency() {
    setupProject(
      appDependOnLibrary = true,
      additionalAppResolvedDependencies = listOf(ideAndroidLibrary("androidx.appcompat:appcompat:1.0.0")),
  ...
  fun testAddingImcompatibleTestDependenciesFromMultipleSources() {
    ...
    setupProject(additionalAppResolvedDependencies = listOf(ideAndroidLibrary("androidx.recyclerview:recyclerview:1.2.0")))

    val (found, missing, warning) =
      analyzer.analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.MATERIAL.getDependency("+"))).get(TIMEOUT, TimeUnit.SECONDS)

    assertThat(warning).isEmpty()
    assertThat(missing).isEmpty()
    assertThat(found)
      .containsExactly(GoogleMavenArtifactId.MATERIAL.getDependency("+"), GoogleMavenArtifactId.MATERIAL.getComponent("1.3.0"))
  }
  ...
  fun testAddingKotlinStdlibDependenciesFromMultipleSources() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(
          listOf(
  ...
  fun testGetAvailableDependencyWithRequiredVersionMatching() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.SUPPORT_APPCOMPAT_V7.getDependency("+")))
        .get(TIMEOUT, TimeUnit.SECONDS)
  ...
  fun testGetAvailableDependencyWhenUnavailable() {
    setupProject()
    val (found, missing, warning) =
      analyzer.analyzeDependencyCompatibility(listOf(Dependency.parse("nonexistent:dependency123:+"))).get(TIMEOUT, TimeUnit.SECONDS)
  ...
  fun testGetAvailableDependenciesWhenUnavailable() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(
          listOf(Dependency.parse("nonexistent:dependency123:+"), Dependency.parse("nonexistent:dependency456:+"))
        )
        .get(TIMEOUT, TimeUnit.SECONDS)
    ...
    assertThat(missing).containsExactly(Dependency.parse("nonexistent:dependency123:+"), Dependency.parse("nonexistent:dependency456:+"))
    assertThat(found).isEmpty()
  }
  ...
  fun testWithExplicitVersion() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.SUPPORT_APPCOMPAT_V7.getDependency("23.1.0")))
        .get(TIMEOUT, TimeUnit.SECONDS)
  ...
  fun testWithExplicitPreviewVersion() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.CONSTRAINT_LAYOUT.getDependency("1.1.0-beta3")))
        .get(TIMEOUT, TimeUnit.SECONDS)
  ...
  fun testWithExplicitNonExistingVersion() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.SUPPORT_APPCOMPAT_V7.getDependency("[22.17.3,22.17.4)!!")))
        .get(TIMEOUT, TimeUnit.SECONDS)
  ...
  fun testComponentWithExplicitNonExistingVersion() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeComponentCompatibility(listOf(GoogleMavenArtifactId.SUPPORT_APPCOMPAT_V7.getComponent("22.17.3")))
        .get(TIMEOUT, TimeUnit.SECONDS)
  ...
  fun testWithMajorVersion() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.SUPPORT_APPCOMPAT_V7.getDependency("23.+")))
        .get(TIMEOUT, TimeUnit.SECONDS)
  ...
  fun testWithMajorMinorVersion() {
    ...
    val (found, missing, warning) =
      analyzer
        .analyzeDependencyCompatibility(listOf(GoogleMavenArtifactId.SUPPORT_APPCOMPAT_V7.getDependency("22.2.+")))
        .get(TIMEOUT, TimeUnit.SECONDS)
  ...
  private fun setupProject(
    ...
    myFixture.addFileToProject("library1/build.gradle", config(additionalLibrary1DeclaredDependencies.map { "implementation" to "'$it'" }))

    setupTestProjectFromAndroidModel(
      project,
      @Suppress("DEPRECATION") project.baseDir!!.toIoFile(),
      JavaModuleModelBuilder.rootModuleBuilder,
      AndroidModuleModelBuilder(
        ":app",
        "debug",
        AndroidProjectBuilder(
          androidModuleDependencyList = { if (appDependOnLibrary) listOf(AndroidModuleDependency(":library1", "debug")) else emptyList() },
    ...
    analyzer =
      GradleDependencyCompatibilityAnalyzer(
