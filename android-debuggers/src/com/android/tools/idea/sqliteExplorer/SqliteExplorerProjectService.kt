...
class SqliteExplorerProjectServiceImpl @JvmOverloads constructor(
  ...
  private val factory: SqliteEditorViewFactory = SqliteEditorViewFactoryImpl(),
  ...
  private val createController: (SqliteController.Model) -> SqliteController = { myModel ->
    SqliteControllerImpl(
      ...
      myModel,
      factory,
      // TODO move the creation of this view in the view factory
  ...
  override fun openSqliteDatabase(file: VirtualFile): ListenableFuture<SqliteDatabase> {
    val sqliteServiceFuture = SqliteServiceFactoryImpl().getSqliteService(file, taskExecutor)

    val openSqliteServiceFuture = taskExecutor.transform(sqliteServiceFuture) { sqliteService ->
      // TODO(b/139525976)
      val name = file.path.split("data/data/").getOrNull(1)?.replace("databases/", "") ?: file.path
      val database = SqliteDatabase(name, sqliteService)
      lock.withLock { databaseToFile[database] = file }

      return@transform database
    }

    toolWindowManager.getToolWindow(SqliteExplorerToolWindowFactory.TOOL_WINDOW_ID).show { controller.addSqliteDatabase(openSqliteServiceFuture) }
