...
/**
 * IndexingMetricsCollector collects indexing-related metrics from the $LOGS_DIR/indexing-diagnostic
 * directory.
 *
 ...
 * <li>numberOfIndexedFilesWritingIndexValue: number of file indexings that led to indexes being
 *   updated;</li>
 * <li>numberOfIndexedFilesWithNothingToWrite: number of file indexings that didn't lead to indexes
 *   being updated;</li>
 * <li>numberOfFilesIndexedByExtensions: number of file indexings performed by extensions;</li>
 * <li>numberOfFilesIndexedWithoutExtensions: number of file indexings performed not by
 *   extensions;</li>
 ...
 * <li>processingSpeedOfBaseLanguageAvg_$LANGUAGE: average processing speed for a given
 *   language;</li>
...
class IndexingMetrics(private val indexingDiagnosticsDir: Path) {
  fun get(project: AndroidProject): List<IndexingMetric> {
    val indexDiagnosticDirectoryChildren =
      Files.list(indexingDiagnosticsDir).filter { it.toFile().isDirectory }.use { it.toList() }
    ...
    val projectIndexDiagnosticDirectory =
      indexDiagnosticDirectoryChildren.let { perProjectDirs ->
        ...
        if (projectName == null) {
          perProjectDirs.singleOrNull()
            ?: error(
              "Only one project diagnostic dir is expected: ${perProjectDirs.joinToString()}"
            )
        } else {
  ...
  data class IndexingMetrics(val jsonIndexDiagnostics: List<JsonIndexingActivityDiagnostic>) {
    private val scanningHistories: List<JsonProjectScanningHistory>
      get() =
        jsonIndexDiagnostics
          .map { it.projectIndexingActivityHistory }
          .filterIsInstance<JsonProjectScanningHistory>()
    ...
    private val indexingHistories: List<JsonProjectDumbIndexingHistory>
      get() =
        jsonIndexDiagnostics
          .map { it.projectIndexingActivityHistory }
          .filterIsInstance<JsonProjectDumbIndexingHistory>()
          .sortedBy { it.times.updatingStart.instant }
      ...
      get() =
        jsonIndexDiagnostics.sumOf {
          TimeUnit.NANOSECONDS.toMillis(
            it.projectIndexingActivityHistory.times.dumbWallTimeWithPauses.nano
          )
        }
    ...
    private val totalIndexingTimeWithoutPauses: Long
      get() =
        TimeUnit.NANOSECONDS.toMillis(
          indexingHistories.sumOf { it.times.totalWallTimeWithoutPauses.nano }
        )

    private val totalScanFilesTimeWithoutPauses: Long
      get() =
        TimeUnit.NANOSECONDS.toMillis(
          scanningHistories.sumOf { it.times.totalWallTimeWithoutPauses.nano }
        )
    ...
    private val totalPausedTime: Long
      get() =
        TimeUnit.NANOSECONDS.toMillis(
          jsonIndexDiagnostics.sumOf {
            it.projectIndexingActivityHistory.times.wallTimeOnPause.nano
          }
        )
    ...
    private val totalNumberOfIndexedFiles: Int
      get() =
        indexingHistories.sumOf { history ->
          history.fileProviderStatistics.sumOf { it.totalNumberOfIndexedFiles }
        }

    private val totalNumberOfIndexedFilesWritingIndexValues: Int
      get() =
        indexingHistories.sumOf { history ->
          history.fileProviderStatistics.sumOf {
            it.totalNumberOfIndexedFiles - it.totalNumberOfNothingToWriteFiles
          }
        }
    ...
    private val totalNumberOfIndexedFilesWithNothingToWrite: Int
      get() =
        indexingHistories.sumOf { history ->
          history.fileProviderStatistics.sumOf { it.totalNumberOfNothingToWriteFiles }
        }
        ...
        jsonIndexDiagnostics.sumOf {
          when (val fileCount = it.projectIndexingActivityHistory.fileCount) {
            is JsonProjectScanningFileCount ->
              fileCount.numberOfFilesIndexedByInfrastructureExtensionsDuringScan
    ...
    private val processingSpeedPerFileTypeWorst: Map<String, Int>
      get() {
        return indexingHistories
          .flatMap { it.totalStatsPerFileType }
          .groupBy { it.fileType }
          .mapValues {
            it.value.minOf { jsonStatsPerFileType ->
              jsonStatsPerFileType.totalProcessingSpeed.toKiloBytesPerSecond()
            }
          }
      }
        ...
        return indexingHistories
          .flatMap { history ->
            history.totalStatsPerFileType.map {
              Triple(
                it.fileType,
                it.partOfTotalProcessingTime.partition * history.times.totalWallTimeWithPauses.nano,
                it.totalFilesSize,
              )
            }
          }
    ...
    private fun Collection<Triple<String, Double, JsonFileSize>>.computeAverageSpeed():
      Map<String, Int> =
      groupBy { it.first }
        .mapValues { entry ->
          JsonProcessingSpeed(
              entry.value.sumOf { it.third.bytes },
              entry.value.sumOf { it.second.toLong() },
            )
    ...
    private val processingSpeedPerBaseLanguageWorst: Map<String, Int>
      get() {
        return indexingHistories
          .flatMap { it.totalStatsPerBaseLanguage }
          .groupBy { it.language }
          .mapValues {
            it.value.minOf { jsonStatsPerParentLanguage ->
              jsonStatsPerParentLanguage.totalProcessingSpeed.toKiloBytesPerSecond()
            }
          }
      }
      ...
      get() {
        ...
        indexingHistories.forEach { indexingHistory ->
          indexingHistory.totalStatsPerFileType.forEach { totalStatsPerFileType ->
            val duration =
              (indexingHistory.times.totalWallTimeWithPauses.nano *
                  totalStatsPerFileType.partOfTotalProcessingTime.partition)
                .toLong()
            indexingDurationMap[totalStatsPerFileType.fileType] =
              indexingDurationMap[totalStatsPerFileType.fileType]?.let { it + duration } ?: duration
          }
        }
    ...
    private fun getProcessingSpeedOfFileTypes(
      mapFileTypeToSpeed: Map<String, Int>,
      suffix: String,
    ): List<IndexingMetric> =
    ...
    private fun getProcessingSpeedOfBaseLanguages(
      mapBaseLanguageToSpeed: Map<String, Int>,
      suffix: String,
    ): List<IndexingMetric> =
      mapBaseLanguageToSpeed.map {
        IndexingMetric(
          "processingSpeedOfBaseLanguage${suffix}_${prepareMetricLabel(it.key)}",
          it.value.toLong(),
        )
      }

    private fun getProcessingTimeOfFileType(
      mapFileTypeToDuration: Map<String, Long>
    ): List<IndexingMetric> =
      mapFileTypeToDuration.map {
        IndexingMetric(
          "processingTime_${prepareMetricLabel(it.key)}",
          TimeUnit.NANOSECONDS.toMillis(it.value),
        )
      }

    fun getIndexingMetrics(): List<IndexingMetric> {
      val numberOfIndexedFiles = totalNumberOfIndexedFiles.toLong()
      val numberOfFilesFullyIndexedByExtensions =
        totalNumberOfFilesFullyIndexedByExtensions.toLong()
      return listOf(
        ...
        IndexingMetric(
          "numberOfIndexedFilesWritingIndexValue",
          totalNumberOfIndexedFilesWritingIndexValues.toLong(),
        ),
        IndexingMetric(
          "numberOfIndexedFilesWithNothingToWrite",
          totalNumberOfIndexedFilesWithNothingToWrite.toLong(),
        ),
        IndexingMetric("numberOfFilesIndexedByExtensions", numberOfFilesFullyIndexedByExtensions),
        IndexingMetric(
          "numberOfFilesIndexedWithoutExtensions",
          numberOfIndexedFiles - numberOfFilesFullyIndexedByExtensions,
        ),
