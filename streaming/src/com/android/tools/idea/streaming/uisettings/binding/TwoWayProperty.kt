...
/**
 * A property of type [T] that is intended to be used in UI models. A [ReadOnlyProperty] is read but not changed from the UI, however it can
 * be changed from the controller.
 */
internal interface ReadOnlyProperty<T> {
  ...
  /** Creates a boolean property that yields true only if both the current boolean property and [other] is true. */
  fun and(other: ReadOnlyProperty<Boolean>): ReadOnlyProperty<Boolean>

  /** Creates a boolean property that yields the opposite value of the current boolean property. */
  fun not(): ReadOnlyProperty<Boolean>
}

/**
 * A property of type [T] that can be changed either from the UI or from the controller. Note: the controller listeners are NOT fired when
 * the property is changed by [setFromUi], and the [uiChangeListener] is NOT fired when the property is changed by [setFromController].
 */
internal interface TwoWayProperty<T> : ReadOnlyProperty<T> {
  ...
  /**
   * Create a property of type [U] that maps a value from this property via the functions: [toTarget] and the inverse function [fromTarget].
   */
