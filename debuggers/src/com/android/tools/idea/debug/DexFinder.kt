...
object DexFinder {
  ...
  data class Result(val dex: Dex?, val status: DexSearchStatus) {
    companion object {
      // We use a NOT_FOUND value to cache fetch failures.
      val NOT_FOUND = Result(null, DexSearchStatus.DEX_NOT_FOUND)
    }
  }
  ...
  /**
   * Finds a [Dex] file that contains a [location].
   *
   ...
   */
  suspend  fun findDex(debugProcess: DebugProcessImpl, expression: KtElement, location: Location): Result {
    val type = location.declaringType()
    val cache = DexCache.instance(debugProcess)
    cache.typeToSearchResult[type]?.let { return it }
    val result = findDexImpl(debugProcess, expression, location)
    return result.also {
      cache.typeToSearchResult[type] = it
    }
  }
...
private suspend fun findDexImpl(debugProcess: DebugProcessImpl, expression: KtElement, location: Location): Result {
  return try {
    findDexViaApkProvider(debugProcess, expression, location)?.let { Result(it, DexSearchStatus.FOUND) }
    ?: findDexViaInstaller(debugProcess, location)?.let { Result(it, DexSearchStatus.FOUND) }
    ?: Result.NOT_FOUND
  } catch (_: Exception) {
    Result.NOT_FOUND
  }
...
private suspend fun findDexViaApkProvider(debugProcess: DebugProcessImpl, expression: KtElement, location: Location): Dex? {
  val apkProvider = debugProcess.androidRunConfiguration?.apkProvider ?: return null
...
private fun findDexViaInstaller(debugProcess: DebugProcessImpl, location: Location): Dex? {
  val device = debugProcess.connectedDevice ?: return null
