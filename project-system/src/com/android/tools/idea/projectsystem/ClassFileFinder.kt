...
/**
 * A [ClassFileFinder] searches build output to find the class file corresponding to a fully-qualified class name.
 *
 * Because the build system is responsible for generating these class files, implementations of [ClassFileFinder] are build system-specific.
 * To retrieve class files in a build system-agnostic way, callers should go through the [AndroidModuleSystem] abstraction.
 */
interface ClassFileFinder {
  /**
   * @return the [ClassContent] corresponding to the class file for the given fully-qualified class name, or null if the class file can't be
   *   found.
   */
