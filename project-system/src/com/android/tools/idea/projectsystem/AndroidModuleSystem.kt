...
/**
 * Provides a build-system-agnostic interface to the build system. Instances of this interface contain methods that apply to a specific
 * [Module].
 */
interface AndroidModuleSystem : SampleDataDirectoryProvider, ModuleHierarchyProvider {
  val androidModel: AndroidModel?
    get() = null
  ...
  val type: Type
    get() =
      when (module.androidFacet?.properties?.PROJECT_TYPE) {
   ...
   * @return a list of templates created from each of the android facet's source providers. In cases where the source provider returns
   *   multiple paths, we always take the first match.
   *
   * TODO: Figure out and document the rest of the contracts for this method, such as how targetDirectory is used, what the source set names
   *   are used for, and why the result is a list
   */
  ...
  @Throws(DependencyManagementException::class) fun hasResolvedDependency(id: WellKnownMavenArtifactId, scope: DependencyScopeType): Boolean
  ...
  /**
   * Returns the Android modules that this module transitively depends on for resources. As Android modules, each module in the returned
   * list will have an associated AndroidFacet.
   *
   * Where supported, the modules will be returned in overlay order to help with resource resolution, but this is only to support legacy
   * callers. New callers should avoid making such assumptions and instead determine the overlay order explicitly if necessary.
   *
   * TODO(b/118317486): Remove this API once resource module dependencies can accurately be determined from order entries for all supported
   *   build systems.
   */
  ...
  /**
   * Returns the Android modules that directly depend on this module for resources. As Android modules, each module in the returned list
   * will have an associated AndroidFacet.
   *
   * TODO(b/118317486): Remove this API once resource module dependencies can accurately be determined from order entries for all supported
   *   build systems.
   */
  ...
  /**
   * Returns the overrides that the underlying build system applies when computing the module's merged manifest.
   *
  ...
  /**
   * Returns the manifest placeholders that the underlying build system applies when computing the module's merged manifest.
   *
   ...
   *
   * The resource package name is equivalent to the "package" attribute of the module's merged manifest once it has been built. Depending on
   * the build system, however, this method may be optimized to avoid the costs of merged manifest computation.
   *
   * The returned package name is guaranteed to reflect the latest contents of the Android manifests including changes that haven't been
   * saved yet but is NOT guaranteed to reflect the latest contents of the build configuration if the project hasn't been re-synced with the
   * build configuration yet.
   */
   ...
   *
   * Some project systems may be unable to retrieve the package name before the project has been successfully built. The returned
   * [ApplicationIdProvider] will throw [ApkProvisionException]'s or return a name derived from incomplete configuration in this case.
   *
   ...
   * system.
   */
  fun getApplicationIdProvider(): ApplicationIdProvider =
   ...
   *
   * This is a seam for [Module.getModuleWithDependenciesAndLibrariesScope] that allows project systems that have not expressed their module
   * level dependencies accurately to IntelliJ (typically for performance reasons) to provide a different scope than what the module itself
   * would.
   */
  ...
  val supportsAndroidResources: Boolean
    get() = true

  /**
   * Whether the R class generated for this module is transitive.
   *
   * If it is transitive it will contain all of the resources defined in its transitive dependencies alongside those defined in this module.
   * If non-transitive it will only contain the resources defined in this module.
   */
  val isRClassTransitive: Boolean
    get() = true
  ...
  val isMlModelBindingEnabled: Boolean
    get() = false
  ...
  val isDebuggable: Boolean
    get() = AndroidModel.get(module)?.isDebuggable ?: false
  ...
  val desugarLibraryConfigFilesKnown: Boolean
    get() = false

  /** A user visible message, such as 'Only supported for project using Android Gradle plugin '8.1.0-alpha05' and above' */
  val desugarLibraryConfigFilesNotKnownUserMessage: String?
    get() = "Only supported for Gradle projects"
  ...
  /**
   * Is this module an Android module that contains Android entities that are potentially relevant for production? Returning `true` from
   * this implies the presence of an [AndroidFacet] for the [module].
   */
  ...
  fun isValidForAndroidRunConfiguration() =
    when (type) {
      Type.TYPE_APP,
      Type.TYPE_DYNAMIC_FEATURE -> true

      Type.TYPE_ATOM,
      Type.TYPE_FEATURE,
      Type.TYPE_INSTANTAPP, // Legacy not-supported module types.
      Type.TYPE_LIBRARY,
  ...
  fun isValidForAndroidTestRunConfiguration() =
    when (type) {
      Type.TYPE_TEST -> true
 ...
 *
 * These overrides are divided into two categories: [directOverrides], known properties of the merged manifest that are directly overridden
 * (e.g. the application ID), and [placeholders], identifiers in the contributing manifest which the build system replaces with arbitrary
 * plain text during merged manifest computation.
 */
...
/**
 * An identifier for a registered dependency, either currently existent or whose registration might be requested of a
 * [RegisteringModuleSystem].
 */
 ...
 *
 * Not all [AndroidModuleSystem]s can introspect or (particularly) modify the project's definition to include new dependencies. Those that
 * can will return a [RegisteringModuleSystem] (usually themselves) from [AndroidModuleSystem.getRegisteringModuleSystem], giving access to
 * these methods. The common entry points for the functionality involve getting ids from a well-known [GoogleMavenArtifactId], but a
 * particular [RegisteringModuleSystem] implementation might provide other entry points (for example, [String] notation applicable to that
 * particular [AndroidModuleSystem].
 *
...
interface RegisteringModuleSystem<T : RegisteredDependencyQueryId, U : RegisteredDependencyId> {
   ...
   * would cause any incompatibilities (for example by introducing versions of libraries with known incompatibilities between themselves or
   * with existing dependencies in the project. Return (as a data class) the list of compatible dependencies, the list of incompatible
   * dependencies, and a [String] warning suitable for displaying to the user.
   */
