...
fun isAndroidTestFile(project: Project, file: VirtualFile?): Boolean =
  ReadAction.nonBlocking<Boolean> {
      val module =
        SlowOperations.knownIssue("IDEA-359567").use { file?.let { ProjectFileIndex.getInstance(project).getModuleForFile(file) } }
...
fun isScreenshotTestFile(project: Project, file: VirtualFile?): Boolean =
  ReadAction.nonBlocking<Boolean> {
      val module =
        SlowOperations.knownIssue("IDEA-359569").use { file?.let { ProjectFileIndex.getInstance(project).getModuleForFile(file) } }
