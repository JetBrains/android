...
fun DeviceFiltersPanel(modifier: Modifier = Modifier, content: @Composable () -> Unit) {
  VerticallyScrollableContainer(modifier.fillMaxSize()) { Column(Modifier.padding(6.dp).testTag("DeviceFilters")) { content() } }
}

/** Holds the state of the filters for a [DeviceTable]. Can be extended to add or adjust filters for subtypes of [DeviceProfile]. */
@Stable
...
open class TextFilterState<in DeviceT : DeviceProfile> : RowFilter<DeviceT> {
  ...
  override fun apply(row: DeviceT): Boolean = searchText.isBlank() || row.name.contains(searchText.trim(), ignoreCase = true)
}

val Manufacturer = DeviceAttribute("OEM") { it.manufacturer }

internal val formFactorOrder: PersistentList<String> = with(FormFactors) { persistentListOf(PHONE, TABLET, WEAR, DESKTOP, TV, AUTO) }

private val formFactorComparator: Comparator<String> =
  compareBy(nullsLast(Ordering.explicit(formFactorOrder))) { formFactor: String -> formFactor.takeIf { it in formFactorOrder } }
    .then(naturalOrder())
